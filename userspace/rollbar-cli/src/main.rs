...
fn run_roll(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Origin: built it for their own sites — productized after demand from other engineers"
    );
    ...
    println!("  Features:");
    println!(
        "    - SDKs for ~30 languages (Ruby, Python, Node, PHP, Java, .NET, Go, Rust, Elixir, Swift, ...)"
    );
    ...
    println!(
        "  Critique: smaller mindshare than Sentry post-2020 (Sentry pulled ahead with self-hosted + perf monitoring)"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "rollbar".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_roll, strip_ext};
