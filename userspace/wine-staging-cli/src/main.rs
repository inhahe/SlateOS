...
fn run_wine_staging(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("wine-staging v9.0 (Slate OS) — Wine with staging patches");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/wine-staging"), "wine-staging");
        assert_eq!(basename(r"C:\bin\wine-staging.exe"), "wine-staging.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("wine-staging.exe"), "wine-staging");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_wine_staging(&["--help".to_string()], "wine-staging"), 0);
        assert_eq!(run_wine_staging(&["-h".to_string()], "wine-staging"), 0);
        let _ = run_wine_staging(&["--version".to_string()], "wine-staging");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_wine_staging(&[], "wine-staging");
    }
}
