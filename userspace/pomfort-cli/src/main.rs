...
//! pomfort-cli — OurOS Pomfort on-set workflow tools
//!
...
fn run_pomfort(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Pomfort Suite (OurOS) — DIT on-set data management & live color");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Pomfort Silverstack 8 / LiveGrade 6 (OurOS)"); return 0; }
    println!("Pomfort Suite (OurOS)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_pomfort(&["--version".to_string()], "pomfort"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_pomfort(&[], "pomfort"), 0);
    }
}
