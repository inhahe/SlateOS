...
fn run_amplitude(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Amplitude (Slate OS) — product analytics platform");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Amplitude 2024 (Slate OS)");
        return 0;
    }
    println!("Amplitude 2024 (Slate OS)");
    ...
    println!(
        "       FY2024 revenue ~$290M, ~$30M+ operating loss (running for cash flow break-even)"
    );
    println!("       ~2,500 customers, ~750 employees");
    println!("  Defining concept — 'product analytics' as a category:");
    println!(
        "    - Different from 'web analytics' (GA) — designed for in-product, logged-in behavior"
    );
    println!(
        "    - Different from BI tools (Looker/Tableau) — pre-built for product/growth team questions"
    );
    println!("    - Behavioral cohorts, funnels, retention, paths analysis as first-class objects");
    println!(
        "    - North Star Framework (NSM) — Amplitude evangelized + open-sourced this growth method"
    );
    println!("  Pricing:");
    ...
    println!(
        "    Pricing controversy: 'MTU' (monthly tracked user) jumps non-linear; 'tracked events' add-on confusing"
    );
    ...
    println!(
        "    - Audience Sync — push cohorts to Facebook Ads, Google Ads, Salesforce, Iterable, Braze"
    );
    ...
    println!(
        "    - AI-driven personalization recommendations (next-best-content, next-best-product)"
    );
    ...
    println!(
        "    - Source integrations: Segment, mParticle, RudderStack, Rivery, Stitch, Fivetran"
    );
    ...
    println!(
        "              Hubspot, Salesforce, Marketo, Mailchimp, Braze, Iterable as downstream"
    );
    println!("              Slack alerts on metric anomalies");
    println!("  Customers: 2,500+ paying customers");
    println!(
        "            Atlassian, Disney+, Ford, Block (Square+Cash App), Instacart, NBCUniversal"
    );
    println!("            Capital One, Hubspot, Hyatt, PayPal, Roblox, Walmart");
    println!("            sweet spot: high-volume B2C and PLG B2B SaaS (50M+ users)");
    println!(
        "  Critique: pricing famously painful at scale — many high-growth customers churn to PostHog/in-house"
    );
    println!("           dashboards can get slow on large volume");
    println!("           SQL access on Growth+ — expensive at lower tiers");
    println!(
        "           competes with PostHog (lower price, open core) on the lower end + Heap/Mixpanel mid-market"
    );
    println!(
        "  Differentiator: most polished product analytics UI + 'North Star Framework' thought leadership + enterprise governance"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_amplitude, strip_ext};
