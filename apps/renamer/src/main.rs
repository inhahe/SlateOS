...
impl RenamerApp {
    ...
    /// The shortcut list, laid over the file list.
    fn render_help(&self, cmds: &mut Vec<RenderCommand>) {
        let w = (WINDOW_WIDTH * 0.78).min(620.0);
        #[expect(
            clippy::cast_precision_loss,
            reason = "eighteen rows is far below f32's integer-exact range"
        )]
        let h = (SHORTCUTS.len() as f32).mul_add(20.0, 56.0);
        let x = (WINDOW_WIDTH - w) / 2.0;
        let y = ((WINDOW_HEIGHT - h) / 2.0).max(TOOLBAR_HEIGHT);
