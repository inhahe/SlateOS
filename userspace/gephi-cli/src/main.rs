...
fn run_gephi(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Gephi v0.10 (Slate OS) — Graph visualization and exploration");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Gephi v0.10.1 (Slate OS)");
        return 0;
    }
    println!("Gephi v0.10.1 (Slate OS) — Graph Visualization");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "gephi".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gephi, strip_ext};
