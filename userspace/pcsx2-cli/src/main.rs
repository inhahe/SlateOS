...
fn run_pcsx2(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("pcsx2 — PlayStation 2 emulator (Slate OS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("PCSX2 v1.7.5491 (Slate OS)");
        return 0;
    }
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/pcsx2"), "pcsx2");
        assert_eq!(basename(r"C:\bin\pcsx2.exe"), "pcsx2.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("pcsx2.exe"), "pcsx2");
