...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "bluetooth-sendto".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sendto, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/bluetooth-sendto"), "bluetooth-sendto");
        assert_eq!(
            basename(r"C:\bin\bluetooth-sendto.exe"),
            "bluetooth-sendto.exe"
        );
