...
// The implementation used to stand here -- "from scratch, FIPS 180-4", 150
// lines, one of ten such copies under `userspace/`. It comes from `sha2` now.
// This crate derives LUKS keys with PBKDF2-SHA256, so the hash decides whether
// a volume unlocks: a divergence would not misbehave, it would simply produce
// a key that never opens the disk it just encrypted.
use sha2::Sha256;
...
fn hmac_sha256(key: &[u8], data: &[u8]) -> [u8; 32] {
    let mut k = [0u8; 64];
    if key.len() > 64 {
        k[..32].copy_from_slice(&sha2::sha256(key));
    } else {
...
impl VeritySuperblock {
    fn new(data_device: &str, hash_device: &str) -> Self {
        ...
        let salt_hash = sha2::sha256(format!("verity-salt-{}", data_device).as_bytes());
        let root_hash =
            sha2::sha256(format!("verity-root-{}-{}", data_device, hash_device).as_bytes());
        Self {
