...
fn run_nestopia(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("nestopia v1.52 (Slate OS) — Nintendo Entertainment System emulator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Nestopia UE v1.52 (Slate OS)");
        return 0;
    }
    let files: Vec<&String> = args.iter().filter(|a| !a.starts_with('-')).collect();
    if files.is_empty() {
        println!("Nestopia UE v1.52 (Slate OS) — NES Emulator");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "nestopia".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_nestopia, strip_ext};
