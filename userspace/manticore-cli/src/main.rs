...
fn run_manticore(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "indexer" => {
                println!("indexer (Slate OS) — Manticore index builder");
            ...
            "indextool" => {
                println!("indextool (Slate OS) — Manticore index diagnostics");
            ...
            _ => {
                println!("searchd (Slate OS) — Manticore Search daemon");
    ...
    if args.iter().any(|a| a == "--version" || a == "-v") { println!("Manticore Search v6.2.12 (Slate OS)"); return 0; }
    match prog {
        ...
        _ => {
            println!("Manticore Search v6.2.12 (Slate OS)");
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_manticore(&["--version".to_string()], "manticore");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_manticore(&[], "manticore");
    }
}
