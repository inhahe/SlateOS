...
fn run_client(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("x2goclient v4.1 (Slate OS) — X2Go remote desktop client");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("x2goclient v4.1 (Slate OS)"); return 0; }
...
fn run_server(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("x2goserver v4.1 (Slate OS) — X2Go server daemon");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("x2goserver v4.1 (Slate OS)"); return 0; }
