...
fn run_bb(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "        target audience: marketers / non-programmers who want to publish to App Store"
    );
    ...
    println!(
        "  2D + 3D: Buildbox 3 added 3D (2018), Buildbox 4 (2023) added improved 3D + better physics"
    );
    println!(
        "  Brainboxes: pre-built template game projects (endless runner, puzzle, action), edit and ship"
    );
    ...
    println!(
        "  Hyper-casual genre: Buildbox is the favorite tool of the genre's prolific publishers"
    );
    println!("  Critique: limited for complex games; great for one specific niche (HC mobile)");
    println!(
        "  Differentiator: ZERO programming required — only true no-code game engine that ships polished products"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "buildbox".to_string());
