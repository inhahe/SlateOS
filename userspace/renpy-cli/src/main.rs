...
fn run_rp(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Multi-platform: Windows, macOS, Linux, iOS, Android, HTML5 (web), Steam Deck (Linux native)"
    );
    println!(
        "  Niche: visual novels — branching dialogue, ADV/NVL modes, sprites + backgrounds + music"
    );
    println!("        also: dating sims, kinetic novels, choice-based RPGs, interactive fiction");
    println!("  Famous Ren'Py games:");
    println!(
        "    - Doki Doki Literature Club (Team Salvato, 2017) — viral horror VN, FREE on Steam"
    );
    ...
    println!(
        "  Pricing: FREE FOREVER, $0 commercial — Ren'Py is one of the great FOSS gifts to game dev"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "renpy".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_rp, strip_ext};
