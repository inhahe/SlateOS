...
const fn build_v1_table() -> SyscallTable {
    ...
    // Fd-relative calls that resolve the *handle* rather than its name.
    // See design-decisions.md §647.
    ...
    // Not part of the copy set above: `mv` rather than `cp` is the caller, and
    // it is the one member that must refuse a cross-mount operation outright
    // rather than fall back to copying.
    handlers[SYS_FS_RENAMEAT_PINNED as usize] = Some(handlers::sys_fs_renameat_pinned);
...
fn test_dispatch_uts_name() -> KernelResult<()> {
    ...
    for (nr, name) in [
        ...
        // Same probe, same two properties, different right. Included here
        // rather than given a rung of its own because the assertion is
        // identical and a second copy would be a second thing to keep in
        // step -- the same argument that made `name_set_gated` shared.
        (SYS_KEYLAYOUT_SET, "SYS_KEYLAYOUT_SET"),
    ] {
        let r = dispatch(
            nr,
            &SyscallArgs {
