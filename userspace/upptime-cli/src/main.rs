...
fn run_upptime(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Upptime v1.37 (Slate OS) — GitHub-powered uptime monitor");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Upptime v1.37.0 (Slate OS)");
        return 0;
    }
    println!("Upptime v1.37.0 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "upptime".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_upptime, strip_ext};
