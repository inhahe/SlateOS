...
fn run_st(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  --goto-anything        Goto Anything (Ctrl/Cmd+P) — fuzzy file/symbol/line jump"
        );
    ...
    println!(
        "          'Evaluation' indefinitely (unlimited free trial; just nag screen every ~30 saves)"
    );
    ...
    println!(
        "  Plugin API: Python 3.3 + 3.8 (dual interpreter) — Package Control by wbond is the de facto registry"
    );
    println!("  Killer features:");
    println!(
        "    - Goto Anything (Ctrl/Cmd+P): fuzzy file finder, also @symbol, :line, #word, > command"
    );
    ...
    println!(
        "    - Project-wide search (Ctrl+Shift+F) — much faster than VS Code for huge codebases"
    );
    println!(
        "  ST4 additions: native Apple Silicon, GPU rendering, multi-select tabs, context-aware autocomplete,"
    );
    ...
    println!(
        "  Differentiator: still the fastest 'feel' of any modern editor; ST inspired VS Code's Quick Open"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "sublime".to_string());
