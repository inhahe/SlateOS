...
fn run_jitterbit(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Jitterbit Harmony 2024 (Slate OS)");
        return 0;
    }
    ...
    println!(
        "  Strategic position: 'unified API + integration + low-code platform for mid-market':"
    );
    println!(
        "                    pitch: 'integrate, automate, orchestrate — one platform for it all'"
    );
    println!(
        "                    target: mid-market + enterprise (especially Salesforce/NetSuite shops)"
    );
    println!(
        "                    primary competitor: Boomi, Workato, MuleSoft (high-end), Celigo (NetSuite)"
    );
    println!(
        "                    secondary: Jitterbit Vinyl competes with OutSystems, Mendix, Microsoft Power Apps"
    );
    println!(
        "                    Jitterbit's wedge: combined iPaaS + low-code + API mgmt + EDI in one platform"
    );
    ...
    println!(
        "    - Compete with: OutSystems + MuleSoft combo, ServiceNow App Engine + Integration"
    );
    ...
    println!(
        "           low-code (Vinyl) acquisition still being integrated — go-to-market overlap unclear"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "jitterbit".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_jitterbit, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_jitterbit(&["--version".to_string()], "jitterbit");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_jitterbit(&[], "jitterbit");
    }
}
