...
fn run_fio(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("fio 3.37 (Slate OS) — Flexible I/O tester");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/fio"), "fio");
        assert_eq!(basename(r"C:\bin\fio.exe"), "fio.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("fio.exe"), "fio");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_fio(&["--help".to_string()], "fio"), 0);
        assert_eq!(run_fio(&["-h".to_string()], "fio"), 0);
        let _ = run_fio(&["--version".to_string()], "fio");
    }
