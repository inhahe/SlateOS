...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_mode_change_count = MODE_CHANGE_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&DISPLAY, DisplayState::new(), self_test_inner);
    MODE_CHANGE_COUNT.store(saved_mode_change_count, Ordering::Relaxed);
    result
}
