...
//! mattermost — OurOS team collaboration platform
//!
...
fn run_mattermost_server(args: Vec<String>) -> i32 {
    ...
    if cmd == "version" || args.iter().any(|a| a == "--version") {
        println!("Mattermost v9.8.0 (OurOS)");
    ...
    println!("{{\"level\":\"info\",\"ts\":1716368400.100,\"msg\":\"Server version: 9.8.0 (OurOS)\"}}");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_mattermost_server(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_mattermost_server(vec![]), 0);
    }
}
