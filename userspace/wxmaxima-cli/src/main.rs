...
fn run_wxmaxima(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("wxmaxima v23.12 (Slate OS) — GUI for Maxima CAS");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("wxmaxima v23.12 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "wxmaxima".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wxmaxima, strip_ext};
