...
//! backintime-cli — OurOS Back In Time snapshot backup
//!
...
fn run_backintime(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("backintime v1.4 (OurOS) — Snapshot-based backup tool");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("backintime v1.4 (OurOS)"); return 0; }
...
fn run_qt(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("backintime-qt v1.4 (OurOS) — Back In Time GUI");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("backintime-qt v1.4 (OurOS)"); return 0; }
