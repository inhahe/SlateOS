...
fn run_gscan2pdf(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("gscan2pdf v2.13 (Slate OS) — Scan to PDF/DjVu/TIFF");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("gscan2pdf v2.13 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "gscan2pdf".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gscan2pdf, strip_ext};
