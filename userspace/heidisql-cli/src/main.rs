...
fn run_heidisql(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("HeidiSQL v12.8 (Slate OS) — Lightweight database client");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("HeidiSQL v12.8.0 (Slate OS)");
        return 0;
    }
    println!("HeidiSQL v12.8.0 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "heidisql".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_heidisql, strip_ext};
