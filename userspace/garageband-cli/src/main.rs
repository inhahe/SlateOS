...
fn run_gb(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("GarageBand 10.4 (Slate OS) — Apple consumer music studio");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("GarageBand 10.4.11 (Slate OS)");
        return 0;
    }
    println!("GarageBand 10.4.11 (Slate OS)");
    ...
    println!(
        "                a song on Rihanna's ANTI album was produced entirely in GarageBand iOS,"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "garageband".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gb, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/garageband"), "garageband");
        assert_eq!(basename(r"C:\bin\garageband.exe"), "garageband.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("garageband.exe"), "garageband");
