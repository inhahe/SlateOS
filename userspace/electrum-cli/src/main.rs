...
fn run_electrum(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Electrum v4.5 (Slate OS) — Lightweight Bitcoin wallet");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("Electrum v4.5 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "electrum".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_electrum, strip_ext};
