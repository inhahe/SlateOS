...
fn run_offlineimap(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("offlineimap v8.0 (Slate OS) — Bidirectional IMAP/Maildir sync");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("offlineimap v8.0 (Slate OS)"); return 0; }
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_offlineimap};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/offlineimap"), "offlineimap");
        assert_eq!(basename(r"C:\bin\offlineimap.exe"), "offlineimap.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("offlineimap.exe"), "offlineimap");
