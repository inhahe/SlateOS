...
fn gen_kernel_file(name: &str) -> KernelResult<Vec<u8>> {
    match name {
        // These three are the same question `/proc/sys/kernel/*` and `uname(2)`
        // answer, and until 2026-08-22 this file answered it differently
        // (`MintOS` / `0.1.0-dev`).  All publishers now read `crate::uname`;
        // `self_test` below compares the generated bytes against procfs's so a
        // generator that stops doing that is caught here rather than by a
        // ported program aborting at start-up.
        "version" => Ok(format!("{}\n", crate::uname::VERSION).into_bytes()),
        "ostype" => Ok(format!("{}\n", crate::uname::SYSNAME).into_bytes()),
        "osrelease" => Ok(format!("{}\n", crate::uname::RELEASE).into_bytes()),
        "hostname" => {
            let h = hostname();
            Ok(format!("{h}\n").into_bytes())
        }
...
pub fn self_test() -> KernelResult<()> {
    ...
    // 2. Read kernel files, and check them against the *other* publisher of the
    //    same three strings rather than against literals.  Until 2026-08-22 this
    //    file answered `MintOS` / `0.1.0-dev` where `/proc/sys/kernel/*` and
    //    `uname(2)` answered `Linux` / `6.6.0-slateos`, and the old assertion
    //    here — `ostype.starts_with(b"MintOS")` — is what let it stay wrong: a
    //    test written against one generator's own literal can only ever agree
    //    with it.  Comparing generated bytes to generated bytes catches both a
    //    drifting literal and a generator that stops reading `crate::uname`,
    //    which asserting the constant here would not.
    let mut procfs = crate::fs::procfs::ProcFs::new();
    for (sys_path, proc_path, expected) in [
        (
            "/kernel/version",
            "/sys/kernel/version",
            crate::uname::VERSION,
        ...
        let ours = fs.read_file(Path::new(sys_path))?;
        let theirs = procfs.read_file(Path::new(proc_path))?;
        let want = format!("{expected}\n").into_bytes();
        assert!(
            ours == want && theirs == want,
            "uname surface disagrees: /sys{sys_path} = {ours:?}, /proc{proc_path} = {theirs:?}, \
             crate::uname says {want:?}"
        );
