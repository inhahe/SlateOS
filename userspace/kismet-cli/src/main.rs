...
//! kismet-cli — OurOS Kismet wireless network detector
//!
...
fn run_kismet(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("kismet v2023.07 (OurOS) — Wireless network detector/sniffer");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("kismet v2023.07 (OurOS)"); return 0; }
...
fn run_kismet_cap(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("kismet_cap_linux_wifi v2023.07 (OurOS) — Wi-Fi capture helper");
        return 0;
    }
    let _ = args;
