...
fn run_ecwid(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Ecwid by Lightspeed (Slate OS) — embeddable e-commerce store you can drop into any existing site"
        );
        println!();
        println!("Options:");
        ...
        println!(
            "  --instant              Instant Site (free landing page if you don't have a site)"
        );
    ...
    println!(
        "  Original: Ecwid Inc. (Ulyanovsk, Russia / Encinitas CA — founded 2009 by Ruslan Fazlyev)"
    );
    println!("  Acquired: by Lightspeed for $500M (mix cash+stock), closed Aug 2021");
    println!(
        "  Founders: Ruslan Fazlyev, 2009 (also founder of X-Cart, an earlier PHP cart platform)"
    );
    println!(
        "          built Ecwid as a JavaScript embed widget — radically different model from Shopify"
    );
    println!(
        "  Founded: 2009 in Ulyanovsk (founder relocated team to US post-2014 Ukraine tensions)"
    );
    ...
    println!(
        "          rebranded post-acquisition: 'Ecwid by Lightspeed' (kept Ecwid name as sub-brand)"
    );
    println!("  Strategic position: 'add a store to a site you already have':");
    println!(
        "                    DIFFERENT pitch from Shopify: 'don't replace your site, add commerce to it'"
    );
    println!(
        "                    primary competitor: Shopify Buy Button + Shopify Lite (discontinued), Snipcart"
    );
    println!(
        "                    competitor for SMBs: Square Online (free + Square processing), GoDaddy Online Store"
    );
    println!(
        "                    sweet spot: existing site (WordPress / Squarespace / Wix / Webflow / custom) wanting commerce"
    );
    println!(
        "                    post-Lightspeed: positioned as 'commerce extension of Lightspeed Retail + Restaurant POS'"
    );
    ...
    println!(
        "    Business — $39/mo — 2,500 products, abandoned cart, advanced reports, subscriptions"
    );
    ...
    println!(
        "            popular with: musicians selling merch, indie creators, café/bakery online ordering, hobbyist sellers"
    );
    println!(
        "            'most popular store on WordPress' for hobby-sellers who don't want WooCommerce complexity"
    );
    println!("            sweet spot: $0-$50K/yr GMV SMBs with existing site");
    println!(
        "  Critique: widget-based architecture limits design control vs full Shopify storefront"
    );
    ...
    println!(
        "           post-Lightspeed acquisition: roadmap focus shifted to omnichannel POS integration vs pure SaaS commerce growth"
    );
    println!(
        "           Lightspeed itself struggling with stock (~$10 vs $100+ peak) — affects long-term Ecwid investment confidence"
    );
    println!("           brand awareness much lower than Shopify or BigCommerce");
    println!(
        "  Differentiator: drop-in embeddable widget that adds commerce to ANY existing site + mobile apps generated for free + Lightspeed POS integration — for SMBs who don't want to migrate their site"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "ecwid".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ecwid, strip_ext};
