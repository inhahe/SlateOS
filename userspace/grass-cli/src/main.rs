...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/grass"), "grass");
        assert_eq!(basename(r"C:\bin\grass.exe"), "grass.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("grass.exe"), "grass");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_grass(&["--help".to_string()]), 0);
        assert_eq!(run_grass(&["-h".to_string()]), 0);
        assert_eq!(run_grass(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_grass(&[]), 0);
    }
}
