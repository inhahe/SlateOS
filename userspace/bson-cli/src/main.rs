...
fn run_bson(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("bsondump v1.0 (Slate OS) — BSON to JSON converter");
        println!();
        println!("Options:");
    ...
    println!(
        "{{\"_id\":{{\"$oid\":\"507f1f77bcf86cd799439011\"}},\"name\":\"test\",\"value\":42}}"
    );
    println!(
        "{{\"_id\":{{\"$oid\":\"507f1f77bcf86cd799439012\"}},\"name\":\"example\",\"value\":99}}"
    );
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "bsondump".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_bson, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/bson"), "bson");
        assert_eq!(basename(r"C:\bin\bson.exe"), "bson.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("bson.exe"), "bson");
