...
//! atlan-cli — OurOS Atlan (modern active metadata + data catalog, Bangalore)
...
fn run_atlan(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Atlan (OurOS) — modern data catalog + active metadata platform");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Atlan 2024 (OurOS)"); return 0; }
    println!("Atlan 2024 (OurOS) — Modern Data Catalog");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_atlan(&["--version".to_string()], "atlan"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_atlan(&[], "atlan"), 0);
    }
}
