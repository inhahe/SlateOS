...
fn run_setBfree(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("setBfree v0.8 (Slate OS) — DSP tonewheel organ emulator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("setBfree v0.8.12 (Slate OS)");
        return 0;
    }
    println!("setBfree v0.8.12 (Slate OS) — Tonewheel Organ");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "setBfree".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_setBfree, strip_ext};
