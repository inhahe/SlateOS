    ...
    match subcmd {
        "list" => {
            ...
            if json {
                println!("{{\"\":[");
                println!(
                    "  {{\"id\":0,\"type\":\"bluetooth\",\"device\":\"hci0\",\"type-desc\":\"Bluetooth\",\"soft\":\"unblocked\",\"hard\":\"unblocked\"}},"
                );
                println!(
                    "  {{\"id\":1,\"type\":\"wlan\",\"device\":\"phy0\",\"type-desc\":\"Wireless LAN\",\"soft\":\"unblocked\",\"hard\":\"unblocked\"}}"
                );
                println!("]}}");
            } else {
        ...
        _ => {
            eprintln!("rfkill: unknown command {}", quoteaf_os(subcmd));
            return 1;
        }
    }
    0
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "rfkill".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_rfkill, strip_ext};
