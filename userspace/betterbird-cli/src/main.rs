...
fn run_betterbird(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("betterbird v115.0 (Slate OS) — Enhanced Thunderbird email client");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("betterbird v115.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "betterbird".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_betterbird, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_betterbird(&["--version".to_string()], "betterbird");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_betterbird(&[], "betterbird");
    }
}
