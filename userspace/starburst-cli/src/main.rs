...
fn run_sb(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Starburst (Slate OS) — commercial Trino — query data wherever it lives (data lakehouse)"
        );
        println!();
        println!("Options:");
    ...
    println!(
        "  Founders: Justin Borgman (CEO), Kamil Bajda-Pawlikowski (CTO), Matt Fuller, Martin Traverso, David Phillips, 2017"
    );
    println!("          Borgman + Bajda-Pawlikowski had founded Hadapt (acquired Teradata 2014)");
    println!(
        "          Traverso + Phillips + Dain Sundstrom were Facebook engineers who created Presto (2012)"
    );
    println!(
        "          all four creators eventually forked Presto to form Trino (2020) over PrestoDB/Presto Software Foundation governance dispute"
    );
    ...
    println!(
        "                    primary competitor: Snowflake (proprietary warehouse), Databricks (Spark lakehouse), Dremio (Apache Iceberg lakehouse)"
    );
    println!(
        "                    AWS Athena, Google BigQuery Omni (federation features) compete on cross-cloud queries"
    );
    println!(
        "                    Starburst's wedge: 'open-format, your-data-anywhere' — query S3 + GCS + Azure + Snowflake + DB without moving data"
    );
    println!(
        "                    natural fit for: enterprises with multi-cloud sprawl, cost-conscious teams, regulatory data residency"
    );
    println!(
        "                    open-source Trino is foundational — Starburst is to Trino what Databricks is to Spark"
    );
    ...
    println!(
        "    Galaxy is the growth driver, Enterprise is the legacy on-prem/private-cloud business"
    );
    println!("  Core architecture (Trino + enterprise extensions):");
    println!("    - Massively parallel SQL execution engine");
    println!(
        "    - 50+ connectors (Snowflake, Postgres, MySQL, Oracle, Cassandra, MongoDB, S3+Iceberg/Hive/Delta/Parquet, ADLS, GCS, Hudi, Kafka, Pinot, Elasticsearch, Redshift)"
    );
    ...
    println!(
        "    - Apache Iceberg + Delta Lake + Hudi native support (formats-agnostic lakehouse)"
    );
    ...
    println!(
        "    - Iceberg is the modern open table format (vs Delta which is Databricks-controlled)"
    );
    ...
    println!(
        "    - Starburst Data are top maintainers of Trino (along with Bloomberg + ex-Facebook contributors)"
    );
    println!(
        "    - Trino used by: Lyft, Netflix (creator), Pinterest, LinkedIn, Twitter/X, Comcast"
    );
    println!(
        "    - Starburst's commercial extensions: more connectors, RBAC, federation, Warp Speed acceleration"
    );
    println!(
        "    - dual-license model — community Trino is Apache 2.0, Starburst extensions are commercial"
    );
    println!("  AI + GenAI:");
    ...
    println!(
        "              Databases: Postgres, MySQL, Oracle, MSSQL, Cassandra, MongoDB, DynamoDB"
    );
    ...
    println!(
        "            Comcast, Bloomberg (deep), Lufthansa, Société Générale, FINRA, AT&T, GE Aerospace"
    );
    println!("            Verizon, ABN AMRO, ING Bank, Allianz, Walmart Connect");
    println!(
        "            sweet spot: F1000 enterprises with multi-cloud + multi-format data sprawl"
    );
    println!(
        "            historically WEAK in: SMB (Snowflake/BigQuery win), pure-play SaaS startups"
    );
    println!(
        "            very strong in: Bloomberg-style finance (real federation needs), telco, retail, healthcare with EHR"
    );
    println!(
        "  Critique: still has Trino's complexity tax — query tuning + cluster sizing is non-trivial"
    );
    println!("           cost can spike if queries aren't optimized + clusters not auto-scaled");
    println!(
        "           catalog/governance features (Gravity) newer than Snowflake's Horizon/Polaris"
    );
    println!(
        "           SaaS Galaxy product younger than Snowflake — fewer enterprise-grade features"
    );
    ...
    println!(
        "           Iceberg vs Delta format wars: Databricks pushing Delta + Iceberg, Starburst leans Iceberg"
    );
    println!(
        "  Differentiator: federated SQL across ANY data source/format + Iceberg lakehouse leadership + Warp Speed acceleration + open-source Trino lineage — for F1000 enterprises with multi-cloud + multi-format sprawl"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "starburst".to_string());
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/starburst"), "starburst");
        assert_eq!(basename(r"C:\bin\starburst.exe"), "starburst.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("starburst.exe"), "starburst");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_sb(&["--help".to_string()], "starburst"), 0);
        assert_eq!(run_sb(&["-h".to_string()], "starburst"), 0);
        let _ = run_sb(&["--version".to_string()], "starburst");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_sb(&[], "starburst");
    }
}
