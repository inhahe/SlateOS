...
//! fwupd-cli — OurOS fwupd firmware update daemon
//!
...
fn run_fwupdmgr(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("fwupdmgr v1.9 (OurOS) — Firmware update manager");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("fwupdmgr v1.9 (OurOS, fwupd)"); return 0; }
...
fn run_fwupdtool(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("fwupdtool v1.9 (OurOS) — Firmware debugging tool");
        println!("  get-plugins      List plugins");
        println!("  get-details CAB  Show firmware details");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("fwupdtool v1.9 (OurOS)"); return 0; }
