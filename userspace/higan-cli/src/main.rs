...
fn run_higan(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("higan v115 (Slate OS) — Cycle-accurate multi-system emulator");
        println!();
        println!("Options:");
    ...
    if files.is_empty() {
        println!("higan v115 (Slate OS) — Cycle-Accurate Emulator");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "higan".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_higan, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_higan(&["--version".to_string()], "higan");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_higan(&[], "higan");
    }
}
