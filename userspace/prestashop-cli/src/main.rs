...
//! prestashop-cli — OurOS PrestaShop (French open-source e-commerce, EU favourite)
...
fn run_presta(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("PrestaShop (OurOS) — French open-source self-hosted e-commerce platform");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("PrestaShop 8.x (OurOS)"); return 0; }
    println!("PrestaShop 8.x (OurOS)");
