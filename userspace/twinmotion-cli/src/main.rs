...
fn run_tm(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Pricing: $499/yr (perpetual licenses still available) — FREE for students/educators"
    );
    ...
    println!(
        "    - Direct Link / Datasmith: live sync with Revit, ArchiCAD, SketchUp, Rhino, Vectorworks"
    );
    ...
    println!(
        "  Audience: architects, BIM users (Revit/ArchiCAD), urban planners, real-estate marketing"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "twinmotion".to_string());
