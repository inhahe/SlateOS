...
fn run_ntopng(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("ntopng v6.0 (Slate OS) — High-speed web-based traffic analysis");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("ntopng v6.0.0 (Slate OS)");
        return 0;
    }
    println!("ntopng v6.0.0 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "ntopng".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ntopng, strip_ext};
