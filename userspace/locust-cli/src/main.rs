...
fn run_locust(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Locust 2.29.0 (Slate OS) — Load testing tool");
        println!();
        println!("Options:");
    ...
    if headless {
        ...
        println!(
            " GET /                                  45     0(0%)  |   23    12    89      18   4.50"
        );
        println!(
            " GET /api/users                         45     0(0%)  |   34    15   123      28   4.50"
        );
        println!(
            " POST /api/login                        23     1(4%)  |   67    31   234      52   2.30"
        );
        println!(
            "----------------------------------------------------------------------------------------------"
        );
        println!(
            " Aggregated                            113     1(1%)  |   38    12   234      28  11.30"
        );
    } else {
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "locust".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_locust, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_locust(&["--version".to_string()]);
    }
