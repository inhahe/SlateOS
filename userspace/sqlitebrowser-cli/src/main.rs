...
fn run_sqlitebrowser(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("sqlitebrowser v3.12 (Slate OS)");
        return 0;
    }
    if let Some(db) = args.iter().find(|a| !a.starts_with('-')) {
        println!("sqlitebrowser: opening {}", quoteaf_os(db));
        println!("  Tables: 5");
        println!("  Views: 2");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "sqlitebrowser".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sqlitebrowser, strip_ext};
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_sqlitebrowser(&["--help".to_string()], "sqlitebrowser"),
            0
        );
