...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/nuxt"), "nuxt");
    ...
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("nuxt.exe"), "nuxt");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_nuxi(&["--help".to_string()]), 0);
        assert_eq!(run_nuxi(&["-h".to_string()]), 0);
        assert_eq!(run_nuxi(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_nuxi(&[]), 0);
    }
}
