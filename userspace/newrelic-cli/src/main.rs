...
fn run_newrelic(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("New Relic CLI 0.81.0 (Slate OS)");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/newrelic"), "newrelic");
        assert_eq!(basename(r"C:\bin\newrelic.exe"), "newrelic.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("newrelic.exe"), "newrelic");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_newrelic(&["--help".to_string()]), 0);
        assert_eq!(run_newrelic(&["-h".to_string()]), 0);
        let _ = run_newrelic(&["--version".to_string()]);
    }
