...
//! vernemq-cli — OurOS VerneMQ MQTT broker
//!
...
fn run_vernemq(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "vmq-admin" => {
                println!("vmq-admin (OurOS) — VerneMQ administration");
            ...
            "vmq-passwd" => {
                println!("vmq-passwd (OurOS) — VerneMQ password management");
                println!("  -c FILE USER       Create/update password file");
                println!("  -D FILE USER       Delete user from file");
            }
            _ => {
                println!("VerneMQ v2.0 (OurOS) — Distributed MQTT broker");
    ...
    if args.iter().any(|a| a == "--version") { println!("VerneMQ v2.0.1 (OurOS)"); return 0; }
    match prog {
        "vmq-admin" => {
