...
//! liquibase-cli — OurOS Liquibase CLI
//!
...
fn run_liquibase(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Liquibase database change management (OurOS).");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Liquibase 4.25.1 (OurOS)");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_liquibase(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_liquibase(vec![]), 0);
    }
}
