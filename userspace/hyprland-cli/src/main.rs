...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/hyprland"), "hyprland");
        assert_eq!(basename(r"C:\bin\hyprland.exe"), "hyprland.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("hyprland.exe"), "hyprland");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_hyprland(&["--help".to_string()], "hyprland"), 0);
        assert_eq!(run_hyprland(&["-h".to_string()], "hyprland"), 0);
        assert_eq!(run_hyprland(&["--version".to_string()], "hyprland"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_hyprland(&[], "hyprland"), 0);
    }
}
