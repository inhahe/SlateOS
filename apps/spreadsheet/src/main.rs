...
impl SpreadsheetApp {
    ...
    pub fn open_file_dialog(&mut self, saving: bool) {
        if saving {
            ...
            self.picker.open_to_write(self.offered_name(".csv"));
        } else {
    ...
    fn ask_where_to_save(&mut self, purpose: PickerFor) {
        ...
        let name = self.offered_name(".spreadsheet");
