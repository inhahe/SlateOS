...
//! stress-cli — OurOS stress system stress tester
//!
...
fn run_stress(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("stress v1.0 (OurOS) — System stress test tool");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("stress v1.0 (OurOS)"); return 0; }
...
fn run_stress_ng(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("stress-ng v0.17 (OurOS) — Advanced stress test tool");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("stress-ng v0.17 (OurOS)"); return 0; }
