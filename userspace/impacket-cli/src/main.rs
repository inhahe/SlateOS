...
fn run_impacket(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "secretsdump" => {
                println!("secretsdump (Slate OS) — Extract credentials from remote host");
            ...
            "ntlmrelayx" => {
                println!("ntlmrelayx (Slate OS) — NTLM relay attack");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Impacket v0.12.0 (Slate OS)");
        return 0;
    }
    match prog {
        "secretsdump" => {
            println!("secretsdump v0.12.0 (Slate OS)");
        ...
        "ntlmrelayx" => {
            println!("ntlmrelayx v0.12.0 (Slate OS)");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "psexec".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_impacket, strip_ext};
