...
fn run_sundials(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!(
            "SUNDIALS v7.0 (Slate OS) — Suite of nonlinear and differential/algebraic solvers"
        );
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("SUNDIALS v7.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "sundials-info".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sundials, strip_ext};
