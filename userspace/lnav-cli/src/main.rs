...
fn run_lnav(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("lnav v0.12 (Slate OS) — Log file navigator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("lnav v0.12 (Slate OS)"); return 0; }
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_lnav};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/lnav"), "lnav");
        assert_eq!(basename(r"C:\bin\lnav.exe"), "lnav.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("lnav.exe"), "lnav");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_lnav(&["--help".to_string()], "lnav"), 0);
        assert_eq!(run_lnav(&["-h".to_string()], "lnav"), 0);
        let _ = run_lnav(&["--version".to_string()], "lnav");
    }
