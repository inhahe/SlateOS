...
//! caribou-cli — OurOS Caribou on-screen keyboard
//!
...
fn run_caribou(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("caribou v0.4 (OurOS) — GNOME on-screen keyboard");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("caribou v0.4 (OurOS)"); return 0; }
...
fn run_preferences(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("caribou-preferences v0.4 (OurOS) — Caribou settings");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("caribou-preferences v0.4 (OurOS)"); return 0; }
