...
fn run_dbeaver(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("DBeaver CE 24.0 (Slate OS) — Universal database manager");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("DBeaver CE 24.0.0 (Slate OS)");
        return 0;
    }
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_dbeaver};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/dbeaver"), "dbeaver");
        assert_eq!(basename(r"C:\bin\dbeaver.exe"), "dbeaver.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("dbeaver.exe"), "dbeaver");
