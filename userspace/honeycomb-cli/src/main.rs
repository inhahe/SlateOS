...
fn run_hc(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Honeycomb.io (Slate OS) — Observability for distributed systems");
        println!();
        println!("Options:");
    ...
    println!(
        "    - Refinery — open-source tail-sampling proxy (keep interesting traces, drop boring)"
    );
    println!(
        "  Culture: Charity Majors evangelizes 'observability != monitoring' + 'test in production'"
    );
    println!("          o11y book ('Observability Engineering' O'Reilly 2022)");
    println!(
        "  Differentiator: built for unknown-unknowns; query any dimension after-the-fact, no pre-aggregation"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "honeycomb".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_hc, strip_ext};
