...
fn run_gitea(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Gitea 1.22 (Slate OS) — Painless self-hosted Git service");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Gitea v1.22.3 (Slate OS)");
        return 0;
    }
    println!("Gitea v1.22.3 (Slate OS)");
    ...
    println!(
        "                    single Go binary, embedded assets, SQLite/MySQL/Postgres backend"
    );
    println!("  Features:");
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gitea, strip_ext};
