...
fn run_pgbouncer(args: Vec<String>) -> i32 {
    ...
    let config = args
        ...
        .map(|s| s.as_str())
        .unwrap_or("/etc/pgbouncer/pgbouncer.ini");
    println!(
        "2025-05-22 10:00:00.000 UTC [12345] LOG kernel file descriptor limit: 1024 (hard: 1048576); max_client_conn: 100, max expected fd use: 112"
    );
    ...
    println!(
        "2025-05-22 10:00:00.003 UTC [12345] LOG process up: PgBouncer 1.22.1, libevent 2.1.12-stable"
    );
    println!(
        "2025-05-22 10:00:00.004 UTC [12345] LOG Config loaded: {}",
        config
    );
    println!();
    println!("Databases:");
    ...
    println!("Stats:");
    println!(
        "  database     total_xact_count  total_query_count  total_server_time  avg_xact_time  avg_query_time"
    );
    println!(
        "  myapp        142890            568234             12345678           0.086          0.021"
    );
    println!(
        "  analytics    8923              34521              8765432            0.982          0.253"
    );
    0
}
