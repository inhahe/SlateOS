...
//! svox-cli — OurOS SVOX pico text-to-speech engine
//!
...
fn run_pico2wave(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("pico2wave v1.0 (OurOS) — SVOX Pico TTS wave file generator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("pico2wave v1.0 (OurOS, SVOX Pico engine)"); return 0; }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_pico2wave(&["--version".to_string()], "svox"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_pico2wave(&[], "svox"), 0);
    }
}
