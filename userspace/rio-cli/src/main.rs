...
fn run_rio(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Rio 0.1.10 (Slate OS) — Hardware-accelerated GPU terminal");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("rio 0.1.10 (Slate OS)");
        return 0;
    }
    let title = args
        .windows(2)
        .find(|w| w[0] == "--window-title")
        .map(|w| w[1].as_str());
    if let Some(t) = title {
        println!("rio: Starting with title {}...", quoteaf_os(t));
    } else {
...
#[cfg(test)]
mod tests {
    use super::{basename, run_rio, strip_ext};
