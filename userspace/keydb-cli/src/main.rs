...
//! keydb-cli — OurOS KeyDB multithreaded Redis fork
//!
...
fn run_keydb(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "keydb-cli" => {
                println!("keydb-cli (OurOS) — KeyDB command-line client");
            ...
            _ => {
                println!("KeyDB v6.3 (OurOS) — Multithreaded Redis fork");
    ...
    if args.iter().any(|a| a == "--version") { println!("KeyDB v6.3.4 (OurOS)"); return 0; }
    match prog {
        "keydb-cli" => {
