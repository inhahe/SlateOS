...
fn run_ac(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Options:");
        println!(
            "  --lite                 Lite — email marketing starter ($15/mo for 1K contacts)"
        );
    ...
    println!(
        "                    competes with Mailchimp + Klaviyo + Constant Contact (less automation depth)"
    );
    ...
    println!(
        "    Professional (1K contacts) — $79/mo (predictive sending + attribution + split testing)"
    );
    println!(
        "    Enterprise (1K contacts) — $145/mo (single sign-on + custom report + dedicated rep)"
    );
    ...
    println!(
        "    - Triggers: site visit, form fill, tag added, email opened, deal stage changed, custom field, anniversary"
    );
    println!(
        "    - Actions: send email, add/remove tag, update field, notify sales rep, wait, if/else, webhook, goal"
    );
    println!(
        "    - Conditional branching: 'if user opened last email AND visited pricing page' → branch"
    );
    println!("    - Site tracking + event tracking integrate with automations");
    println!(
        "    - 800+ pre-built automation recipes shared by community in 'Automation Marketplace'"
    );
    ...
    println!(
        "            very strong in: coaches/consultants, course creators, agencies, indie e-commerce"
    );
    println!("            agency-friendly: white-label + multi-account dashboard");
    println!(
        "  Critique: UI shows its age (still rapidly improving) — feels less modern than HubSpot"
    );
    println!("           reporting is functional but not as polished as Klaviyo or HubSpot");
    println!(
        "           predictive features less sophisticated than Braze/Iterable at enterprise scale"
    );
    println!(
        "           deliverability good for SMB but lower-tier than Klaviyo for high-volume e-commerce"
    );
    ...
    println!(
        "  Differentiator: best automation builder at SMB pricing + integrated CRM + transparent pricing — 'pro automation for indie businesses'"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "activecampaign".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ac, strip_ext};
