...
fn run_simple_scan(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("simple-scan v42.0 (Slate OS) — Simple document scanning");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("simple-scan v42.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "simple-scan".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_simple_scan, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_simple_scan(&["--version".to_string()], "simple-scan");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_simple_scan(&[], "simple-scan");
    }
}
