...
- [ ] OS-level image codec support (all common formats apps can decode/encode via system API):
  - [-] *(Lane F 2026-09-25: PNG, JPEG baseline and progressive, GIF with its animations, WebP -- lossless, lossy, with alpha, and animated -- BMP and ICO decode in `gui/imagecodec`, and TIFF except its fax and JPEG kinds; the others do not yet.)* JPEG, PNG, GIF (animated), BMP, TIFF, WebP, AVIF, HEIC/HEIF, ICO, SVG
