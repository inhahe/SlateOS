...
//! hightouch-cli — OurOS Hightouch (the reverse-ETL category creator)
...
fn run_ht(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Hightouch (OurOS) — reverse-ETL platform (warehouse → SaaS apps)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Hightouch 2024 (OurOS)"); return 0; }
    println!("Hightouch 2024 (OurOS)");
