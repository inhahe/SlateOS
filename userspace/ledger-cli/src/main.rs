...
fn run_ledger(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Ledger Live CLI v2.78 (Slate OS) — Hardware wallet manager");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("Ledger Live CLI v2.78 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "ledger-live".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ledger, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/ledger"), "ledger");
        assert_eq!(basename(r"C:\bin\ledger.exe"), "ledger.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("ledger.exe"), "ledger");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_ledger(&["--help".to_string()], "ledger"), 0);
        assert_eq!(run_ledger(&["-h".to_string()], "ledger"), 0);
        let _ = run_ledger(&["--version".to_string()], "ledger");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_ledger(&[], "ledger");
    }
}
