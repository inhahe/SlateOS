...
run_side() {
  ...
  if [ "$stdin" = "-" ]; then
    env ${ENVV[@]+"${ENVV[@]}"} PATH="$bindir/$side" cut "$@" </dev/null >"$out" 2>"$err"
  else
    printf '%b' "$stdin" | env ${ENVV[@]+"${ENVV[@]}"} PATH="$bindir/$side" cut "$@" >"$out" 2>"$err"
  fi
}
...
run_case()  { compare - "$@"; report "${ENVV[*]:+${ENVV[*]} }cut $*"; }
run_stdin() {
  local input="$1"; shift
  compare "$input" "$@"
...
run_case -c1 posix.txt -z
run_case -c1 -z posix.txt
run_case -c1 posix.txt -- -z
