...
//! psensor-cli — OurOS Psensor hardware temperature monitor
//!
...
fn run_psensor(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("psensor v1.2 (OurOS) — Hardware temperature monitor (GUI)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("psensor v1.2 (OurOS)"); return 0; }
...
fn run_server(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("psensor-server v1.2 (OurOS) — Remote temperature monitoring server");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("psensor-server v1.2 (OurOS)"); return 0; }
