...
fn run_gamemoderun(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!();
        println!("gamemoderun — run a program with GameMode optimizations (Slate OS).");
...
fn run_gamemoded(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "-v") {
        println!("gamemode daemon version 1.8.1 (Slate OS)");
        return 0;
    }
    ...
    println!("gamemoded: starting daemon (Slate OS)");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "gamemoderun".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gamemoderun, strip_ext};
