...
//! humio-cli — OurOS Humio / CrowdStrike Falcon LogScale (log-management, Aarhus DK / Sunnyvale CA)
...
fn run_humio(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Humio (now Falcon LogScale) (OurOS) — index-free log management (CrowdStrike subsidiary)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Falcon LogScale 2024 (was Humio, OurOS)"); return 0; }
    println!("Humio / Falcon LogScale 2024 (OurOS) — Index-Free Log Management");
