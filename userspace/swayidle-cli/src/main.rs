...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "swayidle".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_swayidle, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/swayidle"), "swayidle");
        assert_eq!(basename(r"C:\bin\swayidle.exe"), "swayidle.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("swayidle.exe"), "swayidle");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_swayidle(&["--help".to_string()], "swayidle"), 0);
        assert_eq!(run_swayidle(&["-h".to_string()], "swayidle"), 0);
        let _ = run_swayidle(&["--version".to_string()], "swayidle");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_swayidle(&[], "swayidle");
    }
}
