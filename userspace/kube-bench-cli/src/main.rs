...
//! kube-bench-cli — OurOS kube-bench CIS Kubernetes benchmark
//!
...
fn run_kube_bench(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("kube-bench v0.7.3 (OurOS) — CIS Kubernetes Benchmark");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("kube-bench v0.7.3 (OurOS)");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_kube_bench(&["--version".to_string()], "kube-bench"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_kube_bench(&[], "kube-bench"), 0);
    }
}
