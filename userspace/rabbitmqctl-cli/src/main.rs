...
//! rabbitmqctl-cli — Slate OS RabbitMQ management CLI
//!
...
fn run_rabbitmqctl(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("rabbitmqctl 3.13.0 (Slate OS) — RabbitMQ management CLI");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("rabbitmqctl 3.13.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "rabbitmqctl".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_rabbitmqctl, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/rabbitmqctl"), "rabbitmqctl");
        assert_eq!(basename(r"C:\bin\rabbitmqctl.exe"), "rabbitmqctl.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("rabbitmqctl.exe"), "rabbitmqctl");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_rabbitmqctl(&["--help".to_string()], "rabbitmqctl"), 0);
        assert_eq!(run_rabbitmqctl(&["-h".to_string()], "rabbitmqctl"), 0);
        let _ = run_rabbitmqctl(&["--version".to_string()], "rabbitmqctl");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_rabbitmqctl(&[], "rabbitmqctl");
    }
}
