...
//! TCP connections, UDP sockets, listeners, interface stats,
//! routing, ARP cache, DNS cache, and active services.
//!
//! ## Usage
//!
//! ```text
...
pub fn format_udp_sockets(entries: &[UdpEntry]) -> String {
    let mut out = String::with_capacity(entries.len().saturating_mul(60));
    out.push_str("Proto  Local Address          Rx Queue  Mcast Groups\n");
    out.push_str("─────  ─────────────────────  ────────  ────────────\n");
    ...
    for e in entries {
        ...
        out.push_str(&format!(
            "udp    {:<21}  {:>8}  {:>12}\n",
            local, e.rx_queue, e.mcast_groups
        ));
    }
    out
}
...
pub fn format_interface_stats() -> String {
    ...
    out.push('\n');
    out.push_str("  RX packets: {:<10}  bytes: {}\n");
    // Use actual values.
    let rx_line = format!(
        "  RX packets: {:<10}  bytes: {}\n",
        stats.rx_packets, stats.rx_bytes
    );
    // Clear the template line and add the actual one.
    out.truncate(out.len() - "  RX packets: {:<10}  bytes: {}\n".len());
    out.push_str(&rx_line);
    out.push_str(&format!(
        "  TX packets: {:<10}  bytes: {}\n",
...
pub fn self_test() -> KernelResult<()> {
    ...
    // --- Test 6: UDP formatting ---
    {
        ...
        assert!(formatted.contains("5"), "rx queue");
