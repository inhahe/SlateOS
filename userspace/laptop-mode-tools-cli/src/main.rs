...
//! laptop-mode-tools-cli — Slate OS laptop-mode-tools power savings
//!
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "laptop_mode".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_laptop_mode, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/laptop-mode-tools"), "laptop-mode-tools");
        assert_eq!(
            basename(r"C:\bin\laptop-mode-tools.exe"),
            "laptop-mode-tools.exe"
        );
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_laptop_mode(&["--help".to_string()], "laptop-mode-tools"),
            0
        );
