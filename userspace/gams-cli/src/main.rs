...
fn run_gamside(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("GAMS IDE 45.1.0 (Slate OS)");
        return 0;
    }
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gams, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/gams"), "gams");
        assert_eq!(basename(r"C:\bin\gams.exe"), "gams.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("gams.exe"), "gams");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_gams(&["--help".to_string()]), 0);
        assert_eq!(run_gams(&["-h".to_string()]), 0);
        let _ = run_gams(&["--version".to_string()]);
    }
