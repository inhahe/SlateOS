...
fn run_phpstan(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("PHPStan 1.11.7 (Slate OS)");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/phpstan"), "phpstan");
        assert_eq!(basename(r"C:\bin\phpstan.exe"), "phpstan.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("phpstan.exe"), "phpstan");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_phpstan(&["--help".to_string()]), 0);
        assert_eq!(run_phpstan(&["-h".to_string()]), 0);
        let _ = run_phpstan(&["--version".to_string()]);
    }
