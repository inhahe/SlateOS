...
fn run_gog(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("GOG Galaxy 2.0.78 (Slate OS) — DRM-FREE game store + universal launcher");
        println!();
        println!("Options:");
        println!(
            "  --library              Unified library (GOG + Steam + Epic + Xbox + PSN + Origin etc.)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("GOG Galaxy 2.0.78.10 (Slate OS)");
        return 0;
    }
    println!("GOG Galaxy 2.0.78.10 (Slate OS)");
    ...
    println!(
        "  Launched: GOG.com 2008 (originally 'Good Old Games', dropped name for broader scope)"
    );
    println!("  Philosophy: DRM-FREE. Every game runs offline forever, no online activation");
    println!(
        "             Offline installers downloadable: keep .exe/.bin files, install anywhere"
    );
    println!("  Pricing: regional, often discounted for low-income regions (PPP pricing)");
    println!(
        "  Catalog: classics (Heroes of Might & Magic, Fallout 1/2, Baldur's Gate I/II, etc.)"
    );
    println!("          + indies + AAA where studios agree to DRM-free (CDPR's own games, etc.)");
    println!(
        "  Famous absent: most major AAA publishers refuse DRM-free → narrower catalog vs Steam"
    );
    println!(
        "  GOG Galaxy 2.0: launched 2019 — universal launcher that AGGREGATES all your stores"
    );
    println!(
        "                  into one library (Steam, Epic, Xbox, PSN, Switch, Battle.net, Origin)"
    );
    ...
    println!(
        "  Differentiator: DRM-free preservation + universal launcher for all your other stores"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gog, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_gog(&["--version".to_string()], "gog");
    }
