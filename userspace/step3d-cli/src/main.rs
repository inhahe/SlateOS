...
fn run_step3d(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("step3d v1.0 (Slate OS) — STEP/IGES 3D file viewer");
        println!();
        println!("Options:");
    ...
    let file = args.last().map(|s| s.as_str()).unwrap_or("model.step");
    println!("step3d: viewing {}...", quoteaf_os(file));
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "step3d".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_step3d, strip_ext};
