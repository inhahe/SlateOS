...
fn run_rancher(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("SUSE Rancher 2.9 (Slate OS) — Kubernetes management platform");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("rancher version v2.9.2 (Slate OS)"); return 0; }
    println!("SUSE Rancher 2.9.2 (Slate OS)");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_rancher};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/rancher"), "rancher");
        assert_eq!(basename(r"C:\bin\rancher.exe"), "rancher.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("rancher.exe"), "rancher");
