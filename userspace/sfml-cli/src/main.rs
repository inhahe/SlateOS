...
fn run_sfml_config(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("SFML config 2.6.1 (Slate OS)");
        println!();
        println!("Options:");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "sfml-config".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sfml_config, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_sfml_config(&["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_sfml_config(&[]);
    }
}
