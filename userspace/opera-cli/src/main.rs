...
fn run_op(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Features: built-in VPN (proxy), ad blocker, messengers sidebar (WA/Telegram/IG/FB),"
    );
    println!("            Workspaces, Pinboards, Aria (GPT-based assistant since 2023)");
    println!(
        "  Opera GX: hardware limiters (RAM/CPU/network caps), Twitch/Discord widgets, RGB themes"
    );
...
#[cfg(test)]
mod tests {
    use super::{basename, run_op, strip_ext};
