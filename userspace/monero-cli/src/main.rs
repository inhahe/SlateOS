...
fn run_monero(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Monero Wallet CLI v0.18.3 (Slate OS)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Monero 'Fluorine Fermi' (v0.18.3.3-release) (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "monero-wallet-cli".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_monero, strip_ext};
