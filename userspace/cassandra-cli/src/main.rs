...
fn run_cqlsh(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Cassandra CQL shell (Slate OS).");
        println!();
        println!("Options:");
...
fn run_nodetool(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Cassandra node management tool (Slate OS).");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/cassandra"), "cassandra");
        assert_eq!(basename(r"C:\bin\cassandra.exe"), "cassandra.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("cassandra.exe"), "cassandra");
