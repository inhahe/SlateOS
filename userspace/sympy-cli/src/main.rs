...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/sympy"), "sympy");
        assert_eq!(basename(r"C:\bin\sympy.exe"), "sympy.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("sympy.exe"), "sympy");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_sympy(&["--help".to_string()]), 0);
        assert_eq!(run_sympy(&["-h".to_string()]), 0);
        assert_eq!(run_sympy(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_sympy(&[]), 0);
    }
}
