...
//! crda-cli — OurOS CRDA wireless regulatory domain agent
//!
...
fn run_crda(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("crda v4.15 (OurOS) — Central Regulatory Domain Agent");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("crda v4.15 (OurOS)"); return 0; }
...
fn run_regdbdump(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("regdbdump v4.15 (OurOS) — Dump wireless regulatory database");
        return 0;
    }
    let _ = args;
