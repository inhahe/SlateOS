...
//! ramp-cli — OurOS Ramp (corporate cards + spend mgmt, fastest-growing fintech)
//!
...
fn run_ramp(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Ramp (OurOS) — Corporate cards + expense + bill pay + accounting automation");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Ramp 2024 (OurOS)"); return 0; }
    println!("Ramp 2024 (OurOS)");
