...
//! trino-cli — OurOS Trino (open-source distributed SQL query engine, ex-PrestoSQL)
...
fn run_trino(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Trino (OurOS) — open-source distributed SQL query engine (formerly PrestoSQL)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Trino 445 (OurOS)"); return 0; }
    println!("Trino 445 (OurOS) — Open-source distributed SQL");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_trino(&["--version".to_string()], "trino"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_trino(&[], "trino"), 0);
    }
}
