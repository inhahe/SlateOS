...
fn run_lum(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Options:");
        println!(
            "  --import FILE          Import (SketchUp/Revit/ArchiCAD/Rhino/3ds/FBX/Collada/SKP)"
        );
        println!(
            "  --livesync             LiveSync (real-time bidirectional with Revit/SketchUp/Rhino/ArchiCAD)"
        );
        println!(
            "  --library              Lumion Content Library (1000s of trees/people/cars/furniture)"
        );
    ...
    println!(
        "  Pricing: Lumion Pro €1499/yr, Lumion Standard €749/yr (NL pricing, regional variance)"
    );
    ...
    println!(
        "         hybrid raytracing added Lumion 12, full path-traced still images Lumion 2023+"
    );
    println!(
        "  Workflow: import CAD → place trees + people from library → animate camera → render in MINUTES"
    );
    ...
    println!(
        "  Library: 6800+ models, 1300+ materials, 1300+ sound effects, weather/time-of-day presets"
    );
    println!(
        "  Effects: real-time DOF, volumetric clouds, fog, snow/rain, blooming sun, lens flares"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "lumion".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_lum, strip_ext};
