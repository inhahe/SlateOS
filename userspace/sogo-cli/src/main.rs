...
fn run_sogo(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "sogo-tool" => {
                println!("sogo-tool (Slate OS) — SOGo admin utility");
            ...
            _ => {
                println!("SOGo v5.9 (Slate OS) — Groupware server");
    ...
    if args.iter().any(|a| a == "--version") { println!("SOGo v5.9.1 (Slate OS)"); return 0; }
    match prog {
        "sogo-tool" => {
