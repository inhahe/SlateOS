...
fn run_hcloud(args: Vec<String>) -> i32 {
    ...
    match cmd {
        "context" => match sub {
            ...
            _ => {
                println!("hcloud context {}: see --help.", sub);
            }
        },
        "server" => match sub {
            "list" => {
                ...
                println!(
                    "12345678  my-server     running   116.202.12.34   2a01:4f8:c0c:1234::/64  fsn1-dc14"
                );
                println!(
                    "23456789  web-server    running   116.202.56.78   2a01:4f8:c0c:5678::/64  nbg1-dc3"
                );
            }
            "create" => {
                let name = args
            ...
            _ => {
                println!("hcloud server {}: see --help.", sub);
            }
        },
        "volume" => match sub {
            "list" => {
            ...
            _ => {
                println!("hcloud volume {}: see --help.", sub);
            }
        },
        "server-type" => {
            println!(
                "ID   NAME         DESCRIPTION                CORES  MEMORY    DISK   PRICE (MONTHLY)"
            );
            println!(
                "1    cx11         CX11                       1      2.0 GB    20 GB  EUR 3.98"
            );
            println!(
                "3    cx21         CX21                       2      4.0 GB    40 GB  EUR 5.83"
            );
            println!(
                "5    cx31         CX31                       2      8.0 GB    80 GB  EUR 10.49"
            );
            println!(
                "7    cx41         CX41                       4      16.0 GB   160 GB EUR 18.69"
            );
            println!(
                "22   cpx11        CPX11 (AMD)                2      2.0 GB    40 GB  EUR 4.35"
            );
        }
        "datacenter" => {
...
#[cfg(test)]
mod tests {
    use super::run_hcloud;

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_hcloud(vec!["--help".to_string()]), 0);
        assert_eq!(run_hcloud(vec!["-h".to_string()]), 0);
        let _ = run_hcloud(vec!["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_hcloud(vec![]);
    }
}
