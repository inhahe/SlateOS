...
//! skype-cli — OurOS Microsoft Skype consumer VoIP
//!
...
fn run_sk(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Skype (OurOS) — Microsoft Skype consumer VoIP / video calls");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Skype 8.130.0.207 (OurOS)"); return 0; }
    println!("Skype 8.130.0.207 (OurOS)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_sk(&["--version".to_string()], "skype"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_sk(&[], "skype"), 0);
    }
}
