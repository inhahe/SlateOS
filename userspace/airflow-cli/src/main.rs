...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_airflow};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/airflow"), "airflow");
        assert_eq!(basename(r"C:\bin\airflow.exe"), "airflow.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("airflow.exe"), "airflow");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_airflow(&["--help".to_string()]), 0);
        assert_eq!(run_airflow(&["-h".to_string()]), 0);
        assert_eq!(run_airflow(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_airflow(&[]), 0);
    }
}
