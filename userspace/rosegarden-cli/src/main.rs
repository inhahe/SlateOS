...
fn run_rosegarden(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Rosegarden v23 (Slate OS) — MIDI sequencer and music notation editor");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Rosegarden v23.12 (Slate OS)"); return 0; }
    println!("Rosegarden v23.12 (Slate OS) — MIDI Sequencer");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_rosegarden};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/rosegarden"), "rosegarden");
        assert_eq!(basename(r"C:\bin\rosegarden.exe"), "rosegarden.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("rosegarden.exe"), "rosegarden");
