...
fn run_sameboy(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("SameBoy v0.16 (Slate OS) — Accurate Game Boy / Game Boy Color emulator");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/sameboy"), "sameboy");
        assert_eq!(basename(r"C:\bin\sameboy.exe"), "sameboy.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("sameboy.exe"), "sameboy");
