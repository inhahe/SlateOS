...
//! beyondidentity-cli — OurOS Beyond Identity (passwordless phishing-resistant MFA, Netscape co-founders)
...
fn run_bi(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Beyond Identity (OurOS) — passwordless phishing-resistant MFA on device-bound credentials");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Beyond Identity 2024 (OurOS) — beyondidentity-cli (REST + bi-cli)"); return 0; }
    println!("Beyond Identity 2024 (OurOS) — Passwordless Phishing-Resistant Auth (Netscape Pedigree)");
