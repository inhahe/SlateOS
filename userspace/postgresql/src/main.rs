...
//! postgresql — OurOS PostgreSQL relational database
//!
...
fn run_postgres(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("postgres (PostgreSQL) 16.3 (OurOS)");
        return 0;
    }
    let port = args.iter().position(|a| a == "-p")
        ...
        .unwrap_or(5432);
    println!("2025-05-22 10:00:00.000 UTC [12345] LOG:  starting PostgreSQL 16.3 (OurOS) on x86_64");
...
fn run_psql(args: Vec<String>) -> i32 {
    ...
    // Interactive mode
    println!("psql (16.3 OurOS)");
    println!("Type \"help\" for help.");
    println!();
    println!("postgres=# SELECT version();");
    println!("                                   version");
    println!("-----------------------------------------------------------------------------");
    println!(" PostgreSQL 16.3 (OurOS) on x86_64, compiled by rustc, 64-bit");
    println!("(1 row)");
    println!();
    println!("postgres=# \\q");
    0
...
fn run_pg_dump(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("pg_dump (PostgreSQL) 16.3 (OurOS)");
        return 0;
    }
    ...
    println!("--");
    println!();
    println!("-- Dumped from database version 16.3 (OurOS)");
    println!("-- Dumped by pg_dump version 16.3 (OurOS)");
    println!();
...
fn run_pg_restore(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("pg_restore (PostgreSQL) 16.3 (OurOS)");
        return 0;
    }
...
fn run_createdb(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("createdb (PostgreSQL) 16.3 (OurOS)");
        return 0;
    }
...
fn run_dropdb(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("dropdb (PostgreSQL) 16.3 (OurOS)");
        return 0;
    }
...
fn run_pg_isready(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("pg_isready (PostgreSQL) 16.3 (OurOS)");
        return 0;
    }
...
fn run_initdb(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("initdb (PostgreSQL) 16.3 (OurOS)");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_postgres(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_postgres(vec![]), 0);
    }
}
