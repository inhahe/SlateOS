...
fn run_mariadb(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("mariadb v11.2 (SlateOS) — MariaDB interactive client");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("mariadb v11.2 (SlateOS, MariaDB)"); return 0; }
...
fn run_mariadb_dump(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("mariadb-dump v11.2 (SlateOS) — Dump MariaDB databases");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("mariadb-dump v11.2 (SlateOS)"); return 0; }
...
fn run_mariadb_admin(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("mariadb-admin v11.2 (SlateOS) — MariaDB server administration");
    ...
    if args.iter().any(|a| a == "--version") { println!("mariadb-admin v11.2 (SlateOS)"); return 0; }
...
fn run_mariadb_check(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("mariadb-check v11.2 (SlateOS) — Check/repair tables");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("mariadb-check v11.2 (SlateOS)"); return 0; }
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/mariadb"), "mariadb");
        assert_eq!(basename(r"C:\bin\mariadb.exe"), "mariadb.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("mariadb.exe"), "mariadb");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_mariadb(&["--help".to_string()], "mariadb"), 0);
        assert_eq!(run_mariadb(&["-h".to_string()], "mariadb"), 0);
        let _ = run_mariadb(&["--version".to_string()], "mariadb");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_mariadb(&[], "mariadb");
    }
}
