...
## [A] One unwired module in lane C's tree is blocking all three lanes — raised 2026-09-11

**In short:** a safety check refuses to build the project when it finds a code file that
nothing else uses. It has found one, in another agent's area, and has been refusing since
about 18:55. Because of that, no agent can finish a test run, and a second check then
refuses to upload anyone's work. The fix is one line and takes a minute, but all three
versions of it are in the other agent's files, and the one I could reach is specifically
forbidden. So I need either that agent to act, or your permission to do the forbidden
thing. Nothing is broken or at risk — my work is saved and uploaded — what is stopped is
*checking* it.

### The chain, because each link is ordinary and the total is not

1. `gui/appearance/src/surface.rs` defines public items no other file names. The
   `scan-orphan-modules` gate refuses the build — correctly; its whole purpose is that
   `cargo build` cannot warn about an unused `pub` item and the test suite still passes.
2. That gate runs **after** the full workspace build, so every attempt costs ~8.5 minutes
   first. It is on `origin/main`, so it hits all three lanes.
3. No boot test can complete — including the **release-profile** one.
4. The `pre-push` hook refuses to push once 100 kernel-touching commits have passed since
   the last release boot. We are at 105. Its stated remedy is the release boot test that
   (3) has made impossible.

I pushed using the hook's own documented `ALLOW_STALE_RELEASE=1` override, so no work is
at risk. I still owe that release boot test.

### Why I have not just fixed it

The gate offers three outs and all three are lane C's: wire it up, delete it, or add it
to `scripts/orphan-modules-baseline.txt`. The first two are their code. The third I could
do in one line, and it is the one thing that file forbids outright — its header says the
count *"may fall, never rise"* and that an edit which **adds** a line is *"the failure
this gate exists to prevent, committed by hand"*. Clearing my own path by committing the
exact failure a guard exists to catch is not a trade I will make without you saying so.

Two notices sent to lane C, the second spelling out the push consequence. No response in
~1.5 hours, which most likely means that session is not running.

### The options

* **Start or prompt lane C (recommended).** They wire, delete or baseline it with the
  reason only they know.
  *What changes:* everything clears in a minute and no rule is bent.
* **Authorise me to baseline it on their behalf.** One line naming the module, with a
  comment saying lane A added it, why, and that lane C should replace it with a real
  resolution.
  *What changes:* all three lanes unblock immediately; the debt ledger gains an entry it
  is designed never to gain, and a real finding about lane C's code is silenced until
  someone removes it again.
* **Leave it.** I keep doing work that needs no boot test.
  *What changes:* nothing clears; kernel changes keep accumulating compile-verified but
  untested, and that backlog is the thing that gets expensive, because a boot test that
  finally runs against 15 unverified commits cannot tell you which one broke it.

### If this is never answered

Nothing degrades on its own and nothing is lost. The cost is entirely in accumulation: I
have 14 commits that are compile-verified only, and `cargo check` is the *only* host-side
verification a kernel change can get here — `kernel/Cargo.toml` sets `test = false`
because the kernel provides its own `panic_impl`, so `cargo test -p kernel` runs zero
tests and exits 0. The boot test is not one way to verify kernel work; it is the only
way. Each further unverified commit makes the eventual first green run less informative.
