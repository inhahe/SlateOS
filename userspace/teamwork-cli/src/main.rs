...
fn run_tw(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Teamwork.com (Slate OS) — project management + PSA for client-services agencies");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Teamwork.com 2024 (Slate OS)");
        return 0;
    }
    println!("Teamwork.com 2024 (Slate OS)");
    ...
    println!(
        "          both Cork-based developers, built Teamwork as internal PM for their consultancy Digital Crew"
    );
    println!("          spun out Teamwork as standalone SaaS 2007");
    println!(
        "          legendary Irish bootstrapped success — pre-Stripe, pre-cloud, just shipped"
    );
    println!("  Founded: 2007 in Cork, Ireland (still HQ'd there)");
    println!(
        "          bootstrapped — no outside funding until 2022 ~$70M Series A from Atlassian-Coro"
    );
    println!(
        "          actually it was Cervin Ventures + private investor, not Atlassian — kept independent"
    );
    println!("          ~$100M+ ARR estimated, profitable, ~600 employees");
    println!(
        "          rebrand: was 'Teamwork Projects' + 'Teamwork CRM' + 'Teamwork Desk' as suite, now consolidated under teamwork.com"
    );
    println!("  Strategic position: 'PM purpose-built for agencies billing client work':");
    println!(
        "                    primary competitor: Asana, Monday, ClickUp, Wrike — none as agency-focused"
    );
    ...
    println!(
        "                    legendary: free unlimited client + collaborator logins (unique pricing wedge)"
    );
    ...
    println!(
        "    Grow — $19.99/user/mo (most popular: project budgets, advanced reports, custom fields)"
    );
    ...
    println!(
        "    add-ons: Teamwork CRM, Teamwork Desk (helpdesk), Spaces (docs), Chat — extra per-user"
    );
    ...
    println!(
        "            heavily indie/mid-size agencies, consultancies, design studios, dev shops"
    );
    ...
    println!(
        "            global, but very strong in: Ireland, UK, Australia (founder's geographic ties)"
    );
    println!("            not focused on F1000 enterprise (Wrike/Workfront/Smartsheet win there)");
    println!(
        "  Critique: UI shows its age vs Asana/Monday/ClickUp (improving but still feels 2017-era)"
    );
    ...
    println!(
        "  Differentiator: PM purpose-built for agencies + free unlimited client logins + native time tracking + budgets + invoicing — best PM for agencies billing client work"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "teamwork".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_tw, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_tw(&["--version".to_string()], "teamwork");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_tw(&[], "teamwork");
    }
}
