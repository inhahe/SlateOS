...
fn run_urbackup_client(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("urbackupclientbackend v2.5 (Slate OS) — UrBackup client daemon");
...
fn run_urbackup_ctl(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("urbackupclientctl v2.5 (Slate OS) — UrBackup client control");
        println!();
        println!("Commands:");
...
fn run_urbackup_srv(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("urbackupsrv v2.5 (Slate OS) — UrBackup server daemon");
