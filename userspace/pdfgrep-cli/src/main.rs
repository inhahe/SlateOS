...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/pdfgrep"), "pdfgrep");
        assert_eq!(basename(r"C:\bin\pdfgrep.exe"), "pdfgrep.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("pdfgrep.exe"), "pdfgrep");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_pdfgrep(&["--help".to_string()], "pdfgrep"), 0);
        assert_eq!(run_pdfgrep(&["-h".to_string()], "pdfgrep"), 0);
        assert_eq!(run_pdfgrep(&["--version".to_string()], "pdfgrep"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_pdfgrep(&[], "pdfgrep"), 0);
    }
}
