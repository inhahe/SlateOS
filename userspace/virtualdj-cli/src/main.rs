...
fn run_vdj(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  --stem-separation      Real-Time Stem Separation (vocals/instr/bass/drums live)"
        );
        println!(
            "  --controller           Connect MIDI/HID DJ controller (Pioneer/Numark/Denon/Hercules)"
        );
    ...
    println!(
        "  Pricing: VirtualDJ Home FREE (full features, non-commercial use), Pro $19/mo or $299 lifetime"
    );
    println!("  Killer feature (VDJ 2024+):");
    println!(
        "    Real-Time Stem Separation — separates ANY track into vocals / instruments / bass /"
    );
    ...
    println!(
        "  Library: BPM/key/energy autoanalysis, smart playlists, cloud sync via Cloud Library"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "virtualdj".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_vdj, strip_ext};
