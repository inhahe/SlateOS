...
- **At every moment you must be in exactly one of three states: (1) actively making forward progress on a task, (2) starting a new task you just picked, or (3) blocked waiting on the operator for input you genuinely need before doing anything further.** "Doing nothing" while still pretending to work is never a valid state. A tick whose entire content is "I sampled X and it was already fine, nothing to do" is idling — it produces no artifact, changes no file, and closes no task. If your current line of work has stopped producing changes (e.g. an audit/verification pass that keeps coming back "already correct"), that line of work is **done** — declare it complete and move to the next roadmap task, even a large one. Do not keep re-running a verification loop that yields no edits just to have something to report. If you can't name a blocker (state 3), you are not blocked — pick the next task and start it.
  - **State (3) — genuinely blocked on someone else — is a legitimate state, and in it you SHOULD idle.** Stopping is the right call here, not a failure. **Two things justify it, and both are about waiting on someone else:** (a) **an operator decision** — a design decision, an architectural fork, a user-visible policy, or any irreversible/costly-to-reverse choice genuinely needs the operator's input; or (b) **another lane's work** — the change you need is in a tree you must not write, or a fixture only that lane can rebuild. In either case, *and only when there is no other unblocked task to pick up*, the correct behavior is to **stop**: for (a) post the question to `open-questions.md` (with options, pros/cons, and your recommendation); for (b) file the `requests/` entry or send the notice. Then say in one line what you are waiting on and **stop scheduling autonomous wakeups entirely** — do **not** call `ScheduleWakeup`/keep the `/loop` alive. Let the loop end. Waking up repeatedly only to re-report "still waiting" is itself a form of idling and clutters the transcript. **Resume the loop — at the 60-second cadence — as soon as the block clears:** an answered question, a merged request, a rebuilt fixture, or a new instruction all mean there is work again, and none of them needs to be asked for. Stopping is a pause for a stated reason, not the end of the session. (First, though, make sure you really are fully blocked: if *any* other roadmap task is unblocked, prefer doing that over stopping — stopping is only correct when the block gates *all* forward progress. The reason this should be unusual is not that stopping is discouraged; it is that on this project there is nearly always something left to do, so check you are out of work rather than out of the work you had in mind.)
...
- **Track decisions that need the operator in `open-questions.md`.** When a decision genuinely needs the human (an architectural fork, a user-visible policy, a tradeoff with no obviously-correct answer) and you've deferred it rather than resolved it, add it to `open-questions.md` — the question, the options with pros/cons, your recommendation if you have one, and where in the code it bites. This is distinct from `design-decisions.md` (decisions already *made*) and from `known-issues.md` (bugs/tech-debt): `open-questions.md` is the operator's decision queue. When the operator answers one, move it to `design-decisions.md` (marked `Decided by: Operator`) and remove it from `open-questions.md`.
  - **Write `open-questions.md` for a reader who does not know the subsystem.** The operator has said plainly that the entries are often not understandable — partly terse wording, but mostly **unexplained jargon**. An entry the operator cannot decide from is a failed entry no matter how correct it is. The fix is not more words; it is *different* words in the first few lines, so obey the length rule below as strictly as the rest.
    - **Open every entry with a `**In short:**` paragraph — 2–4 sentences, no jargon at all.** It answers: what is wrong right now, what a user would actually see, and what you are being asked to choose between. If a term of art is unavoidable there, it is the wrong paragraph — rewrite it. This goes *above* the existing detail, which stays for whoever needs it.
    - **Gloss every term of art on first use, in-line, in ≤ 10 words** — even one glossed in another entry, another file, or last week; the operator reads one entry at a time, months apart. Examples of the shape: "ambient authority (permission you get by *being* you, not by holding a token)", "NFC (the Unicode rule for composing é into one character)", "elide (cut text and mark the cut with …)". Assume nothing carries over between entries. If a term genuinely needs a paragraph rather than a clause, that paragraph belongs in the entry — that cost is real and is still cheaper than an unanswerable question.
    - **Give every option a one-line `*What changes:*`** stated as an observable difference, not an implementation ("the clock reads Eastern instead of UTC", "labels end in … instead of mid-word"), so the options can be compared without reading the prose.
    - **Say what happens if it is never answered.** One line: is the current behaviour safe, is anything blocked, does it get worse with time. This is what tells the operator whether to think hard now or wave it through.
