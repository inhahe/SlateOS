...
fn write_str(s: &str) { let _ = io::stdout().write_all(s.as_bytes()); }
fn write_bytes(b: &[u8]) { let _ = io::stdout().write_all(b); }
fn flush() { let _ = io::stdout().flush(); }
fn enter_alternate_screen() { write_str("\x1b[?1049h"); }
fn leave_alternate_screen() { write_str("\x1b[?1049l"); }
...
fn read_byte() -> Option<u8> {
    let mut b = [0u8; 1];
    match io::stdin().lock().read(&mut b) { Ok(1) => Some(b[0]), _ => None }
}
...
fn parse_escape_sequence() -> Key {
    let Some(b2) = read_byte() else { return Key::Escape; };
    match b2 {
        b'[' => {
            let mut params: Vec<u8> = Vec::new();
            loop {
                let Some(b) = read_byte() else { return Key::Unknown(params); };
                if b.is_ascii_uppercase() || b == b'~' { return decode_csi(&params, b); }
                params.push(b);
            }
        }
        b'O' => {
            let Some(b) = read_byte() else { return Key::Escape; };
            match b {
                b'P' => Key::F1, b'H' => Key::Home, b'F' => Key::End,
...
enum UndoOp {
    ...
    ReplaceLines { start_row: usize, old_lines: Vec<String>, new_lines: Vec<String> },
}
...
enum Mode {
    Normal, Insert, VisualChar, VisualLine, Command,
}
...
impl Mode {
    fn name(self) -> &'static str {
        match self {
            ...
            Mode::VisualChar => "VISUAL", Mode::VisualLine => "V-LINE",
...
enum InsertMotion {
    InsertBefore, InsertAfter, InsertBOL, InsertEOL, OpenBelow, OpenAbove,
}
...
impl Editor {
    fn new() -> Self {
        ...
        Self {
            ...
            show_numbers: false,
            filename: None, modified: false,
            undo_stack: Vec::new(), undo_pos: 0,
            register: None,
            message: String::new(),
            visual_anchor: (0, 0),
            cmd_buf: String::new(),
    ...
    fn undo(&mut self) {
        if self.undo_pos == 0 { self.message = "Already at oldest change".to_owned(); return; }
        self.undo_pos -= 1;
    ...
    fn apply_undo_op(&mut self, op: UndoOp) {
        match op {
            ...
            UndoOp::ReplaceLines { start_row, old_lines, new_lines } => {
    ...
    fn redo(&mut self) {
        if self.undo_pos >= self.undo_stack.len() {
            self.message = "Already at newest change".to_owned(); return;
        }
        let op = self.undo_stack[self.undo_pos].clone();
        self.undo_pos += 1;
    ...
    fn apply_redo_op(&mut self, op: UndoOp) {
        match op {
            UndoOp::Insert { row, col, text } => {
                self.insert_text_at(row, col, &text);
                self.row = row; self.col = col + text.len().saturating_sub(1);
            }
            UndoOp::Delete { row, col, text } => {
                self.remove_text_at(row, col, text.len());
                self.row = row; self.col = col;
    ...
    fn insert_text_at(&mut self, row: usize, col: usize, text: &str) {
        ...
        if text.contains('\n') {
            ...
            for p in parts { new_lines.push(p.to_owned()); }
    ...
    fn remove_text_at(&mut self, row: usize, col: usize, count: usize) {
        if row >= self.lines.len() || count == 0 { return; }
        let line = &mut self.lines[row];
    ...
    fn line_num_width(&self) -> usize {
        if self.show_numbers { format!("{}", self.lines.len()).len() + 1 } else { 0 }
    }
    ...
    fn scroll_to_cursor(&mut self) {
        ...
        else if self.row >= self.scroll + visible { self.scroll = self.row + 1 - visible; }
    }
    ...
    fn move_to_bol(&mut self) { self.col = 0; }
    ...
    fn move_word_forward(&mut self, n: usize) {
        for _ in 0..n {
            let line = match self.lines.get(self.row) { Some(l) => l.clone(), None => return };
            let chars: Vec<char> = line.chars().collect();
            let len = chars.len();
            let mut c = self.col.min(len.saturating_sub(1));
            let is_word = |ch: char| ch.is_alphanumeric() || ch == '_';
            if c < len && is_word(chars[c]) {
                while c < len && is_word(chars[c]) { c += 1; }
            } else if c < len && !chars[c].is_whitespace() {
                while c < len && !chars[c].is_whitespace() && !is_word(chars[c]) { c += 1; }
            }
            while c < len && chars[c].is_whitespace() { c += 1; }
            if c >= len && self.row + 1 < self.lines.len() {
                self.row += 1; self.col = 0; self.move_to_first_nonblank(); return;
            }
    ...
    fn move_word_backward(&mut self, n: usize) {
        for _ in 0..n {
            ...
            let is_word = |ch: char| ch.is_alphanumeric() || ch == '_';
            if c > 0 { c -= 1; }
            while c > 0 && chars[c].is_whitespace() { c -= 1; }
            if c > 0 {
                if is_word(chars[c]) {
                    while c > 0 && is_word(chars[c - 1]) { c -= 1; }
                } else {
                    while c > 0 && !chars[c - 1].is_whitespace() && !is_word(chars[c - 1]) { c -= 1; }
                }
            }
            self.col = c;
        }
    ...
    fn move_word_end(&mut self, n: usize) {
        for _ in 0..n {
            ...
            let is_word = |ch: char| ch.is_alphanumeric() || ch == '_';
            if c + 1 < len { c += 1; }
            while c + 1 < len && chars[c].is_whitespace() { c += 1; }
            if is_word(chars[c]) {
                while c + 1 < len && is_word(chars[c + 1]) { c += 1; }
            } else {
                while c + 1 < len && !chars[c + 1].is_whitespace() && !is_word(chars[c + 1]) { c += 1; }
            }
            self.col = c.min(len.saturating_sub(1));
        }
    }
    ...
    fn goto_first_line(&mut self) {
        self.row = 0; self.col = 0; self.move_to_first_nonblank();
    }

    fn goto_last_line(&mut self) {
        self.row = self.lines.len().saturating_sub(1); self.col = 0; self.move_to_first_nonblank();
    }

    fn goto_line(&mut self, n: usize) {
        self.row = n.saturating_sub(1).min(self.lines.len().saturating_sub(1));
        self.col = 0; self.move_to_first_nonblank();
    }
    ...
    fn paste_after(&mut self) {
        match self.register.clone() {
            Some(Register::Lines(lines)) => {
                let at = self.row + 1;
                for (i, line) in lines.iter().enumerate() { self.lines.insert(at + i, line.clone()); }
                self.row = at; self.col = 0; self.move_to_first_nonblank();
                self.modified = true;
            }
            Some(Register::Chars(text)) => {
                let line = &mut self.lines[self.row];
                let pos = if line.is_empty() { 0 } else { (self.col + 1).min(line.len()) };
                line.insert_str(pos, &text);
                self.col = pos + text.len().saturating_sub(1);
                self.modified = true;
            }
            None => { self.message = "Nothing in register".to_owned(); }
        }
    }
    ...
    fn paste_before(&mut self) {
        match self.register.clone() {
            Some(Register::Lines(lines)) => {
                for (i, line) in lines.iter().enumerate() { self.lines.insert(self.row + i, line.clone()); }
                self.col = 0; self.move_to_first_nonblank();
                self.modified = true;
            }
            Some(Register::Chars(text)) => {
    ...
    fn join_lines(&mut self) {
        if self.row + 1 >= self.lines.len() { return; }
        let old_len = self.lines[self.row].len();
        let next = self.lines.remove(self.row + 1);
        if !self.lines[self.row].is_empty() { self.lines[self.row].push(' '); }
    ...
    fn toggle_case(&mut self) {
        let line = match self.lines.get_mut(self.row) { Some(l) => l, None => return };
        if line.is_empty() { return; }
        let c = self.col.min(line.len() - 1);
        if let Some(ch) = line[c..].chars().next() {
            let ch_len = ch.len_utf8();
            let toggled: String = if ch.is_uppercase() { ch.to_lowercase().collect() } else { ch.to_uppercase().collect() };
    ...
    fn replace_char(&mut self, ch: char) {
        let line = match self.lines.get_mut(self.row) { Some(l) => l, None => return };
        if line.is_empty() { line.push(ch); return; }
        let c = self.col.min(line.len() - 1);
    ...
    fn indent_line(&mut self, right: bool) {
        if right {
            ...
            let spaces = self.lines[self.row].chars().take_while(|&c| c == ' ').count();
    ...
    fn open_line_below(&mut self) {
        let indent: String = self.lines[self.row].chars().take_while(|c| c.is_whitespace()).collect();
        self.lines.insert(self.row + 1, indent.clone());
        self.row += 1; self.col = indent.len();
        self.mode = Mode::Insert; self.modified = true;
    }
    ...
    fn search_fwd(&mut self, pattern: &str) {
        ...
        for i in 0..nr {
            let r = (self.row + i + 1) % nr;
            let start_col = if i == 0 { self.col + 1 } else { 0 };
            if let Some(line) = self.lines.get(r) {
                if let Some(pos) = find_after(line, pattern, start_col) {
                    self.row = r; self.col = pos; return;
                }
            }
        }
    ...
    fn search_bwd(&mut self, pattern: &str) {
        ...
        let nr = self.lines.len();
        for i in 0..nr {
            let r = if self.row >= i + 1 { self.row - i - 1 } else { nr - (i + 1 - self.row) };
            if let Some(line) = self.lines.get(r) {
                let before = if i == 0 { self.col.saturating_sub(1) } else { line.len() };
    ...
    fn search_next(&mut self) {
        ...
        if fwd { self.search_fwd(&pat); } else { self.search_bwd(&pat); }
    }

    fn search_prev(&mut self) {
        let pat = self.search_pat.clone();
        let fwd = self.search_forward;
        if fwd { self.search_bwd(&pat); } else { self.search_fwd(&pat); }
    }
    ...
    fn substitute_global(&mut self, pat: &str, rep: &str, all_lines: bool) -> usize {
        ...
        let (range_start, range_end) = if all_lines { (0, self.lines.len()) } else { (self.row, self.row + 1) };
        for i in range_start..range_end {
            ...
            while search_start <= line.len() {
                match line[search_start..].find(pat) {
                    ...
                    None => { new_line.push_str(&line[search_start..]); break; }
                }
            }
    ...
    fn visual_range(&self) -> ((usize, usize), (usize, usize)) {
        ...
        if ar < cr || (ar == cr && ac <= cc) { ((ar, ac), (cr, cc)) } else { ((cr, cc), (ar, ac)) }
    }

    fn visual_line_range(&self) -> (usize, usize) {
        let (ar, _) = self.visual_anchor;
        if ar <= self.row { (ar, self.row) } else { (self.row, ar) }
    }
    ...
    fn indent_visual(&mut self, right: bool) {
        let (start, end) = match self.mode {
            ...
            Mode::VisualChar => { let ((sr, _), (er, _)) = self.visual_range(); (sr, er) }
            _ => return,
        };
    ...
    fn extract_char_range(&self, sr: usize, sc: usize, er: usize, ec: usize) -> String {
        ...
        for r in sr..=er.min(self.lines.len().saturating_sub(1)) {
            ...
            let col_end = if r == er { (ec + 1).min(line.len()) } else { line.len() };
            if r > sr { out.push('\n'); }
    ...
    fn delete_char_range(&mut self, sr: usize, sc: usize, er: usize, ec: usize) {
        if sr == er {
            ...
            if s < e { line.drain(s..e); }
        } else {
            let prefix = self.lines[sr][..sc.min(self.lines[sr].len())].to_owned();
            let suffix = { let l = &self.lines[er]; l[(ec + 1).min(l.len())..].to_owned() };
    ...
    fn exec_command(&mut self, cmd: &str) -> bool {
        ...
        if cmd.chars().all(|c| c.is_ascii_digit()) {
            if let Ok(n) = cmd.parse::<usize>() { self.goto_line(n); }
            return false;
        }
        if cmd == "q" {
            if self.modified { self.message = "No write since last change (use :q! to override)".to_owned(); return false; }
            return true;
        }
        ...
        if cmd == "wq" || cmd == "x" {
            match self.filename.clone() {
                ...
                None => { self.message = "No file name".to_owned(); return false; }
            }
        }
        if let Some(rest) = cmd.strip_prefix("e ") {
            let path = rest.trim().to_owned();
            if self.modified { self.message = "No write since last change (use :e! to override)".to_owned(); return false; }
        ...
        if cmd == "set number" || cmd == "set nu" { self.show_numbers = true; return false; }
        if cmd == "set nonumber" || cmd == "set nonu" { self.show_numbers = false; return false; }
        ...
        if is_percent_sub || is_local_sub {
            ...
            if let Some(sep) = body.chars().next() {
                ...
                if parts.len() >= 2 {
                    ...
                    self.message = if count == 0 { format!("Pattern not found: {}", pat) }
                                   else { format!("{} substitution(s) made", count) };
                } else {
        ...
        match self.last_change.clone() {
            ...
            LastChange::DeleteChar => {
                if let Some(ch) = self.delete_char() {
                    self.push_undo(UndoOp::Delete { row: self.row, col: self.col, text: ch.to_string() });
                }
            }
            LastChange::DeleteLine => {
                let old = self.lines[self.row].clone();
                self.delete_lines(1);
                self.push_undo(UndoOp::ReplaceLines { start_row: self.row, old_lines: vec![old], new_lines: vec![] });
            }
            LastChange::YankLine => { self.yank_lines(1); }
            LastChange::ReplaceChar(ch) => { self.replace_char(ch); }
            LastChange::JoinLines => { self.join_lines(); }
            LastChange::ToggleCase => { self.toggle_case(); }
            LastChange::IndentRight => { self.indent_line(true); }
            LastChange::IndentLeft => { self.indent_line(false); }
            ...
            LastChange::InsertText { motion, text } => {
                match motion {
                    ...
                    InsertMotion::OpenBelow => { self.open_line_below(); self.mode = Mode::Normal; }
                    InsertMotion::OpenAbove => { self.open_line_above(); self.mode = Mode::Normal; }
                    InsertMotion::InsertBefore => {}
                }
                for ch in text.chars() {
                    if ch == '\n' { self.insert_enter(); } else { self.insert_char(ch); }
                }
            }
        }
    }
    ...
    fn handle_normal(&mut self, key: Key) -> bool {
        if let Key::Char(ch) = &key {
            if ch.is_ascii_digit() && (*ch != '0' || !self.count_buf.is_empty()) {
                self.count_buf.push(*ch); return false;
            }
        }
        match key {
            Key::Char('i') => { self.mode = Mode::Insert; self.last_change = LastChange::InsertText { motion: InsertMotion::InsertBefore, text: String::new() }; }
            Key::Char('I') => { self.move_to_first_nonblank(); self.mode = Mode::Insert; self.last_change = LastChange::InsertText { motion: InsertMotion::InsertBOL, text: String::new() }; }
            Key::Char('a') => { self.move_right(1); self.mode = Mode::Insert; self.last_change = LastChange::InsertText { motion: InsertMotion::InsertAfter, text: String::new() }; }
            Key::Char('A') => { self.col = self.line_len(); self.mode = Mode::Insert; self.last_change = LastChange::InsertText { motion: InsertMotion::InsertEOL, text: String::new() }; }
            Key::Char('o') => { self.count_buf.clear(); self.open_line_below(); self.last_change = LastChange::InsertText { motion: InsertMotion::OpenBelow, text: String::new() }; }
            Key::Char('O') => { self.count_buf.clear(); self.open_line_above(); self.last_change = LastChange::InsertText { motion: InsertMotion::OpenAbove, text: String::new() }; }
            Key::Char('v') => { self.visual_anchor = (self.row, self.col); self.mode = Mode::VisualChar; self.count_buf.clear(); }
            Key::Char('V') => { self.visual_anchor = (self.row, self.col); self.mode = Mode::VisualLine; self.count_buf.clear(); }
            Key::Char(':') => { self.cmd_buf.clear(); self.mode = Mode::Command; self.count_buf.clear(); }
            Key::Char('h') | Key::Left => { let n = self.take_count(); self.move_left(n); }
            Key::Char('j') | Key::Down => { let n = self.take_count(); self.move_down(n); }
            Key::Char('k') | Key::Up => { let n = self.take_count(); self.move_up(n); }
            Key::Char('l') | Key::Right => { let n = self.take_count(); self.move_right(n); }
            Key::Char('w') => { let n = self.take_count(); self.move_word_forward(n); }
            Key::Char('b') => { let n = self.take_count(); self.move_word_backward(n); }
            Key::Char('e') => { let n = self.take_count(); self.move_word_end(n); }
            Key::Char('0') => { self.count_buf.clear(); self.move_to_bol(); }
            Key::Char('$') => { self.count_buf.clear(); self.move_to_eol(); }
            Key::Char('^') => { self.count_buf.clear(); self.move_to_first_nonblank(); }
            Key::Char('g') => {
                let n = self.take_count();
                let next = read_key();
                if let Key::Char('g') = next {
                    if n > 1 { self.goto_line(n); } else { self.goto_first_line(); }
                }
            }
            Key::Char('G') => {
                let n_raw = self.count_buf.parse::<usize>().unwrap_or(0);
                self.count_buf.clear();
                if n_raw > 0 { self.goto_line(n_raw); } else { self.goto_last_line(); }
            }
            Key::Ctrl('f') | Key::PageDown => { let n = self.take_count(); self.page_down(n); }
            Key::Ctrl('b') | Key::PageUp => { let n = self.take_count(); self.page_up(n); }
            Key::Ctrl('d') => { let n = self.take_count(); let h = self.term_rows / 2; self.move_down(h * n); }
            Key::Ctrl('u') => { let n = self.take_count(); let h = self.term_rows / 2; self.move_up(h * n); }
            Key::Char('x') => {
                let (row, col) = (self.row, self.col);
                let n = self.take_count();
                for _ in 0..n { self.delete_char(); }
                self.push_undo(UndoOp::Delete { row, col, text: "x".to_owned() });
            ...
            Key::Char('d') => {
                ...
                match next {
                    Key::Char('d') => {
                        ...
                        self.delete_lines(n);
                        self.push_undo(UndoOp::ReplaceLines { start_row: start, old_lines: old, new_lines: vec![] });
                    ...
                    Key::Char('w') => {
                        ...
                        self.push_undo(UndoOp::ReplaceLines { start_row: row, old_lines: vec![old], new_lines: vec![self.lines[row].clone()] });
                    }
                    _ => {}
                }
            }
            ...
            Key::Char('p') => { self.count_buf.clear(); self.paste_after(); self.last_change = LastChange::PasteAfter; }
            Key::Char('P') => { self.count_buf.clear(); self.paste_before(); self.last_change = LastChange::PasteBefore; }
            Key::Char('u') => { self.count_buf.clear(); self.undo(); }
            Key::Ctrl('r') => { self.count_buf.clear(); self.redo(); }
            Key::Char('r') => {
                self.count_buf.clear();
                if let Key::Char(ch) = read_key() {
                    self.replace_char(ch); self.last_change = LastChange::ReplaceChar(ch);
                }
            }
            Key::Char('J') => {
                ...
                for _ in 0..n { self.join_lines(); }
                self.push_undo(UndoOp::ReplaceLines { start_row: row, old_lines: old, new_lines: vec![self.lines[row].clone()] });
            ...
            Key::Char('>') => {
                if read_key() == Key::Char('>') {
                    ...
                    let row = self.row;
                    let old = self.lines[row].clone();
                    for _ in 0..n { self.indent_line(true); }
                    self.push_undo(UndoOp::ReplaceLines { start_row: row, old_lines: vec![old], new_lines: vec![self.lines[row].clone()] });
            ...
            Key::Char('<') => {
                if read_key() == Key::Char('<') {
                    ...
                    let row = self.row;
                    let old = self.lines[row].clone();
                    for _ in 0..n { self.indent_line(false); }
                    self.push_undo(UndoOp::ReplaceLines { start_row: row, old_lines: vec![old], new_lines: vec![self.lines[row].clone()] });
                    self.last_change = LastChange::IndentLeft;
                }
            }
            Key::Char('/') => { self.count_buf.clear(); self.mode = Mode::Command; self.cmd_buf.clear(); self.cmd_buf.push('/'); }
            Key::Char('?') => { self.count_buf.clear(); self.mode = Mode::Command; self.cmd_buf.clear(); self.cmd_buf.push('?'); }
            Key::Char('n') => { self.count_buf.clear(); self.search_next(); }
            Key::Char('N') => { self.count_buf.clear(); self.search_prev(); }
            Key::Char('.') => { self.count_buf.clear(); self.repeat_last_change(); }
            Key::Escape => { self.count_buf.clear(); self.message.clear(); }
    ...
    fn handle_insert(&mut self, key: Key) {
        match key {
            ...
            Key::Enter => {
                self.insert_enter();
                if let LastChange::InsertText { text, .. } = &mut self.last_change { text.push('\n'); }
            }
            Key::Backspace => {
                self.insert_backspace();
                if let LastChange::InsertText { text, .. } = &mut self.last_change { text.pop(); }
            }
            Key::Char(ch) => {
                self.insert_char(ch);
                if let LastChange::InsertText { text, .. } = &mut self.last_change { text.push(ch); }
            }
            Key::Left => { self.move_left(1); }
            Key::Right => { self.move_right(1); }
            Key::Up => { self.move_up(1); }
            Key::Down => { self.move_down(1); }
            Key::Home => { self.move_to_bol(); }
    ...
    fn handle_visual(&mut self, key: Key) {
        match key {
            ...
            Key::Char('y') => { self.yank_visual(); }
            Key::Char('d') | Key::Char('x') => { self.delete_visual(); }
            Key::Char('>') => { self.indent_visual(true); }
            Key::Char('<') => { self.indent_visual(false); }
            Key::Char('h') | Key::Left => { self.move_left(1); }
            Key::Char('j') | Key::Down => { self.move_down(1); }
            Key::Char('k') | Key::Up => { self.move_up(1); }
            Key::Char('l') | Key::Right => { self.move_right(1); }
            Key::Char('w') => { self.move_word_forward(1); }
            Key::Char('b') => { self.move_word_backward(1); }
            Key::Char('e') => { self.move_word_end(1); }
            Key::Char('0') => { self.move_to_bol(); }
            Key::Char('$') => { self.move_to_eol(); }
            Key::Char('^') => { self.move_to_first_nonblank(); }
            Key::Char('G') => { self.goto_last_line(); }
            _ => {}
        }
        self.clamp_cursor();
        self.scroll_to_cursor();
    ...
    fn handle_command(&mut self, key: Key) -> bool {
        match key {
            Key::Escape => { self.cmd_buf.clear(); self.mode = Mode::Normal; }
            Key::Enter => {
                let cmd = self.cmd_buf.clone();
                self.cmd_buf.clear();
                self.mode = Mode::Normal;
                if let Some(pat) = cmd.strip_prefix('/') {
                    if !pat.is_empty() { self.search_pat = pat.to_owned(); }
                    self.search_forward = true;
                    let p = self.search_pat.clone(); self.search_fwd(&p);
                } else if let Some(pat) = cmd.strip_prefix('?') {
                    if !pat.is_empty() { self.search_pat = pat.to_owned(); }
                    self.search_forward = false;
                    let p = self.search_pat.clone(); self.search_bwd(&p);
                } else {
                    return self.exec_command(&cmd);
                }
            }
            Key::Backspace => {
                if self.cmd_buf.is_empty() { self.mode = Mode::Normal; } else { self.cmd_buf.pop(); }
            }
            Key::Char(ch) => { self.cmd_buf.push(ch); }
            _ => {}
        }
        false
    }
    ...
    fn render(&self, out: &mut Vec<u8>) {
        ...
        for screen_row in 0..visible_rows {
            ...
            if buf_row < self.lines.len() {
                ...
                for (byte_idx, ch) in line.char_indices() {
                    if col_idx >= text_w { break; }
                    let should_hl = byte_idx >= hl_start && byte_idx < hl_end;
                    if should_hl && !in_hl { out.extend_from_slice(b"\x1b[7m"); in_hl = true; }
                    else if !should_hl && in_hl { out.extend_from_slice(b"\x1b[27m"); in_hl = false; }
                    if ch == '\t' {
                        out.extend_from_slice(b"    "); col_idx += 4;
                    } else {
                        let mut buf = [0u8; 4];
                ...
                if in_hl { out.extend_from_slice(b"\x1b[0m"); }
            } else {
        ...
        let col_display = self.line_num_width()
            + self.lines.get(self.row)
                .map(|l| l[..self.col.min(l.len())].chars().map(|c| if c == '\t' { 4usize } else { 1 }).sum::<usize>())
                .unwrap_or(0);
        ...
        match self.mode {
            Mode::VisualLine => {
                ...
                if buf_row >= sr && buf_row <= er {
                    (0, self.lines.get(buf_row).map(|l| l.len()).unwrap_or(0).max(1))
                } else { (0, 0) }
            }
            Mode::VisualChar => {
                let ((sr, sc), (er, ec)) = self.visual_range();
                if buf_row < sr || buf_row > er { return (0, 0); }
                let line = match self.lines.get(buf_row) { Some(l) => l, None => return (0, 0) };
                let ll = line.len();
    ...
    fn render_status(&self, out: &mut Vec<u8>) {
        ...
        let left_trunc = if left.len() > max_left { &left[..max_left] } else { &left };
        let pad = self.term_cols.saturating_sub(left_trunc.len() + right.len());
...
fn run(mut editor: Editor) {
    ...
    loop {
        let (rows, cols) = get_terminal_size();
        editor.term_rows = rows; editor.term_cols = cols;
        ...
        let quit = match editor.mode {
            ...
            Mode::Insert => { editor.handle_insert(key); false }
            Mode::VisualChar | Mode::VisualLine => { editor.handle_visual(key); false }
            Mode::Command => editor.handle_command(key),
        };
        if quit { break; }
        if !matches!(editor.mode, Mode::Command) { editor.message.clear(); }
    }
    drop(_raw);
...
fn main() {
    ...
    if args.len() >= 2 {
        if let Err(e) = editor.load_file(&args[1]) {
            eprintln!("vi: {}", e);
            std::process::exit(1);
        }
    }
...
mod tests {
    ...
    fn make_editor(lines: &[&str]) -> Editor {
        ...
        if e.lines.is_empty() { e.lines.push(String::new()); }
        e
    }
    ...
    fn test_move_left_clamps() {
        ...
        e.col = 0; e.move_left(10); assert_eq!(e.col, 0);
    }

    #[test]
    fn test_move_right_clamps_normal() {
        let mut e = make_editor(&["hi"]);
        e.col = 0; e.move_right(100); assert_eq!(e.col, 1);
    }

    #[test]
    fn test_move_up_clamps() {
        let mut e = make_editor(&["a", "b"]);
        e.row = 0; e.move_up(10); assert_eq!(e.row, 0);
    }

    #[test]
    fn test_move_down_clamps() {
        let mut e = make_editor(&["a", "b", "c"]);
        e.move_down(100); assert_eq!(e.row, 2);
    }

    #[test]
    fn test_move_to_bol() {
        let mut e = make_editor(&["hello"]);
        e.col = 4; e.move_to_bol(); assert_eq!(e.col, 0);
    }

    #[test]
    fn test_move_to_eol_normal() {
        let mut e = make_editor(&["hello"]);
        e.move_to_eol(); assert_eq!(e.col, 4);
    }

    #[test]
    fn test_first_nonblank() {
        let mut e = make_editor(&["   hello"]);
        e.move_to_first_nonblank(); assert_eq!(e.col, 3);
    }

    #[test]
    fn test_goto_first_line() {
        let mut e = make_editor(&["a", "b", "c"]);
        e.row = 2; e.goto_first_line(); assert_eq!(e.row, 0);
    }

    #[test]
    fn test_goto_last_line() {
        let mut e = make_editor(&["a", "b", "c"]);
        e.goto_last_line(); assert_eq!(e.row, 2);
    }

    #[test]
    fn test_goto_line_number() {
        let mut e = make_editor(&["a", "b", "c", "d"]);
        e.goto_line(3); assert_eq!(e.row, 2);
    }

    #[test]
    fn test_word_forward() {
        let mut e = make_editor(&["hello world foo"]);
        e.col = 0; e.move_word_forward(1); assert_eq!(e.col, 6);
    }

    #[test]
    fn test_word_backward() {
        let mut e = make_editor(&["hello world"]);
        e.col = 10; e.move_word_backward(1); assert_eq!(e.col, 6);
    }
    ...
    fn test_yank_paste_after() {
        let mut e = make_editor(&["a", "b", "c"]);
        e.row = 0; e.yank_lines(1); e.row = 1; e.paste_after();
    ...
    fn test_paste_before() {
        ...
        e.row = 0; e.yank_lines(1); e.row = 1; e.paste_before();
    ...
    fn test_indent_right() {
        let mut e = make_editor(&["hello"]);
        e.indent_line(true); assert_eq!(e.lines[0], "    hello");
    }

    #[test]
    fn test_indent_left() {
        let mut e = make_editor(&["    hello"]);
        e.indent_line(false); assert_eq!(e.lines[0], "hello");
    }
    ...
    fn test_insert_char() {
        ...
        e.mode = Mode::Insert; e.col = 3; e.insert_char('l');
    ...
    fn test_insert_enter_splits() {
        let mut e = make_editor(&["hello"]);
        e.mode = Mode::Insert; e.col = 3; e.insert_enter();
    ...
    fn test_insert_backspace_merges() {
        ...
        e.mode = Mode::Insert; e.row = 1; e.col = 0;
        e.insert_backspace();
        assert_eq!(e.lines.len(), 1);
        assert_eq!(e.lines[0], "hello");
    }
    ...
    fn test_undo_replace_lines() {
        ...
        e.push_undo(UndoOp::ReplaceLines {
            start_row: 0, old_lines: vec!["original".to_owned()], new_lines: vec!["hello".to_owned()],
        });
        e.undo();
    ...
    fn test_redo_restores() {
        let mut e = make_editor(&["hello"]);
        e.push_undo(UndoOp::ReplaceLines {
            start_row: 0, old_lines: vec!["old".to_owned()], new_lines: vec!["hello".to_owned()],
        });
        e.undo(); assert_eq!(e.lines[0], "old");
        e.redo(); assert_eq!(e.lines[0], "hello");
    }

    #[test]
    fn test_new_change_clears_redo() {
        let mut e = make_editor(&["a"]);
        e.push_undo(UndoOp::ReplaceLines { start_row: 0, old_lines: vec!["x".to_owned()], new_lines: vec!["a".to_owned()] });
        e.undo();
        e.push_undo(UndoOp::ReplaceLines { start_row: 0, old_lines: vec!["x".to_owned()], new_lines: vec!["y".to_owned()] });
    ...
    fn test_search_fwd_finds() {
        let mut e = make_editor(&["hello world"]);
        e.row = 0; e.col = 0; e.search_fwd("world");
        assert_eq!(e.col, 6);
    }
    ...
    fn test_search_fwd_wraps() {
        ...
        e.row = 2; e.col = 0; e.search_fwd("abc");
        assert_eq!(e.row, 0);
    }
    ...
    #[test]
    fn test_exec_q_unmodified() { let mut e = make_editor(&["a"]); assert!(e.exec_command("q")); }

    #[test]
    fn test_exec_q_modified_stays() {
        let mut e = make_editor(&["a"]); e.modified = true;
        assert!(!e.exec_command("q")); assert!(e.message.contains("No write"));
    }

    #[test]
    fn test_exec_q_force() { let mut e = make_editor(&["a"]); e.modified = true; assert!(e.exec_command("q!")); }

    #[test]
    fn test_exec_set_number() {
        let mut e = Editor::new();
        e.exec_command("set number"); assert!(e.show_numbers);
        e.exec_command("set nonumber"); assert!(!e.show_numbers);
    }

    #[test]
    fn test_exec_goto_line() {
        let mut e = make_editor(&["a", "b", "c", "d", "e"]);
        e.exec_command("3"); assert_eq!(e.row, 2);
    }
    ...
    fn test_visual_line_range_normalized() {
        let mut e = make_editor(&["a", "b", "c"]);
        e.mode = Mode::VisualLine; e.visual_anchor = (2, 0); e.row = 0;
    ...
    fn test_visual_char_range_normalized() {
        ...
        e.mode = Mode::VisualChar; e.visual_anchor = (0, 4); e.row = 0; e.col = 1;
        let ((sr, sc), (er, ec)) = e.visual_range();
        assert_eq!((sr, sc), (0, 1)); assert_eq!((er, ec), (0, 4));
    }
    ...
    fn test_open_line_below() {
        ...
        e.row = 0; e.open_line_below();
        assert_eq!(e.lines.len(), 2); assert_eq!(e.row, 1); assert_eq!(e.mode, Mode::Insert);
    }

    #[test]
    fn test_open_line_above() {
        let mut e = make_editor(&["first"]);
        e.row = 0; e.open_line_above();
        assert_eq!(e.lines.len(), 2); assert_eq!(e.row, 0); assert_eq!(e.mode, Mode::Insert);
    }
    ...
    fn test_line_num_width_disabled() {
        let e = make_editor(&["a", "b"]); assert_eq!(e.line_num_width(), 0);
    }

    #[test]
    fn test_line_num_width_enabled_ten_lines() {
        let lines: Vec<&str> = vec!["a"; 10];
        let mut e = make_editor(&lines); e.show_numbers = true;
