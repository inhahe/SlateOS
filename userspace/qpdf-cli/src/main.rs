...
fn run_qpdf(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("QPDF 11.9.1 (Slate OS) — PDF transformation tool");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("qpdf version 11.9.1 (Slate OS)");
        return 0;
    }
    ...
    let input = args
        ...
        .map(|s| s.as_str())
        .unwrap_or("input.pdf");
    println!("qpdf: Processing {}...", quoteaf_os(input));
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_qpdf, strip_ext};
