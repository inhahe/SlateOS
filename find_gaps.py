...
with open('kernel/src/syscall/linux.rs', encoding='utf-8') as f:
    text = f.read()

defined = {}
for m in re.finditer(r'pub const (\w+): u64 = (\d+);', text):
    name = m.group(1)
    val = int(m.group(2))
    if val < 1000:
        defined[val] = name
...
linux_x86_64 = {
    ...
    461: 'lsm_list_modules', 462: 'mseal',
}

missing = []
for num, name in linux_x86_64.items():
    if num not in defined:
        missing.append((num, name))
missing.sort()
print(f'Linux x86_64 syscalls NOT defined as constants: {len(missing)}')
