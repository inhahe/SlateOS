...
fn run_sbl(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Switchboard Live v3.4 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "switchboardlive".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sbl, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/switchboardlive"), "switchboardlive");
        assert_eq!(
            basename(r"C:\bin\switchboardlive.exe"),
            "switchboardlive.exe"
        );
