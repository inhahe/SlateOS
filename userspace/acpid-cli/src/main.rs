...
fn run_acpid(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("acpid v2.0 (Slate OS) — ACPI event daemon");
        println!();
        println!("Options:");
...
fn run_listen(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("acpi_listen v2.0 (Slate OS) — Listen for ACPI events");
        println!();
        println!("Options:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "acpid".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_acpid, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_acpid(&["--version".to_string()], "acpid");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_acpid(&[], "acpid");
    }
}
