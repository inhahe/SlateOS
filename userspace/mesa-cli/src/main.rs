...
//! mesa-cli — OurOS Mesa 3D graphics library tools CLI
//!
...
fn run_eglinfo(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("eglinfo — display EGL information (OurOS).");
        return 0;
    }
...
fn run_es2_info(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("es2_info — OpenGL ES 2.0 implementation info (OurOS).");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_eglinfo(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_eglinfo(&[]), 0);
    }
}
