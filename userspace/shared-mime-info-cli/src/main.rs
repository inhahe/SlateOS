...
fn run_update_mime(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("update-mime-database v2.4 (Slate OS)");
        return 0;
    }
    let dir = args
        .iter()
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "update-mime-database".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_update_mime, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/shared-mime-info"), "shared-mime-info");
        assert_eq!(
            basename(r"C:\bin\shared-mime-info.exe"),
            "shared-mime-info.exe"
        );
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_update_mime(&["--help".to_string()], "shared-mime-info"),
            0
        );
