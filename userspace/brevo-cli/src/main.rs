...
fn run_brevo(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Brevo (Slate OS) — multi-channel marketing + sales + transactional from one platform"
        );
        println!();
        println!("Options:");
    ...
    println!(
        "          Thiberge previously ran a consulting agency in India serving European SMBs"
    );
    println!("          built Sendinblue to solve his own clients' transactional email problems");
    println!(
        "          rebranded Sendinblue → Brevo May 2023 to drop the 'sendin' (email-only) connotation"
    );
    println!("  Founded: 2012 in Paris with operations in Delhi (engineering hub)");
    println!(
        "          bootstrapped early, then raised €140M Bridgepoint Series C 2020 (mostly secondary)"
    );
    println!("          ~€150M+ ARR estimated, profitable, 800+ employees");
    println!(
        "          one of largest European SaaS companies (along with Mirakl, Aircall, Doctolib)"
    );
    println!(
        "  Strategic position: 'European HubSpot for SMBs — multi-channel + transactional in one':"
    );
    println!("                    SMB sweet spot, especially in Europe + LATAM + India + SE Asia");
    println!(
        "                    competes with Mailchimp (less automation), HubSpot (more expensive)"
    );
    println!("                    competes with ActiveCampaign (similar pricing, less channels)");
    println!("                    competes with SendGrid for transactional");
    println!(
        "                    pitch: 'one platform for marketing + transactional + chat + CRM'"
    );
    ...
    println!(
        "    Business — from $18/mo (20K emails/mo) — multi-user, A/B test, send time opt, automations"
    );
    println!(
        "    BrevoPlus (Enterprise) — custom — dedicated IP + advanced security + priority support"
    );
    println!("    SMS + WhatsApp — separate pay-as-you-go pricing (€0.0445/SMS in France, etc.)");
    println!(
        "    Transactional — separate (SendGrid-like): from $15/mo (20K) up to enterprise volumes"
    );
    println!(
        "    contact-count is NOT the pricing axis — email-count is (refreshing differentiator)"
    );
    println!("  Channels (broad for SMB pricing):");
    println!("    - Email (own infrastructure + dedicated IP options)");
    println!(
        "    - Transactional email (SMTP + REST API — competes with SendGrid + Postmark + Mailgun)"
    );
    ...
    println!(
        "    - Triggers: form sign-up, page visit, link click, attribute change, event from API"
    );
    println!(
        "    - Actions: send email, send SMS, add/remove from list, update field, webhook, score adjustment, wait"
    );
    println!(
        "    - Marketing automations library: welcome, anniversary, abandoned cart, post-purchase, win-back"
    );
    ...
    println!(
        "            Louis Vuitton (LVMH, transactional), Carrefour, Michelin, Eurosport (some marketing)"
    );
    println!("            massive SMB long-tail in France, Germany, Spain, Italy, India, Brazil");
    println!(
        "            sweet spot: SMBs + dev shops wanting transactional + marketing in one bill"
    );
    println!(
        "  Critique: UI shows European/translated origin — sometimes inconsistent vs HubSpot's polish"
    );
    println!(
        "           Sales/CRM features less mature than dedicated SMB CRMs (ActiveCampaign, HubSpot)"
    );
    ...
    println!(
        "  Differentiator: marketing + transactional + chat in one tool, generous pricing, GDPR-native, free unlimited contacts — best multi-channel value for SMBs"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "brevo".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_brevo, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_brevo(&["--version".to_string()], "brevo");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_brevo(&[], "brevo");
    }
}
