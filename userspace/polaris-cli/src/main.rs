...
fn run_polaris(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Polaris v8.5.0 (Slate OS) — Kubernetes best practices validator");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("Polaris v8.5.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "polaris".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_polaris, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/polaris"), "polaris");
        assert_eq!(basename(r"C:\bin\polaris.exe"), "polaris.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("polaris.exe"), "polaris");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_polaris(&["--help".to_string()], "polaris"), 0);
        assert_eq!(run_polaris(&["-h".to_string()], "polaris"), 0);
        let _ = run_polaris(&["--version".to_string()], "polaris");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_polaris(&[], "polaris");
    }
}
