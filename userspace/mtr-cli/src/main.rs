...
fn run_mtr(args: Vec<String>) -> i32 {
    ...
    if json {
        println!("{{\"report\": {{");
        println!(
            "  \"mtr\": {{\"src\": \"myhost\", \"dst\": \"{}\", \"psize\": 64}},",
            host
        );
        println!("  \"hubs\": [");
        println!(
            "    {{\"count\": {}, \"host\": \"gateway\", \"Loss%\": 0.0, \"Avg\": 0.5}},",
            count
        );
        println!(
            "    {{\"count\": {}, \"host\": \"isp-router\", \"Loss%\": 0.0, \"Avg\": 5.2}},",
            count
        );
        println!(
            "    {{\"count\": {}, \"host\": \"{}\", \"Loss%\": 0.0, \"Avg\": 12.3}}",
            count, host
        );
        println!("  ]");
        println!("}}}}");
    ...
    if report || wide {
        ...
        if no_dns {
            println!(
                "  1.|-- 192.168.1.1                 0.0%    {}   0.4   0.5   0.3   0.8   0.1",
                count
            );
            println!(
                "  2.|-- 10.0.0.1                    0.0%    {}   4.8   5.2   4.5   6.1   0.4",
                count
            );
            println!(
                "  3.|-- 72.14.215.85                0.0%    {}   8.2   8.5   7.9   9.3   0.3",
                count
            );
            println!(
                "  4.|-- 108.170.252.129             0.0%    {}  10.1  10.5   9.8  11.2   0.4",
                count
            );
            println!(
                "  5.|-- {}                 0.0%    {}  12.1  12.3  11.8  13.1   0.3",
                host, count
            );
        } else {
            println!(
                "  1.|-- gateway                     0.0%    {}   0.4   0.5   0.3   0.8   0.1",
                count
            );
            println!(
                "  2.|-- isp-router.example.net      0.0%    {}   4.8   5.2   4.5   6.1   0.4",
                count
            );
            println!(
                "  3.|-- core-rtr1.example.net       0.0%    {}   8.2   8.5   7.9   9.3   0.3",
                count
            );
            println!(
                "  4.|-- edge-rtr2.example.net       0.0%    {}  10.1  10.5   9.8  11.2   0.4",
                count
            );
            println!(
                "  5.|-- dns.google ({})   0.0%    {}  12.1  12.3  11.8  13.1   0.3",
                host, count
            );
        }
    } else {
