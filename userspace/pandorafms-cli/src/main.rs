...
fn run_pandora(args: &[String], prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Pandora FMS v777 (Slate OS)");
        return 0;
    }
    match prog {
        "pandora_agent" => {
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "pandora_server".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_pandora, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_pandora(&["--version".to_string()], "pandorafms");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_pandora(&[], "pandorafms");
    }
}
