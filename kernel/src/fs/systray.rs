...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_click_count = CLICK_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&TRAY, TrayState::new(), self_test_inner);
    ADD_COUNT.store(saved_add_count, Ordering::Relaxed);
    CLICK_COUNT.store(saved_click_count, Ordering::Relaxed);
    result
}
