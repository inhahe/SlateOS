...
fn run_gusto(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "    - Accountant-friendly: 30+ accounting tool integrations (QuickBooks, Xero, etc.)"
    );
    println!("    - Gusto Embedded (white-label API — used by Square Payroll, Bench, etc.)");
    println!(
        "  Cultural angle: famous 'wall of love' (customer photos at HQ), explicit company values"
    );
    println!(
        "  Critique: light on enterprise features (no global payroll, weak in 200+ employee org)"
    );
    println!("           Plus tier needed for time tracking is annoying for SMBs");
    println!(
        "  Differentiator: best UX in SMB payroll — onboarding new employees feels like a consumer app"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "gusto".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gusto, strip_ext};
