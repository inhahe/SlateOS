...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_add_count = ADD_COUNT.load(Ordering::Relaxed);
    let saved_refresh_count = REFRESH_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&WIDGETS, WidgetState::new(), self_test_inner);
    ADD_COUNT.store(saved_add_count, Ordering::Relaxed);
    REFRESH_COUNT.store(saved_refresh_count, Ordering::Relaxed);
    result
}
