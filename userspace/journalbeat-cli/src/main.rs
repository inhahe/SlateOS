...
//! journalbeat-cli — Slate OS Journalbeat log shipper
//!
...
fn run_journalbeat(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Journalbeat v8.14 (Slate OS) — Journal log shipper");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("journalbeat v8.14.3 (Slate OS)");
        return 0;
    }
    println!("Journalbeat v8.14.3 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "journalbeat".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_journalbeat, strip_ext};
