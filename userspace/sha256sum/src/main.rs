...
///
/// Delegates to the shared `sha2` crate.  This crate used to carry its own
/// SHA-256, which is the one place a private copy is least defensible: the
/// whole purpose of `sha256sum` is to produce a digest that some other
/// program, on some other machine, will compare against its own.  A digest
/// nobody else computes the same way is not a checksum.
///
/// The SHA-1 and SHA-512 implementations above and below stay local, because
/// `sha2` deliberately provides only SHA-256.
fn sha256(data: &[u8]) -> Vec<u8> {
    sha2::sha256(data).to_vec()
}
