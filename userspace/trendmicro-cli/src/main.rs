...
fn run_tm(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Trend Micro Maximum Security 17.8 (Slate OS) — Consumer + enterprise security");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Trend Micro Maximum Security 17.8.1308 (Slate OS)");
        return 0;
    }
    println!("Trend Micro Maximum Security 17.8.1308 (Slate OS)");
    ...
    println!(
        "  Network: TippingPoint IPS, Deep Discovery (APT detection), Smart Protection Network"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "trendmicro".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_tm, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_tm(&["--version".to_string()], "trendmicro");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_tm(&[], "trendmicro");
    }
}
