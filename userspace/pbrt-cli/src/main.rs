...
#[cfg(test)]
mod tests {
    use super::{basename, run_pbrt, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/pbrt"), "pbrt");
        assert_eq!(basename(r"C:\bin\pbrt.exe"), "pbrt.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("pbrt.exe"), "pbrt");
