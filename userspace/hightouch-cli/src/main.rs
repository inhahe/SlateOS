...
fn run_ht(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "          three engineers (ex-Segment, ex-Splunk, ex-startups), all under 30 at founding"
    );
    ...
    println!(
        "          fierce rivalry with Census (founded 2018-2019, similar pitch, similar funding stage)"
    );
    println!(
        "  Strategic position: 'data warehouse is the new system of record — Hightouch activates it':"
    );
    println!(
        "                    primary competitor: Census (very close rival), Polytomic, Workato (less data-focused)"
    );
    println!(
        "                    competing against legacy CDPs: Segment, mParticle, Tealium (Hightouch pitch: 'don't pay CDP, use your warehouse')"
    );
    println!("                    'Composable CDP' category leader");
    println!("                    sells deep into data teams + marketing ops + sales ops");
    println!(
        "                    bigger than Census by most measures (deals, ARR, employee count)"
    );
    ...
    println!(
        "    - Connect to data warehouse as source (Snowflake, BigQuery, Redshift, Databricks, Postgres)"
    );
    println!("    - Define 'Model' via SQL — what data + computed fields you want to send");
    println!(
        "    - Define 'Sync' — destination + field mapping + sync mode (full sync, change-data-capture)"
    );
    println!("    - Run on schedule or trigger via webhook + dbt completion");
    println!(
        "    - 200+ destinations: Salesforce, HubSpot, Marketo, Klaviyo, Braze, Iterable, Customer.io,"
    );
    println!(
        "      Meta Ads, Google Ads, TikTok, LinkedIn Ads, Snowflake, Snowpipe, Segment, mParticle,"
    );
    println!(
        "      Notion, Slack, Mixpanel, Amplitude, Zendesk, Intercom, Stripe, Snowflake, Pinterest, etc."
    );
    ...
    println!(
        "    - Hightouch becoming a full 'composable CDP' stack: ingest + storage + identity + activation"
    );
    ...
    println!(
        "              Ad platforms (Meta, Google, TikTok, LinkedIn, Snap, Pinterest, Reddit, Microsoft, Criteo)"
    );
    println!(
        "              CRM (Salesforce, HubSpot, Pipedrive, Microsoft Dynamics, Outreach, Salesloft, Apollo)"
    );
    println!(
        "              ESPs (Marketo, Klaviyo, Braze, Iterable, Customer.io, ActiveCampaign, Mailchimp)"
    );
    println!("              Support (Zendesk, Intercom, Freshdesk, Front, Help Scout)");
    println!(
        "              Product analytics (Mixpanel, Amplitude, Heap, Pendo, FullStory, Hotjar, PostHog)"
    );
    ...
    println!(
        "            Spotify (some teams), PetSmart, Calendly, Cars.com, Plaid, Warner Bros Discovery"
    );
    println!("            Imperfect Foods, Sweetgreen, Lucid, Ramp, Brex, Carta");
    println!(
        "            sweet spot: data-mature companies with Snowflake/BigQuery + dbt + activation needs"
    );
    ...
    println!(
        "           sync latency: minutes (not real-time) for warehouse-driven syncs — Personalization API closes this"
    );
    ...
    println!(
        "           customer-studio adoption requires data team buy-in (not pure marketing tool)"
    );
    println!(
        "  Differentiator: category creator + best-in-class destination coverage + dbt-native + composable CDP push + AI Decisioning — for data-mature orgs that want activation without buying Segment"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "hightouch".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ht, strip_ext};
