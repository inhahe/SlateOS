...
fn run_aria2c(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  -x, --max-connection-per-server=<NUM>  Max connections per server (default: 1)"
        );
        ...
        println!(
            "  --file-allocation=<METHOD>   File allocation method (none/prealloc/trunc/falloc)"
        );
    ...
    if args.iter().any(|a| a == "--version" || a == "-v") {
        println!("aria2 version 1.37.0 (Slate OS)");
    ...
    let urls: Vec<&str> = args
        .iter()
        .filter(|a| a.starts_with("http") || a.starts_with("ftp") || a.starts_with("magnet"))
        .map(|s| s.as_str())
        .collect();
    ...
    if args.iter().any(|a| a.contains("enable-rpc")) {
        let port = args
            .iter()
            .find_map(|a| a.strip_prefix("--rpc-listen-port="))
            .unwrap_or("6800");
        println!(
            "05/22 10:00:00 [NOTICE] IPv4 RPC: listening on TCP port {}",
            port
        );
    ...
    for url in &urls {
        ...
        let splits = args
            .iter()
            .find_map(|a| a.strip_prefix("-s").or_else(|| a.strip_prefix("--split=")))
