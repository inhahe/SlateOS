...
fn run_bedtools(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        println!("bedtools: flexible tools for genome analysis (v2.31.0, Slate OS)");
        println!();
    ...
    match subcmd {
        "--version" => println!("bedtools v2.31.0 (Slate OS)"),
        "intersect" => {
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_bedtools(&["--help".to_string()]), 0);
        assert_eq!(run_bedtools(&["-h".to_string()]), 0);
        let _ = run_bedtools(&["--version".to_string()]);
    }
