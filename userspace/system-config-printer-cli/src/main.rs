...
fn run_config_printer(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("system-config-printer v1.5 (Slate OS) — Printer configuration tool");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("system-config-printer v1.5 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "system-config-printer".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_config_printer, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(
            basename("/usr/bin/system-config-printer"),
            "system-config-printer"
        );
        assert_eq!(
            basename(r"C:\bin\system-config-printer.exe"),
            "system-config-printer.exe"
        );
    ...
    fn strip_ext_removes_extension() {
        assert_eq!(
            strip_ext("system-config-printer.exe"),
            "system-config-printer"
        );
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_config_printer(&["--help".to_string()], "system-config-printer"),
            0
        );
        assert_eq!(
            run_config_printer(&["-h".to_string()], "system-config-printer"),
            0
        );
