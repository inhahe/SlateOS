...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "kubescape".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kubescape, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/kubescape"), "kubescape");
        assert_eq!(basename(r"C:\bin\kubescape.exe"), "kubescape.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("kubescape.exe"), "kubescape");
