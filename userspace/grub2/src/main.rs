    ...
    while i < args.len() {
        ...
        if arg == "--target" {
            ...
            opts.boot_directory = args
                .get(i)
                .ok_or_else(|| GrubError::InvalidArgs("--boot-directory requires a value".into()))?
                .clone();
            ...
            opts.bootloader_id = args
                .get(i)
                .ok_or_else(|| GrubError::InvalidArgs("--bootloader-id requires a value".into()))?
                .clone();
...
fn run_install(args: &[String]) -> Result<(), GrubError> {
    ...
    if is_efi {
        let efi_dir = opts.efi_directory.as_deref().unwrap_or(EFI_FALLBACK_DIR);
...
fn generate_linux_entries(
    ...
    for (idx, kernel) in kernels.iter().enumerate() {
        ...
        // Recovery entry.
        let recovery_title = format!("{os_name}, with Linux {} (recovery mode)", kernel.version);
        let _ = writeln!(
            cfg,
...
fn generate_os_prober_section(cfg: &mut String) {
    ...
    let _ = writeln!(
        cfg,
        "# OS prober would scan for other operating systems here."
    );
...
fn parse_probe_args(args: &[String]) -> Result<ProbeOptions, GrubError> {
    ...
    let valid_targets = ["device", "fs", "fs_uuid", "fs_label", "drive", "partmap"];
...
"#;
    ...
    fn test_generate_grub_config_full() {
        ...
        let opts = MkconfigOptions {
            ...
            os_release_path: os_release_dir
                .join("os-release")
                .to_str()
                .unwrap()
                .to_string(),
    ...
    fn test_generate_grub_config_os_prober_disabled() {
        ...
        let defaults_dir = dir.join("etc_default");
        fs::create_dir_all(&defaults_dir).unwrap();
        fs::write(defaults_dir.join("grub"), "GRUB_DISABLE_OS_PROBER=true\n").unwrap();
    ...
    fn test_parse_install_args_efi_directory() {
        let args = vec!["--efi-directory".to_string(), "/boot/efi".to_string()];
        let opts = parse_install_args(&args).expect("parse");
    ...
    fn test_parse_editenv_args_custom_file() {
        let args = vec!["/tmp/grubenv".to_string(), "list".to_string()];
        let ea = parse_editenv_args(&args).expect("parse");
    ...
    fn test_editenv_unset() {
        ...
        let args = vec![path_str.clone(), "set".to_string(), "key=val".to_string()];
        run_editenv(&args).expect("set");

        let args = vec![path_str.clone(), "unset".to_string(), "key".to_string()];
    ...
    fn test_editenv_set_bad_format() {
        ...
        let args = vec![path_str, "set".to_string(), "noequals".to_string()];
        assert!(run_editenv(&args).is_err());
    ...
    fn test_editenv_set_empty_key() {
        ...
        let args = vec![path_str, "set".to_string(), "=value".to_string()];
        assert!(run_editenv(&args).is_err());
    ...
    fn test_personality_with_unix_path() {
        assert_eq!(extract_prog_name("/usr/sbin/grub-install"), "grub-install");
    }
    ...
    fn test_grubenv_serialize_truncation() {
        ...
        for i in 0..100 {
            env.set(
                &format!("long_variable_name_{i}"),
                &format!("long_value_{i}_padding"),
            );
        }
        let data = env.serialize();
        assert_eq!(data.len(), GRUB_ENV_SIZE);
