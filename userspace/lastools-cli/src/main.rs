...
//! lastools-cli — OurOS LAStools LiDAR processing
//!
...
fn run_lastools(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "lasinfo" => {
                println!("lasinfo (OurOS) — Display LAS/LAZ file information");
                println!("  -i FILE        Input LAS/LAZ file");
                println!("  -compute_density  Compute point density");
            }
            "las2txt" => {
                println!("las2txt (OurOS) — Convert LAS/LAZ to text");
                println!("  -i FILE        Input LAS/LAZ file");
    ...
    if args.iter().any(|a| a == "--version") { println!("LAStools v2.0 (OurOS)"); return 0; }
    match prog {
        "lasinfo" => {
        ...
        _ => {
            println!("LAStools v2.0 (OurOS) — LiDAR Processing");
