...
fn run_sparrow(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Sparrow Wallet v1.9 (Slate OS) — Bitcoin desktop wallet");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Sparrow Wallet v1.9 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "sparrow".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sparrow, strip_ext};
