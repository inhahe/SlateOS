...
fn run_pihole(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Pi-hole v5.18 (Slate OS) — Network-wide ad blocking");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Pi-hole v5.18.2 (Slate OS)"); return 0; }
    println!("Pi-hole v5.18.2 (Slate OS)");
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_pihole(&["--version".to_string()], "pihole");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_pihole(&[], "pihole");
    }
}
