...
fn run_skanlite(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("skanlite v23.08 (Slate OS) — KDE scanner application");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("skanlite v23.08 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "skanlite".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_skanlite, strip_ext};
