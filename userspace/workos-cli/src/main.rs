...
fn run_wos(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("WorkOS (Slate OS) — Enterprise readiness APIs for SaaS apps");
        println!();
        println!("Options:");
        ...
        println!(
            "  --fga                  FGA (Fine-Grained Authorization, Google Zanzibar-style)"
        );
        println!("  --version              Show version");
        return 0;
    }
    if args.iter().any(|a| a == "--version") {
        println!("WorkOS 2024 (Slate OS)");
        return 0;
    }
    println!("WorkOS 2024 (Slate OS)");
    ...
    println!(
        "          Grinich: prolific essayist/blog poster, advocate for 'SaaS enterprise readiness'"
    );
    println!("  Funding: Lachy Groom, Lightspeed, Benchmark + Stripe");
    println!("          ~$80M raised, est. ~$1B valuation 2024");
    println!(
        "  Strategy: 'Stripe for SSO' — APIs that let a SaaS startup sell into the enterprise overnight"
    );
    println!(
        "           NOT a full IdP — sits BETWEEN apps and customer-provided IdPs (Okta/Entra/etc.)"
    );
    ...
    println!(
        "  Customers: OpenAI, Vercel, Plaid, Webflow, Loom, Cursor, Anthropic Console, Perplexity"
    );
    ...
    println!(
        "       supports: Okta, Entra ID/Azure AD, Google Workspace, OneLogin, JumpCloud, ADFS, generic SAML, generic OIDC"
    );
    println!(
        "    2. Directory Sync — SCIM 2.0 endpoint hosted by WorkOS → maps to your user DB via webhooks"
    );
    println!(
        "       supports: Azure AD, Okta, Google Workspace, OneLogin, Rippling, JumpCloud, BambooHR + others"
    );
    ...
    println!(
        "    6. Admin Portal — pre-built UI where customer's IT admin configures their SSO/SCIM (no engineering work)"
    );
    println!(
        "    7. Audit Logs — pump events into WorkOS → customers query/export via API (SIEM-ready)"
    );
    println!(
        "    8. FGA (Fine-Grained Authorization) — Google Zanzibar-style relation tuples, $100K MAU free tier"
    );
    ...
    println!(
        "  Competitive position vs Auth0/Okta: 'AGI-friendly' — WorkOS doesn't try to BE the IdP, it lets customers bring their own"
    );
    ...
    println!(
        "  Differentiator: best-in-class developer experience + Admin Portal removes per-customer engineering"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "workos".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wos, strip_ext};
