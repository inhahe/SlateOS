...
//! crio-cli — Slate OS CRI-O container runtime
//!
...
fn run_crictl(args: &[String]) -> i32 {
    ...
    match subcmd {
        ...
        "ps" => {
            ...
            println!(
                "a1b2c3d4e5f6    docker.io/library/nginx       5 hours ago     Running   web-server    0        aabb11223344"
            );
            println!(
                "f6e5d4c3b2a1    docker.io/library/redis       3 hours ago     Running   cache         0        aabb11223344"
            );
        }
        "pods" => {
            println!(
            ...
            println!(
                "aabb11223344    5 hours ago     Ready   webapp-pod        default     0        (default)"
            );
            println!(
                "ccdd55667788    2 hours ago     Ready   monitoring-pod    kube-sys    0        (default)"
            );
        }
        "images" => {
        ...
        _ => println!("crictl: command {} completed", quoteaf_os(subcmd)),
    }
    0
}
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "crictl".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_crictl, strip_ext};
