...
fn run_rsyslogd(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("rsyslogd v8.2312 (Slate OS)");
        return 0;
    }
...
fn run_logger(args: &[String], _prog: &str) -> i32 {
    ...
    let msg: Vec<&str> = args
        .iter()
        .filter(|a| !a.starts_with('-'))
        .map(|s| s.as_str())
        .collect();
    // The message is every non-option argv word joined by spaces, so it is
    // wholly the caller's; the hand-written quotes around it end at the
    // first `'` the caller cares to type.
    println!("logger: logged {}", quoteaf_os(msg.join(" ")));
    0
}
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "rsyslogd".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_rsyslogd, strip_ext};
