...
fn run_ads(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Azure Data Studio v1.48 (Slate OS) — Cross-platform database tool");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Azure Data Studio v1.48.1 (Slate OS)");
        return 0;
    }
    println!("Azure Data Studio v1.48.1 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "azuredatastudio".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ads, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/azuredatastudio"), "azuredatastudio");
        assert_eq!(
            basename(r"C:\bin\azuredatastudio.exe"),
            "azuredatastudio.exe"
        );
