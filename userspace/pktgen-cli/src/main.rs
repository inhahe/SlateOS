...
//! pktgen-cli — Slate OS packet generator
//!
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "pktgen".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_pktgen, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/pktgen"), "pktgen");
        assert_eq!(basename(r"C:\bin\pktgen.exe"), "pktgen.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("pktgen.exe"), "pktgen");
