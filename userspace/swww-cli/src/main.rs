...
fn run_swww(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("swww v0.9 (Slate OS) — Animated wallpaper daemon for Wayland");
        println!();
        println!("Commands:");
    ...
    match cmd {
        ...
        "img" => {
            ...
            let transition = args
                .iter()
                .skip_while(|a| a.as_str() != "--transition-type")
                .nth(1)
                .map(|s| s.as_str())
                .unwrap_or("simple");
        ...
        _ => println!("swww: unknown command {}", quoteaf_os(cmd)),
    }
    0
}
...
fn run_daemon(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("swww-daemon v0.9 (Slate OS) — Wallpaper daemon process");
        println!();
        println!("Options:");
...
#[cfg(test)]
mod tests {
    use super::{basename, run_swww, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/swww"), "swww");
        assert_eq!(basename(r"C:\bin\swww.exe"), "swww.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("swww.exe"), "swww");
