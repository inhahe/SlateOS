...
fn run_mkcert(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("mkcert v1.4 (Slate OS) — Zero-config local CA for development certs");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("mkcert v1.4.4 (Slate OS)"); return 0; }
    println!("mkcert v1.4.4 (Slate OS)");
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_mkcert(&["--version".to_string()], "mkcert");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_mkcert(&[], "mkcert");
    }
}
