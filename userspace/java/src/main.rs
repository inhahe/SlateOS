...
fn run_java(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "--version" || a == "-version") {
        ...
        println!("OpenJDK Runtime Environment (Slate OS build 21.0.2+13)");
        println!("OpenJDK 64-Bit Server VM (Slate OS build 21.0.2+13, mixed mode)");
        return 0;
    }
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_java(vec!["--help".to_string()]), 0);
        assert_eq!(run_java(vec!["-h".to_string()]), 0);
        let _ = run_java(vec!["--version".to_string()]);
    }
