...
//! smartmontools-cli — OurOS SMART disk monitoring
//!
...
fn run_smartctl(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("smartctl v7.4 (OurOS) — SMART disk monitoring");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("smartctl v7.4 (OurOS, smartmontools)"); return 0; }
...
fn run_smartd(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("smartd v7.4 (OurOS) — SMART monitoring daemon");
        println!("  -n              Don't fork (foreground)");
        println!("  -q LEVEL        Quiet mode (never, errors, nodev)");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("smartd v7.4 (OurOS)"); return 0; }
