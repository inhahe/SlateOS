...
fn run_raco(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Racket Command-Line Tools (Slate OS)");
        println!();
        println!("Commands:");
    ...
    match subcmd {
        "pkg" => {
            ...
            match action {
                ...
                _ => println!("raco pkg: {} completed", quoteaf_os(action)),
            }
        }
        "test" => {
        ...
        _ => println!("raco: {} completed", quoteaf_os(subcmd)),
    }
    0
}
...
fn main() {
    ...
    let code = match prog.as_str() {
        "raco" => run_raco(&rest),
        "drracket" => {
            println!("DrRacket IDE v8.12 (Slate OS)");
            0
        }
        _ => run_racket(&rest),
    };
...
#[cfg(test)]
mod tests {
    use super::{basename, run_racket, strip_ext};
