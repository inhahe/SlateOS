...
pub fn dispatch_linux(nr: u64, args: &SyscallArgs) -> SyscallResult {
    match nr {
        ...
        nr::MBIND => sys_mbind(args),
        nr::SET_MEMPOLICY => sys_set_mempolicy(args),
        nr::GET_MEMPOLICY => sys_get_mempolicy(args),
        nr::MIGRATE_PAGES => sys_migrate_pages(args),
        nr::MOVE_PAGES => sys_move_pages(args),
        nr::SCHED_SETATTR => sys_sched_setattr(args),
        nr::SCHED_GETATTR => sys_sched_getattr(args),
        nr::LANDLOCK_CREATE_RULESET => sys_landlock_create_ruleset(args),
        nr::LANDLOCK_ADD_RULE => sys_landlock_add_rule(args),
