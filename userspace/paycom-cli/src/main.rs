...
fn run_pc(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Options:");
        println!(
            "  --beti                 Beti (Better Employee Transaction Interface — employees run their own payroll)"
        );
    ...
    println!(
        "          Richison: Oklahoma football lineage, philanthropy, OKC Thunder NBA team minority owner"
    );
    println!(
        "  History: founded as one of the first cloud payroll companies (Application Service Provider era)"
    );
    ...
    println!(
        "  Strategy: single-database HCM (every module shares one employee record — no duplicate data)"
    );
    println!(
        "           sales is famously aggressive (Glassdoor reviews + Indeed: Paycom comes up)"
    );
    ...
    println!(
        "    catches errors before payday (vs traditional 'find errors after the check is wrong')"
    );
    ...
    println!(
        "  Customers: mid-market US — Hilton, Newegg, Toys R Us (RIP), Gentle Giant Moving, etc."
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "paycom".to_string());
