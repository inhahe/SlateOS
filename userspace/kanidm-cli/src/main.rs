...
//! kanidm-cli — OurOS Kanidm identity management
//!
...
fn run_kanidm(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "kanidmd" => {
                println!("kanidmd (OurOS) — Kanidm identity server");
            ...
            _ => {
                println!("kanidm (OurOS) — Kanidm client CLI");
    ...
    if args.iter().any(|a| a == "--version") { println!("Kanidm v1.3.3 (OurOS)"); return 0; }
    match prog {
        "kanidmd" => {
            println!("Kanidm Server v1.3.3 (OurOS)");
