...
fn run_dbcli(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("dbcli v1.0.0 (Slate OS) — Generic database CLI with auto-completion");
        println!();
        println!("Options:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "dbcli".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_dbcli, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_dbcli(&["--version".to_string()], "dbcli");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_dbcli(&[], "dbcli");
    }
}
