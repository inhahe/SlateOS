...
fn run_httping(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("httping v3.5 (Slate OS) — Measure HTTP latency");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-S") {
        println!(
            "connected to {} (200), connect=1.2ms transfer=3.8ms total=5.0ms",
            url
        );
        println!(
            "connected to {} (200), connect=1.1ms transfer=3.6ms total=4.7ms",
            url
        );
        println!(
            "connected to {} (200), connect=1.3ms transfer=4.1ms total=5.4ms",
            url
        );
    } else {
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "httping".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_httping, strip_ext};
