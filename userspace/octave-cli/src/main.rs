...
//! octave-cli — OurOS GNU Octave CLI
//!
...
fn run_octave(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("GNU Octave — numerical computation (OurOS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version" || a == "-v") {
        println!("GNU Octave, version 8.4.0 (OurOS)");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_octave(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_octave(vec![]), 0);
    }
}
