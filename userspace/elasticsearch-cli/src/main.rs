...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/elasticsearch"), "elasticsearch");
        assert_eq!(basename(r"C:\bin\elasticsearch.exe"), "elasticsearch.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("elasticsearch.exe"), "elasticsearch");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_elasticsearch(&["--help".to_string()]), 0);
        assert_eq!(run_elasticsearch(&["-h".to_string()]), 0);
        assert_eq!(run_elasticsearch(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_elasticsearch(&[]), 0);
    }
}
