...
fn run_puppet(args: &[String]) -> i32 {
    ...
    match subcmd {
        ...
        "apply" => {
            ...
            println!(
                "Notice: /Stage[main]/Main/Service[nginx]/ensure: changed 'stopped' to 'running'"
            );
...
fn run_facter(args: &[String]) -> i32 {
    ...
    if let Some(fact) = query {
        match fact {
            "os" => println!(
                "{{name => Slate OS, family => Slate OS, release => {{major => 1, full => 1.0}}}}"
            ),
            "networking" => println!(
                "{{hostname => slateos-desktop, ip => 192.168.1.100, mac => 00:11:22:33:44:55}}"
            ),
            "memory" => println!(
                "{{system => {{total => 16.00 GiB, used => 8.50 GiB, available => 7.50 GiB}}}}"
            ),
...
#[cfg(test)]
mod tests {
    use super::{basename, run_puppet, strip_ext};
