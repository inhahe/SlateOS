...
fn run_skaffold(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Skaffold v2.11.1 (Slate OS) — Kubernetes dev workflow tool");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version" || a == "version") {
        println!("Skaffold v2.11.1 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "skaffold".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_skaffold, strip_ext};
