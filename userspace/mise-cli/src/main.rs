...
fn run_mise(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("mise 2024.11.0 (Slate OS) — Polyglot dev tool version manager");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/mise"), "mise");
        assert_eq!(basename(r"C:\bin\mise.exe"), "mise.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("mise.exe"), "mise");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_mise(&["--help".to_string()], "mise"), 0);
        assert_eq!(run_mise(&["-h".to_string()], "mise"), 0);
        let _ = run_mise(&["--version".to_string()], "mise");
    }
