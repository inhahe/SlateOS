...
fn run_mailspring(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("mailspring v1.13 (Slate OS) — Modern email client");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("mailspring v1.13 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "mailspring".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_mailspring, strip_ext};
