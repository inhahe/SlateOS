...
//! cfssl — Slate OS CloudFlare PKI/TLS toolkit
//!
...
fn run_cfssl(args: Vec<String>) -> i32 {
    ...
    match cmd {
        "version" => {
            println!("Version: 1.6.5 (Slate OS)");
        ...
        "certinfo" => {
            ...
            println!("  \"issuer\": {{\"common_name\": \"Slate OS Root CA\"}},");
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_cfssl(vec!["--help".to_string()]), 0);
        assert_eq!(run_cfssl(vec!["-h".to_string()]), 0);
        let _ = run_cfssl(vec!["--version".to_string()]);
    }
