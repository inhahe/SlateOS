//! Doug Lea's allocator in Alex Crichton's Rust port, dlmalloc-rs 0.2.14: the
//! heap behind `malloc`. Vendored with local changes, each marked in the code
//! with `LOCAL CHANGE (SlateOS)` or `LOCAL ADDITION (SlateOS)`; `VENDORED.md`
//! beside this file has the upstream version, checksums, licence and the list.
//!
//! The code is kept as upstream wrote it, so that comparing it with a later
//! release shows real changes and not restyling. That is why this file alone is
//! exempt from the workspace lints: they fire throughout on upstream's idiom --
//! pointer arithmetic on chunk headers, `as` casts between sizes, indexing bin
//! arrays whose bounds the algorithm itself establishes -- and a rewrite to
//! satisfy them would trade decades of use for a style.
#![allow(
    clippy::all,
    clippy::pedantic,
    clippy::unwrap_used,
...
// LOCAL CHANGE (SlateOS): on in this crate's unit-test builds, where a broken
// invariant should fail the test at the point it breaks. Upstream ties it to
// its `debug` feature, which this crate does not have. See `VENDORED.md`.
...
impl<A: Allocator> Dlmalloc<A> {
    ...
    unsafe fn sys_alloc(&mut self, size: usize) -> *mut u8 {
        self.check_malloc_state();
        // LOCAL ADDITION (SlateOS): C dlmalloc's `mmap_threshold` path, which
        // dlmalloc-rs 0.2.14 does not carry. A large request gets a mapping of
        // its own, so freeing it gives the memory back to the kernel at once
        ...
        // self.init_top(mn, tbase as usize + tsize - mn as usize - top_foot_size);
            ...
            // LOCAL CHANGE (SlateOS): never merge a new region with an
            // address-contiguous segment, in either direction. Upstream extends
            // the neighbouring segment, and a merged segment is later released
            // as one range; the kernel's native `munmap` drops a VMA record only
            // when handed that mapping's own base address, so the second
            // mapping's record would outlive its memory. One segment is one
            // mapping, always. See `VENDORED.md`.
    ...
    /// C dlmalloc's `mmap_alloc`: a chunk in a mapping of its own, laid out
    /// exactly as `mmap_resize` and `free` expect of an "mmapped" chunk.
    ///
    /// LOCAL ADDITION (SlateOS): not in dlmalloc-rs 0.2.14. See `VENDORED.md`.
    unsafe fn mmap_alloc(&mut self, nb: usize) -> *mut u8 {
        // C computes this with wrapping arithmetic and then checks
        // `mmsize > nb` for the wrap; checked arithmetic says the same thing
        // without an overflow that a debug build would panic on.
        let page_mask = self.system_allocator.page_size() - 1;
        let mmsize = match nb
            .checked_add(6 * mem::size_of::<usize>() + self.malloc_alignment() - 1)
            .and_then(|n| n.checked_add(page_mask))
        {
            Some(n) => n & !page_mask,
            // A request this large cannot be mapped.
            None => return ptr::null_mut(),
        };
        let (mm, allocsize, _flags) = self.system_allocator.alloc(mmsize);
        if mm.is_null() {
            return ptr::null_mut();
        }
        let offset = self.align_offset(Chunk::to_mem(mm.cast()));
        let psize = allocsize - offset - self.mmap_foot_pad();
        let p = mm.wrapping_add(offset).cast::<Chunk>();
        (*p).prev_foot = offset;
        (*p).head = psize;
        (*Chunk::plus_offset(p, psize)).head = Chunk::fencepost_head();
        (*Chunk::plus_offset(p, psize + mem::size_of::<usize>())).head = 0;
        if self.least_addr.is_null() || mm < self.least_addr {
            self.least_addr = mm;
        }
    ...
    /// The number of bytes the block at `mem` can hold, which may exceed what
    /// was asked for; 0 for a null or free pointer. C dlmalloc's
    /// `dlmalloc_usable_size`, which dlmalloc-rs does not carry.
    ///
    /// LOCAL ADDITION (SlateOS, 2026-09-25): not in dlmalloc-rs 0.2.14. See
    /// `VENDORED.md`.
    pub unsafe fn usable_size(&self, mem: *mut u8) -> usize {
        ...
        if Chunk::inuse(p) {
            Chunk::size(p) - self.overhead_for(p)
        } else {
            0
        }
    }
...
mod tests {
    use super::*;
    // LOCAL CHANGE (SlateOS): upstream runs these against its own `System`;
    // here they run against the system interface this libc actually uses.
    use super::super::SlateSystem;

    struct System;

    impl System {
        const fn new() -> SlateSystem {
            SlateSystem
        }
    }
