...
#[cfg(test)]
mod tests {
    use super::{basename, run_tox, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/tox"), "tox");
        assert_eq!(basename(r"C:\bin\tox.exe"), "tox.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("tox.exe"), "tox");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_tox(&["--help".to_string()]), 0);
        assert_eq!(run_tox(&["-h".to_string()]), 0);
        let _ = run_tox(&["--version".to_string()]);
    }
