...
fn run_bamboo(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "          Atlassian discontinued Bamboo Cloud Jan 2017 (Bitbucket Pipelines replaced it)"
    );
    println!(
        "          Bamboo Server EOL Feb 2024 — Data Center (large enterprise) is the only edition now"
    );
    ...
    println!(
        "    - Deployment projects: build artifact → environments (Dev/Staging/Prod) with approvals"
    );
    ...
    println!(
        "  Architecture: Java/Tomcat server + remote agents (push or pull) + Postgres/MySQL/Oracle DB"
    );
    ...
    println!(
        "  Differentiator: enterprise on-prem CI for Atlassian-stack shops — when cloud isn't allowed"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_bamboo, strip_ext};
