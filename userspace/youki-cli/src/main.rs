...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "youki".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_youki, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/youki"), "youki");
        assert_eq!(basename(r"C:\bin\youki.exe"), "youki.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("youki.exe"), "youki");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_youki(&["--help".to_string()], "youki"), 0);
        assert_eq!(run_youki(&["-h".to_string()], "youki"), 0);
        let _ = run_youki(&["--version".to_string()], "youki");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_youki(&[], "youki");
    }
}
