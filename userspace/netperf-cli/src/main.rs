...
fn run_netperf(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("netperf v2.7 (Slate OS) — Network performance benchmark");
        println!();
        println!("Options:");
    ...
    let test = args
        ...
        .nth(1)
        .map(|s| s.as_str())
        .unwrap_or("TCP_STREAM");
    println!(
        "MIGRATED {} {} from 0.0.0.0 (0.0.0.0) port 0 to {} () port 0",
        test, test, host
    );
    match test {
        "TCP_RR" => {
...
fn run_netserver(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("netserver v2.7 (Slate OS) — netperf server daemon");
        println!();
        println!("Options:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "netperf".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_netperf, strip_ext};
