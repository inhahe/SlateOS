...
fn run_vsc(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  AI features (native): Copilot Chat, Copilot Workspaces, Edit with Copilot, Inline Chat"
    );
    println!(
        "  Variants: VS Code, VS Code Insiders, VSCodium (MIT-pure), code-server (web), Theia (alt)"
    );
    println!(
        "  Differentiator: huge extension ecosystem + remote dev + Copilot integration + free"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_vsc, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/vscode"), "vscode");
        assert_eq!(basename(r"C:\bin\vscode.exe"), "vscode.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("vscode.exe"), "vscode");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_vsc(&["--help".to_string()], "vscode"), 0);
        assert_eq!(run_vsc(&["-h".to_string()], "vscode"), 0);
        let _ = run_vsc(&["--version".to_string()], "vscode");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_vsc(&[], "vscode");
    }
}
