...
fn run_gprof(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("gprof — display call graph profile (OurOS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("gprof (GNU Binutils) 2.42 (OurOS)");
        return 0;
    }
...
fn run_gcov(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("gcov — test coverage analysis (OurOS).");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_gprof(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_gprof(&[]), 0);
    }
}
