...
fn run_iar(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("IAR Embedded Workbench 9.60 (Slate OS) — Premium embedded IDE+compiler");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("IAR Embedded Workbench 9.60.4 (Slate OS)"); return 0; }
    println!("IAR Embedded Workbench 9.60.4 (Slate OS)");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_iar};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/iar"), "iar");
        assert_eq!(basename(r"C:\bin\iar.exe"), "iar.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("iar.exe"), "iar");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_iar(&["--help".to_string()], "iar"), 0);
        assert_eq!(run_iar(&["-h".to_string()], "iar"), 0);
        let _ = run_iar(&["--version".to_string()], "iar");
    }
