    ...
    if args.iter().any(|a| a == "--help" || a == "-h") {
        match prog {
            "image2xy" => {
                ...
                println!("image2xy v0.85 (Slate OS) — Source extraction from image");
                println!("  -o FILE    Output file");
                println!("  -w         Use median-filtered background");
            }
            "wcs-xy2rd" | "wcs-rd2xy" => {
                println!("Usage: {} WCS_FILE X Y", prog);
                println!("{} v0.85 (Slate OS) — Coordinate conversion", prog);
            }
            _ => {
                println!("Usage: solve-field [OPTIONS] IMAGE");
                println!("solve-field v0.85 (Slate OS) — Astrometric plate solver");
                println!();
                println!("Options:");
