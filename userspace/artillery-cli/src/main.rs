...
fn run_artillery(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Artillery load testing CLI (Slate OS).");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("artillery 2.0.6 (Slate OS)");
        return 0;
    }
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_artillery(vec!["--help".to_string()]), 0);
        assert_eq!(run_artillery(vec!["-h".to_string()]), 0);
        let _ = run_artillery(vec!["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_artillery(vec![]);
    }
}
