...
fn run_hyprpaper(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("hyprpaper v0.7 (Slate OS)");
        return 0;
    }
...
fn run_hyprctl_paper(args: &[String], _prog: &str) -> i32 {
    ...
    match cmd {
        ...
        _ => println!("hyprctl-paper: unknown command {}", quoteaf_os(cmd)),
    }
    0
}
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "hyprpaper".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_hyprpaper, strip_ext};
