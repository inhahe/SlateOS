...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/djvu"), "djvu");
        assert_eq!(basename(r"C:\bin\djvu.exe"), "djvu.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("djvu.exe"), "djvu");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_djvu("djvu", &["--help".to_string()]), 0);
        assert_eq!(run_djvu("djvu", &["-h".to_string()]), 0);
        assert_eq!(run_djvu("djvu", &["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_djvu("djvu", &[]), 0);
    }
}
