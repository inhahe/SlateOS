...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "vegas".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_vegas, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/vegas"), "vegas");
        assert_eq!(basename(r"C:\bin\vegas.exe"), "vegas.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("vegas.exe"), "vegas");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_vegas(&["--help".to_string()], "vegas"), 0);
        assert_eq!(run_vegas(&["-h".to_string()], "vegas"), 0);
        let _ = run_vegas(&["--version".to_string()], "vegas");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_vegas(&[], "vegas");
    }
}
