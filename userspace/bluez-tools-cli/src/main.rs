...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "bt-adapter".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_bt_adapter, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/bluez-tools"), "bluez-tools");
        assert_eq!(basename(r"C:\bin\bluez-tools.exe"), "bluez-tools.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("bluez-tools.exe"), "bluez-tools");
