...
//! dehancer-cli — OurOS Dehancer film emulation plug-in
//!
...
fn run_dh(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Dehancer Pro 7 (OurOS) — Premium film emulation");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Dehancer Pro 7.5.0 (OurOS)"); return 0; }
    println!("Dehancer Pro 7.5.0 (OurOS)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_dh(&["--version".to_string()], "dehancer"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_dh(&[], "dehancer"), 0);
    }
}
