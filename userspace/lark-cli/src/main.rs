...
//! lark-cli — OurOS Lark (ByteDance super-app: chat + docs + sheets + video + email)
...
fn run_lark(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Lark (OurOS) — ByteDance's all-in-one work super-app (chat + docs + meetings + email + base)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Lark 2024 (OurOS)"); return 0; }
    println!("Lark 2024 (OurOS)");
