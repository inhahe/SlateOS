...
fn run_yad(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("yad v13.0 (Slate OS) — Yet Another Dialog");
        println!();
    ...
    if args.iter().any(|a| a == "--version") { println!("yad v13.0 (Slate OS)"); return 0; }
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_yad};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/yad"), "yad");
        assert_eq!(basename(r"C:\bin\yad.exe"), "yad.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("yad.exe"), "yad");
