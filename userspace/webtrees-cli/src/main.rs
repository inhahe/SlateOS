...
fn run_webtrees(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("webtrees v2.1 (Slate OS) — Online genealogy application");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/webtrees"), "webtrees");
        assert_eq!(basename(r"C:\bin\webtrees.exe"), "webtrees.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("webtrees.exe"), "webtrees");
