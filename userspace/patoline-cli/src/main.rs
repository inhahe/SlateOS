...
fn run_patoline(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Patoline v0.2 (Slate OS) — Modern typesetting system");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Patoline v0.2 (Slate OS)");
        return 0;
    }
    println!("Patoline v0.2 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "patoline".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_patoline, strip_ext};
