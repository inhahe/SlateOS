...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_commit_count = COMMIT_COUNT.load(Ordering::Relaxed);
    let saved_key_count = KEY_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&STATE, State::new(), self_test_inner);
    COMMIT_COUNT.store(saved_commit_count, Ordering::Relaxed);
    KEY_COUNT.store(saved_key_count, Ordering::Relaxed);
    result
}
