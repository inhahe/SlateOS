...
fn run_bs(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("BugSnag / SmartBear Insight Hub (Slate OS) — App stability monitoring");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("BugSnag / SmartBear Insight Hub (Slate OS)");
        return 0;
    }
    println!("BugSnag / SmartBear Insight Hub (Slate OS)");
    ...
    println!(
        "  History: started focused on mobile crash reporting — JS error tracking added later"
    );
    ...
    println!(
        "    - First-class iOS/Android/React Native/Flutter/Unity support (mobile crash heritage)"
    );
    ...
    println!(
        "  SmartBear context: testing tools conglomerate (Swagger/OpenAPI, TestComplete, ReadyAPI, Cucumber)"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "bugsnag".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_bs, strip_ext};
