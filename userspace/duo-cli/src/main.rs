...
fn run_duo(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "           Dug Song: prolific security researcher (DSniff, fragroute, libdnet, Arbor Networks)"
    );
    println!("           Jon Oberheide: known for Android security research");
    println!(
        "  History: bootstrapped + Silicon Valley funded — Benchmark, Geodesic, Index + others"
    );
    ...
    println!(
        "           strong with Higher Ed (the green push notification on student/staff phones)"
    );
    ...
    println!(
        "  Pricing: Free up to 10 users (5 apps), Essentials $3/user/mo, Advantage $6, Premier $9"
    );
    println!("  Killer features:");
    println!(
        "    - Duo Push: one-tap MFA approval (mobile push) — UX gold standard, the original 'tap to approve'"
    );
    println!("    - Universal Prompt: modernized UI replacing Traditional Prompt (2022+)");
    println!(
        "    - Device Health: check OS version, screen lock, FDE, MDM enrollment before allowing access"
    );
    println!(
        "    - Risk-Based Authentication: skip MFA on known good context, step-up on anomalies"
    );
    println!("    - Phishing-resistant FIDO2/WebAuthn (security keys + passkeys)");
    println!(
        "    - Verified Push: requires entering 3-digit code from login screen (defeats push fatigue/MFA bombing)"
    );
    ...
    println!(
        "  Trust + reputation: Duo has avoided major breaches; identity provider that hasn't been LAPSUS$'d"
    );
    println!(
        "  Customers: Yale, Duke, Stanford, UC Berkeley (higher ed lead), Etsy, Twilio, Lyft, US Federal"
    );
    ...
    println!(
        "           became 'just another part of Cisco' post-acquisition — less standalone identity"
    );
    println!(
        "  Differentiator: simplest 'add MFA in an afternoon' product — push UX defined the category"
    );
    0
}
