...
fn run_wayvnc(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("wayvnc v0.8 (SlateOS) — VNC server for wlroots compositors");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("wayvnc v0.8 (SlateOS)"); return 0; }
...
fn run_wayvncctl(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("wayvncctl v0.8 (SlateOS) — Control wayvnc");
        println!();
        println!("Commands:");
