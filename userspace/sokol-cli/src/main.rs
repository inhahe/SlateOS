...
fn run_sokol(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("sokol-shdc v1.0.0 (Slate OS) — Sokol shader cross-compiler");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-V" || a == "--version") {
        println!("sokol-shdc v1.0.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "sokol-shdc".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sokol, strip_ext};
