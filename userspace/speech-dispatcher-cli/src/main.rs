...
//! speech-dispatcher-cli — Slate OS Speech Dispatcher
//!
...
fn run_spd_say(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("spd-say v0.11 (Slate OS) — Speak text via Speech Dispatcher");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn basename_strips_path() {
        ...
        assert_eq!(
            basename(r"C:\bin\speech-dispatcher.exe"),
            "speech-dispatcher.exe"
        );
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_dispatcher(&["--help".to_string()], "speech-dispatcher"),
            0
        );
