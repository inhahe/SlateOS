...
//! virt-cli — OurOS virt-manager CLI tools
//!
...
fn run_virt_install(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("virt-install — provision new virtual machines (OurOS).");
        println!();
        println!("Options:");
...
fn run_virt_clone(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("virt-clone — clone existing VMs (OurOS).");
        println!();
        println!("Options:");
...
fn run_virt_viewer(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("virt-viewer — VM console viewer (OurOS).");
        println!();
        println!("Options:");
...
fn run_virt_xml(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("virt-xml — edit libvirt XML (OurOS).");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_virt_install(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_virt_install(&[]), 0);
    }
}
