...
fn run_wkhtmltopdf(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("{} v0.12.6 (Slate OS) — HTML to PDF converter", prog);
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("{} v0.12.6 (Slate OS) with patched Qt", prog);
        return 0;
    }
    if args.len() < 2 {
...
fn run_wkhtmltoimage(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        println!("Usage: {} [OPTIONS] <input> <output>", prog);
        println!("{} v0.12.6 (Slate OS) — HTML to image converter", prog);
        println!();
        println!("Options:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "wkhtmltopdf".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wkhtmltopdf, strip_ext};
