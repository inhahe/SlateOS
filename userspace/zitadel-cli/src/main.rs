...
fn run_zitadel(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("ZITADEL v2.50 (Slate OS) — Cloud-native identity infrastructure");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("ZITADEL v2.50.4 (Slate OS)");
        return 0;
    }
    println!("ZITADEL v2.50.4 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "zitadel".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_zitadel, strip_ext};
