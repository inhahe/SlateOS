...
//! bun — Slate OS Bun JavaScript runtime and toolkit
//!
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_bun(vec!["--help".to_string()]), 0);
        assert_eq!(run_bun(vec!["-h".to_string()]), 0);
        let _ = run_bun(vec!["--version".to_string()]);
    }
