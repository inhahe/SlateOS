...
//! lilypond-cli — OurOS LilyPond music engraver
//!
...
fn run_lilypond(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("GNU LilyPond 2.24.3 (OurOS)");
...
fn run_lilypond_book(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("lilypond-book (GNU LilyPond) 2.24.3 (OurOS)");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_lilypond(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_lilypond(&[]), 0);
    }
}
