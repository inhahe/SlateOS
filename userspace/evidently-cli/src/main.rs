...
fn run_evidently(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Evidently v0.4 (Slate OS) — ML model monitoring & evaluation");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Evidently v0.4.33 (Slate OS)");
        return 0;
    }
    println!("Evidently v0.4.33 (Slate OS) — Model Monitoring");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "evidently".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_evidently, strip_ext};
