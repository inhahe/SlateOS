...
pub fn self_test() -> KernelResult<()> {
    ...
    // --- 3: several entries, including a directory and a binary payload --
    {
        let originals = [
            ...
            (b"c.bin".to_vec(), vec![0xAB_u8; 300], false),
        ];
        let writes: Vec<ZipWriteEntry> = originals
            .iter()
            .map(|(name, data, store_only)| ZipWriteEntry {
                name: name.clone(),
                data: data.clone(),
        ...
        for ((name, data, _), entry) in originals.iter().zip(parsed.iter()) {
            if &entry.name != name || &extract_entry(&archive, entry)? != data {
                return Err(KernelError::CorruptedData);
            }
        }
        crate::serial_println!("[zip]   multi-entry round-trip OK (4 entries)");
    }
