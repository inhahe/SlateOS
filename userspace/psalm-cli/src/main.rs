...
//! psalm-cli — OurOS Psalm static analysis tool for PHP
//!
...
fn run_psalter(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Psalter 5.25.0 — Psalm's code fixer (OurOS)");
        println!();
        println!("Options:");
...
fn run_psalm_plugin(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Psalm Plugin Manager 5.25.0 (OurOS)");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_psalm(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_psalm(&[]), 0);
    }
}
