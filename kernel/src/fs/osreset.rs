...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_next_checkpoint_id = NEXT_CHECKPOINT_ID.load(Ordering::Relaxed);
    let saved_next_plan_id = NEXT_PLAN_ID.load(Ordering::Relaxed);
    ...
    NEXT_CHECKPOINT_ID.store(saved_next_checkpoint_id, Ordering::Relaxed);
    NEXT_PLAN_ID.store(saved_next_plan_id, Ordering::Relaxed);
