...
run_side() { local side=$1; shift; diff_run timeout -k 2 30 env ${ENVV[@]+"${ENVV[@]}"} PATH="$bindir/$side" paste "$@"; }
...
run_case()  { compare - "$@"; report "${ENVV[*]:+${ENVV[*]} }paste $*"; }
run_stdin() {
  local input="$1"; shift
  compare "$input" "$@"
...
run_case posix.txt -s
run_case -s posix.txt
run_case posix.txt -- -s
