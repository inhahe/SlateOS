...
fn run_liquidctl(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("liquidctl v1.13 (Slate OS) — Liquid cooler and RGB control");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("liquidctl v1.13 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "liquidctl".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_liquidctl, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_liquidctl(&["--version".to_string()], "liquidctl");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_liquidctl(&[], "liquidctl");
    }
}
