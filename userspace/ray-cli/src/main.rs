...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/ray"), "ray");
        assert_eq!(basename(r"C:\bin\ray.exe"), "ray.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("ray.exe"), "ray");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_ray(&["--help".to_string()]), 0);
        assert_eq!(run_ray(&["-h".to_string()]), 0);
        assert_eq!(run_ray(&["--version".to_string()]), 0);
    }
