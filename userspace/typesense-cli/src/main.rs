...
fn run_typesense(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Typesense v0.25 (Slate OS) — Open-source search engine");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Typesense v0.25.2 (Slate OS)");
        return 0;
    }
    println!("Typesense v0.25.2 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "typesense-server".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_typesense, strip_ext};
