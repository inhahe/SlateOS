...
fn run_scmp_sys_resolver(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Resolve syscall names to numbers and vice versa (Slate OS).");
        return 0;
    }
...
fn run_firejail(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("firejail — Linux namespaces sandbox (Slate OS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("firejail version 0.9.72 (Slate OS)");
        return 0;
    }
...
fn run_bwrap(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("bwrap — bubblewrap sandbox (Slate OS).");
        println!();
        println!("Options:");
    ...
    let command = args
        ...
        .map(|s| s.as_str())
        .unwrap_or("sh");
    println!("bwrap: running {} in sandbox", quoteaf_os(command));
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_scmp_sys_resolver, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_scmp_sys_resolver(&["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_scmp_sys_resolver(&[]);
    }
}
