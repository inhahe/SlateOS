...
fn run_coqui(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Coqui TTS v0.22 (Slate OS) — Deep learning text-to-speech");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("Coqui TTS v0.22 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "coqui-tts".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_coqui, strip_ext};
