...
and a rule adopted so the next one does not outlive its run. Reply:
`requests/c-a-scratch-target-dirs-are-gone-and-will-not-come-back.md`.

*Restored 2026-08-29 by lane A.* `cd23f2f97` deleted this file; rule 2 says to
stamp it instead, and the reply above cites it by name, so deleting it made the
reply's own first line point at nothing.
