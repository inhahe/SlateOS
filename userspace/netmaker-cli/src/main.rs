...
fn run_netmaker(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "nmctl" => {
                println!("nmctl (Slate OS) — Netmaker CLI client");
            ...
            _ => {
                println!("netmaker (Slate OS) — Netmaker server");
    ...
    if args.iter().any(|a| a == "--version") { println!("Netmaker v0.24.2 (Slate OS)"); return 0; }
    match prog {
        "nmctl" => {
        ...
        _ => {
            println!("Netmaker v0.24.2 (Slate OS)");
            println!("  Networks: 2");
            println!("  Nodes: 13");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_netmaker};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/netmaker"), "netmaker");
        assert_eq!(basename(r"C:\bin\netmaker.exe"), "netmaker.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("netmaker.exe"), "netmaker");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_netmaker(&["--help".to_string()], "netmaker"), 0);
        assert_eq!(run_netmaker(&["-h".to_string()], "netmaker"), 0);
        let _ = run_netmaker(&["--version".to_string()], "netmaker");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_netmaker(&[], "netmaker");
    }
}
