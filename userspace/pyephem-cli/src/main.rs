...
fn run_ephem(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("ephem v4.1 (Slate OS) — Astronomical ephemeris calculator");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("PyEphem v4.1.5 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "ephem".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ephem, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_ephem(&["--version".to_string()], "pyephem");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_ephem(&[], "pyephem");
    }
}
