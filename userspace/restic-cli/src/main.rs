...
//! restic-cli — OurOS Restic backup CLI
//!
...
fn run_restic(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Restic backup program (OurOS).");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_restic(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_restic(vec![]), 0);
    }
}
