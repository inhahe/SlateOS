...
fn run_scilab(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Scilab v2024.1 (Slate OS) — Open-source numerical computing");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Scilab v2024.1 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "scilab".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_scilab, strip_ext};
