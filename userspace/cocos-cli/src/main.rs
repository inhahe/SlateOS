...
fn run_cocos(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  --build TARGET         Build (web / native / minigame [wechat/bytedance/etc] / android)"
        );
    ...
    println!(
        "         cocos2d-x C++ port (2010 China) → cocos2d-js → cocos2d-html5 → Cocos Creator (2016)"
    );
    println!(
        "  Pricing: FREE — Cocos Engine itself is free; Cocos Service (cloud) and Asset Store paid"
    );
    println!(
        "  Languages: TypeScript (preferred) or JavaScript, with optional C++ binding extensions"
    );
    println!("  Targets: web (HTML5), native (C++ for Windows/Mac/Linux/iOS/Android),");
    println!(
        "           Chinese mini-games (WeChat, ByteDance/Douyin, Alipay, QQ — 1B+ users each)"
    );
    ...
    println!(
        "  Differentiator: best-of-class HTML5 / mini-game export, lightweight runtime (~250KB)"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "cocos".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_cocos, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/cocos"), "cocos");
        assert_eq!(basename(r"C:\bin\cocos.exe"), "cocos.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("cocos.exe"), "cocos");
