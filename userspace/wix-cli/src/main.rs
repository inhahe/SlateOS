...
fn run_wix(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Wix (Slate OS) — drag-and-drop website builder + commerce + business apps (NASDAQ:WIX)"
        );
        println!();
        println!("Options:");
    ...
    println!(
        "  Founders: Avishai Abrahami (CEO), Nadav Abrahami (his brother), Giora Kaplan, 2006"
    );
    println!(
        "          founded after a personal frustration trying to build a website for one of their startups"
    );
    ...
    println!(
        "                    primary competitor: Squarespace (smaller, design-led), GoDaddy Websites + Marketing, Webflow (designer-friendlier)"
    );
    println!("                    commerce competitor: Shopify (more commerce-deep), Squarespace");
    println!("                    booking competitor: Calendly, Acuity, Booksy");
    println!(
        "                    pitch: '1 platform for any kind of small biz' — restaurants + retail + services + creators"
    );
    println!(
        "                    massively heavy advertising — Wix Super Bowl ads, YouTuber sponsorships, TikTok"
    );
    println!(
        "                    aggressively international: ~50% of revenue outside the Americas"
    );
    ...
    println!(
        "    - Wix ADI (Artificial Design Intelligence — auto-generates a site from a few questions)"
    );
    ...
    println!(
        "    - Wix's bet to capture the designer/agency market it historically lost to Webflow"
    );
    ...
    println!(
        "    - Wix Stores (e-commerce — multiple shipping zones, dropshipping, POS, multichannel)"
    );
    ...
    println!(
        "            heavy in: small biz worldwide, freelancers, restaurants, fitness studios"
    );
    println!(
        "            very international: huge in LATAM, Eastern Europe, Israel obviously, MENA, SE Asia"
    );
    println!(
        "            celebrity/brand: Karlie Kloss site, NICE-Network for Israel arts, many NGOs"
    );
    ...
    println!(
        "           reputation: 'cheap-looking' sites by amateurs — Wix Studio targets this perception"
    );
    ...
    println!(
        "  Differentiator: most all-in-one platform (site + commerce + bookings + restaurants + POS + dev platform) at SMB pricing, with Velo for developers — for any small biz wanting one vendor for everything"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wix, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/wix"), "wix");
        assert_eq!(basename(r"C:\bin\wix.exe"), "wix.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("wix.exe"), "wix");
