...
//! stalwart-cli — OurOS Stalwart Mail Server
//!
...
fn run_stalwart(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Stalwart Mail v0.8 (OurOS) — All-in-one mail server");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Stalwart Mail v0.8.5 (OurOS)"); return 0; }
    println!("Stalwart Mail v0.8.5 (OurOS)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_stalwart(&["--version".to_string()], "stalwart"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_stalwart(&[], "stalwart"), 0);
    }
}
