...
use crate::sfnt::{Span, i16_at, u16_at, u32_at};
...
/// font cannot make parsing quadratic. Real fonts use single digits; the
...
fn parse_gpos(data: &[u8], span: Span) -> Option<Vec<usize>> {
    ...
    // Which lookups the `kern` feature uses. Ascending and deduplicated,
    // because lookups apply in LookupList order regardless of the order a
    // feature happens to list them in.
    ...
    for i in 0..usize::from(feature_count) {
        ...
        if data.get(rec..rec.checked_add(4)?)? != b"kern" {
            continue;
        }
    ...
    for idx in indices {
        ...
        collect_pair_pos(data, lookup, &mut subtables);
...
/// Append the `PairPos` subtables of one lookup, unwrapping extensions.
fn collect_pair_pos(data: &[u8], lookup: usize, out: &mut Vec<usize>) {
    ...
    if kind != LOOKUP_PAIR_POS && kind != LOOKUP_EXTENSION {
        return;
    }
    for i in 0..usize::from(count) {
        ...
        let wrapped = sub.checked_add(2).and_then(|o| u16_at(data, o));
        if wrapped != Some(LOOKUP_PAIR_POS) {
            continue;
        }
...
fn coverage_index(data: &[u8], table: usize, glyph: u16) -> Option<u16> {
...
/// a real class that a subtable may kern, so an unlisted glyph is `Some(0)`
/// rather than `None`.
fn glyph_class(data: &[u8], table: usize, glyph: u16) -> Option<u16> {
    match u16_at(data, table)? {
        1 => {
...
fn range_containing(data: &[u8], first: usize, count: usize, glyph: u16) -> Option<usize> {
...
fn binary_search(count: usize, probe: impl Fn(usize) -> Option<core::cmp::Ordering>) -> Option<usize> {
    let mut lo = 0usize;
    let mut hi = count;
    while lo < hi {
