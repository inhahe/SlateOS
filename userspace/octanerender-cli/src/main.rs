...
//! octanerender-cli — OurOS OTOY Octane Render (GPU path tracer)
//!
...
fn run_octane(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("OTOY Octane Render 2024 (OurOS) — GPU-only spectral path tracer");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Octane Render 2024.1.1 (OurOS)"); return 0; }
    println!("Octane Render 2024.1.1 (OurOS)");
