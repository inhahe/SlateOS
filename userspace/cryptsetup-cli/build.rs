...
//! executable whose name contains an installer keyword such as "setup" — which
//! includes both the real `cryptsetup-cli.exe` binary and the unit-test harness
//! exe that Cargo names `cryptsetup-cli-<hash>.exe`. Without a manifest
//! declaring `asInvoker`, `cargo test` cannot even launch the harness: it fails
...
fn main() {
    ...
    embed_manifest::embed_manifest(embed_manifest::new_manifest("SlateOS.cryptsetup-cli"))
