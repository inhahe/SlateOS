...
fn run_jpgicc(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("jpgicc v2.16 (Slate OS) — Apply ICC profiles to JPEG files");
        println!();
        println!("Options:");
...
fn run_linkicc(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("linkicc v2.16 (Slate OS) — Link multiple ICC profiles into a device link");
        println!();
        println!("Options:");
        println!("  -t INTENT         Rendering intent (0-3)");
        println!("  -b                Black point compensation");
...
fn run_transicc(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("transicc v2.16 (Slate OS) — Transform colors between ICC profiles");
        println!();
        println!("Options:");
        println!("  -i PROFILE        Input ICC profile");
        println!("  -o PROFILE        Output ICC profile");
...
fn run_tificc(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("tificc v2.16 (Slate OS) — Apply ICC profiles to TIFF files");
        println!();
        println!("Options:");
        println!("  -i PROFILE        Input ICC profile");
        println!("  -o PROFILE        Output ICC profile");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "jpgicc".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_jpgicc, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_jpgicc(&["--version".to_string()], "littlecms");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_jpgicc(&[], "littlecms");
    }
}
