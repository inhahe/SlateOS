...
fn run_shellcheck(args: Vec<String>) -> i32 {
    ...
    match format {
        "json" => {
            println!("[");
            println!(
                "  {{\"file\":\"{}\",\"line\":3,\"column\":1,\"level\":\"warning\",\"code\":2034,\"message\":\"x appears unused. Verify use (or export if used externally).\"}},",
                target
            );
            println!(
            ...
            println!(
                "  {{\"file\":\"{}\",\"line\":12,\"column\":3,\"level\":\"info\",\"code\":2046,\"message\":\"Quote this to prevent word splitting.\"}}",
                target
            );
            println!("]");
        }
        "gcc" => {
            println!(
                "{}:3:1: warning: x appears unused. Verify use (or export if used externally). [SC2034]",
                target
            );
            println!(
                "{}:7:5: error: Double quote to prevent globbing and word splitting. [SC2086]",
                target
            );
            println!(
                "{}:12:3: info: Quote this to prevent word splitting. [SC2046]",
                target
            );
        }
        _ => {
