...
fn run_freshdesk(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  --omnichannel          Omnichannel bundle (Support + Messaging + Contact Center)"
        );
    ...
    println!(
        "  Origin story: Girish Mathrubootham read a Hacker News thread about Zendesk's 60-300% price hike"
    );
    println!(
        "              built Freshdesk as the cheaper, friendlier alternative — launched 2011"
    );
    ...
    println!(
        "          Pro $49/agent/mo (custom roles, multi-product, advanced reporting, sandbox)"
    );
    println!(
        "          Enterprise $79/agent/mo (audit logs, IP allow-list, custom objects, agent shifts)"
    );
    println!(
        "          Omnichannel bundles: Growth+ $29/agent/mo, Pro+ $69/agent/mo, Enterprise+ $109/agent/mo"
    );
    ...
    println!(
        "              Slack, Salesforce, Hubspot, Jira, Office 365, Google Workspace, Shopify"
    );
    ...
    println!(
        "            Bridgestone, Klarna, Decathlon, Pearson, American Express (parts), Honda"
    );
    println!("            sweet spot: SMB + mid-market, 10-1,000 agents");
    println!("            strong globally — Asia, Europe, LATAM (vs Zendesk's NA strength)");
    println!(
        "  Critique: AI features still maturing — Freddy lags Intercom's Fin in deflection numbers"
    );
    println!("           UI improvements lag Zendesk Agent Workspace");
    println!(
        "           pricing tier jumps non-linear — Enterprise needed for many 'enterprise' basics"
    );
    println!(
        "           Freshworks broader story (sales + IT + support combined) inconsistent in execution"
    );
    println!(
        "  Differentiator: best free tier in helpdesk category + global price-competitive vs Zendesk"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "freshdesk".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_freshdesk, strip_ext};
