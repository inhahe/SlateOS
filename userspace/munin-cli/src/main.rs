...
//! munin-cli — OurOS Munin network monitoring
//!
...
fn run_munin_node(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("munin-node v2.0 (OurOS) — Munin monitoring node agent");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("munin-node v2.0 (OurOS)"); return 0; }
...
fn run_munin_run(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("munin-run v2.0 (OurOS) — Run a Munin plugin manually");
        return 0;
    }
    if args.is_empty() {
...
fn run_munin_update(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("munin-update v2.0 (OurOS) — Fetch data from munin nodes");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("munin-update v2.0 (OurOS)"); return 0; }
...
fn run_munin_cron(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("munin-cron v2.0 (OurOS) — Periodic data collection");
        return 0;
    }
    let _ = args;
