...
//! zsnes-cli — OurOS ZSNES Super Nintendo emulator
//!
...
fn run_zsnes(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("zsnes v2.0 (OurOS) — Super Nintendo Entertainment System emulator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("ZSNES v2.0.12 (OurOS)"); return 0; }
    let files: Vec<&String> = args.iter().filter(|a| !a.starts_with('-')).collect();
    if files.is_empty() {
        println!("ZSNES v2.0.12 (OurOS) — SNES Emulator");
