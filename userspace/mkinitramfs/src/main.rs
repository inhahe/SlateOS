//! OurOS initial RAM filesystem builder.
//!
...
fn generate_init_script(config: &InitramfsConfig) -> String {
    ...
    script.push_str("# OurOS initramfs init script\n");
...
fn lsinitramfs_main(args: &[String]) -> i32 {
    ...
    for arg in args {
        match arg.as_str() {
            ...
            "--version" => {
                println!("lsinitramfs (OurOS) {VERSION}");
                return 0;
            }
...
fn mkinitramfs_main(args: &[String]) -> i32 {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--version" => {
                println!("mkinitramfs (OurOS) {VERSION}");
                return 0;
            }
            s if !s.starts_with('-') => {
...
fn update_initramfs_main(args: &[String]) -> i32 {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--version" => {
                println!("update-initramfs (OurOS) {VERSION}");
                return 0;
            }
            other => {
