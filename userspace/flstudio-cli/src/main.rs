...
//! flstudio-cli — Slate OS FL Studio (Image-Line DAW, pattern-based)
//!
...
fn run_fl(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Creator: Didier Dambrin (a.k.a. gol) — solo dev of original FruityLoops 1.0 (Dec 1997)"
    );
    println!(
        "  Rename: 'FruityLoops' → 'FL Studio' in 2003 (Hasbro Fruit Loops trademark dispute)"
    );
    ...
    println!(
        "  Workflow: pattern-based (step sequencer + piano roll) → arrange patterns in playlist"
    );
    ...
    println!(
        "  Native plugins: Sytrus, Harmor, Harmless, Fruity Reeverb 2, Maximus, Soundgoodizer,"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "flstudio".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_fl, strip_ext};
