...
fn run_wl_screencast(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("wl-screencast v0.1 (Slate OS)");
        return 0;
    }
    let output = args
        .iter()
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "wl-screencast".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wl_screencast, strip_ext};
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_wl_screencast(&["--help".to_string()], "wl-screencast"),
            0
        );
