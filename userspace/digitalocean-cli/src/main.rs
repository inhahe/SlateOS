...
fn run_do(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "DigitalOcean (Slate OS) — developer cloud (Droplets + App Platform + Managed services, NYSE:DOCN)"
        );
        println!();
        println!("Options:");
        ...
        println!(
            "  --databases            Managed Databases (Postgres, MySQL, Redis, MongoDB, Kafka)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("DigitalOcean 2024 (Slate OS) — doctl 1.x");
        return 0;
    }
    ...
    println!(
        "  Founders: Ben Uretsky + Moisey Uretsky + Mitch Wainer + Jeff Carr + Alec Hartman, 2011"
    );
    ...
    println!(
        "          Yancey Spruill: CEO 2019-2024, replaced by Paddy Srinivasan (former CTO of NetSuite)"
    );
    ...
    println!(
        "                    pitch: 'the developer cloud that doesn't punish you for not being a Fortune 500'"
    );
    ...
    println!(
        "                    DigitalOcean's wedge: predictable pricing + best DX + tutorials + ~600+ docs"
    );
    println!("                    'AWS for the rest of us' = simplified UI + SKUs + pricing");
    println!(
        "                    AI push: Paperspace acquisition $111M (Mar 2023) for GPU compute"
    );
    ...
    println!(
        "    doctl compute droplet create my-server --size s-1vcpu-1gb --image ubuntu-22-04-x64 --region nyc3"
    );
    println!("    doctl compute droplet list");
    println!("    doctl compute droplet delete my-server");
    println!(
        "    doctl kubernetes cluster create my-k8s --region nyc3 --size s-2vcpu-4gb --count 3"
    );
    ...
    println!(
        "           Cloudways acquisition $350M was richly priced for managed hosting margins"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "digitalocean".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_do, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_do(&["--version".to_string()], "digitalocean");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_do(&[], "digitalocean");
    }
}
