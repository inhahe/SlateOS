...
//! mosquitto-cli — Slate OS Mosquitto MQTT CLI
//!
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/mosquitto"), "mosquitto");
        assert_eq!(basename(r"C:\bin\mosquitto.exe"), "mosquitto.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("mosquitto.exe"), "mosquitto");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_pub(&["--help".to_string()]), 0);
        assert_eq!(run_pub(&["-h".to_string()]), 0);
        let _ = run_pub(&["--version".to_string()]);
    }
