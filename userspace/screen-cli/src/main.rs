...
//! screen-cli — Slate OS GNU Screen CLI
//!
...
fn run_screen(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("GNU Screen — terminal multiplexer (Slate OS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-v" || a == "--version") {
        println!("Screen version 4.09.01 (Slate OS)");
        return 0;
    }
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_screen(vec!["--help".to_string()]), 0);
        assert_eq!(run_screen(vec!["-h".to_string()]), 0);
        let _ = run_screen(vec!["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_screen(vec![]);
    }
}
