...
fn run_hive(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Hive (Slate OS) — project management built by user votes (democratic product development)"
        );
        println!();
        println!("Options:");
    ...
    println!(
        "                    primary competitor: Asana, Monday.com, ClickUp, Trello, Basecamp"
    );
    println!(
        "                    Hive's wedge: rapid feature shipping based on user votes (every feature went through 'Hive.com/forum')"
    );
    println!(
        "                    'productivity platform' positioning + chat + meeting notes + time tracking + analytics in one"
    );
    println!(
        "                    customer base skews to media + creative agencies + NYC-area companies"
    );
    ...
    println!(
        "    add-ons: HiveAnalytics ($5/user/mo), HiveTime (timesheets), HiveAutomate (workflow automation)"
    );
    ...
    println!(
        "    - Productivity dashboards: who's done what, project health, workload distribution"
    );
    ...
    println!(
        "    - Marketing slogan: 'the world's first democratically-built productivity platform'"
    );
    ...
    println!(
        "            Google (some teams), Starbucks (departments), Toyota (regional), Comcast"
    );
    ...
    println!(
        "            sweet spot: 50-500 person media + creative + consulting orgs in NYC area"
    );
    ...
    println!(
        "           features-per-tier model confusing (HiveAnalytics + HiveTime + HiveAutomate all separate add-ons)"
    );
    ...
    println!(
        "  Differentiator: built-by-user-votes democratic product dev + 'everything in one app' (chat + notes + email + actions) + NYC media verticals — for teams who want one tool not five"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_hive, strip_ext};
