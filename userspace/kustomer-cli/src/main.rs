...
fn run_kustomer(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Kustomer (Slate OS) — customer-centric (not ticket-centric) support platform");
        println!();
        println!("Options:");
        println!(
            "  --enterprise           Enterprise tier (custom pricing, typically $89-$139/seat/mo)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Kustomer 2024 (Slate OS)");
        return 0;
    }
    println!("Kustomer 2024 (Slate OS)");
    println!(
        "  Vendor: Kustomer, Inc. (NYC, NY — owned by Meta then divested back to founders 2023)"
    );
    ...
    println!(
        "    - Meta (then Facebook) acquired Nov 2020 for $1B announced (delayed by EU regulators to Feb 2022)"
    );
    println!("    - Meta divested back to original investors (incl. founders) Jul 2023");
    println!(
        "      reasons: Meta deprioritized B2B, EU competition pressure post-WhatsApp Business focus"
    );
    ...
    println!(
        "    - Every customer has ONE unified Timeline showing ALL interactions across all channels"
    );
    println!(
        "    - Order history, conversations, returns, page views, app sessions — all in chronological order"
    );
    ...
    println!(
        "            Glovo, Sweetgreen, Ring, ThirdLove, Rent the Runway, Glossier, Away, Bombas"
    );
    println!("            sweet spot: 50-500 agents, B2C high-volume + relationship-driven");
    println!("  Critique: enterprise-priced — not for SMB");
    println!(
        "           Meta era distracted product roadmap; recovery still in progress under independence"
    );
    ...
    println!(
        "  Differentiator: most genuinely customer-centric (not ticket-centric) support data model — Timeline is the product"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "kustomer".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kustomer, strip_ext};
