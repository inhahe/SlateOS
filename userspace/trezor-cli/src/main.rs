...
fn run_trezor(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("trezorctl v0.14 (Slate OS) — Trezor hardware wallet CLI");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("trezorctl v0.14 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "trezorctl".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_trezor, strip_ext};
