...
//! irsim-cli — OurOS IRSIM switch-level simulator
//!
...
fn run_irsim(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("IRSIM v9.7 (OurOS) — Switch-level digital circuit simulator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("IRSIM v9.7.103 (OurOS)"); return 0; }
    println!("IRSIM v9.7.103 (OurOS) — Switch-Level Simulator");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_irsim(&["--version".to_string()], "irsim"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_irsim(&[], "irsim"), 0);
    }
}
