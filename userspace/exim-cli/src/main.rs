...
fn run_exim(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Exim v4.97 (Slate OS) — Mail Transfer Agent");
        println!();
        println!("Options:");
...
#[cfg(test)]
mod tests {
    use super::{basename, run_exim, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/exim"), "exim");
        assert_eq!(basename(r"C:\bin\exim.exe"), "exim.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("exim.exe"), "exim");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_exim(&["--help".to_string()], "exim"), 0);
        assert_eq!(run_exim(&["-h".to_string()], "exim"), 0);
        let _ = run_exim(&["--version".to_string()], "exim");
    }
