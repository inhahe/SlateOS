...
fn run_ttfautohint(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("ttfautohint v1.8 (Slate OS)");
        return 0;
    }
    let input = args.first().map(|s| s.as_str()).unwrap_or("input.ttf");
    println!("ttfautohint: processing {}...", quoteaf_os(input));
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "ttfautohint".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ttfautohint, strip_ext};
