...
fn run_pendo(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Pendo (Slate OS) — product experience platform (analytics + guides + feedback)");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/pendo"), "pendo");
        assert_eq!(basename(r"C:\bin\pendo.exe"), "pendo.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("pendo.exe"), "pendo");
