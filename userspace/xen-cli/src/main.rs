...
fn run_xen(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Xen Project 4.19 (Slate OS) — Open-source type-1 hypervisor");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Xen 4.19.0 (Slate OS)");
        return 0;
    }
    println!("Xen Project 4.19.0 (Slate OS)");
...
#[cfg(test)]
mod tests {
    use super::{basename, run_xen, strip_ext};
