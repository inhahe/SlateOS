...
fn run_freetype(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("freetype-info v2.13 (Slate OS) — FreeType font information tool");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("FreeType v2.13 (Slate OS)");
        return 0;
    }
    let file = args
        .iter()
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "freetype-info".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_freetype, strip_ext};
