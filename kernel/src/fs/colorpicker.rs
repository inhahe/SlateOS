...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_pick_count = PICK_COUNT.load(Ordering::Relaxed);
    let saved_sample_count = SAMPLE_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&STATE, State::new(), self_test_inner);
    NEXT_ID.store(saved_next_id, Ordering::Relaxed);
    PICK_COUNT.store(saved_pick_count, Ordering::Relaxed);
    SAMPLE_COUNT.store(saved_sample_count, Ordering::Relaxed);
    result
}
