...
fn run_embree(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Embree v4.3 (Slate OS) — High-performance ray tracing kernels");
        println!();
        println!("Commands:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "embree".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_embree, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/embree"), "embree");
        assert_eq!(basename(r"C:\bin\embree.exe"), "embree.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("embree.exe"), "embree");
