...
//! foreman-cli — OurOS Foreman lifecycle management
//!
...
fn run_foreman(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("foreman v3.9 (OurOS) — Lifecycle management tool");
        println!();
    ...
    if args.iter().any(|a| a == "--version") { println!("foreman v3.9 (OurOS)"); return 0; }
...
fn run_hammer(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("hammer v3.9 (OurOS) — Foreman CLI client");
        println!();
        println!("Resources:");
    ...
    if args.iter().any(|a| a == "--version") { println!("hammer v3.9 (OurOS, Foreman CLI)"); return 0; }
    if args.len() >= 2 {
        ...
        println!("  1  | web-01       | OurOS 1.0    | OK");
        println!("  2  | db-01        | OurOS 1.0    | OK");
        println!("  3  | app-01       | OurOS 1.0    | OK");
    } else {
...
fn run_foreman_rake(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("foreman-rake v3.9 (OurOS) — Foreman maintenance tasks");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_foreman(&["--version".to_string()], "foreman"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_foreman(&[], "foreman"), 0);
    }
}
