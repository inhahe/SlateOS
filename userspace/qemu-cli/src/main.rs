...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/qemu"), "qemu");
        assert_eq!(basename(r"C:\bin\qemu.exe"), "qemu.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("qemu.exe"), "qemu");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_qemu_system(&["--help".to_string()]), 0);
        assert_eq!(run_qemu_system(&["-h".to_string()]), 0);
        assert_eq!(run_qemu_system(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_qemu_system(&[]), 0);
    }
}
