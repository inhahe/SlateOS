...
fn run_chafa(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("chafa 1.14.1 (Slate OS) — Terminal graphics");
        println!();
        println!("Options:");
    ...
    let file = args
        ...
        .map(|s| s.as_str())
        .unwrap_or("-");
    println!("chafa: Rendering {} to terminal...", quoteaf_os(file));
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "chafa".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_chafa, strip_ext};
