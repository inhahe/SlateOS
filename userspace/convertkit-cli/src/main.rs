...
fn run_ck(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "ConvertKit / Kit (Slate OS) — email marketing for creators (newsletters, courses, podcasts)"
        );
        println!();
        println!("Options:");
    ...
    println!(
        "          Barry was a designer + indie author who needed email for his book launches"
    );
    println!(
        "          built ConvertKit himself initially, scaled it as a one-person shop until ~2014"
    );
    println!(
        "          legendary 'public failure' story: nearly shut down 2014, pivoted to creators, became hit"
    );
    ...
    println!(
        "          rebranded to 'Kit' Aug 2024 (Barry: 'we're not just convert + kit anymore')"
    );
    println!("  Strategic position: 'email + audience for creators, not e-commerce or marketers':");
    println!(
        "                    laser-focused on bloggers, YouTubers, podcasters, course creators, authors, newsletter writers"
    );
    println!("                    NOT for e-commerce (would use Klaviyo)");
    println!("                    NOT for B2B (would use HubSpot or ActiveCampaign)");
    println!(
        "                    competes mostly with Substack (newsletter platform), Beehiiv (next-gen creator), MailerLite"
    );
    ...
    println!(
        "       Newsletter Referral System + advanced reporting + Facebook custom audiences + priority support"
    );
    ...
    println!(
        "    - Landing pages + forms (looks like a creator's website, not a marketer's funnel)"
    );
    ...
    println!(
        "    - Newsletter Referral System (Creator Pro): readers refer friends to unlock rewards (like Morning Brew)"
    );
    ...
    println!(
        "            Tim Ferriss, James Clear, Pat Flynn, Casey Neistat, Brené Brown, Seth Godin"
    );
    println!(
        "            Heath Brothers, Maria Popova (Marginalian), and tens of thousands of newsletter writers"
    );
    println!(
        "  Critique: weaker for e-commerce (no real cart/product feed/abandoned cart vs Klaviyo)"
    );
    ...
    println!(
        "           Substack threat: free + built-in audience + paid subscriptions out of the box"
    );
    println!("           Beehiiv threat: better creator UX + Beehiiv Boosts grew faster");
    println!("           rebrand to 'Kit' caused confusion + SEO loss");
    println!(
        "  Differentiator: best email tool for non-techy creators + first-class commerce + sponsor network — 'newsletter business in a box'"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "convertkit".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ck, strip_ext};
