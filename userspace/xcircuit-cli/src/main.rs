...
fn run_xcircuit(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("XCircuit v3.10 (Slate OS) — Publication-quality schematic editor");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("XCircuit v3.10.30 (Slate OS)");
        return 0;
    }
    println!("XCircuit v3.10.30 (Slate OS) — Schematic Editor");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "xcircuit".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_xcircuit, strip_ext};
