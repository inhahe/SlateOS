...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_garage};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/garage"), "garage");
        assert_eq!(basename(r"C:\bin\garage.exe"), "garage.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("garage.exe"), "garage");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_garage(&["--help".to_string()], "garage"), 0);
        assert_eq!(run_garage(&["-h".to_string()], "garage"), 0);
        assert_eq!(run_garage(&["--version".to_string()], "garage"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_garage(&[], "garage"), 0);
    }
}
