...
fn run_sensors(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("sensors — print hardware monitoring sensor readings (Slate OS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("sensors version 3.6.0+git (Slate OS)");
        return 0;
    }
...
fn run_sensors_detect(_args: &[String]) -> i32 {
    println!("# sensors-detect version 3.6.0+git (Slate OS)");
...
fn run_pwmconfig(_args: &[String]) -> i32 {
    println!("# pwmconfig — fan speed configuration (Slate OS)");
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_sensors(&["--version".to_string()]);
    }
