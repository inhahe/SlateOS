...
fn run_wx(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Cisco Webex (Slate OS) — Enterprise meetings, messaging, calling");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Cisco Webex App 44.11.0 (Slate OS)"); return 0; }
    println!("Cisco Webex App 44.11.0 (Slate OS)");
