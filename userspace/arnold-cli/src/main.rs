...
fn run_arnold(args: &[String], _prog: &str) -> i32 {
    ...
    println!("Arnold 7.3.4.0 (Slate OS)");
    println!(
        "  Vendor: Autodesk Inc. — acquired Solid Angle (original Arnold dev) Apr 2016 for $60M"
    );
    println!(
        "  Original developer: Solid Angle SL (Madrid, Spain) — Marcos Fajardo (founder 2004)"
    );
    println!("  History: Arnold 1.0 internal use Sony Pictures Imageworks 2009, public 2013");
    println!(
        "  Famous breakthrough: 'Monster House' (2006) — first feature fully rendered in Arnold"
    );
    println!("  Oscar: 2017 Academy Sci-Tech Award for the design of Arnold (Fajardo + team)");
    println!(
        "  Pricing: $585/yr single-user, $235/yr per render-only node, MtoA/HtoA free for plugin owners"
    );
    println!(
        "  Integrations: Maya (native — MtoA), Houdini (HtoA), 3ds Max, Cinema 4D, Katana, Softimage"
    );
    println!(
        "  Engine: unbiased Monte Carlo path tracer, ray-traced from camera (no light cache cheating)"
    );
    ...
    println!(
        "  Strengths: rock-solid memory efficiency (handles billions of polygons), simple controls,"
    );
    println!("             noise-free convergence, robust hair / fur / volumetrics");
    println!(
        "  Famous users: ILM, Sony Pictures Imageworks, MPC, DNEG, Framestore, Method, Animal Logic"
    );
    println!(
        "  Films: Gravity, The Avengers, Pacific Rim, Game of Thrones, Lion King 2019, every Marvel film"
    );
    println!("  Hollywood standard: top-3 production renderer alongside RenderMan + V-Ray");
    println!(
        "  Differentiator: predictable PT convergence + no special-case tweaks needed for film output"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "arnold".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_arnold, strip_ext};
