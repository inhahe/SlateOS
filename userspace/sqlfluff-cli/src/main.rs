...
fn run_sqlfluff(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("SQLFluff v3.0.0 (Slate OS) — SQL linter and formatter");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("SQLFluff v3.0.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "sqlfluff".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sqlfluff, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/sqlfluff"), "sqlfluff");
        assert_eq!(basename(r"C:\bin\sqlfluff.exe"), "sqlfluff.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("sqlfluff.exe"), "sqlfluff");
