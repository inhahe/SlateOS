...
fn run_krita(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Krita 5.2 (Slate OS) — Open-source digital painting (KDE Frameworks)");
        println!();
        println!("Options:");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "krita".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_krita, strip_ext};
