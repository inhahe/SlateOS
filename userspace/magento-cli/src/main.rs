...
//! magento-cli — OurOS Adobe Commerce / Magento (enterprise self-hosted e-commerce)
...
fn run_magento(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Adobe Commerce / Magento (OurOS) — enterprise self-hosted + cloud e-commerce");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Adobe Commerce / Magento 2.4 (OurOS)"); return 0; }
    println!("Adobe Commerce / Magento 2.4 (OurOS)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_magento(&["--version".to_string()], "magento"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_magento(&[], "magento"), 0);
    }
}
