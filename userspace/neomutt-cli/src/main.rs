...
fn run_neomutt(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("neomutt v2024.01 (Slate OS) — Terminal email client (NeoMutt)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-v") {
        println!("NeoMutt 2024-01-01 (Slate OS)");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "neomutt".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_neomutt, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_neomutt(&["--version".to_string()], "neomutt");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_neomutt(&[], "neomutt");
    }
}
