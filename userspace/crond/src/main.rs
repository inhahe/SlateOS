//! Slate OS Cron Daemon (`crond`) with anacron support
//!
//! # There used to be three of these
//!
//! `userspace/cron` answered to `crond` with a personality that printed
//! `scheduler ready (simulated)` and exited -- while also printing
//! `@reboot: running '<cmd>' as <user>`, which it had not done. `userspace/crond`
...
//! `udisks`/`umount` shape, and it is why `scripts/multicall-aliases.py`
//! exists.
//!
//! The other two are deleted. This one survived because it reads per-user
//! spools, parameterises the crontab parser on whether a user column is
//! present rather than assuming, and had 77 tests before today.
//!
