//! `<linux/fuse.h>` — FUSE (Filesystem in Userspace) constants.
//!
//! FUSE allows implementing filesystems as userspace programs. The
//! kernel FUSE module forwards VFS operations to a userspace daemon
//! via /dev/fuse. Used by sshfs, NTFS-3G, GlusterFS, rclone mount,
//! etc. FUSE opcodes define the request/reply protocol between the
//! kernel and the userspace filesystem implementation.

// ---------------------------------------------------------------------------
// FUSE opcodes (kernel → userspace requests)
// ---------------------------------------------------------------------------

/// Look up a directory entry by name.
pub const FUSE_LOOKUP: u32 = 1;
/// Forget an inode (release kernel reference).
pub const FUSE_FORGET: u32 = 2;
/// Get file attributes.
pub const FUSE_GETATTR: u32 = 3;
/// Set file attributes.
pub const FUSE_SETATTR: u32 = 4;
/// Read symbolic link target.
pub const FUSE_READLINK: u32 = 5;
/// Create a symbolic link.
pub const FUSE_SYMLINK: u32 = 6;
/// Create a special node (device, pipe, socket).
pub const FUSE_MKNOD: u32 = 8;
/// Create a directory.
pub const FUSE_MKDIR: u32 = 9;
/// Remove a file.
pub const FUSE_UNLINK: u32 = 10;
/// Remove a directory.
pub const FUSE_RMDIR: u32 = 11;
/// Rename a file or directory.
pub const FUSE_RENAME: u32 = 12;
/// Create a hard link.
pub const FUSE_LINK: u32 = 13;
/// Open a file.
pub const FUSE_OPEN: u32 = 14;
/// Read data from a file.
pub const FUSE_READ: u32 = 15;
/// Write data to a file.
pub const FUSE_WRITE: u32 = 16;
/// Get filesystem statistics.
pub const FUSE_STATFS: u32 = 17;
/// Release (close) a file.
pub const FUSE_RELEASE: u32 = 18;
/// Synchronize file contents.
pub const FUSE_FSYNC: u32 = 20;
/// Set extended attribute.
pub const FUSE_SETXATTR: u32 = 21;
/// Get extended attribute.
pub const FUSE_GETXATTR: u32 = 22;
/// List extended attributes.
pub const FUSE_LISTXATTR: u32 = 23;
/// Remove extended attribute.
pub const FUSE_REMOVEXATTR: u32 = 24;
/// Flush file (called on each close of an fd).
pub const FUSE_FLUSH: u32 = 25;
/// Initialize FUSE connection.
pub const FUSE_INIT: u32 = 26;
/// Open a directory.
pub const FUSE_OPENDIR: u32 = 27;
/// Read directory entries.
pub const FUSE_READDIR: u32 = 28;
/// Release (close) a directory.
pub const FUSE_RELEASEDIR: u32 = 29;
/// Synchronize directory contents.
pub const FUSE_FSYNCDIR: u32 = 30;
/// Acquire/release/test file lock.
pub const FUSE_GETLK: u32 = 31;
/// Set file lock.
pub const FUSE_SETLK: u32 = 32;
/// Set file lock (blocking).
pub const FUSE_SETLKW: u32 = 33;
/// Check file access permissions.
pub const FUSE_ACCESS: u32 = 34;
/// Create and open a file atomically.
pub const FUSE_CREATE: u32 = 35;
/// Interrupt a pending request.
pub const FUSE_INTERRUPT: u32 = 36;
/// Map block index to device block.
pub const FUSE_BMAP: u32 = 37;
/// Clean up after unmount.
pub const FUSE_DESTROY: u32 = 38;

// ---------------------------------------------------------------------------
// FUSE init flags (capabilities)
// ---------------------------------------------------------------------------

/// Support async reads.
pub const FUSE_ASYNC_READ: u32 = 1 << 0;
/// Support POSIX file locks.
pub const FUSE_POSIX_LOCKS: u32 = 1 << 1;
/// Support atomic O_TRUNC.
pub const FUSE_ATOMIC_O_TRUNC: u32 = 1 << 3;
/// Support big writes (> 4KB).
pub const FUSE_BIG_WRITES: u32 = 1 << 5;
/// Support parallel directory ops.
pub const FUSE_PARALLEL_DIROPS: u32 = 1 << 18;
/// Support POSIX ACLs.
pub const FUSE_POSIX_ACL: u32 = 1 << 20;

// ---------------------------------------------------------------------------
// FUSE minor version
// ---------------------------------------------------------------------------

/// Current FUSE kernel protocol major version.
pub const FUSE_KERNEL_VERSION: u32 = 7;
/// Current FUSE kernel protocol minor version.
pub const FUSE_KERNEL_MINOR_VERSION: u32 = 39;

// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_opcodes_distinct() {
        let ops = [
            FUSE_LOOKUP, FUSE_FORGET, FUSE_GETATTR, FUSE_SETATTR,
            FUSE_READLINK, FUSE_SYMLINK, FUSE_MKNOD, FUSE_MKDIR,
            FUSE_UNLINK, FUSE_RMDIR, FUSE_RENAME, FUSE_LINK,
            FUSE_OPEN, FUSE_READ, FUSE_WRITE, FUSE_STATFS,
            FUSE_RELEASE, FUSE_FSYNC, FUSE_SETXATTR, FUSE_GETXATTR,
            FUSE_LISTXATTR, FUSE_REMOVEXATTR, FUSE_FLUSH, FUSE_INIT,
            FUSE_OPENDIR, FUSE_READDIR, FUSE_RELEASEDIR, FUSE_FSYNCDIR,
            FUSE_GETLK, FUSE_SETLK, FUSE_SETLKW, FUSE_ACCESS,
            FUSE_CREATE, FUSE_INTERRUPT, FUSE_BMAP, FUSE_DESTROY,
        ];
        for i in 0..ops.len() {
            for j in (i + 1)..ops.len() {
                assert_ne!(ops[i], ops[j]);
            }
        }
    }

    #[test]
    fn test_init_flags_no_overlap() {
        let flags = [
            FUSE_ASYNC_READ, FUSE_POSIX_LOCKS,
            FUSE_ATOMIC_O_TRUNC, FUSE_BIG_WRITES,
            FUSE_PARALLEL_DIROPS, FUSE_POSIX_ACL,
        ];
        for i in 0..flags.len() {
            assert!(flags[i].is_power_of_two());
            for j in (i + 1)..flags.len() {
                assert_eq!(flags[i] & flags[j], 0);
            }
        }
    }

    #[test]
    fn test_protocol_version() {
        assert_eq!(FUSE_KERNEL_VERSION, 7);
        assert!(FUSE_KERNEL_MINOR_VERSION > 0);
    }

    #[test]
    fn test_opcode_ordering() {
        // FUSE opcodes are roughly sequential.
        assert!(FUSE_LOOKUP < FUSE_DESTROY);
        assert!(FUSE_INIT > FUSE_FLUSH);
    }
}
