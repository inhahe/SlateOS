...
fn run_ghostty(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Ghostty 1.0.0 (Slate OS) — Fast, feature-rich terminal emulator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("ghostty 1.0.0 (Slate OS)");
        return 0;
    }
    // Handle + actions
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_ghostty(&["--version".to_string()], "ghostty");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_ghostty(&[], "ghostty");
    }
}
