...
fn run_irqbalance(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("irqbalance v1.9 (Slate OS) — Distribute IRQs across CPUs");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("irqbalance v1.9 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "irqbalance".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_irqbalance, strip_ext};
