...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_event_count = EVENT_COUNT.load(Ordering::Relaxed);
    let saved_idle_check_count = IDLE_CHECK_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&POWER, PowerState::new(), self_test_inner);
    EVENT_COUNT.store(saved_event_count, Ordering::Relaxed);
    IDLE_CHECK_COUNT.store(saved_idle_check_count, Ordering::Relaxed);
    result
}
