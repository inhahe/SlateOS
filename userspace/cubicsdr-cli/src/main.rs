...
fn run_cubicsdr(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("CubicSDR v0.2 (Slate OS) — Cross-platform SDR application");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("CubicSDR v0.2.7 (Slate OS)");
        return 0;
    }
    println!("CubicSDR v0.2.7 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "CubicSDR".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_cubicsdr, strip_ext};
