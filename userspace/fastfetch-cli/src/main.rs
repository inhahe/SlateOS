...
fn run_fastfetch(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("fastfetch v2.8 (Slate OS) — Fast system information tool");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("fastfetch v2.8 (Slate OS)"); return 0; }
    ...
    println!(" / /_/ / / / / __/ // /\\ \\      OS: Slate OS 1.0 x86_64");
...
fn run_flashfetch(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("flashfetch v2.8 (Slate OS) — Minimal fastfetch preset");
        return 0;
    }
    println!("Slate OS 1.0 | 0.1.0-slateos | AMD Ryzen 7 | 4.0/16.0 GiB");
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_fastfetch};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/fastfetch"), "fastfetch");
        assert_eq!(basename(r"C:\bin\fastfetch.exe"), "fastfetch.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("fastfetch.exe"), "fastfetch");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_fastfetch(&["--help".to_string()], "fastfetch"), 0);
        assert_eq!(run_fastfetch(&["-h".to_string()], "fastfetch"), 0);
        let _ = run_fastfetch(&["--version".to_string()], "fastfetch");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_fastfetch(&[], "fastfetch");
    }
}
