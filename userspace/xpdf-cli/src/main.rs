...
//! xpdf-cli — OurOS Xpdf PDF utilities
//!
...
fn run_pdfinfo(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("pdfinfo v4.05 (OurOS) — PDF document info");
        return 0;
    }
...
fn run_pdffonts(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("pdffonts v4.05 (OurOS) — List fonts used in PDF");
        return 0;
    }
...
fn run_pdftoppm(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("pdftoppm v4.05 (OurOS) — Convert PDF pages to PPM/PNG/JPEG");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_pdfinfo(&["--version".to_string()], "xpdf"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_pdfinfo(&[], "xpdf"), 0);
    }
}
