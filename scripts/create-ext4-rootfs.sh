...
# --- CMake -P script fixtures (lane B's half of the cmake rung) ---------------
# Requested from lane A in
# `requests/b-a-cmake-needs-a-ring-3-rung-like-the-other-three.md`. cmake is the
# only one of section 4.4's four ports still carrying "shipped is not run", and
# the only one for which no rung was ever asked for.
#
# `-P` runs a CMake SCRIPT: no project, no generator, no compiler. That is the
# honest minimum, for the same reason `--version` was rejected for pkgconf --
# it opens no CMake file at all, and parsing them is the whole of what cmake
# does.
#
# THREE THINGS MEASURED AGAINST A REAL CMAKE (4.3.0-rc2 on the build host)
# before these were written down, because guessing any of them would make the
# rung assert the wrong thing:
#
#  1. `message()` in script mode writes to **stderr**, not stdout. stdout is
#     EMPTY for every script here. A rung modelled on the make one -- which
#     asserts stdout -- would fail forever against a working cmake. So every
#     fixture below states its result by WRITING A FILE, and the assertion is
#     "the artifact exists with these exact bytes".
#  2. A `FATAL_ERROR` exits 1 and stops execution: the `file(WRITE)` after it
#     never runs. Both halves are worth asserting.
#  3. Inputs are resolved from `CMAKE_CURRENT_LIST_FILE`, so /usr/share can be
#     read-only; outputs are written relative, so they land in whatever
#     writable directory the rung chdirs to. Verified by running from a
#     separate cwd and confirming the fixture directory was untouched.
CM_DIR="$STAGE/usr/share/cmake-selftest"
mkdir -p "$CM_DIR/globdir"

# Inputs. Deliberately not generated by the scripts themselves: 04 must read
# something it did not just write, or it proves the write path twice and the
# read path never.
printf 'slateos-input-payload\n' > "$CM_DIR/input.txt"
printf 'alpha\n' > "$CM_DIR/globdir/a.txt"
printf 'beta\n'  > "$CM_DIR/globdir/b.txt"
printf 'gamma\n' > "$CM_DIR/globdir/c.txt"

# 1. Does a script run at all, and can cmake write a file? The cheapest rung
#    and the one that fails first if libc regresses.
#    EXPECT: exit 0, and ./script-ran.txt holds "slateos-cmake-ok\n".
cat > "$CM_DIR/01-script.cmake" <<'EOF'
# PASS => cmake started, found CMAKE_ROOT, parsed a script and wrote a file.
# FAIL => startup, the module tree at /share/cmake-4.4, or file(WRITE) is broken.
cmake_minimum_required(VERSION 3.20)
file(WRITE script-ran.txt "slateos-cmake-ok\n")
EOF

# 2. The interpreter, not just the lexer: set/expand, a string transform, a
#    length, and a branch on it. A cmake that parsed but evaluated nothing
#    would still write a file here -- so the CONTENT is the assertion.
#    EXPECT: exit 0, ./vars.txt holds "SLATEOS/7/len-ok\n".
cat > "$CM_DIR/02-vars.cmake" <<'EOF'
# PASS => variables expand, string(TOUPPER/LENGTH) work, if(EQUAL) branches.
# FAIL => the language is being parsed and not evaluated.
cmake_minimum_required(VERSION 3.20)
set(NAME "slateos")
...
if(LEN EQUAL 7)
  ...
  set(VERDICT "len-WRONG-${LEN}")
endif()
file(WRITE vars.txt "${UPPER}/${LEN}/${VERDICT}\n")
EOF

# 3. THE NEGATIVE CASE, and the reason this set is worth anything. Without it
#    the rung passes against a cmake that cannot report a failure at all --
#    exactly what `slateos-badver` prevents for pkgconf and `05-failure` for
#    make. Two assertions: the exit status is non-zero, AND the file after the
#    FATAL_ERROR was never written.
#    EXPECT: exit 1, stderr contains "slateos-deliberate-failure",
#            ./must-not-exist.txt ABSENT.
cat > "$CM_DIR/03-fatal.cmake" <<'EOF'
# PASS => FATAL_ERROR exits non-zero and stops the script.
# FAIL => errors are being swallowed, or execution continues past a fatal one.
cmake_minimum_required(VERSION 3.20)
message(FATAL_ERROR "slateos-deliberate-failure")
file(WRITE must-not-exist.txt "this line must never run\n")
EOF

# 4. Reading a file off our ext4, not just writing one. The input is staged
#    beside the script and located from CMAKE_CURRENT_LIST_FILE.
#    EXPECT: exit 0, ./read.txt holds "SLATEOS-INPUT-PAYLOAD\n".
cat > "$CM_DIR/04-read.cmake" <<'EOF'
# PASS => file(READ) opened and read a staged file through our VFS.
# FAIL => the read path, or CMAKE_CURRENT_LIST_FILE resolution, is broken.
cmake_minimum_required(VERSION 3.20)
get_filename_component(HERE "${CMAKE_CURRENT_LIST_FILE}" DIRECTORY)
file(READ "${HERE}/input.txt" PAYLOAD)
string(STRIP "${PAYLOAD}" PAYLOAD)
string(TOUPPER "${PAYLOAD}" LOUD)
file(WRITE read.txt "${LOUD}\n")
EOF

# 5. Walking a DIRECTORY, which is the facility cmake needs and neither pkgconf
#    nor make exercises. `file(GLOB)` opens the directory and enumerates it;
#    the assertion is the count AND the sorted names, so a glob that returned
#    everything, nothing, or an unsorted set all fail.
#    EXPECT: exit 0, ./glob.txt holds "3:a.txt,b.txt,c.txt,\n".
cat > "$CM_DIR/05-glob.cmake" <<'EOF'
# PASS => opendir/readdir work through our VFS and the results sort.
# FAIL => directory enumeration is empty, over-broad, or unordered.
cmake_minimum_required(VERSION 3.20)
get_filename_component(HERE "${CMAKE_CURRENT_LIST_FILE}" DIRECTORY)
file(GLOB FOUND "${HERE}/globdir/*.txt")
list(SORT FOUND)
list(LENGTH FOUND N)
set(NAMES "")
foreach(f IN LISTS FOUND)
  get_filename_component(base "${f}" NAME)
  string(APPEND NAMES "${base},")
endforeach()
file(WRITE glob.txt "${N}:${NAMES}\n")
EOF

echo "[rootfs] staged cmake fixtures in /usr/share/cmake-selftest: 01-script, 02-vars, 03-fatal, 04-read, 05-glob"
...
#
# ...and, for the fastpy family, *the fastpy compiler*, which was the one input
# neither gate could see. A fastpy fixture's ELF is generated by
# `compiler.codegen` from a source that never changes, so a codegen fix rewrites
# the binary while every file named above stands still. That is not
# hypothetical: `fastpy-minishell.elf` was built 2026-08-21 16:05, fastpy fixed
# the list-element-kind defect that broke it at 18:03, and both this gate and
# `ctest-fixtures.py` went on calling the fixture current
# (BUG-FASTPY-MINISHELL-EXITS-0-WITHOUT-FORKING). The compiler is found the same
# way `ctest-fixtures.py::_fastpy_dir` finds it — $FASTPY_DIR, then a sibling of
# the repo root named exactly `fastpy` — and stood in for by the newest `.py`
# under its `compiler/` package, which is the same mtime instrument the rest of
# this loop uses. Not found means the input is skipped: this script runs under
# WSL, cannot rebuild a fastpy fixture anyway, and a gate that fails closed on a
# checkout it merely could not locate would block every image build on a machine
# that has no fastpy at all.
#
...
# The newest source file of the fastpy compiler, or empty if no checkout is
# reachable.  Computed once: the loop below runs 70 times and this walks a few
# hundred files.  `-printf` is GNU find, which is what WSL has; the `|| true`
# keeps a missing directory from killing the script under `set -e`, the same
# guard the sysroot check above uses for the same reason.
FASTPY_NEWEST=""
_fastpy_root=""
if [ -n "${FASTPY_DIR:-}" ] && [ -f "$FASTPY_DIR/compiler/__init__.py" ]; then
