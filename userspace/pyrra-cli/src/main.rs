...
fn run_pyrra(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Pyrra v0.7.0 (Slate OS) — SLO management tool");
        println!();
        println!("Commands:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "pyrra".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_pyrra, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/pyrra"), "pyrra");
        assert_eq!(basename(r"C:\bin\pyrra.exe"), "pyrra.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("pyrra.exe"), "pyrra");
