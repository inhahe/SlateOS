...
fn run_ramp(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Founders: Eric Glyman + Karim Atiyeh + Gene Lee (previously Paribus, sold to Capital One 2016)"
    );
    ...
    println!(
        "    - 'Savings Insights' — AI flags duplicate subscriptions, price hikes, unused seats"
    );
    ...
    println!(
        "  Underwriting: 'fund-based' — credit limits based on cash in bank, not personal guarantee"
    );
    println!(
        "  Integrations: QuickBooks, NetSuite, Sage Intacct, Xero, Workday, Microsoft Dynamics"
    );
    println!(
        "  AI Push: 'Ramp Intelligence' — agentic finance ops (auto-categorize, auto-approve, auto-pay)"
    );
    ...
    println!(
        "  Differentiator: spend-savings AI angle + 1.5% cashback floor + free spend platform — Concur killer"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ramp, strip_ext};
