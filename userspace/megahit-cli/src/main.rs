...
fn run_megahit(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("MEGAHIT v1.2 (Slate OS) — Ultra-fast metagenome assembler");
        println!();
        println!("Input:");
    ...
    if args.iter().any(|a| a == "--version") { println!("MEGAHIT v1.2.9 (Slate OS)"); return 0; }
    println!("MEGAHIT v1.2.9 (Slate OS) — Metagenome Assembly");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_megahit};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/megahit"), "megahit");
        assert_eq!(basename(r"C:\bin\megahit.exe"), "megahit.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("megahit.exe"), "megahit");
