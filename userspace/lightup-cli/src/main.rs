...
fn run_lightup(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Lightup (Slate OS) — no-code AI data quality (acquired by Acceldata Apr 2024)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Lightup 2024 (Slate OS)");
        return 0;
    }
    println!("Lightup 2024 (Slate OS) — No-Code AI Data Quality");
    ...
    println!(
        "         Lightup brand sunsetting; tech being integrated into Acceldata Torch module"
    );
    println!("  Strategic position (pre-acquisition):");
    println!(
        "                    pitch: 'pre-built data quality monitors — turn on, ship insights'"
    );
    println!("                    target: non-technical data quality teams (analysts, stewards)");
    println!(
        "                    primary competitor (pre-acq): Monte Carlo, Anomalo, Bigeye, Soda"
    );
    println!(
        "                    Lightup's wedge: no-code UI + pre-built monitor library + ML auto-thresholding"
    );
    println!(
        "                    differentiator: 'data analysts can use it, not just data engineers'"
    );
    ...
    println!(
        "    - Pre-built monitor library: null rate, uniqueness, range, regex, business-rule, etc."
    );
    ...
    println!(
        "    - Anomaly attribution: identifies which segment (region, product, customer cohort) is off"
    );
    ...
    println!(
        "           independent product lifespan ending — full Acceldata integration mid-2025"
    );
    ...
    println!(
        "           consolidation in observability space: ~5+ similar startups, market shaking out"
    );
    ...
    println!(
        "           pattern: Datadog/Metaplane, Coalesce/Castor, Acceldata/Lightup — observability consolidation 2024"
    );
    println!(
        "  Differentiator: pre-built no-code monitor library + ML auto-thresholding + analyst-friendly UI + Stanford-PhD-led product + now backed by Acceldata's multi-pillar platform — the no-code data quality choice for non-engineering data teams"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "lightup".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_lightup, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_lightup(&["--version".to_string()], "lightup");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_lightup(&[], "lightup");
    }
}
