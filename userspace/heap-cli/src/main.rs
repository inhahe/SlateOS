...
fn run_heap(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "    - Acquired by Contentsquare Sep 2023 for reported $400M (down round to acquisition)"
    );
    println!("    - Contentsquare = Paris-based session replay + UX analytics (~$1.4B valuation)");
    println!(
        "    - combined entity covers product analytics (Heap) + session replay (CS) + voice-of-customer"
    );
    ...
    println!(
        "    - Define 'events' retroactively after the fact — no instrumenting code in advance"
    );
    println!(
        "    - Contrast with Amplitude/Mixpanel: those require explicit track() calls per event"
    );
    ...
    println!(
        "    - Event Visualizer — point-and-click to define events from raw auto-capture data"
    );
    ...
    println!(
        "    - Auto-surface 'friction points' — drop-offs the user didn't define a funnel for"
    );
    ...
    println!(
        "            Twilio (early flagship), Asana, Microsoft (parts), AppLovin, Zendesk (parts)"
    );
    println!("            Pluralsight, Toast, Stitch Fix, Northwestern Mutual");
    println!(
        "            sweet spot: B2C + B2B SaaS with high event diversity + small data eng teams"
    );
    println!(
        "  Critique: storage costs add up — auto-capture means you pay for everything captured"
    );
    println!(
        "           sometimes auto-captured events lack the semantic meaning manual events have"
    );
    ...
    println!(
        "  Differentiator: only major product analytics tool with TRUE retroactive event definition (auto-capture from day 1)"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_heap, strip_ext};
