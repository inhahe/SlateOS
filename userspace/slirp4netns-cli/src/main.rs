...
fn run_slirp4netns(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("slirp4netns v1.2 (Slate OS) — User-mode networking for rootless containers");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("slirp4netns v1.2.3 (Slate OS)"); return 0; }
    println!("slirp4netns v1.2.3 (Slate OS)");
    println!("  PID: 12345");
    println!("  TAP: tap0");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_slirp4netns};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/slirp4netns"), "slirp4netns");
        assert_eq!(basename(r"C:\bin\slirp4netns.exe"), "slirp4netns.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("slirp4netns.exe"), "slirp4netns");
