...
//! postfix — OurOS mail transfer agent (MTA)
//!
...
fn run_postfix(args: Vec<String>) -> i32 {
    ...
    match cmd {
        ...
        "--version" | "version" => {
            println!("postfix (Postfix) 3.8.6 (OurOS)");
            0
        }
        "start" => {
...
fn run_sendmail(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "-V" || a == "--version") {
        println!("sendmail: Postfix sendmail compatibility interface 3.8.6 (OurOS)");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_postfix(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_postfix(vec![]), 0);
    }
}
