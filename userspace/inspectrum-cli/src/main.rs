...
//! inspectrum-cli — OurOS inspectrum signal analyzer
//!
...
fn run_inspectrum(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("inspectrum v0.2 (OurOS) — RF signal spectrum analyzer");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("inspectrum v0.2.3 (OurOS)"); return 0; }
    let files: Vec<&String> = args.iter().filter(|a| !a.starts_with('-')).collect();
    if files.is_empty() {
        println!("inspectrum v0.2.3 (OurOS) — Signal Analyzer");
