...
//! avast-cli — OurOS Gen Digital Avast One
//!
...
fn run_avast(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Avast One 24.10 (OurOS) — Consumer security/privacy/performance");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Avast One 24.10.9223 (OurOS)"); return 0; }
    println!("Avast One 24.10.9223 (OurOS)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_avast(&["--version".to_string()], "avast"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_avast(&[], "avast"), 0);
    }
}
