...
fn kind_to_handle_type(kind: crate::fdtable::HandleKind) -> u8 {
    ...
    match kind {
        ...
        // Epoll and Timerfd fds are per-process userspace state and
        // cannot be meaningfully transferred to a child.  Map to FILE
        // so the function is total; build_fd_map filters these entries
        // out before they reach this conversion.
        HandleKind::Epoll | HandleKind::Timerfd => fd_handle_type::FILE,
    }
}
...
fn build_fd_map(
    ...
    while idx < MAX_FD_MAP {
        ...
        if let Some(entry) = fdtable::get_fd(fd) {
            ...
            // Skip epoll/timerfd fds — the instance state lives in the
            // parent's userspace memory and cannot be transferred to
            // the child.
