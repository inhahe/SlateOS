...
fn run_zmq(args: &[String], prog_name: &str) -> i32 {
    ...
    match prog_name {
        "zmq-send" => {
            let endpoint = args
                .iter()
                .find(|a| a.starts_with("tcp://") || a.starts_with("ipc://"))
                .map(|s| s.as_str())
                .unwrap_or("tcp://localhost:5555");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "zmq-send".to_string());
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/zeromq"), "zeromq");
        assert_eq!(basename(r"C:\bin\zeromq.exe"), "zeromq.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("zeromq.exe"), "zeromq");
