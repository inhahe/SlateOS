...
fn run_kmag(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("kmag v24.05 (Slate OS) — KDE screen magnifier");
        println!();
        println!("Options:");
    ...
    let mode = args
        ...
        .nth(1)
        .map(|s| s.as_str())
        .unwrap_or("follow-mouse");
    let zoom = args
        .iter()
        .skip_while(|a| a.as_str() != "--zoom")
        .nth(1)
        .map(|s| s.as_str())
        .unwrap_or("2");
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kmag, strip_ext};
