...
//! vals-cli — OurOS vals secret reference resolver
//!
...
fn run_vals(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("vals v0.37.0 (OurOS) — Helmfile's secret reference resolver");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_vals(&["--version".to_string()], "vals"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_vals(&[], "vals"), 0);
    }
}
