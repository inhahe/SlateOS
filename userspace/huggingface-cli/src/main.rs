...
fn run_hf(args: Vec<String>) -> i32 {
    ...
    match cmd {
            ...
            match sub {
                "create" => {
                    ...
                    let repo_type = args
                        ...
                        .unwrap_or("model");
                    println!(
                        "Created {} repo: https://huggingface.co/user/{}",
                        repo_type, name
                    );
                }
                "list" => {
        ...
        "download" => {
            ...
            println!(
                "Downloaded to /home/user/.cache/huggingface/hub/models--{}",
                repo
            );
            0
        }
        ...
        "scan-cache" => {
            println!(
                "REPOS         SIZE      BLOBS     REFS      LAST_ACCESSED     LAST_MODIFIED     REPO ID"
            );
            println!(
                "models/bert   440.2 MB  4         2         2024-01-15        2024-01-15        bert-base-uncased"
            );
            println!(
                "models/gpt2   548.1 MB  5         1         2024-01-14        2024-01-10        gpt2"
            );
            println!(
                "datasets/imdb  84.6 MB  2         1         2024-01-12        2024-01-08        imdb"
            );
            println!();
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_hf(vec!["--help".to_string()]), 0);
        assert_eq!(run_hf(vec!["-h".to_string()]), 0);
        let _ = run_hf(vec!["--version".to_string()]);
    }
