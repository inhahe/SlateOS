...
impl Firewall {
    ...
    fn save_to_file(&self) -> io::Result<()> {
        ...
        writeln!(
            f,
            "# Auto-generated by fw(1).  Manual edits are preserved on load."
        )?;
...
fn cmd_list(fw: &Firewall, json: bool) {
    ...
    println!();
    println!(
        "Total: {} rule(s).  Default inbound policy: {}.",
        fw.rules.len(),
        fw.default_policy.as_str().to_lowercase()
    );
}
...
fn cmd_delete(fw: &mut Firewall, num_str: &str) {
    ...
    if num > fw.rules.len() {
        eprintln!(
            "Rule #{num} does not exist (only {} rules defined).",
            fw.rules.len()
        );
        process::exit(1);
    }
...
fn cmd_policy(fw: &mut Firewall, value: &str) {
    ...
    println!(
        "Default inbound policy set to: {}",
        policy.as_str().to_lowercase()
    );
}
...
mod tests {
    ...
    fn test_parse_multiple_rules() {
        ...
        let rules: Vec<Rule> = lines.iter().filter_map(|l| Rule::from_line(l)).collect();
