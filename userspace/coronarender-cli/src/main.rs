...
fn run_corona(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Corona Renderer 12 (Slate OS) — Unbiased path tracer for ArchViz");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Corona Renderer 12 (Hotfix 1) (Slate OS)");
        return 0;
    }
    println!("Corona Renderer 12 (Hotfix 1) (Slate OS)");
    ...
    println!(
        "  Materials: Corona Material (PBR), Corona Layered Material, Corona Sky/Sun, Corona Volume"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "corona".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_corona, strip_ext};
