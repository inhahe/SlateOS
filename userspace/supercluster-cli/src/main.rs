...
//! supercluster-cli — OurOS Supercluster point clustering tool
//!
...
fn run_supercluster(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Supercluster v8.0 (OurOS) — Fast geospatial point clustering");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Supercluster v8.0 (OurOS)");
        return 0;
    }
    let file = args.iter()
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_supercluster(&["--version".to_string()], "supercluster"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_supercluster(&[], "supercluster"), 0);
    }
}
