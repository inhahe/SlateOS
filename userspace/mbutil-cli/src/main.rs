...
fn run_mbutil(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("mbutil v0.3 (Slate OS) — MBTiles import/export utility");
        println!();
        println!("Commands:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "mbutil".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_mbutil, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/mbutil"), "mbutil");
        assert_eq!(basename(r"C:\bin\mbutil.exe"), "mbutil.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("mbutil.exe"), "mbutil");
