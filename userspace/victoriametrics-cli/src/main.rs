...
//! victoriametrics-cli — Slate OS VictoriaMetrics CLI tools
//!
...
fn run_vmctl(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("vmctl v1.97.0 (Slate OS) — VictoriaMetrics migration tool");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("vmctl v1.97.0 (Slate OS)");
        return 0;
    }
...
fn run_vmbackup(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("vmbackup v1.97.0 (Slate OS) — VictoriaMetrics backup tool");
        println!();
        println!("Options:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "vmctl".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_vmctl, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/victoriametrics"), "victoriametrics");
        assert_eq!(
            basename(r"C:\bin\victoriametrics.exe"),
            "victoriametrics.exe"
        );
