...
fn run_openresty(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "resty" => {
                println!("resty (Slate OS) — OpenResty Lua script runner");
            ...
            _ => {
                println!("OpenResty v1.25 (Slate OS) — Nginx + LuaJIT web platform");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/openresty"), "openresty");
        assert_eq!(basename(r"C:\bin\openresty.exe"), "openresty.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("openresty.exe"), "openresty");
