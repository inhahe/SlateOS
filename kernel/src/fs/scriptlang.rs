...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_next_engine_id = NEXT_ENGINE_ID.load(Ordering::Relaxed);
    let saved_next_context_id = NEXT_CONTEXT_ID.load(Ordering::Relaxed);
    let saved_eval_count = EVAL_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&STATE, State::new(), self_test_inner);
    NEXT_ENGINE_ID.store(saved_next_engine_id, Ordering::Relaxed);
    NEXT_CONTEXT_ID.store(saved_next_context_id, Ordering::Relaxed);
    EVAL_COUNT.store(saved_eval_count, Ordering::Relaxed);
    result
}
