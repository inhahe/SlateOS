...
fn run_construct(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Construct 3 r400 (Slate OS) — Visual game development");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Construct 3 r400 (Slate OS)"); return 0; }
    println!("Construct 3 r400 (Slate OS)");
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_construct(&["--version".to_string()], "construct");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_construct(&[], "construct");
    }
}
