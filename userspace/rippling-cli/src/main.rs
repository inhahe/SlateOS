...
fn run_rip(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Founder: Parker Conrad (previously Zenefits CEO — ousted 2016 over licensing scandal)"
    );
    println!("  Funding: Founders Fund, Kleiner Perkins, Sequoia, Coatue + others");
    println!("          $1.4B+ raised, $13.5B valuation Aug 2024");
    println!(
        "  Strategy: 'Employee Graph' — one identity → all employee systems (HR, IT, finance)"
    );
    println!(
        "           pitch: 'onboard a new hire in 90 seconds: payroll + email + Slack + 1Password + laptop'"
    );
    ...
    println!(
        "    HR Cloud: payroll, benefits, time, recruiting, learning, performance, ATS, surveys"
    );
    println!(
        "    IT Cloud: SSO, identity (SCIM provisioning), MDM (Mac/Win), device buy/ship/manage,"
    );
    println!(
        "              password manager, automated app provisioning (Slack/G Suite/Salesforce/etc.)"
    );
    println!(
        "    Finance Cloud: corporate cards, expense management, bill pay, travel booking, spend mgmt"
    );
    ...
    println!(
        "  Differentiator: only platform with HR + IT + Finance unified on one employee graph"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "rippling".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_rip, strip_ext};
