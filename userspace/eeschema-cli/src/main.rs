...
fn run_eeschema(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("eeschema v8.0 (Slate OS, KiCad)");
        return 0;
    }
...
fn run_pcbnew(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("pcbnew v8.0 (Slate OS, KiCad)");
        return 0;
    }
...
fn run_gerbview(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("gerbview v8.0 (Slate OS, KiCad)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "eeschema".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_eeschema, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_eeschema(&["--version".to_string()], "eeschema");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_eeschema(&[], "eeschema");
    }
}
