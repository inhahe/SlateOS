...
fn run_geotiff(args: &[String], prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("libgeotiff v1.7.1 (Slate OS)");
        return 0;
    }
    match prog {
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "listgeo".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_geotiff, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_geotiff(&["--version".to_string()], "geotiff");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_geotiff(&[], "geotiff");
    }
}
