...
//! arnold-cli — OurOS Autodesk Arnold (Oscar-winning production renderer)
//!
...
fn run_arnold(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Autodesk Arnold 7 (OurOS) — Unbiased Monte Carlo path tracer");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Arnold 7.3.4.0 (OurOS)"); return 0; }
    println!("Arnold 7.3.4.0 (OurOS)");
