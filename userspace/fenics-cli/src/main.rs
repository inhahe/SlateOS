...
//! fenics-cli — OurOS FEniCS finite element framework
//!
...
fn run_fenics(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("FEniCS/DOLFINx v0.8 (OurOS) — Finite element framework");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("FEniCS/DOLFINx v0.8.0 (OurOS)");
        println!("  PETSc: 3.20, SLEPc: 3.20");
        return 0;
    }
    match prog {
        "ffc" => {
            println!("FFC: FEniCS Form Compiler v0.8 (OurOS)");
        ...
        _ => {
            println!("DOLFINx v0.8 (OurOS) — Running FEM solver");
