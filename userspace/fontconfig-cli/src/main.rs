...
fn run_fc_list(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("fc-list v2.15 (SlateOS) — List available fonts");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("fc-list v2.15 (SlateOS)"); return 0; }
...
fn run_fc_match(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("fc-match v2.15 (SlateOS) — Match font pattern");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("fc-match v2.15 (SlateOS)"); return 0; }
...
fn run_fc_cache(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("fc-cache v2.15 (SlateOS) — Build font cache");
        println!("  -f    Force rebuild");
        println!("  -v    Verbose");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("fc-cache v2.15 (SlateOS)"); return 0; }
...
fn run_fc_query(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("fc-query v2.15 (SlateOS) — Query font files");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("fc-query v2.15 (SlateOS)"); return 0; }
...
fn run_fc_scan(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("fc-scan v2.15 (SlateOS) — Scan font files and directories");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("fc-scan v2.15 (SlateOS)"); return 0; }
