...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_open_ops = OPEN_OPS.load(Ordering::Relaxed);
    let saved_nav_ops = NAV_OPS.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&PICKER, PickerState::new(), self_test_inner);
    OPEN_OPS.store(saved_open_ops, Ordering::Relaxed);
    NAV_OPS.store(saved_nav_ops, Ordering::Relaxed);
    result
}
