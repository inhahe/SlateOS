...
##### Tier 1 — Icon Theme
- [-] App icons, file type icons, system tray icons, folder icons — *2026-09-26 (`design-decisions.md` §880): `appearance::icons` finds an icon by its freedesktop name in the chosen theme, with a built-in set compiled in (folders and the start menu's places, document, picture, audio, home, settings, terminal, power, window, computer, bin, search, hard disk, link). Drawn by the start menu's places column and power button, and -- since later the same day -- by the desktop's icons, each type as its own picture in its own hue (they were emoji, which a face without emoji draws as boxes). The taskbar and the tray still draw words, and apps have no icon of their own until a program can say which it is (`open-questions.md` on the application registry).*
