...
fn run_fb(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "FreshBooks (Slate OS) — Cloud accounting for freelancers, agencies, service businesses"
        );
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("FreshBooks 2024 (Slate OS)");
        return 0;
    }
    println!("FreshBooks 2024 (Slate OS)");
    ...
    println!(
        "          built FreshBooks because he accidentally invoiced a client over old QuickBooks → wanted simpler tool"
    );
    ...
    println!(
        "          2017 rebuild ('FreshBooks Classic' → 'New FreshBooks') controversial — lost some loyal users"
    );
    ...
    println!(
        "  Customers: independent contractors, lawyers, consultants, photographers, small agencies"
    );
    println!(
        "  Critique: weaker accounting depth vs QuickBooks/Xero (no inventory, weak fixed assets)"
    );
    println!("           Lite tier's 5-client cap is aggressive");
    println!(
        "  Differentiator: best invoicing UX in the market + time tracking → invoice loop for service businesses"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "freshbooks".to_string());
