...
fn run_gucharmap(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("gucharmap v15.1 (Slate OS) — Unicode character map");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("gucharmap v15.1 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "gucharmap".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gucharmap, strip_ext};
