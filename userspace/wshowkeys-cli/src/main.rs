...
fn run_wshowkeys(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("wshowkeys v1.0 (Slate OS) — Display keypresses on Wayland");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("wshowkeys v1.0 (Slate OS)");
        return 0;
    }

    let anchor = args
        .iter()
        .skip_while(|a| a.as_str() != "-a")
        .nth(1)
        .map(|s| s.as_str())
        .unwrap_or("bottom");
    let size = args
        ...
        .unwrap_or("24");
    println!(
        "wshowkeys: displaying keypresses (anchor={}, font_size={}px)",
        anchor, size
    );
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "wshowkeys".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wshowkeys, strip_ext};
