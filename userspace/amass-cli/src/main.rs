...
//! amass-cli — Slate OS OWASP Amass attack surface mapper
//!
...
fn run_amass(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Amass v4.2 (Slate OS) — OWASP attack surface mapping");
        println!();
        println!("Subcommands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Amass v4.2.0 (Slate OS)");
        return 0;
    }
    println!("Amass v4.2.0 (Slate OS) — Attack Surface Mapping");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "amass".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_amass, strip_ext};
