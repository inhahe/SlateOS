...
fn run_census(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  --embedded             Census Embedded (white-label reverse-ETL for SaaS vendors)"
        );
    ...
    println!(
        "          Jabes ex-product at Couchbase + Mixpanel, repeat founder (sold a startup to Salesforce)"
    );
    println!(
        "          built Census initially as a 'CRM sync' product, expanded into full reverse ETL category"
    );
    println!("          quieter + more 'enterprise-respectable' positioning than Hightouch");
    println!(
        "  Founded: 2018 in San Francisco (around the same time as Hightouch — natural rivalry from day one)"
    );
    println!("          raised ~$80M total (Sequoia, A16Z, Insight)");
    println!("          ~$30M+ ARR estimated (private)");
    println!("          ~150-200 employees");
    println!(
        "          coined-the-term 'Operational Analytics' (vs Hightouch's 'Reverse ETL') — same category"
    );
    println!("  Strategic position: 'data activation for the modern data stack':");
    println!(
        "                    primary competitor: Hightouch (very close rival), Polytomic, Workato"
    );
    println!(
        "                    Hightouch slightly bigger by most measures but Census wins big enterprises"
    );
    println!("                    enterprise-respectable tone (vs Hightouch's louder marketing)");
    println!(
        "                    differentiator: Census Embedded (white-label for SaaS to build in-product reverse-ETL)"
    );
    println!(
        "                    'modern data stack' partner-of-choice: Snowflake, dbt, Fivetran sales motion overlap"
    );
    ...
    println!(
        "    - Connect warehouse (Snowflake, BigQuery, Redshift, Databricks, Postgres) as source"
    );
    ...
    println!(
        "    - 200+ destinations (Salesforce, HubSpot, Marketo, Klaviyo, Iterable, Braze, etc.)"
    );
    ...
    println!(
        "    - SaaS product can offer 'sync your data to X' to its customers without building infra"
    );
    println!(
        "    - Customers of: Sigma Computing, Mode, Hex, Klaviyo (uses Census Embedded for Klaviyo CDP layer)"
    );
    ...
    println!(
        "    - Census's answer to 'composable CDP' positioning — vs Hightouch Customer Studio"
    );
    ...
    println!(
        "              CRM (Salesforce, HubSpot, Pipedrive, Microsoft Dynamics, Outreach, Salesloft)"
    );
    println!(
        "              Marketing (Marketo, Klaviyo, Braze, Iterable, Customer.io, ActiveCampaign)"
    );
    ...
    println!(
        "            Notion, Figma, Lyft (some teams), Mux, Loom, Plaid, Carta, Ramp, Brex (some shared with Hightouch)"
    );
    ...
    println!(
        "            embedded Customers: Klaviyo, Sigma, Hex, Mode build reverse-ETL into their products via Census"
    );
    println!(
        "  Critique: very similar product to Hightouch — differentiation is positioning, not feature gap"
    );
    println!(
        "           smaller than Hightouch in deals + employee count, but wins enterprise + embedded plays"
    );
    ...
    println!(
        "  Differentiator: Census Embedded (white-label reverse-ETL for SaaS vendors) + enterprise-respectable positioning + Datasets semantic layer — for B2B SaaS vendors building data sync as a feature + enterprise data teams"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "census".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_census, strip_ext};
