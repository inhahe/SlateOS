...
fn run_alacritty(args: &[String], prog: &str) -> i32 {
    if prog == "alacritty-msg" {
        ...
        match cmd {
            ...
            _ => println!("alacritty msg: unknown command {}", quoteaf_os(cmd)),
        }
        return 0;
    }
...
#[cfg(test)]
mod tests {
    use super::{basename, run_alacritty, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/alacritty"), "alacritty");
        assert_eq!(basename(r"C:\bin\alacritty.exe"), "alacritty.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("alacritty.exe"), "alacritty");
