...
fn run_dm(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "dmstats" => {
                println!("dmstats (Slate OS) — Device-mapper statistics");
            ...
            _ => {
                println!("dmsetup (Slate OS) — Device-mapper management");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("dmsetup v1.02.196 (Slate OS)");
        return 0;
    }
    match prog {
        "dmstats" => {
            println!("dmstats (Slate OS)");
        ...
        _ => {
            println!("dmsetup (Slate OS) — Device-Mapper");
            println!("  Devices:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "dmsetup".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_dm, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_dm(&["--version".to_string()], "dm");
    }
