...
fn run_acr(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Acronis Cyber Protect Home Office 2024 / Cyber Protect 16 (Slate OS)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Acronis Cyber Protect Home Office 2024 41478 (Slate OS)");
        return 0;
    }
    println!("Acronis Cyber Protect Home Office 2024 build 41478 (Slate OS)");
    println!(
        "  Vendor: Acronis International GmbH (Schaffhausen, Switzerland; founded 2003 Singapore)"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "acronis".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_acr, strip_ext};
