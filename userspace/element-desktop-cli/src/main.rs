...
fn run_element(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("element-desktop v1.11 (Slate OS) — Matrix client");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("element-desktop v1.11 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "element-desktop".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_element, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/element-desktop"), "element-desktop");
        assert_eq!(
            basename(r"C:\bin\element-desktop.exe"),
            "element-desktop.exe"
        );
