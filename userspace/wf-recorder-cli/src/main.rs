...
fn run_wf_recorder(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("wf-recorder v0.4 (Slate OS) — Screen recording for Wayland");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("wf-recorder v0.4 (Slate OS)");
        return 0;
    }
    let file = args
        .iter()
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "wf-recorder".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wf_recorder, strip_ext};
