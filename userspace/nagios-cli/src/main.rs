...
//! nagios-cli — OurOS Nagios monitoring system
//!
...
fn run_nagios(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("nagios v4.5 (OurOS) — Host/service monitoring system");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("nagios v4.5 (OurOS)"); return 0; }
...
fn run_nagiostats(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("nagiostats v4.5 (OurOS) — Nagios performance statistics");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("nagiostats v4.5 (OurOS)"); return 0; }
...
fn run_nagios_check(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("nagios-check v4.5 (OurOS) — Run Nagios check plugins");
        return 0;
    }
    if args.is_empty() {
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_nagios(&["--version".to_string()], "nagios"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_nagios(&[], "nagios"), 0);
    }
}
