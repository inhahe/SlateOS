...
fn run_kubeseal(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("kubeseal v0.27.0 (Slate OS) — Sealed Secrets encryption");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-V" || a == "--version") {
        println!("kubeseal v0.27.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "kubeseal".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kubeseal, strip_ext};
