...
fn run_rm(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "RPG Maker MZ (Slate OS) — Kadokawa's classic JRPG-style maker (current 'MZ' version)"
        );
        println!();
        println!("Options:");
    ...
    println!(
        "         Originally by ASCII (1992), then Enterbrain, then Kadokawa Future Publishing"
    );
    ...
    println!(
        "  Niche: top-down JRPG-style games — tile maps, turn-based combat, item shops, towns"
    );
    ...
    println!(
        "  Differentiator: no programming needed for basic RPGs — event commands cover everything"
    );
    println!("  Plugins: 1000s of community + paid plugins (Yanfly Engine, VisuStella, etc.)");
    println!(
        "  Asset packs: huge market for character sprites / tilesets / music on the official store"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "rpgmaker".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_rm, strip_ext};
