...
fn run_sclang(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("SuperCollider Language 3.13.0 (Slate OS)");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("sclang 3.13.0 (Slate OS)");
        return 0;
    }
    let file = args
        .iter()
        .find(|a| a.ends_with(".scd") || a.ends_with(".sc"))
        .map(|s| s.as_str());
...
fn run_scsynth(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("scsynth 3.13.0 (Slate OS)");
        return 0;
    }
    let port = args
        .windows(2)
...
fn run_scide(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("SuperCollider IDE 3.13.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "sclang".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sclang, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_sclang(&["--version".to_string()]);
    }
