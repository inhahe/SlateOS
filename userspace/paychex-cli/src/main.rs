...
fn run_px(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "          Golisano targeted businesses ADP considered too small (under 50 employees)"
    );
    ...
    println!(
        "  Mergers + acquisitions: Advance Partners 2015, HR Outsourcing 2018 ($1.2B Oasis), Lessor Group 2022"
    );
    println!(
        "  Critique: pricier than online-only competitors (Gusto, Square Payroll) for similar features"
    );
    println!(
        "           still relies on account-manager relationship — friction vs self-serve modern UX"
    );
    println!(
        "  Differentiator: dedicated account manager + decades of compliance expertise for SMB"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "paychex".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_px, strip_ext};
