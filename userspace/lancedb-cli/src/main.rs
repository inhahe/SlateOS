...
//! lancedb-cli — OurOS LanceDB (multi-modal Rust vector DB on Lance columnar format, SF, private)
...
fn run_lance(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("LanceDB (OurOS) — multi-modal Rust vector DB on Lance columnar format");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("LanceDB 2024 (OurOS) — lancedb CLI 0.x"); return 0; }
    println!("LanceDB 2024 (OurOS) — Multi-Modal Vector Database on Lance Columnar Format");
