...
fn run_skychart(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Cartes du Ciel / SkyChart v4.3 (Slate OS) — Star chart generator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Cartes du Ciel v4.3 (Slate OS)");
        return 0;
    }
    println!("Cartes du Ciel v4.3 (Slate OS) — Sky Chart");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "skychart".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_skychart, strip_ext};
