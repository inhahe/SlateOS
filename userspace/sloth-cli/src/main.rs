...
fn run_sloth(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Sloth v0.11.0 (Slate OS) — SLO generation framework");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("Sloth v0.11.0 (Slate OS)");
        return 0;
    }
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sloth, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_sloth(&["--version".to_string()], "sloth");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_sloth(&[], "sloth");
    }
}
