//! OurOS Multi-Personality Utility: mktemp / id / groups / whoami
//!
...
fn print_mktemp_help() {
    println!("mktemp (OurOS) 0.1.0 -- Create temporary files or directories safely");
    println!();
    println!("USAGE:");
...
fn print_id_help() {
    println!("id (OurOS) 0.1.0 -- Display user and group IDs");
    println!();
    println!("USAGE:");
...
fn run_groups(args: &[String]) -> i32 {
    ...
    while i < args.len() {
        ...
        match arg.as_str() {
            "-h" | "--help" => {
                println!("groups (OurOS) 0.1.0 -- Display group memberships");
                println!();
                println!("USAGE:");
                println!("  groups [USER...]");
                println!();
