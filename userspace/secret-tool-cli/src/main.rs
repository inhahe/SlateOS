...
fn run_secret_tool(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("secret-tool v0.20 (Slate OS) — Secret storage CLI");
        println!();
        println!("Commands:");
...
#[cfg(test)]
mod tests {
    use super::{basename, run_secret_tool, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_secret_tool(&["--version".to_string()], "secret-tool");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_secret_tool(&[], "secret-tool");
    }
}
