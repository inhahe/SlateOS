...
fn run_kopano(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "kopano-admin" => {
                println!("kopano-admin (Slate OS) — Kopano user management");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Kopano v8.7.27 (Slate OS)");
        return 0;
    }
    match prog {
        "kopano-admin" => {
        ...
        _ => {
            println!("Kopano Server v8.7.27 (Slate OS)");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "kopano-server".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kopano, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_kopano(&["--version".to_string()], "kopano");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_kopano(&[], "kopano");
    }
}
