...
//! bemenu-cli — OurOS bemenu dynamic menu
//!
...
fn run_bemenu(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("bemenu v0.6 (OurOS) — Dynamic menu (dmenu clone for Wayland)");
        println!();
        println!("Options:");
...
fn run_bemenu_run(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("bemenu-run v0.6 (OurOS) — Application launcher (lists PATH commands)");
        println!();
