...
fn run_cribl(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Cribl Stream v4.7 (Slate OS) — Observability data pipeline");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Cribl Stream v4.7.2 (Slate OS)"); return 0; }
    println!("Cribl Stream v4.7.2 (Slate OS)");
    println!("  Mode: leader");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_cribl};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/cribl"), "cribl");
        assert_eq!(basename(r"C:\bin\cribl.exe"), "cribl.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("cribl.exe"), "cribl");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_cribl(&["--help".to_string()], "cribl"), 0);
        assert_eq!(run_cribl(&["-h".to_string()], "cribl"), 0);
        let _ = run_cribl(&["--version".to_string()], "cribl");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_cribl(&[], "cribl");
    }
}
