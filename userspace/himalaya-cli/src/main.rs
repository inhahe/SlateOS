...
//! himalaya-cli — OurOS Himalaya CLI email client
//!
...
fn run_himalaya(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("himalaya v1.0 (OurOS) — CLI email client (Rust)");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_himalaya(&["--version".to_string()], "himalaya"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_himalaya(&[], "himalaya"), 0);
    }
}
