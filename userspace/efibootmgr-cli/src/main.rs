...
//! efibootmgr-cli — OurOS EFI boot manager
//!
...
fn run_efibootmgr(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("efibootmgr — EFI boot manager (OurOS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("efibootmgr version 18 (OurOS)");
        return 0;
    }
    ...
    if verbose {
        println!("Boot0001* OurOS\tHD(1,GPT,abcdef01-2345-6789-abcd-ef0123456789,0x800,0x100000)/File(\\EFI\\ouros\\grubx64.efi)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_efibootmgr(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_efibootmgr(&[]), 0);
    }
}
