...
fn run_cellwriter(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("cellwriter v1.3 (Slate OS) — Handwriting recognition input panel");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("CellWriter v1.3.6 (Slate OS)");
        return 0;
    }
    ...
    println!("CellWriter v1.3.6 (Slate OS) — Handwriting Input");
    println!("  Cells: 8");
    println!("  Profile: default");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "cellwriter".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_cellwriter, strip_ext};
