...
fn run_zigbee2mqtt(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Zigbee2MQTT 1.35.1 (Slate OS)");
...
fn run_zbcli(args: &[String]) -> i32 {
    ...
    match subcmd {
        "version" | "--version" => println!("zbcli 1.35.1 (Zigbee2MQTT, Slate OS)"),
        "devices" => {
        ...
        _ => println!("zbcli: {} completed", quoteaf_os(subcmd)),
    }
    0
}
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "zbcli".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_zigbee2mqtt, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_zigbee2mqtt(&["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_zigbee2mqtt(&[]);
    }
}
