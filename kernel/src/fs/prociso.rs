...
pub fn init_defaults() {
    ...
    *guard = Some(State {
        namespaces: alloc::vec![
            Namespace {
                id: 1,
                ns_type: NsType::Mount,
                name: String::from("root_mnt"),
                isolation: IsolationLevel::None,
                parent_id: None,
                processes: alloc::vec![1],
                created_ns: now,
            },
            Namespace {
                id: 2,
                ns_type: NsType::Pid,
                name: String::from("root_pid"),
                isolation: IsolationLevel::None,
                parent_id: None,
                processes: alloc::vec![1],
                created_ns: now,
...
pub fn create_namespace(
    ...
    with_state(|state| {
        ...
        if state.namespaces.iter().any(|n| n.name == name) {
            return Err(KernelError::AlreadyExists);
        }
        if let Some(pid) = parent {
            if !state.namespaces.iter().any(|n| n.id == pid) {
...
pub fn delete_container(id: u32) -> KernelResult<()> {
    with_state(|state| {
        ...
        for &ns_id in &container.namespaces {
            state
                .namespaces
                .retain(|n| n.id != ns_id || n.processes.is_empty());
        }
        Ok(())
    })
}
...
pub fn list_containers() -> Vec<Container> {
    STATE
        .lock()
        .as_ref()
        .map_or(Vec::new(), |s| s.containers.clone())
}
...
pub fn get_namespace(id: u32) -> Option<Namespace> {
    STATE
        .lock()
        .as_ref()
        .and_then(|s| s.namespaces.iter().find(|n| n.id == id).cloned())
}
...
pub fn self_test() {
    ...
    // 2: Create namespace.
    let id =
        create_namespace(NsType::User, "test_user", IsolationLevel::Full, None).expect("create");
    assert!(id >= 4);
    ...
    // 4: Create container.
    let cid = create_container("test_container", &[NsType::Mount, NsType::Pid, NsType::Net])
        .expect("cont");
    ...
    // 6: Nested namespace.
    let parent = create_namespace(NsType::Mount, "parent_mnt", IsolationLevel::Partial, None)
        .expect("parent");
    let child = create_namespace(
        NsType::Mount,
        "child_mnt",
        IsolationLevel::Full,
        Some(parent),
