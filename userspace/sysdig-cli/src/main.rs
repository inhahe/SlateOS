...
//! sysdig-cli — OurOS Sysdig (Falco creator, runtime security + monitoring, Davis CA)
...
fn run_sysdig(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Sysdig (OurOS) — runtime cloud security (Falco creator)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Sysdig + Falco 0.39 (OurOS)"); return 0; }
    println!("Sysdig 2024 (OurOS) — Runtime Cloud Security");
