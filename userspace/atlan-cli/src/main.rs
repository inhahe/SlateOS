...
fn run_atlan(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "          Prukalpa: World Economic Forum Young Global Leader, prolific data Substack"
    );
    ...
    println!(
        "         earlier: Sequoia India, Waterbridge, angels (Postman + Freshworks founders)"
    );
    println!("  ARR: estimated $50M-100M+ ARR (growing fast, not public)");
    println!(
        "  Strategic position: 'data catalog 3.0' — third wave after Informatica + Collibra/Alation:"
    );
    println!("                    1st gen: Informatica/IBM/SAP (enterprise on-prem, $$$)");
    println!(
        "                    2nd gen: Collibra + Alation (2010s, cloud-aware, still enterprise-heavy)"
    );
    println!(
        "                    3rd gen: Atlan + Castor + Select Star (modern data stack-native, fast onboarding)"
    );
    println!(
        "                    Atlan's wedge: 'data team product' UX + dbt/Snowflake/Looker-native + asset-based pricing"
    );
    println!(
        "                    Galaxy-style canvas + Notion-style docs + Slack-style collaboration"
    );
    println!(
        "                    bet: every modern data team needs a catalog, and old vendors are unusable"
    );
    ...
    println!(
        "    Enterprise — $250K-2M+/yr (SSO, SCIM, on-prem option, audit logs, dedicated CSM)"
    );
    println!("    asset-based + user-based hybrid pricing model");
    println!("  Core platform:");
    println!(
        "    - Active metadata: bidirectional sync with all data tools (not just passive scraping)"
    );
    println!("    - Open API + webhooks: query metadata, push annotations from anywhere");
    println!(
        "    - Column-level lineage across Snowflake/BigQuery/Redshift/Databricks → dbt → BI tools"
    );
    println!(
        "    - Asset 360 view: SQL, owners, freshness, quality, lineage, business context all in one page"
    );
    println!("    - Slack + Teams integration: data context follows the conversation");
    println!(
        "    - Atlan Playbooks: no-code automation (auto-tag PII, alert on schema drift, propagate ownership)"
    );
    println!("  Connectors (60+):");
    println!(
        "    - Warehouses: Snowflake, BigQuery, Databricks, Redshift, Synapse, ClickHouse, Postgres"
    );
    println!(
        "    - Transformation: dbt Cloud + Core (deep integration — Atlan was an early dbt partner)"
    );
    ...
    println!(
        "    - dbt Labs (Tristan Handy + team) publicly recommends Atlan as catalog of choice"
    );
    ...
    println!(
        "    - Account_usage view ingestion: query history, access patterns, popularity scores"
    );
    ...
    println!(
        "    - vs passive catalogs (Collibra/Alation): metadata is read + written bidirectionally"
    );
    ...
    println!(
        "    - vs Collibra: Atlan faster onboarding (hours vs months), better dbt/Snowflake-native UX"
    );
    println!(
        "    - vs Alation: similar — Atlan's UX is the bet, Alation has more enterprise feature breadth"
    );
    println!(
        "    - vs data.world: data.world more semantic/knowledge graph-leaning, Atlan more workflow"
    );
    println!("    - vs Select Star: similar generation, Atlan larger + more enterprise traction");
    println!(
        "    - vs Castor (now CastorDoc, acquired by Coalesce 2024): direct competitor, Atlan bigger"
    );
    println!(
        "    - vs DataHub (LinkedIn OSS + Acryl commercial): DataHub more eng-first, Atlan more business-friendly"
    );
    println!(
        "    - vs Microsoft Purview / Google Dataplex / AWS DataZone: cloud-native catalogs, free w/ cloud — Atlan justifies premium via UX + cross-cloud"
    );
    ...
    println!(
        "           overlap with dbt Cloud's own catalog features (dbt Explorer) — dbt could disintermediate"
    );
    println!(
        "           Snowflake Horizon governance + Databricks Unity Catalog compete from the warehouse side"
    );
    println!("           catalog category historically slow ROI — buyers struggle to justify");
    println!(
        "           less mature than Collibra for highly-regulated industries (banking, pharma)"
    );
    println!(
        "           remote-first culture + India HQ: occasional time-zone friction for US enterprise sales"
    );
    println!(
        "  Differentiator: dbt-native + Snowflake-native + active metadata thesis + Notion-style UX + remote-first culture — the modern-data-stack catalog of choice, beating legacy Collibra/Alation on speed-to-value"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "atlan".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_atlan, strip_ext};
