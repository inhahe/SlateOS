...
fn run_livechat(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "        rebranded from LiveChat Software → Text in 2023 to reflect multi-product portfolio"
    );
    ...
    println!(
        "          originally bought existing live chat code from US devs, modernized iteratively"
    );
    ...
    println!(
        "          Team $41/agent/mo (unlimited history, multiple branding, advanced reports)"
    );
    ...
    println!(
        "    - integration with ChatBot.com for deflection (single auth, shared customer record)"
    );
    ...
    println!(
        "            Mercedes-Benz (regional sites), Pearson, Adobe (some teams), Atlassian (parts)"
    );
    ...
    println!(
        "  Critique: pure-play chat ceiling — less of a fit if you want unified email/chat/social helpdesk"
    );
    println!("           pricing higher than Tidio/Tawk.to for solo entrepreneurs");
    println!("           AI features lag Intercom Fin / Zendesk Advanced AI");
    println!(
        "           Text S.A. rebrand created some brand confusion (LiveChat vs Text vs ChatBot.com)"
    );
    println!(
        "  Differentiator: 22 years of pure focus on chat — deepest chat-only widget feature set + profitable Polish public company"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "livechat".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_livechat, strip_ext};
