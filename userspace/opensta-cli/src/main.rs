...
fn run_sta(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("OpenSTA v2.5.0 (Slate OS)");
        return 0;
    }
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sta, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/opensta"), "opensta");
        assert_eq!(basename(r"C:\bin\opensta.exe"), "opensta.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("opensta.exe"), "opensta");
