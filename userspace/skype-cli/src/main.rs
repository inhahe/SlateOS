...
fn run_sk(args: &[String], _prog: &str) -> i32 {
    ...
    println!("Skype 8.130.0.207 (Slate OS)");
    println!(
        "  Owner: Microsoft (acquired May 2011 from eBay+silver lake/canada pension for $8.5B)"
    );
    ...
    println!(
        "  Decline: Microsoft shifted enterprise to Teams (2017), consumer focus on Teams free"
    );
    println!(
        "  Skype for Business: discontinued Jul 2021 (online), Jul 2025 on-prem (Teams replaces)"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "skype".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sk, strip_ext};
