...
fn run_gluster(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!();
        println!("gluster — GlusterFS CLI (Slate OS).");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("glusterfs 11.1 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "gluster".to_string());
    let rest: Vec<String> = args.into_iter().skip(1).collect();
    let code = match prog.as_str() {
        "glusterd" => {
            println!("glusterd: started (Slate OS)");
            0
        }
        "glusterfsd" => {
            println!("glusterfsd: brick process started");
            0
        }
        _ => run_gluster(&rest),
    };
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gluster, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_gluster(&["--version".to_string()]);
    }
