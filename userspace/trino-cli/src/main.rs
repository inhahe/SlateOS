...
fn run_trino(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Trino (Slate OS) — open-source distributed SQL query engine (formerly PrestoSQL)"
        );
        println!();
        println!("Options:");
        ...
        println!(
            "  --foundation           Trino Software Foundation (501(c)(3) nonprofit governance)"
        );
        println!("  --version              Show version");
        return 0;
    }
    if args.iter().any(|a| a == "--version") {
        println!("Trino 445 (Slate OS)");
        return 0;
    }
    ...
    println!(
        "            board: Martin Traverso, Dain Sundstrom, David Phillips + community members"
    );
    println!("            independent governance after fork from PrestoDB (Facebook-controlled)");
    println!(
        "  Creators: Martin Traverso, Dain Sundstrom, David Phillips, Eric Hwang, 2012 at Facebook"
    );
    println!(
        "          built Presto inside Facebook to query 300+ PB Hive warehouse + replace Hadoop MapReduce SQL"
    );
    println!("          open-sourced Nov 2013 as Presto");
    println!(
        "          all left Facebook 2018 to form Presto Software Foundation (independent governance)"
    );
    ...
    println!(
        "          renamed again to 'Trino' Dec 2020 due to trademark concerns with Facebook + Linux Foundation"
    );
    println!("  Origin story (the dramatic fork):");
    println!(
        "    - Facebook donated Presto trademark to Linux Foundation as 'PrestoDB Software Foundation'"
    );
    println!(
        "    - PrestoDB stayed Facebook-controlled; PrestoSQL/Trino became truly community-led"
    );
    println!(
        "    - For ~2 years (2019-2020) there were TWO 'Presto' projects causing massive confusion"
    );
    println!("    - Trino rename (Dec 2020) was meant to end confusion + signal independence");
    println!(
        "    - PrestoDB (Facebook-led) still exists but has far less development velocity than Trino"
    );
    ...
    println!(
        "    Commercial: GCP Athena equivalent (BigQuery Federation), Azure Synapse Serverless"
    );
    ...
    println!(
        "    - Pluggable Connector SPI: read from any source (Hive, Iceberg, Delta, Postgres, MySQL, MongoDB)"
    );
    ...
    println!(
        "    - Fault-tolerant execution (since v382) — checkpoints + retry instead of restart"
    );
    ...
    println!(
        "    - Netflix: original adopter, 7+ PB Trino daily, internal data platform centerpiece"
    );
    ...
    println!(
        "    - Starburst Data (commercial Trino) employs many top maintainers but doesn't control project"
    );
    ...
    println!(
        "  Critique: operating Trino requires data engineering expertise (cluster sizing, GC tuning, connector configuration)"
    );
    println!(
        "           cost-based optimizer occasionally produces poor plans on Iceberg/Delta — needs tuning"
    );
    println!(
        "           memory-heavy: spilling to disk available since v336 but performance varies"
    );
    println!(
        "           catalog/governance: native RBAC is basic — typically use Apache Ranger, Privacera, or Starburst Gravity"
    );
    println!(
        "           ecosystem fragmentation: Trino vs PrestoDB vs Athena (all somewhat compatible, somewhat not)"
    );
    println!(
        "           commercial vendors (Starburst especially) drive most enterprise feature roadmap"
    );
    println!(
        "           competing with: Spark SQL (Databricks), BigQuery, Snowflake — all have commercial backing"
    );
    println!(
        "  Differentiator: truly open-source + community-governed + 50+ connectors + federation across any source + battle-tested at Netflix/Lyft/Pinterest scale + 1,100+ contributors — the free distributed SQL engine that powers most lakehouses"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_trino, strip_ext};
