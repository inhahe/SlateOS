...
fn run_brex(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Founders: Henrique Dubugras + Pedro Franceschi (Brazil — sold Pagar.me to Stone for $150M before age 22)"
    );
    ...
    println!(
        "  History: 2017 launched first corporate card for startups (no personal guarantee, daily payoff)"
    );
    println!("          2020: pivoted to broader 'financial OS for startups'");
    println!(
        "          2022: famously pulled out of SMB market ('focus on growth-stage + enterprise')"
    );
    ...
    println!(
        "    - Daily payoff card (not 30-day cycle) — preserves credit limit for fast-burning startups"
    );
    println!(
        "    - Underwriting based on cash balance (not personal credit) — perfect for funded startups"
    );
    println!(
        "    - Brex Cash (formerly Brex Empower) — business account with FDIC sweep + MMF yield (~4.7% Mar 2024)"
    );
    ...
    println!(
        "  Rewards: Brex card cashback varies (1-7x on certain categories like SaaS, Apple, dining)"
    );
    println!("          Brex Points redeemable for travel, statements, gift cards");
    println!(
        "  Customers: 30K startups + scale-ups — Anthropic, Coinbase, Robinhood, Plaid, Notion"
    );
    ...
    println!(
        "  Differentiator: deep startup-CFO product (Cash account + Premium) + premium category rewards"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_brex, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_brex(&["--version".to_string()], "brex");
    }
