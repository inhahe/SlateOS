...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_store_ops = STORE_OPS.load(Ordering::Relaxed);
    let saved_retrieve_ops = RETRIEVE_OPS.load(Ordering::Relaxed);
    let result =
        crate::fs::selftest::with_pristine(&STORE, CredentialStore::new(), self_test_inner);
    STORE_OPS.store(saved_store_ops, Ordering::Relaxed);
    RETRIEVE_OPS.store(saved_retrieve_ops, Ordering::Relaxed);
    result
}
