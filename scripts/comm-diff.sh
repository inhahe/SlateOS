...
run_side() {
  ...
  diff_run timeout -k 2 30 env ${ENVV[@]+"${ENVV[@]}"} "LC_ALL=$loc" PATH="$bindir/$side" comm "$@"
}
...
run_case()  { compare - C "$@"; report "${ENVV[*]:+${ENVV[*]} }comm $*"; }
...
run_utf8()  { compare - C.UTF-8 "$@"; report "${ENVV[*]:+${ENVV[*]} }comm $* [C.UTF-8]"; }
...
# Through `run_diag`, as every diagnostic here is: the third operand is refused
# as `extra operand`, whose quotes are §351's curly ones in every locale.
ENVV=(POSIXLY_CORRECT=1)
run_diag a.txt b.txt -1
