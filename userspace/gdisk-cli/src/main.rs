...
//! gdisk-cli — OurOS gdisk GPT partition tool
//!
...
fn run_gdisk(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("gdisk v1.0 (OurOS) — Interactive GPT partition editor");
        println!();
    ...
    if args.iter().any(|a| a == "--version") { println!("gdisk v1.0 (OurOS)"); return 0; }
...
fn run_sgdisk(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("sgdisk v1.0 (OurOS) — Scriptable GPT partition editor");
        println!("  -p            Print partition table");
    ...
    if args.iter().any(|a| a == "--version") { println!("sgdisk v1.0 (OurOS)"); return 0; }
    if args.iter().any(|a| a == "-p") {
        println!("Number  Start      End         Size       Code  Name");
        println!("   1    2048       1050623     512.0 MiB  EF00  EFI system");
        println!("   2    1050624    967098367   461.0 GiB  8300  OurOS root");
...
fn run_cgdisk(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("cgdisk v1.0 (OurOS) — Curses-based GPT partition editor");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("cgdisk v1.0 (OurOS)"); return 0; }
...
fn run_fixparts(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("fixparts v1.0 (OurOS) — Fix MBR partition table problems");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("fixparts v1.0 (OurOS)"); return 0; }
