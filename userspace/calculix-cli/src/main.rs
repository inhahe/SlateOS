...
//! calculix-cli — OurOS CalculiX finite element solver
//!
...
fn run_ccx(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("CalculiX CrunchiX v2.21 (OurOS) — FEM solver");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("CalculiX CrunchiX v2.21 (OurOS)"); return 0; }
    let input = args.windows(2).find(|w| w[0] == "-i").map(|w| w[1].as_str());
    if input.is_none() {
        eprintln!("ccx: error: no input file (-i)");
        return 1;
    }
    println!("CalculiX CrunchiX v2.21 (OurOS)");
...
fn run_cgx(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("CalculiX GraphiX v2.21 (OurOS) — FEM pre/post-processor");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("CalculiX GraphiX v2.21 (OurOS)"); return 0; }
    println!("CalculiX GraphiX v2.21 (OurOS) — Pre/Post-Processor");
