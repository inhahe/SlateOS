...
fn run_waves(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Waves Audio (Slate OS) — Industry-leading plug-in catalog (Waves V14)");
        println!();
    ...
    if args.iter().any(|a| a == "--version") { println!("Waves V14 (Slate OS)"); return 0; }
    println!("Waves V14 (Slate OS)");
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_waves(&["--version".to_string()], "waves");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_waves(&[], "waves");
    }
}
