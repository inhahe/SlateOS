...
fn run_ap(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Affinity Photo 2 (Slate OS) — Pro raster photo editor (Photoshop alternative)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Affinity Photo 2.5.7 (Slate OS)");
        return 0;
    }
    println!("Affinity Photo 2.5.7 (Slate OS)");
    println!("  Vendor: Serif Europe Ltd (Nottingham, UK; founded 1987)");
    println!(
        "  Acquired by: Canva (Mar 2024, undisclosed terms — significant for design industry)"
    );
    ...
    println!(
        "                 Affinity v2 Universal License $164.99 (one-time, all 3 apps Win/Mac/iPad)"
    );
    println!(
        "  Engine: custom C++ engine, GPU-accelerated (Metal/D3D), 64-bit color, 1000x100K px"
    );
    println!(
        "  Personas: workflow-mode switching (Photo / Liquify / Develop / Tone Mapping / Export)"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "affinityphoto".to_string());
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_ap(&["--version".to_string()], "affinityphoto");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_ap(&[], "affinityphoto");
    }
}
