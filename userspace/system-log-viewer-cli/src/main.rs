...
fn run_system_log(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("gnome-system-log v43.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "gnome-system-log".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_system_log, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/system-log-viewer"), "system-log-viewer");
        assert_eq!(
            basename(r"C:\bin\system-log-viewer.exe"),
            "system-log-viewer.exe"
        );
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_system_log(&["--help".to_string()], "system-log-viewer"),
            0
        );
