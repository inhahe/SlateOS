...
//! servicebus-cli — OurOS Azure Service Bus (Microsoft's enterprise broker, AMQP 1.0, Redmond WA)
...
fn run_sb(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Azure Service Bus (OurOS) — Microsoft's enterprise messaging broker on Azure");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Azure Service Bus 2024 (OurOS) — servicebus CLI (az + Service Bus Explorer)"); return 0; }
    println!("Azure Service Bus 2024 (OurOS) — Enterprise Messaging on Azure (AMQP 1.0-native)");
