...
- [-] Random background on boot or daily rotation (with exclusion filters) — *Reader landed 2026-09-17. `appearance.yaml` now carries `wallpaper.folder`, `wallpaper.interval_secs` and `wallpaper.shuffle`; the shell honours them, scanning the folder only when it changes rather than on every settings read. **The bigger find was that nothing turned the clock**: `WallpaperManager::tick` had no caller outside its own tests, so a slideshow could never have advanced and the time-of-day gradient could never have changed, whatever was configured. `step_frame` ticks it now. The folder is percent-encoded under the same §426 marker as the picture, since it is a path too. **Still to do:** exclusion filters, and a control in the Settings app — the reader lands first on purpose, because a control writing a value nothing reads is what design-decisions 852 is about.*
