...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "authentik".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_authentik, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/authentik"), "authentik");
        assert_eq!(basename(r"C:\bin\authentik.exe"), "authentik.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("authentik.exe"), "authentik");
