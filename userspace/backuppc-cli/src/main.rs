...
fn run_backuppc(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("backuppc v4.4 (Slate OS) — Enterprise-grade backup system");
        println!();
        println!("Options:");
...
fn run_backuppc_nightly(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("backuppc-nightly v4.4 (Slate OS) — Nightly maintenance");
        return 0;
    }
    let _ = args;
...
fn run_backuppc_servermesg(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("backuppc-servermesg v4.4 (Slate OS) — Send messages to server");
        return 0;
    }
