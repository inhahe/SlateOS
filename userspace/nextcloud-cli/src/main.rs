...
fn run_nextcloud(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Nextcloud v29.0 (Slate OS) — Self-hosted file sync & share");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Nextcloud v29.0.4 (Slate OS)"); return 0; }
    println!("Nextcloud v29.0.4 (Slate OS)");
    println!("  Users: 45");
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_nextcloud(&["--version".to_string()], "nextcloud");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_nextcloud(&[], "nextcloud");
    }
}
