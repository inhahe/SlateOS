...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/containerd"), "containerd");
        assert_eq!(basename(r"C:\bin\containerd.exe"), "containerd.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("containerd.exe"), "containerd");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_ctr(&["--help".to_string()]), 0);
        assert_eq!(run_ctr(&["-h".to_string()]), 0);
        assert_eq!(run_ctr(&["--version".to_string()]), 0);
    }
