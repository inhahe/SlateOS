...
fn run_aqua(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Aqua Security (Slate OS) — container + cloud-native security platform");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Aqua Security 2024 + Trivy 0.55 (Slate OS)");
        return 0;
    }
    println!("Aqua Security 2024 (Slate OS) — Cloud-Native Security");
    ...
    println!(
        "          one of the original three 'container security' vendors (with Twistlock + StackRox)"
    );
    println!(
        "          Twistlock acquired by Palo Alto (2019, $410M), StackRox by Red Hat (2021, $250M)"
    );
    ...
    println!(
        "         earlier: Lightspeed, Microsoft Ventures, Insight, IBM Ventures, TLV Partners"
    );
    println!("         private + profitable as of 2024");
    println!(
        "  Strategic position: 'cloud-native security from build to runtime — open-source DNA':"
    );
    println!(
        "                    pitch: 'protect containers, K8s, serverless, and cloud — across the lifecycle'"
    );
    println!("                    target: cloud-native engineering teams + Kubernetes operators");
    println!(
        "                    primary competitor: Wiz, Palo Alto Prisma Cloud, CrowdStrike Falcon Cloud, Sysdig"
    );
    println!(
        "                    Aqua's wedge: deepest container/K8s heritage + OSS (Trivy + Tracee) leadership"
    );
    println!(
        "                    moat: free OSS adoption funnel — Trivy 22K+ stars, default in 80%+ K8s CI scans"
    );
    println!("                    challenge: CSPM/agentless features behind Wiz");
    println!("  Pricing:");
    println!(
        "    Trivy OSS — FREE, Apache 2.0 (the world's most-used container vulnerability scanner)"
    );
    ...
    println!(
        "    - Scans: container images, K8s manifests, Terraform, Dockerfile, CycloneDX SBOMs"
    );
    println!(
        "    - Default scanner in: GitHub Container Registry, Harbor, AWS ECR, GCP Artifact Registry"
    );
    ...
    println!(
        "    1. Aqua Repository Scanning — image scanning in registries (Trivy-powered + commercial features)"
    );
    ...
    println!(
        "    - Discovered HiddenWasp Linux malware, EleKtra-Leak (AWS keys in GitHub), and many K8s attacks"
    );
    ...
    println!(
        "    - Cloud Provider: AWS Security Hub, Microsoft Defender, Google Security Command Center"
    );
    ...
    println!(
        "           DevSecOps positioning sometimes too dev-friendly for security-team-led buying"
    );
    ...
    println!(
        "           Israeli engineering concentration = post-Oct 2023 sourcing scrutiny in some sectors"
    );
    println!("           IPO talked about for years — still private, growth pace below peers");
    println!(
        "  Differentiator: deepest container + Kubernetes security heritage + Trivy OSS distribution (22K+ stars) + Tracee eBPF leadership + Aqua Nautilus research team + full lifecycle build-to-runtime coverage — the cloud-native security platform with the strongest open-source community foundation"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "aquasec".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_aqua, strip_ext};
