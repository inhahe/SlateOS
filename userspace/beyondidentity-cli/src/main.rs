...
fn run_bi(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Beyond Identity (Slate OS) — passwordless phishing-resistant MFA on device-bound credentials"
        );
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Beyond Identity 2024 (Slate OS) — beyondidentity-cli (REST + bi-cli)");
        return 0;
    }
    println!(
        "Beyond Identity 2024 (Slate OS) — Passwordless Phishing-Resistant Auth (Netscape Pedigree)"
    );
    println!("  Vendor: Beyond Identity, Inc. (New York, NY — private)");
    println!("  Founders: Jim Clark + TJ Jermoluk, 2020");
    println!(
        "          Jim Clark: Netscape co-founder (Netscape Navigator, 1994 IPO), Silicon Graphics co-founder, Healtheon"
    );
    println!(
        "          TJ Jermoluk: ex-CEO @Home Network (cable broadband internet pioneer), ex-Healtheon"
    );
    println!("          Started Beyond Identity to 'fix authentication's fundamental problems'");
    println!(
        "          'Same team that brought the web to the masses, now fixing how the web logs you in'"
    );
    println!(
        "          Industry credibility: Jim Clark = legendary serial entrepreneur, $billions exits"
    );
    ...
    println!(
        "  Strategic position: 'eliminate passwords entirely via device-bound cryptographic keys':"
    );
    println!(
        "                    pitch: 'no passwords, no OTPs, no shared secrets — only device-bound keys + biometric'"
    );
    println!("                    target: workforce + CIAM at security-forward enterprises");
    println!(
        "                    primary competitor: Okta + Duo, Microsoft Authenticator, YubiKey, Hypr"
    );
    println!(
        "                    secondary: Auth0, Stytch, Clerk for CIAM; SailPoint + Ping for workforce"
    );
    println!(
        "                    BI wedge: device-bound cryptographic credentials = phishing-resistant by design"
    );
    println!("                    + no passwords on the wire ever");
    println!("                    + no OTPs (which are phishable)");
    println!(
        "                    + biometric unlocks the local credential (TEE/Secure Enclave/TPM)"
    );
    ...
    println!(
        "    - Netscape Communications co-founder (Netscape Navigator browser, 1994 IPO defined dotcom)"
    );
    ...
    println!(
        "    bi-ssh login                                             # initialize device-bound SSH key"
    );
    println!(
        "    ssh user@host                                            # auto-authenticated via BI"
    );
    println!("    # Git commit signing:");
    println!("    git config --global gpg.format ssh");
    println!("    git config --global user.signingKey <BI-key>");
    println!(
        "    git commit -S -m 'signed commit'                        # signed with BI device key"
    );
    ...
    println!(
        "           passwordless UX still requires user adjustment (no more typing passwords)"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "beyondidentity".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_bi, strip_ext};
