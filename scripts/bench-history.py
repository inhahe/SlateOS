...
#: A call to a bare local function: `bench_foo(` with no `::` before it.
CALL_NAME_RE = re.compile(r'(?<![:.\w])([a-z_][a-z0-9_]*)\s*\(')


def reachable_bench_fns(text):
    """Bodies of the functions in `bench.rs` reachable from `run_all`, by name.

    `None` if `run_all` itself cannot be found, which callers must treat as *do not
    narrow* rather than as *nothing is reachable*.

    WHY THIS EXISTS. `declared_series` excuses a series that vanished from the records
    if `bench.rs` still contains a `score("x")` or `track("x")` for it. That trusted
    the presence of the text without asking whether the enclosing function is still
    CALLED. Delete one line from `run_all` and leave the function behind, and the
    series stops being recorded, keeps its `track(` text, and has its disappearance
    excused -- which is precisely the silent coverage loss the guard exists to catch.
    Nothing else closes it: an uncalled `fn` is `dead_code`, which is a warning, and
    this build emits about eighteen thousand of them.

    WHICH WAY IT FAILS, because that is the part worth choosing deliberately. A call
    made through a function pointer or generated by a macro is not seen, so a function
    that really does run can look unreachable. That makes the gate STRICTER -- its
    series is no longer excused -- and it only bites when combined with an actual gap
    in the records, which a running benchmark does not produce. The opposite error,
    excusing too much, is the one that loses coverage in silence, so the analysis is
    pointed to fail in the direction that is merely noisy.
    """
    # `BENCH_FN_RE` is a PER-LINE pattern -- its other caller uses `.match(line)`, so it
    # carries no `re.M` and `finditer` over the whole file matches at most once, at
    # offset 0. Scanning the file needs the multiline flag, and deriving it from the same
    # `.pattern` rather than retyping it means the two cannot drift apart. Compiled here
    # rather than at module level because `BENCH_FN_RE` is defined further down the file.
    fn_at_line_start = re.compile(BENCH_FN_RE.pattern, re.M)
    bodies = {}
    for m in fn_at_line_start.finditer(text):
        body = rustscan.fn_body(text, m.start())
        if body is not None:
            bodies[m.group(1)] = body
    if "run_all" not in bodies:
        return None
    seen = {}
    queue = ["run_all"]
    while queue:
        ...
        if body is None:
            continue
        seen[name] = body
        for callee in CALL_NAME_RE.findall(body):
...
def declared_series(bench_source=BENCH_SOURCE):
    ...
    try:
        ...
        return set()
    reachable = reachable_bench_fns(text)
    if reachable is None:
        # No `run_all` found. Fall back to counting every score/track call, which is
        # the looser answer -- deliberately. A scanner that cannot find the root must
        # not be the thing that makes this gate stricter: being strict here means
        # refusing to excuse a removal, and an unexcused removal is a pre-build
        # failure that only a new boot record can clear. That is the deadlock this
        # function was written to end, and a parse failure must not recreate it.
        return set(SCORE_CALL_RE.findall(text)) | set(TRACK_CALL_RE.findall(text))
    out: set[str] = set()
    for fn, body in reachable.items():
        del fn
        out |= set(SCORE_CALL_RE.findall(body))
        out |= set(TRACK_CALL_RE.findall(body))
    return out
