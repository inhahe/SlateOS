...
fn run_bareos_dir(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("bareos-dir v23.0 (Slate OS)");
        return 0;
    }
...
fn run_bareos_sd(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("bareos-sd v23.0 (Slate OS)");
        return 0;
    }
...
fn run_bareos_fd(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("bareos-fd v23.0 (Slate OS)");
        return 0;
    }
...
fn run_bconsole(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("bconsole v23.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "bareos-dir".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_bareos_dir, strip_ext};
