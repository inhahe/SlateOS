...
//! contour-cli — OurOS Contour terminal emulator
//!
...
fn run_contour(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("contour v0.4 (OurOS) — Modern terminal emulator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("contour v0.4 (OurOS)"); return 0; }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_contour(&["--version".to_string()], "contour"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_contour(&[], "contour"), 0);
    }
}
