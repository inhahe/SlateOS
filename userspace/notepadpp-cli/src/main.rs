...
//! notepadpp-cli — Slate OS Notepad++ (Don Ho's Windows-only free editor)
//!
...
fn run_npp(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Platform: Windows-only (Win32 / Win64 — Wine-friendly on Linux via Bottles/Lutris)"
    );
    println!("  Pricing: FREE — donation-supported, distributed via notepad-plus-plus.org");
    println!(
        "  Engine: C++ with the Scintilla editing component (also used by SciTE, Geany, Code::Blocks)"
    );
    ...
    println!(
        "  Plugin Admin: built-in plugin manager — install Compare, NppExec, XML Tools, JSON Viewer, etc."
    );
    println!(
        "  Politics: Don Ho is famous for issuing politically-themed releases (Tiananmen, Stand with Ukraine,"
    );
    println!("           Stand with Hong Kong, Free Uyghur) — release names + about-box messaging");
    println!(
        "  Usage: still THE default editor for many Windows sysadmins / lightweight scripting tasks"
    );
    println!(
        "  Differentiator: FREE, FAST, Windows-native (no Electron), strong regex/multi-cursor, donations not subscription"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "notepadpp".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_npp, strip_ext};
