...
fn run_xc(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Options:");
        println!(
            "  --new                  New project (App / Framework / Package / Swift Playground)"
        );
        ...
        println!(
            "  --predictive-code      Predictive Code Completion (on-device ML, Apple Silicon)"
        );
    ...
    println!(
        "  Pricing: FREE — but Apple Developer Program required for App Store submission ($99/yr)"
    );
    println!(
        "  Languages supported: Swift (Apple's modern language), Objective-C, C, C++, Metal Shading Language"
    );
    println!(
        "  Targets: iOS 12+, iPadOS 13+, macOS 11+, watchOS 6+, tvOS 14+, visionOS (Apple Vision Pro)"
    );
    println!("  Components:");
    ...
    println!(
        "  Differentiator: only IDE that ships Apple frameworks officially (SDKs only via Xcode)"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_xc, strip_ext};
