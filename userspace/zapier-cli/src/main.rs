...
fn run_zapier(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Zapier (Slate OS) — workflow automation for SMB + prosumers (private, profitable)"
        );
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Zapier 2024 (Slate OS)");
        return 0;
    }
    println!("Zapier 2024 (Slate OS) — Automation for the Rest of Us");
    println!(
        "  Vendor: Zapier, Inc. (incorporated Delaware, HQ San Francisco — fully remote since 2011)"
    );
    ...
    println!(
        "          one of the iconic fully-remote companies in tech (no HQ, distributed workforce 800+)"
    );
    ...
    println!(
        "                    target: SMB + freelancer + marketer + ops person (NOT IT department)"
    );
    println!(
        "                    primary competitor: Make (was Integromat), n8n, Microsoft Power Automate (lower-tier)"
    );
    println!(
        "                    secondary (enterprise tier): Workato, Tray.io, Boomi (Zapier moves upmarket slowly)"
    );
    println!(
        "                    Zapier's wedge: 7,000+ app integrations (most in industry) + simplicity + free tier"
    );
    println!(
        "                    'the app store of automation' — most app integrations of any iPaaS"
    );
    ...
    println!(
        "    - Featured integration partner for many SaaS startups (Zapier integration = legitimacy)"
    );
    ...
    println!(
        "    - Mainstream SaaS: Salesforce, HubSpot, Gmail, Slack, Microsoft 365, Google Sheets, Notion"
    );
    println!(
        "    - Niche tools: every podcast platform, every form builder, every newsletter tool"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "zapier".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_zapier, strip_ext};
