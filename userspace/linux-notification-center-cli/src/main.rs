...
fn run_notification_center(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("notification-center v2.2 (Slate OS) — Notification center");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("notification-center v2.2 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "notification-center".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_notification_center, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(
            basename("/usr/bin/linux-notification-center"),
            "linux-notification-center"
        );
        assert_eq!(
            basename(r"C:\bin\linux-notification-center.exe"),
            "linux-notification-center.exe"
        );
    ...
    fn strip_ext_removes_extension() {
        assert_eq!(
            strip_ext("linux-notification-center.exe"),
            "linux-notification-center"
        );
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_notification_center(&["--help".to_string()], "linux-notification-center"),
            0
        );
        assert_eq!(
            run_notification_center(&["-h".to_string()], "linux-notification-center"),
            0
        );
