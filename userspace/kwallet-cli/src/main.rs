...
fn run_kwalletd(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("kwalletd6 v6.0 (Slate OS)");
        return 0;
    }
...
fn run_manager(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("kwalletmanager5 v23.08 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "kwalletd6".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kwalletd, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_kwalletd(&["--version".to_string()], "kwallet");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_kwalletd(&[], "kwallet");
    }
}
