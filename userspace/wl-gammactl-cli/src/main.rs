...
fn run_wl_gammactl(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("wl-gammactl v0.1 (Slate OS)");
        return 0;
    }
    ...
    if args.is_empty() {
        ...
        let gamma = args
            .iter()
            .skip_while(|a| a.as_str() != "-g")
            .nth(1)
            .map(|s| s.as_str())
            .unwrap_or("1.0");
        println!(
            "wl-gammactl: brightness={} contrast={} gamma={}",
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "wl-gammactl".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wl_gammactl, strip_ext};
