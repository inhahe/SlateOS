...
fn run_vega(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        match prog {
            "vl2vg" => {
                ...
                println!("vl2vg v5.0 (Slate OS) — Compile Vega-Lite to Vega");
            ...
            _ => {
                ...
                println!("{} v5.0 (Slate OS) — Render Vega spec to {}", prog, format);
                println!("  -o FILE    Output file");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("vega-cli v5.28.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "vg2svg".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_vega, strip_ext};
