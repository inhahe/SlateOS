...
fn run_cura(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Ultimaker Cura 5.6.0 (Slate OS)");
        println!("CuraEngine 5.6.0");
        return 0;
    }
    let file = args
        .iter()
        .find(|a| a.ends_with(".stl") || a.ends_with(".3mf") || a.ends_with(".obj"))
        .map(|s| s.as_str());
    if let Some(f) = file {
...
fn run_curaengine(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("CuraEngine 5.6.0 (Slate OS)");
        return 0;
    }
...
#[cfg(test)]
mod tests {
    use super::{basename, run_cura, strip_ext};
