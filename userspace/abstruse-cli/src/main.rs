...
fn run_abstruse(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Abstruse CI v2.8 (Slate OS) — Distributed CI/CD platform");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Abstruse CI v2.8.0 (Slate OS)"); return 0; }
    println!("Abstruse CI v2.8.0 (Slate OS)");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_abstruse};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/abstruse"), "abstruse");
        assert_eq!(basename(r"C:\bin\abstruse.exe"), "abstruse.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("abstruse.exe"), "abstruse");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_abstruse(&["--help".to_string()], "abstruse"), 0);
        assert_eq!(run_abstruse(&["-h".to_string()], "abstruse"), 0);
        let _ = run_abstruse(&["--version".to_string()], "abstruse");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_abstruse(&[], "abstruse");
    }
}
