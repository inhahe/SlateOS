...
//! tensorflow-cli — OurOS TensorFlow machine learning framework
//!
...
fn run_tensorflow(args: &[String]) -> i32 {
    ...
    match subcmd {
        "version" | "--version" => {
            println!("TensorFlow 2.15.0 (OurOS)");
...
fn run_saved_model_cli(args: &[String]) -> i32 {
    ...
    match subcmd {
        "--version" => println!("saved_model_cli 2.15.0 (TensorFlow, OurOS)"),
        "show" => {
...
fn run_tflite(args: &[String]) -> i32 {
    ...
    let subcmd = args.first().map(|s| s.as_str()).unwrap_or("version");
    match subcmd {
        "version" | "--version" => println!("TensorFlow Lite 2.15.0 (OurOS)"),
        "info" => {
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_tensorflow(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_tensorflow(&[]), 0);
    }
}
