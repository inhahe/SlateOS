...
fn run_ls(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Lightstep / ServiceNow Cloud Observability (Slate OS) — Distributed tracing pioneer"
        );
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("ServiceNow Cloud Observability (formerly Lightstep) 2024 (Slate OS)");
        return 0;
    }
    println!("ServiceNow Cloud Observability (formerly Lightstep) 2024 (Slate OS)");
    println!(
        "  Vendor: ServiceNow Inc. (NYSE:NOW) — acquired Lightstep May 2021 (~$300M, undisclosed)"
    );
    println!("          rebranded 'ServiceNow Cloud Observability' 2023");
    println!(
        "  Founders: Ben Sigelman + Daniel Spoonhower + Spencer Rugaber (San Francisco, 2015)"
    );
    println!(
        "           Ben Sigelman — co-creator of Google Dapper (the 2010 paper that founded distributed tracing)"
    );
    println!(
        "           also co-created OpenTracing (2016), OpenCensus, OpenTelemetry merger (2019)"
    );
    println!(
        "  History: built on Sigelman's Dapper experience — first commercial tracing at scale"
    );
    ...
    println!(
        "          decline in mindshare post-acquisition (sales focus shifted to ServiceNow customers)"
    );
    ...
    println!(
        "  Strategy: ServiceNow positions observability as part of broader 'AIOps + ITOM' suite"
    );
    println!("           goal: unified IT ops platform from monitoring → incident → CMDB → ticket");
    println!(
        "  Cultural impact (independent of present): Ben Sigelman shaped the entire OpenTelemetry standard"
    );
    println!(
        "                                            most modern APM SDKs trace lineage to OpenTracing/Lightstep"
    );
    println!(
        "  Differentiator: distributed-tracing-first design + true OTel native + tail-sampling at scale"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "lightstep".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ls, strip_ext};
