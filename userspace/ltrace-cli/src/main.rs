...
//! ltrace-cli — OurOS library call tracer
//!
...
fn run_ltrace(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("ltrace — library call tracer (OurOS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-V" || a == "--version") {
        println!("ltrace version 0.7.3 (OurOS)");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_ltrace(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_ltrace(&[]), 0);
    }
}
