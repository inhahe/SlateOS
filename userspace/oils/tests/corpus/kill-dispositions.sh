...
for s in CHLD URG WINCH CONT; do
  { sleep 0.4; echo alive; } & sleep 0.1; kill -"$s" %1; echo "$s rc=$?"; sleep 0.1; jobs; wait
done
...
for s in INT QUIT; do
  sleep 0.4 & sleep 0.1; kill -"$s" %1; echo "$s rc=$?"; sleep 0.1; jobs; wait
  { sleep 0.4; echo alive; } & sleep 0.1; kill -"$s" %1; echo "$s rc=$?"; sleep 0.1; jobs; wait
  ( sleep 0.4; echo alive ) & sleep 0.1; kill -"$s" %1; echo "$s rc=$?"; sleep 0.1; jobs; wait
  sleep 0.4 | cat & sleep 0.1; kill -"$s" %1; echo "$s rc=$?"; sleep 0.1; jobs; wait
done
