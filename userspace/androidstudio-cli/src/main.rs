...
fn run_as(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Android Studio Ladybug 2024.2.2 (Slate OS) — Google's IDE for Android development"
        );
        println!();
        println!("Options:");
        ...
        println!(
            "  --gemini               Gemini in Android Studio (AI assistant — free + paid tiers)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Android Studio Ladybug | 2024.2.2 Patch 1 (Slate OS)");
        return 0;
    }
    println!("Android Studio Ladybug | 2024.2.2 Patch 1 (Slate OS)");
    println!("  Vendor: Google LLC");
    println!(
        "  Origin: replaced Eclipse-based ADT (Android Developer Tools) plugin — May 2013 announce"
    );
    ...
    println!(
        "  Languages: Kotlin (preferred, Google-blessed 2019), Java (legacy), C++ (NDK), Dart (via plugin → Flutter)"
    );
    println!(
        "  UI frameworks: Jetpack Compose (modern declarative — like SwiftUI), XML Views (legacy)"
    );
    println!("  Compose Preview: live UI preview as you type (inspired by SwiftUI Previews)");
    println!(
        "  Emulator: AVD Manager — boots Android x86/ARM images in QEMU+KVM/HAXM, snapshot resume"
    );
    println!("           Real device debugging via USB or wireless ADB");
    println!(
        "  Gradle: Android Gradle Plugin (AGP) is the build system — slow but extremely flexible"
    );
    ...
    println!(
        "  Gemini in Android Studio: AI code suggestions, Studio Bot — free Code Completions, paid for chat"
    );
    println!(
        "  Modules: app, feature modules, dynamic feature delivery, App Bundles (.aab), Play Asset Delivery"
    );
    println!(
        "  Differentiator: only official IDE supported by Google for Android dev (+ first-class Wear/Auto/TV)"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "androidstudio".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_as, strip_ext};
