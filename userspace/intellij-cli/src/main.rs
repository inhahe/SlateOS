...
fn run_ij(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "    Ultimate Edition: $599/yr (individual), $169/yr after 3 years (loyalty discount)"
    );
    ...
    println!(
        "  Language: written in Java + Kotlin (JetBrains' own language — also they invented Kotlin!)"
    );
    ...
    println!(
        "  AI Assistant ($10/mo extra): code completion, refactoring suggestions, commit messages"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "intellij".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ij, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_ij(&["--version".to_string()], "intellij");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_ij(&[], "intellij");
    }
}
