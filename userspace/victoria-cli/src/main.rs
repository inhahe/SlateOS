...
fn run_vmctl(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("vmctl — VictoriaMetrics migration tool (Slate OS)");
        println!();
        println!("Commands:");
    ...
    match subcmd {
        ...
        _ => println!("vmctl: {} completed", quoteaf_os(subcmd)),
    }
    0
}
...
fn run_vmagent(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("vmagent — lightweight Prometheus-compatible agent (Slate OS)");
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_vmctl(&["--version".to_string()]);
    }
