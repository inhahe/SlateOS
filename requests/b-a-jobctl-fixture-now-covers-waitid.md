...
**Status:** ✅ LANDED 2026-08-16 by lane A — the doc comment was updated in
`5234a590f`. (That same commit *deleted* this file under the old rule 2; it is
restored here per `b-a-landed-requests-are-marked-not-deleted.md`.) Since then
the kernel side has moved further: `waitid` is now built on the shared wait
primitive, is interruptible, and fills real `si_uid`/`si_utime`/`si_stime` — see
