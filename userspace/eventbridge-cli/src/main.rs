...
//! eventbridge-cli — OurOS AWS EventBridge (serverless event bus, formerly CloudWatch Events, 2019)
...
fn run_eb(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("AWS EventBridge (OurOS) — serverless event bus (formerly CloudWatch Events, 2019)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("AWS EventBridge 2024 (OurOS) — eventbridge CLI (aws-cli v2)"); return 0; }
    println!("AWS EventBridge 2024 (OurOS) — Serverless Event Bus + Routing + Schedule + Pipes");
