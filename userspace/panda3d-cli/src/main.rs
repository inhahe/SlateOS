...
fn run_panda3d(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Panda3D v1.10 (Slate OS) — Open-source 3D engine (Python/C++)");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Panda3D v1.10.14 (Slate OS)");
        return 0;
    }
    println!("Panda3D v1.10.14 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "panda3d".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_panda3d, strip_ext};
