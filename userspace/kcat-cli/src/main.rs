...
//! kcat-cli — OurOS kcat (kafkacat) Kafka tool
//!
...
fn run_kcat(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("kcat v1.7.0 (OurOS) — Kafka cat (producer/consumer/metadata)");
        println!();
        println!("Modes:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_kcat(&["--version".to_string()], "kcat"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_kcat(&[], "kcat"), 0);
    }
}
