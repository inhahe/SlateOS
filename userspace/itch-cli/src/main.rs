...
fn run_itch(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  --bundles              Owned bundles (Bundle for Racial Justice and Equality, etc.)"
        );
    ...
    println!(
        "    - Bundle for Racial Justice and Equality (2020): $8.1M raised, 1741 games included"
    );
    println!("    - Annual GMTK Jam (Mark Brown's), Ludum Dare archive");
    println!(
        "    - Hosts Celeste / Hotline Miami / Hyper Light Drifter / Undertale (initial release)"
    );
    ...
    println!(
        "  Differentiator: most creator-friendly digital store on the internet, no gatekeeping"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_itch, strip_ext};
