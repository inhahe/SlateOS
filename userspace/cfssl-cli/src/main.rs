...
//! cfssl-cli — OurOS CloudFlare's PKI toolkit
//!
...
fn run_cfssl(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "cfssljson" => {
                println!("cfssljson (OurOS) — Extract certs/keys from cfssl JSON output");
                println!("  -bare              Output bare files (no prefix)");
                println!("  -f FILE            Input JSON file (default: stdin)");
            }
            "mkbundle" => {
                println!("mkbundle (OurOS) — Build certificate pool bundle");
                println!("  -f FILE            Bundle file output");
                println!("  -nw N              Number of workers");
            }
            _ => {
                println!("cfssl (OurOS) — CloudFlare PKI/TLS toolkit");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_cfssl(&["--version".to_string()], "cfssl"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_cfssl(&[], "cfssl"), 0);
    }
}
