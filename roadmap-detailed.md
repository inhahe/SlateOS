...
- [ ] **The "pick an app" dialog names who asked.** When a file with no association for its extension is launched, the chooser dialog does not just ask *what to open it with* — it first says **who wants it opened**, so the user can tell a deliberate double-click apart from a background process trying to get something executed. The dialog shows, for the requesting side:
  - [ ] **Process** — PID, process name, and start time, so the identity is checkable against process explorer (§4.3) while the dialog is still up.
  - [ ] **Application** — the owning *installed application* the process belongs to (package/manifest name and its display name + icon), not just the executable, since a helper binary buried in a package tells the user nothing. Falls back to the executable path when the process belongs to no installed package.
  - [ ] **User** — the account the requesting process runs as, plus its sandbox/namespace label if it is confined. A launch request arriving from a different user than the one at the keyboard is called out visibly, not merely listed.
  - [ ] **Full command** — the requesting process's complete command line as invoked (argv joined with the same quoting rules the shell would need to re-run it), plus the exact launch request itself: the file path being opened and any arguments the requester attached. Long command lines are elided with the full text available on expand and copyable to the clipboard.
  - [ ] The whole "who asked" block is derived from the launch IPC's sender identity — the kernel-attested peer of the channel the request arrived on, not a self-reported field in the message — so a requester cannot lie about any of the four. Rendering is defensive: control characters and bidi overrides in a command line or path are escaped, never interpreted, so a crafted argv cannot forge dialog chrome.
  - [ ] The same block appears in the association dialog reached from "Open With → Choose another app" on a file that *does* have an association, for consistency, and every shown-to-user launch request is logged (JSON-lines) with the same four fields whether or not the user goes on to pick an app.
...
- [ ] **The per-drive queue applies here too** (section 4.1, "Serialise bulk copy/move per drive"): a command-line copy whose drive is already busy queues behind the running operation rather than competing with it, and says so rather than appearing to hang. "Same mechanism as file explorer drag-drop" is what makes this automatic — the scheduling belongs to the shared engine, not to either front end — but the CLI needs its own answer to two questions the GUI answers with a window: what it *prints* while queued (what it is waiting for, and that Ctrl-C cancels a queued operation without touching the disk), and whether a non-interactive invocation should wait or fail fast, which a `--no-queue` / `--wait` pair settles explicitly rather than by guessing at the caller.
...
- [-] *(Lane F 2026-09-25: recovery now drops every surface whose owning connection is gone — `Server::recover`, design-decisions §1304; asking each live client which surfaces it owns is not built.)* Surface tree rebuild from live-client enumeration. Rather than incrementally adding/removing surfaces based on client requests, the compositor must support rebuilding its surface list from scratch by re-enumerating all live clients and asking each "what surfaces do you currently own?" Any surface in the old list not claimed by a live client in the new enumeration is dropped. Runs on the full-redraw trigger; can also run periodically as a sanity sweep.
- [x] *(Lane F 2026-09-25: `Present::reset` — the DRM presenter programs every live head's mode again and forgets its buffers' contents; no overlay or cursor plane is in use yet, the pointer being drawn in software. §1304.)* Hardware overlay / DRM plane reset. The full-redraw path must also flush all hardware planes (DRM overlay planes, cursor planes, direct-scanout buffers) back to compositor-managed state. Some classes of artifact live in hardware planes that bypass the normal compositor buffer; recompositing the main plane doesn't touch them.
- [x] *(Lane F 2026-09-25: Ctrl+Super+R, compositor-owned, and the shell request `RecoverDisplay`; `Server::recover`, design-decisions §1304. The surface rebuild it triggers is the connection-level sweep noted two items up.)* Full-screen redraw / artifact-recovery path. User-triggered operation (shortcut: e.g. Ctrl+Super+R) and IPC method that:
...
- [-] Widget support — *verified 2026-09-25: the desktop's right-click menu adds a clock, a calendar or a system monitor; widgets move and are removed, and the layout is saved and restored (`widgets.yaml`). `WidgetKind` defines five more -- weather, notes, RSS, music, photo frame -- that nothing offered; notes are offered since 2026-09-25 ("Add widget > Note"): written in where they sit, on the toolkit's multi-line field, and kept with the layout at every change.*
...
##### Tier 1 — Icon Theme
- [-] App icons, file type icons, system tray icons, folder icons — *2026-09-26 (`design-decisions.md` §880): `appearance::icons` finds an icon by its freedesktop name in the chosen theme, with a built-in set compiled in (folders and the start menu's places, document, picture, audio, home, settings, terminal, power, window, computer, bin, search, hard disk, link). Drawn by the start menu's places column and power button, and -- since later the same day -- by the desktop's icons, each type as its own picture in its own hue (they were emoji, which a face without emoji draws as boxes). Then (§881) every picture the shell drew as a character became an icon, sixty-four in the built-in set: the taskbar's start button, bell and tray chevron, every on-screen overlay's, the login screen's, and the widgets'. What is left: the icons *programs* put in the tray are still characters they send (a wire change, lane F's), and apps have no icon of their own until a program can say which it is (`open-questions.md` on the application registry).*
- [x] SVG-based with color token substitution (monochrome icon sets automatically match theme accent color) — *2026-09-26: an icon drawn in `currentColor` takes the colour it is drawn in; one with colours of its own keeps them. The toolkit's SVG renderer learned to inherit the root element's presentation attributes, which is how most icon sets are written.*
...
- [x] Fit options: fit with letterbox, fill with crop — 2026-09-14. All six (`Fill`, `Fit`, `Stretch`, `Tile`, `Center`, `Span`) are settable: `ImageFit` moved into `appearance` so it is one model, `AppearanceSettings::wallpaper_fit` carries it, and the Settings app's Wallpaper page offers it. See design-decisions 852 and its follow-through. *"User can scroll to center" is not done and is a separate thing — there is no pan offset, only the six placements. **Corrected 2026-09-17: settable was as far as it went.** The renderer passed the *screen's* dimensions where `compute_image_rect` expects the *picture's*, and under that assumption all six modes return the same full-screen rectangle — `Fill` and `Fit` scale by a ratio of exactly 1, `Center` and `Tile` hand back the rectangle they were given — so the setting drew the identical picture in all six positions from the day it shipped. The function itself was right, and its six unit tests passed throughout, because they call it directly with sizes of their own. `WallpaperManager::note_image_size` now carries the real size from the one place that decodes a picture (`session.rs`), bound to the image id so a slideshow that advances mid-decode cannot impose the outgoing picture's proportions on the incoming one. design-decisions §856 once more: it was stored, and nothing obeyed it.*
- [-] Random background on boot or daily rotation (with exclusion filters) — *Reader landed 2026-09-17. `appearance.yaml` now carries `wallpaper.folder`, `wallpaper.interval_secs` and `wallpaper.shuffle`; the shell honours them, scanning the folder only when it changes rather than on every settings read. **The bigger find was that nothing turned the clock**: `WallpaperManager::tick` had no caller outside its own tests, so a slideshow could never have advanced and the time-of-day gradient could never have changed, whatever was configured. `step_frame` ticks it now. The folder is percent-encoded under the same §426 marker as the picture, since it is a path too. **Control landed the same day**, after the reader: the Wallpaper page has a Rotation section — choose a folder, how often it changes, whether it shuffles, and a way to stop. The interval is a fixed list rather than a number field, the same judgement the lock delay and thumbnail sizes make. The detail rows appear only once a folder is chosen, since three controls of which two govern nothing read as broken rather than as inapplicable. **Exclusion filters landed 2026-09-17**: `wallpaper.exclude` in `appearance.yaml` is a list of glob patterns matched against each picture's **file name**, since a rotation reads one folder and a pattern that could cross directories would have nothing to cross. That is why it uses the flat matcher in `gui/globmatch` and not the path-aware one in `apps/backup` — see `known-issues.md` `TD-C-THE-TWO-GLOB-MATCHERS-ARE-NOT-DUPLICATES`. A name with no text form matches nothing and stays in the rotation, since excluding a picture because its name could not be read would be the wrong way round. **Control landed 2026-09-17**, after the reader as the rest of this feature did: the Rotation section lists each saved pattern with a Remove button, and a text field adds one. The field is the page's first, so it arrived as a reusable `text_field_row` rather than a one-off — focus is an underline and a caret rather than a change of fill, since a field that changes colour when focused makes the *text* look like it changed. A draft is held apart from the saved list so that typing is not a change to the settings: writing each keystroke through would leave `holiday`, `holiday-` and `holiday-2` behind in the file, none of them a pattern the user meant. Patterns are trimmed, empties refused, duplicates ignored. **And nothing woke the desktop for it, found 2026-09-26**: `step_frame` ticks the wallpaper only on frames something else asked for, so on a desktop nobody touched the rotation never turned -- and a picture it did turn to was never drawn, since the background was painted only at start (`known-issues.md` `TD-C-THE-DESKTOP-BACKGROUND-WAS-PAINTED-ONCE-AND-NEVER-AGAIN`). The shell now sleeps until the next picture is due (`WallpaperManager::next_change_in`; a dynamic wallpaper once a minute), a rotation chosen while the desktop is up arms that at once, and a folder of one picture no longer re-reads it every interval.*
- [x] Login screen background (easy way to match desktop background) — 2026-09-17. `appearance.yaml` gains `login.background`: `theme`, `color`, `desktop`, `image` or `gradient`. **`desktop` is the easy way `design.txt`:1247 asks for, and it stores no filename** — it asks the wallpaper what is up at the moment it paints, so it keeps step with a rotation folder for free; a stored path would have frozen the greeter on whichever picture was up when the box was ticked. See design-decisions §859. The greeter had had `SameAsDesktop` and `CustomImage` variants since it was written and drew neither: nothing ever read a file for them, so both landed on the theme colour. The session now uploads to the greeter's own surface (images belong to the window that uploaded them, so this is a second upload of one picture, not a shared id), via an `upload_picture` helper shared with the wallpaper rather than a second copy of the decode. A picture that cannot be read costs the picture and not the machine — the greeter is the one screen where failing closed locks the user out — and the reason is kept in `Session::login_background_error` rather than swallowed. **Control landed the same day**, after the reader as before: the Wallpaper page has a Login screen section — a Background dropdown and, for a picture, a chooser. The dropdown offers **four** styles and not five (three on the day it landed; a solid colour joined it hours later, once the accent picker gave the app somewhere to pick one): the missing one is the gradient, which needs *two* colours, and `ColorPickerDialog` has no title — so asking for them in sequence would put up two identical dialogs with nothing to say which was the top and which the bottom (§856 refusing a control that cannot finish the job). Adding a title was considered and declined for now: `DialogLayout`'s own documentation records a bug caused by six derivations of that layout drifting apart, and a gradient is not something `design.txt` asks of the login screen. One that is already in force is listed as a fourth entry rather than misreported as "Theme colour", so the first click does not silently discard hand-written colours. Choosing "A picture" opens the picker rather than setting the mode, because an `image` mode with no `login.image` reads back as the theme — a setting that would silently undo itself if the picker were cancelled. The three picture dialogs now share one opener, since the filter list is the part worth not copying: bare extensions instead of globs once made every directory list as empty.

#### Hotkeys
- [x] Set hotkey: capture from keyboard, show existing binding, select function or arbitrary command — **the other half landed 2026-09-24** (`gui/desktop/src/shortcut_editor.rs`): on the card, **F2** chooses a different action for a row's keys and **Insert** adds a shortcut, both through a searchable list of every action that shows the keys each one already has (`design.txt`:1318's "can search for functions, shows you any hotkeys already applied to that function"); **"Run a program…"** at the top of that list binds an arbitrary program, pre-filled with the current one when changing such a shortcut (one program path and no arguments: a shortcut stores a program, not a command line. This said until 2026-09-25 that the Run box took the same, "splitting a line into words needs a quoting rule nobody has chosen"; the Run box has one now, §870, should a shortcut ever want arguments); and keys already in use are no longer refused — the card names what holds them and **Enter moves them**, Escape keeps them ("shows you the function that's already taken … and lets you modify … its hotkey"). The card's bottom line lists its keys when there is nothing to report, so the editor is discoverable. The editor is a state machine of its own, addressed by chord rather than row, so a rebind that re-sorts the list keeps the keyboard on the binding just edited. *What was true before, for the record: capture and show were done, choosing was not. The shell's shortcut card (`ToggleShortcutCard`) lists every binding, and Enter on a row records a new chord: modifier-only presses are ignored while recording, Escape cancels, a collision is refused with the name of what already holds the chord, and the save reports its own failure rather than claiming success. What is missing is the other half — there is no way to point a chord at a *different* action, or at an arbitrary command, so `HotkeyAction::LaunchApp(String)` is reachable only by hand-editing the file. `design.txt`:1318 also asks to search the function list and to show which chords an action already has.*
- [x] Modify/delete hotkeys from list — modify was already there (the shortcut card: arrows select, Enter records a new chord, conflicts are refused with a reason, and the save reports its own failures). **Delete landed 2026-09-17**: Delete on a row unbinds the action. The work was not the unbinding but making it *stay* — `load_shortcuts` merges onto the shipped defaults so that a shortcut added in a later version still reaches a user who has customised theirs, which means a deletion had to be written down rather than merely left out, as `none=<action>`. See design-decisions §860. Choosing what a chord does (F2) and adding one (Insert) landed 2026-09-24 — see the bullet above. Delete now also says which keys the action still has.
- [~] Minimal defaults (Alt+F4, Alt+Tab, Ctrl+C/V/X, Ctrl+Z, Print Screen) — *blocked on **C-Q24**. Alt+F4, Alt+Tab (with Alt+Shift+Tab) and Print Screen are bound. **Ctrl+C/V/X and Ctrl+Z are deliberately not**, and should not be: they belong to whichever application has focus, and a global grab would break every one of them — the same reasoning `register_defaults` already gives for leaving Ctrl+R to applications that reload. The open question is the opposite of this bullet: `design.txt`:1320 asks for very few defaults "or possibly no" and the shell binds 31, with no recorded decision either way.*
- [-] Available functions: monitor off, minimize all, change desktops, logoff, etc. — *31 actions exist. Of the four this bullet names: **minimise all** is `ShowDesktop` and works; **change desktops** is `PreviousDesktop`/`NextDesktop`/`SwitchDesktop(u8)` and works. **Monitor off is not built and deliberately so** — there is no display-power verb anywhere in `gui/compositor`, so the action could only be accepted and dropped, which is exactly what `TD-C-BRIGHTNESS-KEYS-ARE-NOT-KEYS` already records for the brightness keys. **Logoff** ends the process (`take_login_power`, drained in `main.rs`) but starts no new session, because the shell has no channel to the process server — `TD-SHELL-HAS-NOWHERE-TO-SEND-A-LAUNCH`. Both wait on a verb that does not exist yet rather than on the hotkey table.*
...
#### Input Fields
- [x] Single-line and multiline — *single-line is done and shared: `guitk::textinput::TextInput` (state) over `guitk::textedit` (the editing primitives and the `SingleLine` renderer, which draws caret, selection and the horizontal scroll that keeps a long string's caret visible). Multiline since 2026-09-25: `guitk::textarea::TextArea` (caret and selection over wrapped lines, Up/Down holding the column, Page Up/Down, mouse click, drag and double click, a scrolled view, and undo/redo by the word) and `textarea::draw`; first used by the desktop's notes. `WidgetKind::TextArea` still holds only a value and a placeholder, and twenty-two files still hand-roll typing against `KeyEvent::types_text`; `apps/settings` was the first converted, on 2026-09-17, and `apps/notes`, `apps/stickynotes` and `apps/email` are asked to move onto the multi-line one (`requests/c-e-a-multi-line-text-field-for-the-apps-that-edit-text.md`).*
- [x] Word wrap option — *2026-09-25: `TextArea::set_wrap`. On by default, at the box's width; off, lines are paragraphs and the view follows the caret sideways (`design-decisions.md` §546's rule, as the single-line field has it).*
...
- [x] Path bar with autocomplete (absolute or relative paths) — `guitk::pathbar` is the widget, `ExplorerState::completions_for` supplies candidates from the filesystem (relative via `.`, absolute via the leading `/`), sorted so the list cannot reorder between keystrokes. Verified against the code 2026-09-16.
...
- [-] **Optional metadata labels in thumbnail / icon view.** Even in thumbnail (icon/grid) view — not just details view — the user can choose to show per-item **filename**, **date** (modified/created/taken, user-selectable which), and/or **size** beneath or beside each thumbnail. Each of the three is an independent toggle (any combination, including all off for a pure-image wall or all on for a labeled contact-sheet look), exposed in the view menu and persisted with the same per-folder / global-default preference mechanism as the detail columns. The point is that switching to thumbnail mode should not force the user to give up seeing basic file metadata: they get the visual preview *and* the name/date/size at a glance if they want it. Values reuse the same typed metadata source as the detail columns (so the date/size shown match, and sorting within thumbnail view is consistent). Long filenames wrap/ellipsize to a configurable line count so the grid stays even. *Built 2026-09-16: three independent toggles on the folder menu (name, date modified, size), ticked where drawn, saved and applied at start-up; all three off is the pure-image wall. Values come from `entry_value`, the same function the detail cells use, so the two views cannot disagree about a file. The cell grows one line per label and its height is decided by the line count rather than any name, so the grid stays even; the default is the name alone at exactly the height the view had before. **Three parts of this bullet are not built**: the date is "modified" only and not user-selectable between modified/created/taken; the choice is saved globally and not per-folder as the detail columns are; and the line count is one, ellipsized, rather than configurable.*
- [-] **Thumbnail cache — stored in a dedicated OS directory, never alongside the source files.** *Verified 2026-09-16: the cache exists and is wired in production (`ThumbnailGenerator::with_default_disk_cache`), it lives under the per-user path and never beside sources (`DiskCache::default_location`), and invalidation works — `cache_filename` keys on path, mtime and the size cap, so any change misses and regenerates. The size cap landed the same day: `enforce_cap` bounds the directory at a
default 256 MiB, oldest-written evicted first, with the budget as a parameter.
What remains of this bullet is §4.1's install-time sizing (needs a
filesystem-capacity reading that does not exist) and pressure-aware shrinking
(lane A's kernel shrinker).* Generated thumbnails are persisted in a centralized cache so re-listing a directory is fast and so the same thumbnail can be served to other consumers (file picker, image viewer, taskbar previews, etc.) without re-running the generator. The cache lives in a dedicated OS-managed directory (under the per-user state path, e.g. `~/.cache/os/thumbnails/` — exact path TBD; never in the user's source directories). Rationale:
...
- [-] View options: list, thumbnails (any size), column view, order by any column *Verified 2026-09-16: `ViewMode` has List, Icons and Details, and columns sort via `ColumnManager::sort_by`, so three of the four hold. Size became choosable the same day: a submenu on the folder menu offers 64, 96, 128 and 192 pixels, ticked at the one in force, saved and applied at the next start-up. **Four sizes, not an arbitrary number** -- `ThumbConfig::size` is a `u32` and `DiskCache` keys entries on it, so arbitrary sizes need only a different control, not different machinery. Whether "any size" means "the user picks" or "any integer" is the one word left between this and `[x]`.*
- [-] **Manual (custom) file ordering with persistence.** *Started 2026-09-16: drag a row past a 4-pixel threshold and the folder enters Custom order, saved against the folder in the `explorer` settings document (`apps/explorer/src/manualorder.rs`). The precedence rule holds and is pinned by a test: a column sort overrides the arrangement and switching back restores it, because the arrangement lives on disk against the folder while the sort mode is view state — sorting by Name writes nothing. Folders are **not** forced to the top in Custom, since the spec has the user arranging files and folders together and a folder that sprang back to the top would fight that. Newcomers land at the end, which falls out of sorting by name first and letting the arrangement tie. An insertion line marks where the drop will land — the top edge of the row it would go before, the bottom edge of the last row when it would go at the end, and nothing at all before the drag threshold is crossed, so a plain click never makes the folder look about to rearrange itself. The line is drawn from the rectangles the last *frame* registered, the same source the hit-testing uses, because recomputing them would be a second copy of the layout arithmetic and the first disagreement would put the line between two rows the user is not looking at. Dragging a row that is part of the selection moves the whole selection. **Still to do:** the shared file-explorer component, so the open/save dialogs inherit it. A file whose name is not text keeps its place for the session but cannot be written down — C-Q24.* Original note: *genuinely unstarted — no manual sort mode, no stored order. Note for whoever starts it: the per-folder preference mechanism it needs now exists as `apps/explorer/src/columnprefs.rs`, which already saves a column set against a folder path and refuses a path it cannot write down.* The user can drag files/folders into an arbitrary order within a directory and that ordering is remembered — it persists across navigations, refreshes, and reboots (stored per-directory in user-side state, keyed by directory identity, not baked into the filesystem). This is a distinct sort mode ("Custom" / "Manual"): while it is active, drag-to-reorder is enabled and new/renamed items land in a defined place (e.g. appended, then draggable). **Precedence rule:** enabling any "order by *column*" (name, size, date, type, etc.) **temporarily** overrides the manual order — the view shows the column sort while that sort is active — but it does **not** discard the saved manual order; switching back to Custom restores the user's hand-arranged sequence intact. So column sorts are a non-destructive temporary view on top of the persisted manual arrangement. Applies to the shared file-explorer component, so the file open/save dialogs inherit it.
- [-] Optional preview panel: shows currently selected image/video, movable and resizable (uses dockable panel widget) *Started 2026-09-17. Toggled from the folder menu, resizable by dragging the divider, and both the on/off state and the split are remembered. Shows the selected file's thumbnail rather than decoding the original: a preview pane is a few hundred pixels wide, the thumbnail is already decoded and uploaded, and re-reading a large photo on every arrow-key press would stall the window — the thumbnail-size setting is the lever if it looks soft. A file with no thumbnail says so rather than showing a blank panel, because "no picture" and "not a picture" are different answers. A pane too narrow for both at their minimums shows the listing alone and **keeps the preference**, so shrinking the window does not silently forget it. **Movable done 2026-09-17:** the panel sits on any of the four sides, chosen from the same menu, and the choice is remembered. `preview_split` is stored as the *listing's* share whichever side the panel is on, so moving it mirrors the layout instead of resizing it. Top and bottom get their own minimums, smaller than the side ones, because a few rows is still a usable listing where a few pixels of width is not. **Still to do:** video preview needs a decoder this does not have.*
- [-] Search feature with checkboxes for what to search: *Started 2026-09-16. Ctrl+F opens a Find prompt; results replace the listing and Escape restores the folder it was started from. `apps/explorer/src/search.rs`. **There are no checkboxes yet, deliberately** — of the five sources listed below only the first has a backing, and design-decisions 856 rules out offering a control nothing obeys. A "file contents" box with no indexer behind it would make search quietly worse, not better: a user who ticks it gets fewer results than if they had left it alone. The boxes arrive with their backing, one at a time.*
  - [x] Filename/path (always available, no indexer needed) *Done 2026-09-16. Matches **bytes**, not a decoded name, so `report<FF>.txt` is still found by "report" and by ".txt" — `apps/filesearch` skips such a name (honestly, and it counts them) but skipped is still a file you cannot find. ASCII-case-insensitive; non-ASCII compares exactly, because case folding outside ASCII is language-dependent and a find-as-you-type box is not where to choose a locale. Breadth-first so a truncated walk returns the near matches; capped at 2000 results and 16 levels, and it says when a cap stopped it. Unreadable folders are counted and reported, because "not found" and "not looked at" are different answers and only one means the file is absent. **Path as well as name, added the same day after re-reading this bullet:** a query containing `/` matches the path below the search root (`sub/notes` finds `sub/notes.txt`), one without it stays a name query — matching the whole path unconditionally would make `a` return every file under any ancestor containing an `a`. The relative path is built during the walk with `/` between components rather than derived with `strip_prefix`, because `Path` joins with a backslash on the Windows test host: a derived path would make path queries work on SlateOS and fail under test, which is a green suite about the wrong machine. A backslash is **not** a separator here, on either host, because `design.txt` permits it in a filename.*
...
- [x] Drop zones for drag-and-drop *Verified 2026-09-16: `apps/explorer/src/dropzone.rs` — `DropZoneManager` is rebuilt each frame and registers a zone per row and per sidebar item; all three views call it.*
- [x] Atomic copy/move/delete with undo, resume on interruption *Verified 2026-09-16: copies go through a scratch name and are renamed into place (`atomic_copy_file`), `UndoStack` is reachable on Ctrl+Z and tested, and `OperationJournal` is opened by the executor so an interrupted plan can be continued. See the bullet below for the one part of "resume" that is not wired.*
- [-] **Durable, resumable bulk file operations (survive crash / restart / accidental abort).** *Verified 2026-09-16, then corrected on reading the file format. The journal is real and is opened by `OperationExecutor`, so a plan **that is running** avoids re-doing completed work. It cannot support crash recovery as it stands, and the reason is not the missing start-up scan I first assumed: the file holds a plan **id** and the indices that finished, and nothing else. Not the sources, not the destinations, not the operation. So after a crash the record on disk can say "actions 0..7 of plan 4391 are done" to a program that no longer knows what plan 4391 was. Crash recovery needs the plan persisted too, which is a format change and not a scan. Journals also live in the destination directory (`dir/.fileop-journal`), so whatever reads them will find them by visiting that directory rather than by scanning at start-up.* A long-running mass copy/move/delete must be journaled to persistent OS state as it runs — a per-operation record of the source set, destination, per-file progress (which items are done, in-flight, pending), and the chosen conflict resolutions — so that if it is interrupted for *any* reason (app crash, system restart, power loss, the user accidentally aborting or closing the window), the OS knows the operation was still in progress and can resume it rather than leaving a half-finished mess. On next login (or when the filesystem/service layer comes back up), the user is **reminded**, when appropriate, that "N file operations were interrupted" and offered resume/discard. There is also a **central place** — a "File Operations" / "Transfers" view (surfaced in file explorer and/or the notification pane) — where the user can see all interrupted *and* in-progress bulk operations and resume, retry, or cancel them individually. Design ties: reuse the same operation-journal that powers "resume on interruption" above and the `files.transfer_progress` event (§4.10 service API); deletes to the recycle bin are trivially resumable, permanent deletes and moves journal each item's committed/uncommitted state so resume never double-copies or loses a file. Idempotent replay: each journaled step records enough to re-check the target state and skip already-completed items.
- [-] **Serialise bulk copy/move per drive: queue an operation whose drive is already busy rather than running both at once.** When the user starts a directory copy or move, check every drive it will touch — the source's and the destination's — against the drives every *in-flight* operation is touching. If any drive is shared, the new operation does not start: it goes into a queue and begins when the operation ahead of it finishes.  **[C] 2026-09-14: the rule and the view are both built.** Admission is keyed on a resolved `DriveSet`, per-drive queues are independent, and the engine was made steppable and chunked to get there. The Transfers view lists what is running and what is waiting, a waiting row names the operation it is waiting *for*, and the four controls this description asks for are there: reorder up and down, start now, and cancel — cancelling a waiting one before it has written anything, which is free. Start-now overrides the drive check deliberately, because this description calls the rule a default and not a prohibition. **One clause of this entry is still not built:** *“an operation waiting in the queue is journalled as `Queued` and survives a restart in that state, so an interrupted queue resumes as a queue rather than starting everything at once on next login”*. The queue is in memory only; closing the explorer forgets what was waiting. That belongs with “Durable, resumable bulk file operations” above, which owns the journal the state would be written to, and is why this stays `[-]`. The two *unmeasured* things are recorded in `known-issues.md`: whether interleaving two operations on one thread keeps two devices busy, and whether the same-drive case really is slower interleaved than in turn.

  **Why, and the second reason matters more than the first.**

  1. *It is faster.* Two operations on one drive do not get half the throughput each; they get less than that. They interleave two access patterns into a single device queue, so on a spinning disk the head seeks between the two working sets instead of streaming, and on any device — SSD, USB stick, network share — neither stream gets a contiguous run, the readahead for each is repeatedly invalidated by the other, and the writeback cache is split between two destinations. Doing the same total work one operation after another finishes sooner *and* gives the user a meaningful progress bar, because the first operation's estimate is not being perturbed by a second one competing with it.
  2. *Fewer operations in flight means fewer that can be left half-done.* Every concurrently-running operation is one more thing that a crash, a power loss, a forced unmount, a yanked USB stick or an accidental "cancel all" leaves in a partially-applied state that has to be journalled, detected on restart and resumed or unwound. Serialising per drive does not make any single operation safer, but it reduces how many are simultaneously exposed — and it means a user who pulls a drive mid-operation loses at most one operation's worth of half-copied state on it, not several interleaved ones.

  **What "the same drive" means.** The comparison is on the *backing device or volume*, resolved from the path — not on the mount path, and not on the path prefix. Two directories under different mount points can be on one physical disk, and one directory tree can span devices via a bind mount or a mounted subvolume. A copy from `D:` to `E:` conflicts with any in-flight operation touching `D:` **or** `E:`, because it loads both. A copy whose source and destination are the same drive loads that drive twice over and is the case this rule most wants to protect. A network or removable volume counts as a drive like any other. Devices for which the ordering does not help — a RAM disk, say — may be exempted, but the exemption is a per-device-class property with a stated reason, never a guess from the path.

  **What the user sees.** A queued operation appears immediately in the "File Operations" / Transfers view alongside the running ones, in state *Queued*, saying what it is waiting for ("waiting for: Copy 12 GB to D:\Backup"). Nothing about it is hidden: the user can reorder the queue, cancel a queued operation before it ever starts (which is free — nothing has been written), or override and start it now, because the rule is a default and not a prohibition, and the user may know something the scheduler does not. Starting an operation must never *silently* do nothing; a copy that is queued rather than started has to say so, or the user will start it again.

  **How it fits what is already specified.** This is a scheduling policy over the existing bulk-operation engine, not a new engine: the operation is created, journalled and made visible exactly as it is today (see "Durable, resumable bulk file operations" above and `files.transfer_progress` in the §4.10 service API) — only its *start* is deferred. Consequences that follow and must be handled: an operation waiting in the queue is journalled as `Queued` and survives a restart in that state, so an interrupted queue resumes as a queue rather than starting everything at once on next login; conflict resolutions the user chose up front are held with the queued operation rather than being asked again when it finally starts; and a queued operation whose source or destination has disappeared by the time its turn comes fails cleanly at start rather than at enqueue, which is the only moment at which the check can be honest. The per-drive queues are independent: an operation on `F:` starts immediately while two are queued on `D:`, which is the whole point of keying the rule on the drive rather than on a single global lock.
...
- [-] Text editor app: status bar (line/col, encoding, language, indentation mode) *2026-09-16: line/col, language and indentation mode are drawn — indentation was added on this date, the document already knew `use_spaces` and `tab_width` and nothing showed them. The bar also carries the line ending and a line count, which this bullet does not ask for. **Encoding is the one missing**, and it is not a drawing job: a `Document` has no encoding field and nothing detects one, which is the sibling bullet about open/save with encoding detection.*
...
- [ ] **Port LibreOffice (office suite — Writer/Calc/Impress/Draw; ~10M lines C++, MPL-2.0).** The productivity suite is the last big "can I actually use this as my daily driver?" gap once the browser and IDE are up; MPL-2.0 permits shipping and patching it freely. Scope notes for whoever tackles it:
  - [ ] **The port is a VCL backend, not a fork.** LibreOffice already abstracts every platform behind **VCL** (its Visual Class Library) with swappable backends (`gtk3`, `qt5`/`qt6`, `win`, `osx`). The bulk of the work is writing a **new VCL backend targeting our GUI toolkit/compositor** (§gui-toolkit) — window creation, event loop integration, menus, dialogs, clipboard/drag-drop (§gui-toolkit clipboard), printing (§CUPS port), and a2d/GPU drawing surface — rather than touching the 10M lines of application logic. Fall back to the **Qt backend over a ported Qt** if our own toolkit backend proves slower to land; decide once Qt's port status is known.
  - [ ] **Dependency prerequisites** (each is its own port, most already implied elsewhere): a C++ toolchain + POSIX layer (§4.8), **freetype + harfbuzz + fontconfig** for text shaping/layout (shared with the compositor's font stack), **ICU** (collation, i18n, bidi), **cairo**, **libxml2**, **boost**, **openssl** (document signing/encryption), and **zlib** — LibreOffice's `configure` can consume system copies of all of these, so prefer packaged system libs over its bundled `external/` tarballs to keep the content-addressed store deduplicated (§pkg).
  - [ ] **Build system**: `gbuild` (a large GNU-make system) driven by an autotools `configure`. Needs `make`, `perl`, and a working `pkg-config` (§4.8). Expect the build itself — not the code — to be the main porting friction; budget for cross-compiling it against our sysroot.
  - [ ] **Optional subsystems to disable first, enable later**: **Java/JVM** (only needed for Base's HSQLDB engine, some wizards, and a few filters — build with `--without-java` initially), and the **Python UNO scripting bridge** (re-point at our **fastpy**/CPython once §4.8's CPython lands so macros and the UNO automation API work).
  - [ ] **UNO component model** is LibreOffice's own component/IPC architecture — it is self-contained and should need no changes, but its bootstrap path resolves component registries from disk; verify it works under the content-addressed store's immutable, generation-based paths (§pkg) rather than assuming a mutable install prefix.
  - [ ] **Interim option**: run the stock Linux build under the **Linux compatibility layer** (same fallback noted below for Firefox) to get a working suite early, and treat the native VCL backend as the quality/perf follow-up.
...
- [-] gcc, cmake, make, pkg-config (via POSIX layer) — pkg-config and make are each proven at *link* level against `toolchain/sysroot/lib/libc.a` (`scripts/pkgconf-spike/`, `scripts/make-spike/`); pkgconf is also staged on the image. Neither has ever executed under the kernel, so neither is `[x]`. gcc and cmake untouched. See `roadmap.md` for the full status.
...
- [x] Login screen background — 2026-09-17. Wallpaper page, Login screen section. See the Desktop Background entry above and design-decisions §859.
