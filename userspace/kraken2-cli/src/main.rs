...
fn run_kraken2(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "kraken2-build" => {
                println!("Kraken2-build v2.1 (Slate OS) — Database builder");
            ...
            "kraken2-inspect" => {
                println!("Kraken2-inspect v2.1 (Slate OS) — Database inspector");
                println!("  --db DIR    Database directory");
            }
            _ => {
                println!("Kraken2 v2.1 (Slate OS) — Taxonomic sequence classifier");
                println!("  --db DIR          Database directory");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Kraken2 v2.1.3 (Slate OS)");
        return 0;
    }
    match prog {
        "kraken2-build" => {
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "kraken2".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kraken2, strip_ext};
