...
fn run_elixir(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Elixir 1.16.1 (Slate OS)");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/elixir"), "elixir");
        assert_eq!(basename(r"C:\bin\elixir.exe"), "elixir.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("elixir.exe"), "elixir");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_elixir(&["--help".to_string()]), 0);
        assert_eq!(run_elixir(&["-h".to_string()]), 0);
        let _ = run_elixir(&["--version".to_string()]);
    }
