...
fn run_magnifier(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("magnifier v1.0 (Slate OS) — Screen magnification tool");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("magnifier v1.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "magnifier".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_magnifier, strip_ext};
