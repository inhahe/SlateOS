...
fn run_centreon(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Centreon v24.04 (Slate OS) — IT infrastructure monitoring");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Centreon v24.04.2 (Slate OS)");
        return 0;
    }
    println!("Centreon v24.04.2 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "centreon".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_centreon, strip_ext};
