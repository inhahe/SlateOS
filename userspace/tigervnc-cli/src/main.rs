...
//! tigervnc-cli — Slate OS TigerVNC server and client
//!
...
fn run_viewer(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("vncviewer v1.14 (Slate OS, TigerVNC)");
        return 0;
    }
...
fn run_server(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("vncserver v1.14 (Slate OS, TigerVNC)");
        return 0;
    }
...
fn run_config(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("vncconfig v1.14 (Slate OS, TigerVNC)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "vncviewer".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_viewer, strip_ext};
