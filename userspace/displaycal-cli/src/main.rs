...
fn run_displaycal(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("displaycal v3.9 (Slate OS) — Display calibration and profiling");
        println!();
        println!("Options:");
...
fn run_apply_profiles(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("displaycal-apply-profiles v3.9 (Slate OS) — Apply display ICC profiles");
        println!();
        println!("Options:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "displaycal".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_displaycal, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/displaycal"), "displaycal");
        assert_eq!(basename(r"C:\bin\displaycal.exe"), "displaycal.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("displaycal.exe"), "displaycal");
