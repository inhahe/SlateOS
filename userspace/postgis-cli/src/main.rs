...
//! postgis-cli — Slate OS PostGIS spatial database utilities
//!
...
fn run_shp2pgsql(args: &[String]) -> i32 {
    ...
    println!("BEGIN;");
    println!(
        "CREATE TABLE \"{}\" (gid serial, \"geom\" geometry(MultiPolygon,4326));",
        table
    );
...
fn run_raster2pgsql(args: &[String]) -> i32 {
    ...
    let file = args
        .iter()
        .find(|a| a.ends_with(".tif") || a.ends_with(".tiff"))
        .map(|s| s.as_str())
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "shp2pgsql".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_shp2pgsql, strip_ext};
