...
fn run_vercel(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Vercel command-line interface (Slate OS).");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version" || a == "-v") {
        println!("Vercel CLI 33.3.0 (Slate OS)");
        return 0;
    }
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_vercel(vec!["--help".to_string()]), 0);
        assert_eq!(run_vercel(vec!["-h".to_string()]), 0);
        let _ = run_vercel(vec!["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_vercel(vec![]);
    }
}
