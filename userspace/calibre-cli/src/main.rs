...
fn run_calibredb(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("calibredb 7.4.0 (Slate OS)");
        println!();
    ...
    match subcmd {
        "--version" => println!("calibredb (calibre 7.4.0, Slate OS)"),
        "list" => {
        ...
        _ => println!("calibredb: {} completed", quoteaf_os(subcmd)),
    }
    0
}
...
fn run_ebook_convert(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("ebook-convert (calibre 7.4.0, Slate OS)");
        return 0;
    }
...
fn run_ebook_meta(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("ebook-meta (calibre 7.4.0, Slate OS)");
        return 0;
    }
    let file = args
        .iter()
...
fn run_calibre(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("calibre 7.4.0 (Slate OS)");
        return 0;
    }
...
#[cfg(test)]
mod tests {
    use super::{basename, run_calibredb, strip_ext};
