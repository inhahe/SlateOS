...
fn run_azote(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("azote v1.12 (Slate OS) — Wallpaper manager for Wayland/X11");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("azote v1.12 (Slate OS)");
        return 0;
    }
    let dir = args
        .iter()
        .skip_while(|a| a.as_str() != "-d")
        .nth(1)
        .map(|s| s.as_str())
        .unwrap_or("~/Pictures");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "azote".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_azote, strip_ext};
