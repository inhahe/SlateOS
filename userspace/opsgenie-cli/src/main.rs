...
fn run_opsgenie(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Opsgenie alert management CLI (Slate OS).");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("opsgenie 0.5.0 (Slate OS)");
        return 0;
    }
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_opsgenie(vec!["--help".to_string()]), 0);
        assert_eq!(run_opsgenie(vec!["-h".to_string()]), 0);
        let _ = run_opsgenie(vec!["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_opsgenie(vec![]);
    }
}
