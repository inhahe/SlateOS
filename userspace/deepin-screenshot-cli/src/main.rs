...
fn run_deepin_screenshot(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("deepin-screenshot v5.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "deepin-screenshot".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_deepin_screenshot, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/deepin-screenshot"), "deepin-screenshot");
        assert_eq!(
            basename(r"C:\bin\deepin-screenshot.exe"),
            "deepin-screenshot.exe"
        );
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_deepin_screenshot(&["--help".to_string()], "deepin-screenshot"),
            0
        );
        assert_eq!(
            run_deepin_screenshot(&["-h".to_string()], "deepin-screenshot"),
            0
        );
