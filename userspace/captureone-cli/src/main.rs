...
fn run_c1(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Capture One Pro 16.5 (SlateOS) — Pro RAW developer + tethering");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Capture One Pro 16.5.2.12 (SlateOS)"); return 0; }
    println!("Capture One Pro 16.5.2.12 (SlateOS)");
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_c1(&["--version".to_string()], "captureone");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_c1(&[], "captureone");
    }
}
