...
//! dovecot — OurOS IMAP/POP3 mail server
//!
...
fn run_dovecot(args: Vec<String>) -> i32 {
    ...
    println!("May 22 10:00:00 dovecot: master: Dovecot v2.3.21 (OurOS) starting up");
...
fn run_doveconf(args: Vec<String>) -> i32 {
    ...
    if nondefault {
        println!("# 2.3.21 (OurOS): /etc/dovecot/dovecot.conf");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_dovecot(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_dovecot(vec![]), 0);
    }
}
