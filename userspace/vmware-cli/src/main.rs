...
//! vmware-cli — OurOS VMware Workstation Pro / Fusion / vSphere
//!
...
fn run_vmw(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("VMware Workstation Pro 17 (OurOS) — Type-2 hypervisor (Linux/Win host)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("VMware Workstation 17.6.2 build-24409262 (OurOS)"); return 0; }
    println!("VMware Workstation Pro 17.6.2 build-24409262 (OurOS)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_vmw(&["--version".to_string()], "vmware"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_vmw(&[], "vmware"), 0);
    }
}
