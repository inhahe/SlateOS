...
fn run_ogr2ogr(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("ogr2ogr v3.9 (Slate OS) — Convert vector geospatial data");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("ogr2ogr v3.9 (Slate OS) — GDAL/OGR");
        return 0;
    }
...
fn run_ogrinfo(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("ogrinfo v3.9 (Slate OS) — List info about OGR data source");
        return 0;
    }
    let file = args
        .iter()
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "ogr2ogr".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ogr2ogr, strip_ext};
