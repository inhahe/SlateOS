...
fn run_timidity(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("TiMidity++ v2.15 (Slate OS) — MIDI to audio converter/player");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("TiMidity++ v2.15.0 (Slate OS)"); return 0; }
    println!("TiMidity++ v2.15.0 (Slate OS)");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_timidity};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/timidity"), "timidity");
        assert_eq!(basename(r"C:\bin\timidity.exe"), "timidity.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("timidity.exe"), "timidity");
