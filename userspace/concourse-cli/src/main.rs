...
fn run_fly(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Concourse CLI (fly) 7.11.0 (Slate OS)");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/concourse"), "concourse");
        assert_eq!(basename(r"C:\bin\concourse.exe"), "concourse.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("concourse.exe"), "concourse");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_fly(&["--help".to_string()]), 0);
        assert_eq!(run_fly(&["-h".to_string()]), 0);
        let _ = run_fly(&["--version".to_string()]);
    }
