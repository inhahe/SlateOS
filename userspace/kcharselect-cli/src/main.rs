...
fn run_kcharselect(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("kcharselect v23.08 (Slate OS) — KDE character selector");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("kcharselect v23.08 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "kcharselect".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kcharselect, strip_ext};
