...
fn run_dh(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Dehancer Pro 7 (Slate OS) — Premium film emulation");
        println!();
        println!("Options:");
        println!(
            "  --film STOCK           Choose film (Kodak Vision3, Ektachrome, Fuji Eterna, etc.)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Dehancer Pro 7.5.0 (Slate OS)");
        return 0;
    }
    println!("Dehancer Pro 7.5.0 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "dehancer".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_dh, strip_ext};
