...
fn run_curs(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Cursor 0.43 (Slate OS) — AI-native code editor (VS Code fork by Anysphere)");
        println!();
        println!("Options:");
        ...
        println!(
            "  --tab                  Tab — predictive code completion (Cursor's killer feature)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Cursor 0.43.6 (Slate OS)");
        return 0;
    }
    println!("Cursor 0.43.6 (Slate OS)");
    ...
    println!(
        "  Built on: VS Code OSS fork (the open-source half) — extension-compatible with VS Code"
    );
    ...
    println!(
        "  Models: GPT-4o, GPT-4 Turbo, Claude 3.5 Sonnet, Claude 3.5 Haiku, Gemini Pro 1.5, o1-preview / o1-mini"
    );
    println!("  Killer features:");
    println!(
        "    - 'Tab' completion: predictive multi-line edits, jump-to-next edit hint (insanely fast UX)"
    );
    println!(
        "    - Cmd+K: select code → write natural-language instruction → AI edits inline with diff"
    );
    println!(
        "    - Cmd+L (Chat/Composer): multi-file edits with chat panel, @-mention files/symbols/docs"
    );
    println!(
        "    - Agent mode (since Oct 2024): autonomous mode — Cursor runs commands, edits files, debugs"
    );
    ...
    println!(
        "  Privacy: 'Privacy Mode' (Pro+) — Anysphere doesn't store code, models don't train on it"
    );
    println!(
        "  Critique: closed source (despite VS Code OSS base), telemetry concerns, fast-moving UX"
    );
    println!(
        "  Differentiator: tab-key prediction UX is unmatched — feels like AI knows your next edit"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_curs, strip_ext};
