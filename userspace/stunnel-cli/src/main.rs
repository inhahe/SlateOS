...
fn run_stunnel(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!();
        println!("stunnel — SSL/TLS tunnel (Slate OS).");
        println!();
        println!("Options:");
...
fn run_socat(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!();
        println!("socat — multipurpose relay (Slate OS).");
        println!();
...
fn run_ncat(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!();
        println!("ncat — networking utility (Slate OS, from Nmap).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Ncat: Version 7.94 (Slate OS)");
        return 0;
    }
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/stunnel"), "stunnel");
        assert_eq!(basename(r"C:\bin\stunnel.exe"), "stunnel.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("stunnel.exe"), "stunnel");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_stunnel(&["--help".to_string()]), 0);
        assert_eq!(run_stunnel(&["-h".to_string()]), 0);
        let _ = run_stunnel(&["--version".to_string()]);
    }
