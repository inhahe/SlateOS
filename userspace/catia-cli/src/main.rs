...
fn run_catia(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Dassault CATIA V5-6R2024 / 3DEXPERIENCE (Slate OS) — Aerospace/automotive CAD");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Dassault CATIA V5-6R2024 (Slate OS)");
        return 0;
    }
    println!("Dassault CATIA V5-6R2024 / 3DEXPERIENCE (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "catia".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_catia, strip_ext};
