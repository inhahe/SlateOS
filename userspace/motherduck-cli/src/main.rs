...
//! motherduck-cli — OurOS MotherDuck (DuckDB cloud, hybrid execution, Seattle, private 2022)
...
fn run_md(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("MotherDuck (OurOS) — DuckDB-in-the-cloud, hybrid local/cloud query execution");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("MotherDuck 2024 (OurOS) — DuckDB CLI 1.x with MotherDuck extension"); return 0; }
    println!("MotherDuck 2024 (OurOS) — DuckDB-in-the-Cloud (Hybrid Local + Cloud)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_md(&["--version".to_string()], "motherduck"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_md(&[], "motherduck"), 0);
    }
}
