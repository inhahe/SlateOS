...
- [-] **Thumbnail cache — stored in a dedicated OS directory, never alongside the source files.** *Verified 2026-09-16: the cache exists and is wired in production (`ThumbnailGenerator::with_default_disk_cache`), it lives under the per-user path and never beside sources (`DiskCache::default_location`), and invalidation works — `cache_filename` keys on path, mtime and the size cap, so any change misses and regenerates. **What is missing is the size cap**: nothing bounds the directory, so it grows for the life of the install. See known-issues TD-C-THE-THUMBNAIL-CACHE-HAS-NO-CEILING.* Generated thumbnails are persisted in a centralized cache so re-listing a directory is fast and so the same thumbnail can be served to other consumers (file picker, image viewer, taskbar previews, etc.) without re-running the generator. The cache lives in a dedicated OS-managed directory (under the per-user state path, e.g. `~/.cache/os/thumbnails/` — exact path TBD; never in the user's source directories). Rationale:
