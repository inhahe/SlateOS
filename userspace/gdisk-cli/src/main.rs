...
fn run_fixparts(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("fixparts v1.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "gdisk".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gdisk, strip_ext};
