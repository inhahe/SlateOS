...
fn run_micro(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("micro 2.0.14 (Slate OS) — A modern and intuitive terminal text editor");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-version") {
        println!("Version: 2.0.14 (Slate OS)");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/micro"), "micro");
        assert_eq!(basename(r"C:\bin\micro.exe"), "micro.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("micro.exe"), "micro");
