...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_tclsh};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/tcl"), "tcl");
        assert_eq!(basename(r"C:\bin\tcl.exe"), "tcl.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("tcl.exe"), "tcl");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_tclsh(&["--help".to_string()]), 0);
        assert_eq!(run_tclsh(&["-h".to_string()]), 0);
        let _ = run_tclsh(&["--version".to_string()]);
    }
