...
fn run_drone(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Drone CLI 1.7.0 (Slate OS)");
        println!();
        println!("Commands:");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_drone};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/drone"), "drone");
        assert_eq!(basename(r"C:\bin\drone.exe"), "drone.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("drone.exe"), "drone");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_drone(&["--help".to_string()]), 0);
        assert_eq!(run_drone(&["-h".to_string()]), 0);
        let _ = run_drone(&["--version".to_string()]);
    }
