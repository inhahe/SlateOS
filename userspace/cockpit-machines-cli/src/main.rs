...
fn run_cockpit_machines(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("cockpit-machines v306 (Slate OS) — Web-based VM management");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("cockpit-machines v306 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "cockpit-machines".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_cockpit_machines, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/cockpit-machines"), "cockpit-machines");
        assert_eq!(
            basename(r"C:\bin\cockpit-machines.exe"),
            "cockpit-machines.exe"
        );
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_cockpit_machines(&["--help".to_string()], "cockpit-machines"),
            0
        );
        assert_eq!(
            run_cockpit_machines(&["-h".to_string()], "cockpit-machines"),
            0
        );
