...
fn run_openblas(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("OpenBLAS v0.3.27 (Slate OS) — BLAS benchmark/info tool");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("OpenBLAS v0.3.27 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "openblas-test".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_openblas, strip_ext};
