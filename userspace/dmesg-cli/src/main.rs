...
fn run_dmesg(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("dmesg — print kernel ring buffer messages (Slate OS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("dmesg from util-linux 2.39 (Slate OS)");
        return 0;
    }
    ...
    if json {
        println!("{{\"dmesg\": [");
        println!(
            "  {{\"pri\": 6, \"time\": 0.000000, \"msg\": \"Slate OS version 1.0.0 (rust 2024) #1 SMP PREEMPT\"}},"
        );
        println!(
            "  {{\"pri\": 6, \"time\": 0.000001, \"msg\": \"Command line: root=/dev/sda2 ro quiet splash\"}},"
        );
        println!(
            "  {{\"pri\": 6, \"time\": 0.000100, \"msg\": \"x86/cpu: Intel(R) Core(TM) i9-13900K\"}}"
        );
        println!("]}}");
        return 0;
    }
    ...
    let messages = [
        (
            ...
            "kern",
            "info",
            "Slate OS version 1.0.0 (rust 2024) #1 SMP PREEMPT",
        ),
        (
            0.000001,
            "kern",
        ...
        (
            ...
            "kern",
            "info",
            "x86/cpu: Intel(R) Core(TM) i9-13900K",
        ),
        (
            0.000200,
            "kern",
            "info",
            "x86/fpu: x87 FPU: SSE, SSE2, SSE3, SSSE3, SSE4.1, SSE4.2, AVX, AVX2, AVX-512",
        ),
        ...
        (
            ...
            "kern",
            "info",
            "Memory: 65536MB (67108864KB) total",
        ),
        (
            0.010000,
            "kern",
            "info",
            "ACPI: RSDP 0x00000000000E0000 000024 (v02 ALASKA)",
        ),
        (
            0.020000,
            "kern",
            "info",
            "ACPI: XSDT 0x000000007FFE0000 0000BC (v01 ALASKA A M I    01072009 AMI  01000013)",
        ),
        (
            0.050000,
            "kern",
        ...
        (
            ...
            "kern",
            "info",
            "Calibrating delay loop... 6000.00 BogoMIPS",
        ),
        (
            0.200000,
            "kern",
            "info",
            "pid_max: default: 4194304 minimum: 301",
        ),
        (
            0.300000,
            "kern",
        ...
        (
            ...
            "kern",
            "info",
            "smpboot: CPU0: Intel(R) Core(TM) i9-13900K (family: 0x6, model: 0xb7)",
        ),
        ...
        (
            ...
            "kern",
            "info",
            "nvme 0000:04:00.0: PCIe Gen4 x4 link",
        ),
        ...
        (
            ...
            "kern",
            "info",
            "ahci 0000:00:17.0: AHCI 0001.0301 32 slots 6 ports",
        ),
        (
            3.000000,
            "kern",
        ...
        (
            ...
            "kern",
            "info",
            "igc 0000:03:00.0: Intel(R) 2.5GbE Network Connection",
        ),
        (
            4.000000,
            "kern",
            "info",
            "iwlwifi 0000:02:00.0: loaded firmware version 83.e8f84e98.0",
        ),
        (
            5.000000,
            "kern",
...
#[cfg(test)]
mod tests {
    use super::{basename, run_dmesg, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/dmesg"), "dmesg");
        assert_eq!(basename(r"C:\bin\dmesg.exe"), "dmesg.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("dmesg.exe"), "dmesg");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_dmesg(&["--help".to_string()]), 0);
        assert_eq!(run_dmesg(&["-h".to_string()]), 0);
        let _ = run_dmesg(&["--version".to_string()]);
    }
