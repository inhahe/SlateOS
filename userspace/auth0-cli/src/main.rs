...
fn run_a0(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Strategy: 'developer-first authentication' — beautiful docs + SDKs for every platform"
    );
    println!("           Auth0 captured mindshare with hands-on developer experience");
    println!(
        "           targeted CIAM (Customer Identity & Access Management) vs Okta's workforce focus"
    );
    ...
    println!(
        "    - 30+ SDKs (Node, Python, Go, .NET, Java, iOS, Android, React, Angular, Vue, ...)"
    );
    println!(
        "    - 60+ identity providers out-of-box (Google, Facebook, Microsoft, GitHub, LinkedIn, ...)"
    );
    ...
    println!(
        "    - Organizations: B2B multi-tenant (each customer = an org with its own users + branding)"
    );
    ...
    println!(
        "  Customers: Stripe, Pfizer, Subway, Mazda, Siemens — anyone needing modern customer login"
    );
    println!("  Critique: pricing complex (MAU-based + feature gates) — surprise bills at scale");
    println!(
        "           Rules → Actions migration painful (Auth0 deprecating old extension model)"
    );
    println!("           post-acquisition some feel slowed-down product velocity");
    println!(
        "  Differentiator: gold standard developer experience for CIAM — fastest 'hello world' to prod login"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_a0, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/auth0"), "auth0");
        assert_eq!(basename(r"C:\bin\auth0.exe"), "auth0.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("auth0.exe"), "auth0");
