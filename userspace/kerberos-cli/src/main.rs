...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "kinit".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kinit, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/kerberos"), "kerberos");
        assert_eq!(basename(r"C:\bin\kerberos.exe"), "kerberos.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("kerberos.exe"), "kerberos");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_kinit(&["--help".to_string()]), 0);
        assert_eq!(run_kinit(&["-h".to_string()]), 0);
        let _ = run_kinit(&["--version".to_string()]);
    }
