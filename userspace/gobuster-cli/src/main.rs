...
fn run_gobuster(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Gobuster v3.6 (Slate OS) — Directory/file & DNS brute-forcer");
        println!();
        println!("Modes:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Gobuster v3.6.0 (Slate OS)");
        return 0;
    }
    println!("Gobuster v3.6.0 (Slate OS)");
    println!("  Mode: dir");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "gobuster".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gobuster, strip_ext};
