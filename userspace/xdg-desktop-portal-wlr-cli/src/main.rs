...
fn run_portal_wlr(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("xdg-desktop-portal-wlr v0.7 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "xdg-desktop-portal-wlr".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_portal_wlr, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(
            basename("/usr/bin/xdg-desktop-portal-wlr"),
            "xdg-desktop-portal-wlr"
        );
        assert_eq!(
            basename(r"C:\bin\xdg-desktop-portal-wlr.exe"),
            "xdg-desktop-portal-wlr.exe"
        );
    ...
    fn strip_ext_removes_extension() {
        assert_eq!(
            strip_ext("xdg-desktop-portal-wlr.exe"),
            "xdg-desktop-portal-wlr"
        );
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_portal_wlr(&["--help".to_string()], "xdg-desktop-portal-wlr"),
            0
        );
        assert_eq!(
            run_portal_wlr(&["-h".to_string()], "xdg-desktop-portal-wlr"),
            0
        );
