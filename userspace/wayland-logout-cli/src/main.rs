...
fn run_wayland_logout(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("wayland-logout v1.4 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "wayland-logout".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wayland_logout, strip_ext};
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_wayland_logout(&["--help".to_string()], "wayland-logout"),
            0
        );
        assert_eq!(run_wayland_logout(&["-h".to_string()], "wayland-logout"), 0);
        let _ = run_wayland_logout(&["--version".to_string()], "wayland-logout");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_wayland_logout(&[], "wayland-logout");
    }
}
