...
fn run_ttfdump(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("ttfdump v0.6 (Slate OS) — Dump TrueType font tables");
        println!();
        println!("Options:");
    ...
    if let Some(tbl) = table {
        println!("Table {} from {}:", quoteaf_os(tbl), file);
        match tbl {
            "head" => {
                println!("  Version: 1.0");
            ...
            _ => println!("  (table data for {})", quoteaf_os(tbl)),
        }
    } else {
        println!("Font: {}", file);
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "ttfdump".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ttfdump, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/ttfdump"), "ttfdump");
        assert_eq!(basename(r"C:\bin\ttfdump.exe"), "ttfdump.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("ttfdump.exe"), "ttfdump");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_ttfdump(&["--help".to_string()], "ttfdump"), 0);
        assert_eq!(run_ttfdump(&["-h".to_string()], "ttfdump"), 0);
        let _ = run_ttfdump(&["--version".to_string()], "ttfdump");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_ttfdump(&[], "ttfdump");
    }
}
