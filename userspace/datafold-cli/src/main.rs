...
fn run_datafold(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Datafold (Slate OS) — data diff + data-aware CI for dbt + SQL changes");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/datafold"), "datafold");
        assert_eq!(basename(r"C:\bin\datafold.exe"), "datafold.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("datafold.exe"), "datafold");
