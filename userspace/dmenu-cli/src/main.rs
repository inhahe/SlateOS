...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "dmenu".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_dmenu, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/dmenu"), "dmenu");
        assert_eq!(basename(r"C:\bin\dmenu.exe"), "dmenu.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("dmenu.exe"), "dmenu");
