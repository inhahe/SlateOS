...
fn run_portal(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("xdg-desktop-portal v1.18 (Slate OS) — Desktop integration portal");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("xdg-desktop-portal v1.18 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "xdg-desktop-portal".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_portal, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(
            basename("/usr/bin/xdg-desktop-portal"),
            "xdg-desktop-portal"
        );
        assert_eq!(
            basename(r"C:\bin\xdg-desktop-portal.exe"),
            "xdg-desktop-portal.exe"
        );
