...
//! akamai-cli — OurOS Akamai (original CDN + edge security + cloud compute, Cambridge MA, NASDAQ:AKAM)
...
fn run_akamai(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Akamai (OurOS) — original CDN, security, edge compute, Linode cloud (NASDAQ:AKAM)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Akamai 2024 (OurOS) — akamai CLI 2.x"); return 0; }
    println!("Akamai 2024 (OurOS) — Cloud Computing, Security, and Content Delivery");
