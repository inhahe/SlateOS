...
fn run_squid(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("squid — web proxy cache (Slate OS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-v") {
        println!("Squid Cache: Version 6.6 (Slate OS)");
    ...
    if let Some(cmd) = k_arg {
        match cmd {
            ...
            _ => println!("squid: command {} completed", quoteaf_os(cmd)),
        }
        return 0;
    }
...
fn run_squidclient(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!();
        println!("squidclient — Squid HTTP client (Slate OS).");
        println!();
        println!("Options:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "squid".to_string());
    let rest: Vec<String> = args.into_iter().skip(1).collect();
    let code = match prog.as_str() {
        "squidclient" => run_squidclient(&rest),
        "purge" => {
            println!("Purging cache entries matching pattern...");
            println!("Purged 42 objects.");
            0
        }
        _ => run_squid(&rest),
    };
...
#[cfg(test)]
mod tests {
    use super::{basename, run_squid, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_squid(&["--version".to_string()]);
    }
