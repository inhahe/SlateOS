...
//! chroma-cli — OurOS Chroma (open-source embedding database, San Francisco, private)
...
fn run_ch(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Chroma (OurOS) — open-source embedding database, AI-application-first DX");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Chroma 2024 (OurOS) — chromadb CLI 0.5.x"); return 0; }
    println!("Chroma 2024 (OurOS) — Open-Source Embedding Database for AI Apps");
