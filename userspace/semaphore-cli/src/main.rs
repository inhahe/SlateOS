...
fn run_sem(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Semaphore CI 2.0 (Slate OS) — Fast continuous integration");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Semaphore CI 2.0 (Slate OS)");
        return 0;
    }
    println!("Semaphore CI 2.0 (Slate OS)");
    println!("  Vendor: Rendered Text d.o.o. (Novi Sad, Serbia + Brooklyn, NY)");
    println!(
        "  Founders: Marko Anastasov + Aleksandar Diklic (Serbian dev shop turned product co.)"
    );
    ...
    println!(
        "          Semaphore 2.0 launched 2018 — full rewrite, YAML config, fan-out pipelines"
    );
    ...
    println!(
        "  Strong communities: Ruby, Elixir, Crystal — first-class language support since Rails-era"
    );
    ...
    println!(
        "  Differentiator: capacity-priced billing model + fastest cold-start + Elixir/Ruby community"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "semaphore".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sem, strip_ext};
