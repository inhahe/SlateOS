...
fn cmd_bzip2(args: &str) {
    ...
    // Compress.
    let compressed = crate::fs::bzip2::bzip2_compress(&file_data, level);
...
fn cmd_unxz(args: &str) {
    ...
    if file_data.len() < 12
        || file_data.get(..6) != Some(&[0xFD, 0x37, 0x7A, 0x58, 0x5A, 0x00])
    {
