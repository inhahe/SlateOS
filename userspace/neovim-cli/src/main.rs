...
fn run_nvim(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Neovim 0.10.2 (Slate OS) — Hyperextensible Vim-based text editor");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Neovim v0.10.2 (Slate OS)");
        return 0;
    }
    println!("Neovim v0.10.2 (Slate OS)");
    ...
    println!(
        "  Maintainer: Justin M. Keyes (lead since 2014), open community of ~700 contributors"
    );
    ...
    println!(
        "         but: async (jobs/RPC), built-in LSP, Tree-sitter, Lua, GUI API for outside frontends"
    );
    ...
    println!(
        "    Famous: telescope.nvim (fuzzy finder), nvim-cmp (completion), neo-tree, gitsigns,"
    );
    println!(
        "           nvim-treesitter, mason.nvim (LSP installer), null-ls.nvim (deprecated → none-ls)"
    );
    println!(
        "  GUIs: Neovide (Rust), Goneovim (Go), Firenvim (browser textarea), VSCode-Neovim extension"
    );
    println!(
        "  Differentiator: keeps modal editing alive + brings it into modern dev ecosystem (LSP+TS+Lua)"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_nvim, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/neovim"), "neovim");
        assert_eq!(basename(r"C:\bin\neovim.exe"), "neovim.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("neovim.exe"), "neovim");
