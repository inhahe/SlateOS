...
fn run_grbl(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("GRBL 1.1h CNC Controller (Slate OS)");
        println!();
        println!("Commands:");
    ...
    match subcmd {
        ...
        _ => println!("grbl: {} completed", quoteaf_os(subcmd)),
    }
    0
}
...
fn run_grbl_send(args: &[String]) -> i32 {
    ...
    let file = args
        .iter()
        .find(|a| a.ends_with(".gcode") || a.ends_with(".nc") || a.ends_with(".ngc"))
        .map(|s| s.as_str())
        .unwrap_or("job.gcode");
    let port = args
        .windows(2)
...
#[cfg(test)]
mod tests {
    use super::{basename, run_grbl, strip_ext};
