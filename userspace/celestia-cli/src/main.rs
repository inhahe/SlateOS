...
fn run_celestia(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("celestia v1.7 (Slate OS) — Real-time 3D space simulator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("celestia v1.7 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "celestia".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_celestia, strip_ext};
