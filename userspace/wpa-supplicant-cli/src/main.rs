...
fn run_wpa_supplicant(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("wpa_supplicant v2.10 (Slate OS) — WPA/WPA2/WPA3 supplicant");
        println!();
        println!("Options:");
...
fn run_wpa_cli(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("wpa_cli v2.10 (Slate OS) — WPA supplicant control");
...
fn run_wpa_passphrase(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("wpa_passphrase v2.10 (Slate OS) — Generate WPA PSK");
        return 0;
    }
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wpa_supplicant, strip_ext};
    ...
    fn help_exits_zero() {
        assert_eq!(
            run_wpa_supplicant(&["--help".to_string()], "wpa-supplicant"),
            0
        );
