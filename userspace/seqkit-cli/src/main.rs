...
fn run_seqkit(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("SeqKit v2.8 (Slate OS) — Ultrafast FASTA/Q toolkit");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/seqkit"), "seqkit");
        assert_eq!(basename(r"C:\bin\seqkit.exe"), "seqkit.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("seqkit.exe"), "seqkit");
