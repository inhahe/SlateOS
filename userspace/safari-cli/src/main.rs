...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/safari"), "safari");
        assert_eq!(basename(r"C:\bin\safari.exe"), "safari.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("safari.exe"), "safari");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_sf(&["--help".to_string()], "safari"), 0);
        assert_eq!(run_sf(&["-h".to_string()], "safari"), 0);
        assert_eq!(run_sf(&["--version".to_string()], "safari"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_sf(&[], "safari"), 0);
    }
}
