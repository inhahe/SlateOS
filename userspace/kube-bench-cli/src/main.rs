...
fn run_kube_bench(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("kube-bench v0.7.3 (Slate OS) — CIS Kubernetes Benchmark");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("kube-bench v0.7.3 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "kube-bench".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kube_bench, strip_ext};
