...
fn run_notifyd(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("xfce4-notifyd v0.9 (Slate OS) — XFCE notification daemon");
        println!("  --version      Show version");
        return 0;
    }
    if args.iter().any(|a| a == "--version") {
        println!("xfce4-notifyd v0.9 (Slate OS)");
        return 0;
    }
...
fn run_notifyd_config(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("xfce4-notifyd-config v0.9 (Slate OS) — Notification settings");
        return 0;
    }
    let _ = args;
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "xfce4-notifyd".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_notifyd, strip_ext};
