...
fn run_rs(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Restream (Slate OS) — Multistream to 30+ platforms simultaneously");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Restream Studio v2.0 (Slate OS)"); return 0; }
    println!("Restream (Slate OS)");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/restream"), "restream");
        assert_eq!(basename(r"C:\bin\restream.exe"), "restream.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("restream.exe"), "restream");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_rs(&["--help".to_string()], "restream"), 0);
        assert_eq!(run_rs(&["-h".to_string()], "restream"), 0);
        let _ = run_rs(&["--version".to_string()], "restream");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_rs(&[], "restream");
    }
}
