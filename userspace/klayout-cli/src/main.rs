...
fn run_klayout(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("KLayout v0.29 (Slate OS) — IC layout viewer and editor");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("KLayout v0.29 (Slate OS)");
        return 0;
    }
    let file = args
        .iter()
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "klayout".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_klayout, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/klayout"), "klayout");
        assert_eq!(basename(r"C:\bin\klayout.exe"), "klayout.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("klayout.exe"), "klayout");
