...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_send_count = SEND_COUNT.load(Ordering::Relaxed);
    let saved_dismiss_count = DISMISS_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&CENTER, NotifCenter::new(), self_test_inner);
    SEND_COUNT.store(saved_send_count, Ordering::Relaxed);
    DISMISS_COUNT.store(saved_dismiss_count, Ordering::Relaxed);
    result
}
