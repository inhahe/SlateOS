...
//! materialize-cli — OurOS Materialize (streaming SQL via incremental view maintenance, NYC, private)
...
fn run_mz(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Materialize (OurOS) — streaming SQL database with incremental view maintenance");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Materialize 2024 (OurOS) — mz CLI 2.x"); return 0; }
    println!("Materialize 2024 (OurOS) — Streaming SQL Database (Incremental View Maintenance)");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_mz(&["--version".to_string()], "materialize"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_mz(&[], "materialize"), 0);
    }
}
