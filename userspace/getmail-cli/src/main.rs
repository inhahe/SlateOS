...
fn run_getmail(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("getmail v6.19 (Slate OS) — Mail retriever with Strstrong filtering");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("getmail v6.19 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "getmail".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_getmail, strip_ext};
