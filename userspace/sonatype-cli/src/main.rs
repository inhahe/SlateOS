...
fn run_son(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Sonatype Nexus 3.69 (Slate OS)");
        return 0;
    }
    ...
    println!(
        "          Jason van Zyl: created Apache Maven build tool (2002) — foundation of Java ecosystem"
    );
    ...
    println!(
        "                    pitch: 'protect your software supply chain from open-source risks'"
    );
    println!(
        "                    target: any company shipping Java + .NET + JavaScript + Python applications"
    );
    println!(
        "                    primary competitor: Snyk (newer, developer-first), JFrog (artifact mgmt overlap)"
    );
    println!(
        "                    secondary: GitHub Advanced Security, Veracode SCA, Mend (was WhiteSource), Black Duck"
    );
    println!(
        "                    Sonatype's moat: 16+ year SCA history + Maven Central operator + 80%+ Fortune 100"
    );
    println!(
        "                    sales motion: enterprise direct + heavy government/regulated industries"
    );
    ...
    println!(
        "    1. Maven Central operator — 80B+ downloads/yr (the foundation of Java ecosystem)"
    );
    println!(
        "    2. Nexus Repository Manager — artifact repository (Maven, npm, PyPI, NuGet, Docker, Helm)"
    );
    ...
    println!(
        "    - Original research: Sonatype State of the Software Supply Chain report (annual)"
    );
    ...
    println!(
        "           GitHub Advanced Security (free for OSS, low-cost for repos) threatens from below"
    );
    ...
    println!(
        "           Maven Central operator role = mission-critical but unmonetized infrastructure"
    );
    println!(
        "  Differentiator: 16+ years SCA leadership + Maven Central operator + 80%+ Fortune 100 + deep policy engine + government/defense customer base + CycloneDX co-author — the enterprise SCA platform for regulated industries that need defensible supply-chain security"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "sonatype".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_son, strip_ext};
