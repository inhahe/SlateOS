...
fn run_logisim(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("logisim v3.8 (Slate OS) — Digital logic designer and simulator");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("logisim v3.8 (Slate OS)"); return 0; }
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_logisim};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/logisim"), "logisim");
        assert_eq!(basename(r"C:\bin\logisim.exe"), "logisim.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("logisim.exe"), "logisim");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_logisim(&["--help".to_string()], "logisim"), 0);
        assert_eq!(run_logisim(&["-h".to_string()], "logisim"), 0);
        let _ = run_logisim(&["--version".to_string()], "logisim");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_logisim(&[], "logisim");
    }
}
