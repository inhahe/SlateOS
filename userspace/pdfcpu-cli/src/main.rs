...
fn run_pdfcpu(args: &[String], _prog: &str) -> i32 {
        ...
        || args.is_empty()
    {
        ...
        println!("pdfcpu 0.8.0 (Slate OS) — PDF processor");
        println!();
        println!("Commands:");
    ...
    match cmd {
        "version" => println!("pdfcpu v0.8.0 (Slate OS)"),
        "info" => {
            let file = args.get(1).map(|s| s.as_str()).unwrap_or("doc.pdf");
            println!("pdfcpu info for {}:", quoteaf_os(file));
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "pdfcpu".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_pdfcpu, strip_ext};
