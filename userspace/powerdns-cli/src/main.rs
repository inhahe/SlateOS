...
fn run_powerdns(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "pdnsutil" => {
                println!("pdnsutil (Slate OS) — PowerDNS zone management");
            ...
            "pdns_control" => {
                println!("pdns_control (Slate OS) — PowerDNS runtime control");
            ...
            _ => {
                println!("pdns_server v4.9 (Slate OS) — PowerDNS authoritative server");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("PowerDNS v4.9.0 (Slate OS)");
        return 0;
    }
    println!("PowerDNS v4.9.0 (Slate OS)");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "pdns_server".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_powerdns, strip_ext};
