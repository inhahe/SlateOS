...
fn run_sonic_pi(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Sonic Pi v4.5 (Slate OS) — The live coding music synth");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("Sonic Pi v4.5 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "sonic-pi".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sonic_pi, strip_ext};
