...
fn run_motion(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Options:");
        println!(
            "  --new                  New project (motion graphic / Final Cut title/effect/transition/generator)"
        );
    ...
    println!(
        "    - Behaviors: procedural animation (drag a 'Throw' behavior onto a layer, it animates),"
    );
    println!("                no keyframing needed for most motion");
    println!(
        "    - Particle Emitter + Replicator: built-in particle/replicator system, vast presets"
    );
    ...
    println!(
        "  Use cases: lower-thirds, broadcast graphics, animated logos, FCP custom transitions"
    );
...
#[cfg(test)]
mod tests {
    use super::{basename, run_motion, strip_ext};
