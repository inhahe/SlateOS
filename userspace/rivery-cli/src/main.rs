...
fn run_rivery(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Rivery (Slate OS) — ELT + transformation + reverse-ETL + orchestration in one platform"
        );
        println!();
        println!("Options:");
        println!("  --starter              Starter — from $599/mo (5 sources, basic ELT)");
        println!(
            "  --professional         Professional — $1,499/mo (more sources, transformations)"
        );
    ...
    println!(
        "          founders had previously sold Logz.io (ELK log management) — repeat data infrastructure operators"
    );
    println!(
        "          Yigal was Logz.io co-founder, brought log-management ops mindset to data pipelines"
    );
    ...
    println!(
        "  Strategic position: 'all-in-one data integration' vs unbundled stack (Fivetran + dbt + Airflow + Hightouch):"
    );
    println!(
        "                    primary competitor: Fivetran (ELT), Matillion (transformation), Hightouch + Census (reverse ETL)"
    );
    println!(
        "                    Rivery pitch: 'why buy 4 tools when you can use 1?' — unified ELT + transformation + orchestration + reverse-ETL"
    );
    println!(
        "                    targets mid-market: companies wanting modern data stack but with limited platform team"
    );
    println!(
        "                    differentiator: pricing per RPU (Rivery Pricing Unit) — single budget across all data flows"
    );
    ...
    println!(
        "    RPU = unit measuring compute used by ELT runs + transformations + reverse-ETL syncs"
    );
    println!(
        "    typically simpler to budget vs Fivetran's MAR (Monthly Active Rows) which scales unpredictably"
    );
    ...
    println!(
        "    - Databases: Postgres, MySQL, MSSQL, Oracle, MongoDB, DynamoDB, Cosmos DB, Snowflake"
    );
    ...
    println!(
        "    - Conditional branching: if upstream succeeded → run transformation; else → notify"
    );
    ...
    println!(
        "    - 'Pre-built Data Models' — Salesforce → warehouse schema with transformations included"
    );
    ...
    println!(
        "    - Lighter than dedicated data catalogs (Atlan, Collibra) — bundled with platform"
    );
    println!("  Customers: ~300+ paying customers");
    println!(
        "            Ladbrokes, Vibes, eToro (Israeli fintech), Trax, Wibbitz, Reali, Stash, Cybereason"
    );
    println!("            Lemonade, Wix (yes, Wix uses Rivery for internal data), Sodexo, Sysdig");
    println!(
        "            sweet spot: mid-market companies $50M-$1B revenue with data needs but small data teams"
    );
    println!(
        "            especially strong: Israeli tech ecosystem, EU mid-market, fintech, mar-tech analytics"
    );
    println!(
        "  Critique: less mature ecosystem than Fivetran (smaller connector library, fewer integrations)"
    );
    println!(
        "           reverse-ETL features lag Hightouch + Census in destination coverage + polish"
    );
    println!(
        "           dbt integration good but most teams keep dbt Cloud separate for advanced cases"
    );
    println!(
        "           RPU pricing simpler than Fivetran MAR but still scales unpredictably at enterprise"
    );
    ...
    println!(
        "  Differentiator: all-in-one ELT + transformation + orchestration + reverse-ETL on single platform + RPU pricing simplicity + pre-built data model kits — for mid-market companies wanting modern data stack without buying 4 tools"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "rivery".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_rivery, strip_ext};
