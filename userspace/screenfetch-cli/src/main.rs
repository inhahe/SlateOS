...
fn run_screenfetch(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("screenfetch v3.9 (Slate OS) — System information screenshot");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("screenfetch v3.9 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "screenfetch".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_screenfetch, strip_ext};
