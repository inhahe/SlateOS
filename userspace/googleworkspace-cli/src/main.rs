...
fn run_gw(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Google Workspace 2024.11 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "googleworkspace".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_gw, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/googleworkspace"), "googleworkspace");
        assert_eq!(
            basename(r"C:\bin\googleworkspace.exe"),
            "googleworkspace.exe"
        );
