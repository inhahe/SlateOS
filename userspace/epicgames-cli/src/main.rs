...
fn run_eg(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Epic Games Launcher 16.5 (Slate OS) — Epic Games Store + Unreal/Fortnite hub");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Epic Games Launcher 16.5.4 (Slate OS)");
        return 0;
    }
    println!("Epic Games Launcher 16.5.4 (Slate OS)");
    println!(
        "  Vendor: Epic Games Inc. (HQ Cary, NC — founded 1991 as 'Potomac Computer Systems')"
    );
    println!(
        "  Founder/CEO: Tim Sweeney (Pennsylvania State Univ. dropout, started Epic at age 20)"
    );
    ...
    println!(
        "  Killer feature #2: Exclusives (Borderlands 3, Metro Exodus, Alan Wake 2, Fortnite)"
    );
    ...
    println!(
        "  UE Marketplace: assets / blueprints / Megascans (free since 2019 acquisition of Quixel)"
    );
    println!(
        "  Legal: Epic v. Apple lawsuit 2020 (App Store fees) — partial win, third-party stores now allowed"
    );
    println!(
        "  Other Epic products: Houseparty (RIP 2021), SuperAwesome, Bandcamp (sold 2024 to Songtradr)"
    );
    println!(
        "  Differentiator: aggressive freebies + exclusives + revenue-share — disrupting Steam"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "epicgames".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_eg, strip_ext};
