...
fn run_cflt(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Confluent (Slate OS) — Apache Kafka cloud + enterprise platform (NASDAQ:CFLT)");
        println!();
        println!("Options:");
        println!("  --cloud                Confluent Cloud (managed Kafka SaaS)");
        println!("  --platform             Confluent Platform (self-hosted enterprise)");
        println!(
            "  --flink                Apache Flink stream processing (managed, Immerok-based)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Confluent 2024 (Slate OS)");
        return 0;
    }
    println!("Confluent 2024 (Slate OS)");
    ...
    println!(
        "  Strategic position: 'event-streaming platform' — Kafka + everything that needs Kafka:"
    );
    println!(
        "                    primary competitor: AWS MSK (Managed Kafka), Aiven, Cloudera, Redpanda (BYOL alternative)"
    );
    println!(
        "                    Streaming alternatives: Apache Pulsar, Amazon Kinesis, Google Pub/Sub, Azure Event Hubs"
    );
    println!(
        "                    Confluent's wedge: original Kafka creators + Schema Registry + Connectors + Flink + ksqlDB"
    );
    println!(
        "                    pitch: 'data in motion' — every enterprise needs real-time pipelines + Kafka is the dominant standard"
    );
    println!(
        "                    enterprise sales motion + heavy investment in cloud-managed offering"
    );
    println!(
        "                    open-source Kafka governance still at Apache Software Foundation"
    );
    ...
    println!(
        "    Confluent Cloud Enterprise — private networking, premium SLA, ~$1K-50K+/mo typical"
    );
    ...
    println!(
        "    - Tiered storage (cold data offloaded to S3) — Confluent Cloud + KIP-405 since 3.6"
    );
    ...
    println!(
        "    - Sources: Postgres, MySQL CDC (Debezium), MongoDB, Salesforce, Stripe, Mailchimp"
    );
    println!(
        "    - Sinks: Snowflake, BigQuery, Databricks, Elasticsearch, MongoDB, S3, Iceberg, Postgres"
    );
    ...
    println!(
        "    - Apache Flink (since acquiring Immerok Jan 2023 for ~$60M) — full state, complex event processing"
    );
    println!(
        "    - Confluent's bet: Flink becomes the default stream-processing layer (vs ksqlDB)"
    );
    ...
    println!(
        "    - Streaming events become lakehouse tables queryable from any engine (Snowflake, Databricks, Athena, Trino)"
    );
    ...
    println!(
        "    confluent kafka cluster create my-cluster --cloud aws --region us-west-2 --type basic"
    );
    ...
    println!(
        "            Goldman Sachs, Citi, Capital One, Wells Fargo, JPMorgan (real-time payments, fraud)"
    );
    println!(
        "            Walmart, Target, eBay, Domino's (real-time inventory, ML personalization)"
    );
    println!("            BMW, Volvo (connected car telemetry), Lufthansa, Sky, Disney+");
    println!(
        "            sweet spot: Fortune 1000 with real-time data needs + microservices + event-driven architecture"
    );
    ...
    println!(
        "           competition: AWS MSK + Aiven offer Kafka cheaper (no Confluent enterprise features)"
    );
    println!("           Redpanda (Apache 2.0 Kafka alternative) growing fast, often 10x cheaper");
    println!(
        "           Kafka itself complex to operate even managed — Schema Registry + Connect + Flink each operational"
    );
    println!(
        "           Apache Pulsar (Streamnative) + Materialize (real-time analytics) compete on adjacent needs"
    );
    println!("           operating losses persistent — stock down from peak, growth slowing");
    println!(
        "           Tableflow + Iceberg integration: behind Snowflake's Snowpipe Streaming in adoption"
    );
    println!(
        "           Apache Kafka itself is open-source — Confluent must justify premium vs free"
    );
    println!(
        "  Differentiator: original Kafka creators + Schema Registry + 200+ Connectors + managed Flink + Cloud-native experience + Stream Governance — the de-facto choice for enterprise event streaming"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "confluent".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_cflt, strip_ext};
