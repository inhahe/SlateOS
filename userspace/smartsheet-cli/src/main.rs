...
fn run_ss(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  --advance              Advance — bundled enterprise edition (Control Center, Data Shuttle)"
        );
    ...
    println!(
        "  Founders: Brent Frei, Mark Mader (CEO since 2006), John Creason, Maria Colacurcio, 2005"
    );
    println!(
        "          Mader was VP at Onyx Software, took CEO role early and is still CEO 19 years later"
    );
    println!("          Frei went on to start Triberg Capital (private equity)");
    println!(
        "          one of the most patient enterprise SaaS builds: 13 years private before IPO"
    );
    ...
    println!(
        "          Vista Equity + Blackstone agreed to take Smartsheet PRIVATE Sep 2024 at $56.50/share (~$8.4B)"
    );
    ...
    println!(
        "                    primary competitor: Asana, Monday.com, Wrike, Adobe Workfront, Microsoft Project"
    );
    println!(
        "                    advantage: Excel-native interface = zero learning curve for analysts/PMs in F1000"
    );
    println!(
        "                    historically strong in: government, manufacturing, professional services, IT PMO"
    );
    println!("                    pitch: 'configurable work platform for any team, any process'");
    println!(
        "                    Vista take-private likely focused on tightening to enterprise-only + raising prices"
    );
    ...
    println!(
        "    - Column types: text/number, contact, date, checkbox, dropdown, symbols, formula, predecessor, attachment"
    );
    ...
    println!(
        "    - Actions: notify user, send approval request, change cell, lock row, copy/move row, request update"
    );
    ...
    println!(
        "    - Common pattern: nightly Salesforce → Smartsheet sync, Smartsheet → Snowflake export"
    );
    ...
    println!(
        "              REST API + JSON + webhooks + Smartsheet API SDKs (Java, Python, .NET, Node, Ruby, C#)"
    );
    println!(
        "  Customers: ~14,000 enterprise customers (>$5K ACV), >85% of Fortune 100 use it somewhere"
    );
    println!("            NASA, Cisco, Pfizer, P&G, BMW, Comcast, McGraw-Hill, US Navy, US Army");
    println!("            Roche, Hilton Hotels, Sodexo, ABB, Carlsberg");
    println!(
        "            sweet spot: enterprise PMO + IT + construction + manufacturing + professional services"
    );
    println!(
        "            historically WEAK in: dev teams (Jira wins), tiny SMBs (Asana/Trello win)"
    );
    println!(
        "  Critique: UI shows spreadsheet roots — modern teams find it dated vs Linear/Monday"
    );
    println!(
        "           grid-centric model limits power for non-grid workflows (whiteboarding, docs)"
    );
    ...
    println!(
        "           per-user pricing means usage caps at scale (Asana/Monday more aggressive on volume discounts)"
    );
    println!(
        "           Vista take-private: typical PE concerns about pricing increases + R&D cuts"
    );
    println!("           mobile experience weaker than desktop");
    println!(
        "  Differentiator: most spreadsheet-native enterprise PM platform + Control Center PPM + Excel power users feel at home — for F1000 PMOs running 100+ projects in parallel"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "smartsheet".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ss, strip_ext};
