...
fn run_al(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Alitu (Slate OS) — Podcast maker for beginners (browser-based)");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Alitu v3.2 (Slate OS)");
        return 0;
    }
    println!("Alitu (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "alitu".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_al, strip_ext};
