...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "bandwhich".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_bandwhich, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/bandwhich"), "bandwhich");
        assert_eq!(basename(r"C:\bin\bandwhich.exe"), "bandwhich.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("bandwhich.exe"), "bandwhich");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_bandwhich(&["--help".to_string()], "bandwhich"), 0);
        assert_eq!(run_bandwhich(&["-h".to_string()], "bandwhich"), 0);
        let _ = run_bandwhich(&["--version".to_string()], "bandwhich");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_bandwhich(&[], "bandwhich");
    }
}
