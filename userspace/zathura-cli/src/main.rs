...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/zathura"), "zathura");
        assert_eq!(basename(r"C:\bin\zathura.exe"), "zathura.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("zathura.exe"), "zathura");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_zathura(&["--help".to_string()], "zathura"), 0);
        assert_eq!(run_zathura(&["-h".to_string()], "zathura"), 0);
        assert_eq!(run_zathura(&["--version".to_string()], "zathura"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_zathura(&[], "zathura"), 0);
    }
}
