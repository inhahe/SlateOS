...
fn run_mdbook(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("mdbook 0.4.37 (Slate OS)");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/mdbook"), "mdbook");
        assert_eq!(basename(r"C:\bin\mdbook.exe"), "mdbook.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("mdbook.exe"), "mdbook");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_mdbook(&["--help".to_string()]), 0);
        assert_eq!(run_mdbook(&["-h".to_string()]), 0);
        let _ = run_mdbook(&["--version".to_string()]);
    }
