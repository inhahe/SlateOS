...
//! weaviate-cli — OurOS Weaviate (open-source vector + AI-native database, Amsterdam, private)
...
fn run_wv(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Weaviate (OurOS) — open-source vector + AI-native database, modules-based architecture");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Weaviate 2024 (OurOS) — weaviate CLI 1.x"); return 0; }
    println!("Weaviate 2024 (OurOS) — Open-Source AI-Native Vector Database");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_wv(&["--version".to_string()], "weaviate"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_wv(&[], "weaviate"), 0);
    }
}
