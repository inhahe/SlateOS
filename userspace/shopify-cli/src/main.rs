...
fn run_shopify(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Shopify (Slate OS) — turn-key e-commerce platform powering ~10%+ of US online retail"
        );
        println!();
        println!("Options:");
    ...
    println!(
        "          Lütke is German-born, moved to Ottawa to be with his then-girlfriend (now wife)"
    );
    println!(
        "          built first version of Shopify to sell snowboards (Snowdevil) because existing e-com tools sucked"
    );
    ...
    println!(
        "          known for 'no meetings' Mondays + 'crew' culture + opinionated technical leadership"
    );
    println!(
        "  Founded: 2006 in Ottawa — went public 2015 NYSE:SHOP at $17 ($28B+ valuation today)"
    );
    ...
    println!(
        "  Strategic position: 'arming the rebels' against Amazon — DTC + omnichannel commerce OS:"
    );
    println!(
        "                    primary competitor: BigCommerce (smaller), Wix Stores, Squarespace Commerce"
    );
    println!(
        "                    enterprise: Salesforce Commerce Cloud + Adobe Commerce (ex-Magento)"
    );
    println!("                    headless/composable: commercetools + Saleor + Medusa");
    println!(
        "                    'unified commerce' pitch: online + POS + B2B + wholesale + marketplaces"
    );
    println!(
        "                    2022 strategic pivot: focus on enterprise (Plus) + Shop App consumer side"
    );
    println!(
        "                    2023 divested fulfillment biz (sold Deliverr/6 River to Flexport for stock + 13% Flexport stake)"
    );
    ...
    println!(
        "    Advanced Shopify — $399/mo (15 staff, advanced reports, custom calculated shipping)"
    );
    ...
    println!(
        "    transaction fees: 0% with Shopify Payments, ~0.5-2% if using third-party gateway"
    );
    println!(
        "    credit card rates: 2.9% + 30¢ (Basic) down to 2.4% + 30¢ (Advanced) for online cards"
    );
    ...
    println!(
        "    - Customers: Allbirds, Gymshark, Mattel, Hasbro, Steve Madden, FIGS, Glossier, Heinz, Nestle, Coca-Cola (D2C lines)"
    );
    println!("  Shopify Payments + Shop Pay:");
    println!("    - Shopify's payment processor (built on Stripe under the hood, white-labeled)");
    println!(
        "    - Shop Pay = accelerated one-click checkout (saves card + address, like Apple Pay)"
    );
    ...
    println!(
        "    - Multi-region selling: currency conversion, localized prices, country-specific URLs"
    );
    println!(
        "    - Markets Pro: Merchant of Record service (Shopify handles duties + taxes + compliance internationally)"
    );
    ...
    println!(
        "            Allbirds, Gymshark, Heinz, Mattel, Hasbro, Nestle, Glossier, FIGS, Steve Madden, Decathlon"
    );
    println!("            Lindt, Coca-Cola (D2C), Kylie Cosmetics, Tesla (merch)");
    println!("            sweet spot: from indie maker → $500M+ DTC brand");
    println!(
        "            very weak in: $1B+ retailer with complex omnichannel (those use SFCC or commercetools)"
    );
    println!(
        "  Critique: cost adds up: subscription + 10-20+ paid apps + Plus rev-share + Payments fee"
    );
    println!("           lock-in via Liquid + apps + Shopify Payments — switching is painful");
    println!(
        "           B2B/wholesale lags BigCommerce + Adobe Commerce + commercetools historically"
    );
    ...
    println!(
        "  Differentiator: best out-of-box DTC commerce experience + Shop Pay + 8K-app ecosystem + Plus enterprise stack — the platform Amazon fears"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_shopify, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_shopify(&["--version".to_string()], "shopify");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_shopify(&[], "shopify");
    }
}
