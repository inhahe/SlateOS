...
fn run_wave(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Wave (Slate OS) — Free accounting + invoicing for small business");
        println!();
        println!("Options:");
    ...
    println!(
        "  Origin: built as 'Tic' accounting tool, repositioned as ad-supported free for small biz"
    );
    println!(
        "  Strategy: FREE core accounting + invoicing forever — monetize via Payments + Payroll + Advisors"
    );
    ...
    println!("  Pricing:");
    println!(
        "    - Accounting + Invoicing: FREE (unlimited transactions, unlimited invoices, double-entry GL)"
    );
    ...
    println!(
        "    - Payroll: $20/mo + $6/employee (CA, NY, FL, TX, WA self-serve states), $40/mo elsewhere"
    );
    ...
    println!(
        "    - Bank/credit card transaction import (manual or via OFX/CSV — Wave Pro for auto-feeds)"
    );
    ...
    println!(
        "  Differentiator: legitimately free double-entry accounting — only major free option in the market"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_wave, strip_ext};
