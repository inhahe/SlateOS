...
fn run_fc_list(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("fc-list v2.15 (Slate OS)");
        return 0;
    }
...
fn run_fc_match(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("fc-match v2.15 (Slate OS)");
        return 0;
    }
...
fn run_fc_cache(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("fc-cache v2.15 (Slate OS)");
        return 0;
    }
...
fn run_fc_query(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("fc-query v2.15 (Slate OS)");
        return 0;
    }
...
fn run_fc_scan(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("fc-scan v2.15 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "fc-list".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_fc_list, strip_ext};
