...
fn run_humio(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Humio (now Falcon LogScale) (Slate OS) — index-free log management (CrowdStrike subsidiary)"
        );
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Falcon LogScale 2024 (was Humio, Slate OS)");
        return 0;
    }
    ...
    println!(
        "  Founders: Geeta Schmidt (CEO) + Kresten Krab Thorup (CTO) + Christian Hvitved (Chief Architect)"
    );
    println!(
        "          founded with thesis: 'index-free architecture for cheap, fast log ingest at any scale'"
    );
    println!("          Kresten Krab Thorup: ex-Trifork CTO + Erlang expert (Humio is Java/Scala)");
    println!(
        "          unique architectural approach: 'stream + scan' instead of 'index + look up'"
    );
    ...
    println!(
        "  Strategic position: 'index-free log management — modern, fast, affordable at any volume':"
    );
    println!(
        "                    pitch: 'logs at any scale without breaking the bank or the search latency'"
    );
    println!(
        "                    target: enterprise security + observability (especially CrowdStrike Falcon customers)"
    );
    println!(
        "                    primary competitor: Splunk (head-to-head), Datadog Logs, Elastic, Sumo Logic"
    );
    println!("                    secondary: Logz.io, Graylog, Mezmo, Cribl (log pipelines)");
    println!(
        "                    Humio's wedge: 1TB/day at ~$1K-$2K with sub-second search latency (vs Splunk's $50K+)"
    );
    println!(
        "                    'Sub-second search at terabyte/petabyte scale' is the marketing claim"
    );
    println!("  Pricing:");
    ...
    println!(
        "       - Combines Falcon Insight EDR data + LogScale storage + Charlotte AI analytics"
    );
    ...
    println!(
        "    - Tight integration with Falcon EDR data (every endpoint detection ships to LogScale)"
    );
    println!("    - Charlotte AI (CrowdStrike LLM) can query LogScale conversationally");
    println!(
        "    - 'Falcon NG-SIEM' = LogScale + Charlotte AI + Falcon Insight + Falcon Identity + Falcon Cloud"
    );
    ...
    println!(
        "    - Cloud: AWS CloudWatch + GuardDuty + VPC Flow + S3, Azure Activity, GCP Cloud Logging"
    );
    ...
    println!(
        "    humio query run 'level=error | groupBy(service) | count() | sort(count, desc)' --repo logs-prod"
    );
    println!("    humio dashboard import --repo logs-prod --file dashboard.json");
    println!(
        "    humio alert create --name 'Crash Rate Spike' --query 'level=fatal' --threshold 10 --window 5m"
    );
    ...
    println!(
        "           index-free architecture trade-off: deep historical queries slower than indexed"
    );
    println!(
        "           Splunk SPL community much larger than LQL — onboarding cost for new users"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "humio".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_humio, strip_ext};
