...
fn run_osquery(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "osqueryd" => {
                println!("osqueryd v5.12 (Slate OS) — osquery daemon");
            ...
            _ => {
                println!("osqueryi v5.12 (Slate OS) — Interactive osquery shell");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("osquery v5.12.1 (Slate OS)");
        return 0;
    }
    match prog {
        "osqueryd" => {
            println!("osqueryd v5.12.1 (Slate OS)");
        ...
        _ => {
            println!("osqueryi v5.12.1 (Slate OS)");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "osqueryi".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_osquery, strip_ext};
