...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/bundler"), "bundler");
        assert_eq!(basename(r"C:\bin\bundler.exe"), "bundler.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("bundler.exe"), "bundler");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_bundler(&["--help".to_string()]), 0);
        assert_eq!(run_bundler(&["-h".to_string()]), 0);
        assert_eq!(run_bundler(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_bundler(&[]), 0);
    }
}
