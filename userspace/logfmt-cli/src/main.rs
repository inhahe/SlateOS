...
//! logfmt-cli — OurOS logfmt parser/formatter
//!
...
fn run_logfmt(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("logfmt v1.0 (OurOS) — Parse and format logfmt-encoded logs");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_logfmt(&["--version".to_string()], "logfmt"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_logfmt(&[], "logfmt"), 0);
    }
}
