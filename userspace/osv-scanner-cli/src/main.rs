...
fn run_osv_scanner(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("OSV-Scanner v1.7.0 (Slate OS) — Vulnerability scanner");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-V" || a == "--version") {
        println!("osv-scanner v1.7.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "osv-scanner".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_osv_scanner, strip_ext};
