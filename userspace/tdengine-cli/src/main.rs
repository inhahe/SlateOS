...
fn run_tdengine(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "taos" => {
                println!("taos (Slate OS) — TDengine command-line client");
            ...
            _ => {
                println!("taosd (Slate OS) — TDengine server daemon");
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("TDengine v3.3.2 (Slate OS)");
        return 0;
    }
    match prog {
        "taos" => {
        ...
        _ => {
            println!("TDengine Server v3.3.2 (Slate OS)");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "taosd".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_tdengine, strip_ext};
