...
fn parse_dumpe2fs_args(args: &[String]) -> Result<Dumpe2fsOptions, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-V" => {
                println!("dumpe2fs 1.47.0 (Slate OS)");
                process::exit(0);
            }
            arg if arg.starts_with('-') => {
                return Err(format!("unknown option: {arg}"));
...
fn parse_debugfs_args(args: &[String]) -> Result<DebugfsOptions, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-V" => {
                println!("debugfs 1.47.0 (Slate OS)");
                process::exit(0);
            }
            arg if arg.starts_with('-') => {
                return Err(format!("unknown option: {arg}"));
...
fn parse_resize2fs_args(args: &[String]) -> Result<Resize2fsOptions, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-V" => {
                println!("resize2fs 1.47.0 (Slate OS)");
                process::exit(0);
            }
            arg if arg.starts_with('-') => {
                return Err(format!("unknown option: {arg}"));
...
fn parse_e2image_args(args: &[String]) -> Result<E2imageOptions, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-I" => opts.install = true,
            "-V" => {
                println!("e2image 1.47.0 (Slate OS)");
                process::exit(0);
            }
            arg if arg.starts_with('-') => {
                return Err(format!("unknown option: {arg}"));
...
fn parse_filefrag_args(args: &[String]) -> Result<FilefragOptions, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-V" => {
                println!("filefrag 1.47.0 (Slate OS)");
                process::exit(0);
            }
            arg if arg.starts_with('-') => {
                return Err(format!("unknown option: {arg}"));
...
mod tests {
    ...
    fn test_superblock_creator_os() {
        ...
        sb.s_creator_os = 5;
        assert_eq!(sb.creator_os_str(), "Slate OS");
    }
