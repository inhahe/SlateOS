...
fn run_fusion(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Blackmagic Fusion 19 Studio (Slate OS) — Node-based compositing & motion graphics"
        );
        println!();
        println!("Options:");
...
#[cfg(test)]
mod tests {
    use super::{basename, run_fusion, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_fusion(&["--version".to_string()], "fusion");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_fusion(&[], "fusion");
    }
}
