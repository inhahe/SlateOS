...
fn run_asciidoctor(args: &[String]) -> i32 {
    ...
    let files: Vec<&str> = args
        .iter()
        .filter(|a| a.ends_with(".adoc") || a.ends_with(".asciidoc"))
        .map(|s| s.as_str())
        .collect();
    let backend = args
        .windows(2)
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "asciidoctor".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_asciidoctor, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/asciidoctor"), "asciidoctor");
        assert_eq!(basename(r"C:\bin\asciidoctor.exe"), "asciidoctor.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("asciidoctor.exe"), "asciidoctor");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_asciidoctor(&["--help".to_string()]), 0);
        assert_eq!(run_asciidoctor(&["-h".to_string()]), 0);
        let _ = run_asciidoctor(&["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_asciidoctor(&[]);
    }
}
