...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_scan_count = SCAN_COUNT.load(Ordering::Relaxed);
    let saved_connect_count = CONNECT_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&STATE, State::new(), self_test_inner);
    SCAN_COUNT.store(saved_scan_count, Ordering::Relaxed);
    CONNECT_COUNT.store(saved_connect_count, Ordering::Relaxed);
    result
}
