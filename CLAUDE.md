...
- **At every moment you must be in exactly one of three states: (1) actively making forward progress on a task, (2) starting a new task you just picked, or (3) blocked waiting on the operator for a genuine design decision.** "Doing nothing" is never a valid state. A tick whose entire content is "I sampled X and it was already fine, nothing to do" is idling — it produces no artifact, changes no file, and closes no task. If your current line of work has stopped producing changes (e.g. an audit/verification pass that keeps coming back "already correct"), that line of work is **done** — declare it complete and move to the next roadmap task, even a large one. Do not keep re-running a verification loop that yields no edits just to have something to report. The only acceptable way to end a tick without having advanced a task is state (3): a specific design question posted to `open-questions.md` that genuinely blocks all unblocked work. If you can't name such a blocker, you are not blocked — pick the next task and start it.
...
- **Dynamic /loop pacing: always schedule the next tick at `delaySeconds=60`.** The only exception is when work is genuinely blocked on a human decision — then schedule no wakeup and let the loop end. On this project roadmap work almost always exists, so 60s should be the norm; do not back off to longer idle intervals.
