...
fn run_runc(args: &[String]) -> i32 {
    ...
    match subcmd {
        ...
        "list" => {
            ...
            println!(
                "mycontainer 1234        running     /run/containers/mycontainer      2024-05-22T08:00:00.000000000Z   root"
            );
        }
        "state" => {
        ...
        "events" => {
            let id = args.get(1).map(|s| s.as_str()).unwrap_or("mycontainer");
            println!(
                "{{\"type\":\"stats\",\"id\":\"{}\",\"data\":{{\"cpu\":{{\"usage\":{{\"total\":123456789}}}},\"memory\":{{\"usage\":{{\"usage\":45678901}}}}}}}}",
                id
            );
        }
        _ => println!("runc: command {} completed", quoteaf_os(subcmd)),
    }
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_runc, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/runc"), "runc");
        assert_eq!(basename(r"C:\bin\runc.exe"), "runc.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("runc.exe"), "runc");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_runc(&["--help".to_string()]), 0);
        assert_eq!(run_runc(&["-h".to_string()]), 0);
        let _ = run_runc(&["--version".to_string()]);
    }
