...
//! collectd-cli — OurOS collectd system statistics daemon
//!
...
fn run_collectd(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("collectd v5.12 (OurOS) — System statistics collection daemon");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("collectd v5.12 (OurOS)"); return 0; }
...
fn run_collectdctl(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("collectdctl v5.12 (OurOS) — Control collectd daemon");
    ...
    if args.first().map(|s| s.as_str()) == Some("listval") {
        println!("ouros/cpu-0/cpu-user");
        println!("ouros/cpu-0/cpu-system");
        println!("ouros/memory/memory-used");
        println!("ouros/memory/memory-free");
        println!("ouros/load/load");
        return 0;
    }
...
fn run_collectdmon(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("collectdmon v5.12 (OurOS) — Monitoring wrapper for collectd");
        return 0;
    }
    let _ = args;
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_collectd(&["--version".to_string()], "collectd"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_collectd(&[], "collectd"), 0);
    }
}
