...
fn run_keepassxc(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("keepassxc v2.7 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "keepassxc".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_keepassxc, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/keepassxc"), "keepassxc");
        assert_eq!(basename(r"C:\bin\keepassxc.exe"), "keepassxc.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("keepassxc.exe"), "keepassxc");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_keepassxc(&["--help".to_string()], "keepassxc"), 0);
        assert_eq!(run_keepassxc(&["-h".to_string()], "keepassxc"), 0);
        let _ = run_keepassxc(&["--version".to_string()], "keepassxc");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_keepassxc(&[], "keepassxc");
    }
}
