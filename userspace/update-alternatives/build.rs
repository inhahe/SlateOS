...
//! which includes both the real `update-alternatives.exe` binary and the
//! unit-test harness exe that Cargo names `update-alternatives-<hash>.exe`.
//! Without a manifest declaring `asInvoker`, `cargo test` cannot even launch
//! the harness: it fails with "The requested operation requires elevation."
//! (os error 740).
//!
...
fn main() {
    ...
    embed_manifest::embed_manifest(embed_manifest::new_manifest("SlateOS.update-alternatives"))
