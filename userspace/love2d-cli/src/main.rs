...
//! love2d-cli — Slate OS LÖVE 2D game framework
//!
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/love2d"), "love2d");
        assert_eq!(basename(r"C:\bin\love2d.exe"), "love2d.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("love2d.exe"), "love2d");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_love(&["--help".to_string()]), 0);
        assert_eq!(run_love(&["-h".to_string()]), 0);
        let _ = run_love(&["--version".to_string()]);
    }
