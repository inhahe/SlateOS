...
//! st-cli — OurOS st (suckless terminal)
//!
...
fn run_st(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("st v0.9 (OurOS) — Simple terminal");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-v") { println!("st v0.9 (OurOS)"); return 0; }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_st(&["--version".to_string()], "st"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_st(&[], "st"), 0);
    }
}
