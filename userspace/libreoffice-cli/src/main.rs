...
fn run_libreoffice(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("libreoffice v24.2 (Slate OS) — Office suite");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("libreoffice v24.2 (Slate OS)");
        return 0;
    }
    let component = match prog {
        ...
        "lobase" => "Base",
        _ => {
            if args.iter().any(|a| a == "--writer") {
                "Writer"
            } else if args.iter().any(|a| a == "--calc") {
                "Calc"
            } else if args.iter().any(|a| a == "--impress") {
                "Impress"
...
#[cfg(test)]
mod tests {
    use super::{basename, run_libreoffice, strip_ext};
