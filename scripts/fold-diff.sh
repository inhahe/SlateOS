...
run_side() { local side=$1; shift; diff_run timeout -k 2 30 env ${ENVV[@]+"${ENVV[@]}"} PATH="$bindir/$side" fold "$@"; }
...
run_case()  { compare - "$@"; report "${ENVV[*]:+${ENVV[*]} }fold $*"; }
run_stdin() {
  local input="$1"; shift
  compare "$input" "$@"
...
run_case posix.txt -w1
run_case -w1 posix.txt
run_case posix.txt -- -w1
