...
fn run_musikcube(args: &[String], prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("musikcube 3.0.2 (Slate OS) — Terminal music player and library");
        println!();
        println!("Options:");
        println!("  -V, --version       Show version");
        return 0;
    ...
    if args.iter().any(|a| a == "-V" || a == "--version") {
        println!("musikcube 3.0.2 (Slate OS)");
        return 0;
    }
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_musikcube(&["--version".to_string()], "musikcube");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_musikcube(&[], "musikcube");
    }
}
