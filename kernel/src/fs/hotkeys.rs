...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_dispatch_count = DISPATCH_COUNT.load(Ordering::Relaxed);
    let saved_hit_count = HIT_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&HOTKEYS, HotkeyState::new(), self_test_inner);
    DISPATCH_COUNT.store(saved_dispatch_count, Ordering::Relaxed);
    HIT_COUNT.store(saved_hit_count, Ordering::Relaxed);
    result
}
