...
//! bombardier-cli — OurOS Bombardier HTTP benchmarking tool
//!
...
fn run_bombardier(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Bombardier 1.2.6 (OurOS) — Fast HTTP(S) benchmarking tool");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_bombardier(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_bombardier(&[]), 0);
    }
}
