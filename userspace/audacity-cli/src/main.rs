...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        assert_eq!(run_audacity(vec!["--help".to_string()]), 0);
        assert_eq!(run_audacity(vec!["-h".to_string()]), 0);
        assert_eq!(run_audacity(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_audacity(vec![]), 0);
    }
}
