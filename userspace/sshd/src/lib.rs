...
// live in `sshwire` now. The cipher in particular had to: RFC 4344 s4 says the
// counter starts at the derived IV and advances once per encrypted block for
// the life of the key, and this crate's `build_ctr` instead computed
// `IV + seq * 256 + block_idx` -- a formula with no basis in the RFC, which
// disagreed with the client on every single block and, past 4 KiB, collided
// with the next packet's own keystream. The client meanwhile restarted the
// counter from the IV at every packet, so it reused one keystream for the
// whole session. A shared type that owns the counter is what makes both of
...
/// Written out by hand rather than derived, because a derived one would print
/// the seed. That is not a hypothetical: `Debug` is what `unwrap`, `expect`,
/// `assert_eq!` and every structured log line reach for, so a derived
/// implementation puts the host's private key into the first panic message or
/// log record that happens to mention a `HostKey`. The public half is safe to
/// show and is the half anyone debugging actually wants, since it is what the
/// client compares against `known_hosts`.
impl core::fmt::Debug for HostKey {
...
impl HostKey {
    ...
    ///
    /// These are the same bytes that go inside the private key container, into
    /// an `authorized_keys` line, and into a client's `known_hosts`. The shared
    /// definition is in `sshwire` so those four spellings cannot drift.
    fn public_key_blob(&self) -> Vec<u8> {
        ed25519_public_blob(&self.public_key)
    }
    ...
    /// the one their client shows needs the two to be the same string, which
    /// is why `sshwire` names its two encoders apart rather than making one of
    /// them a default: `base64_encode` is the unpadded one this needs, and
    /// `base64_encode_padded` is the one a key file needs.
    ...
    fn load_from_file(path: &str) -> Result<Self, SshdError> {
        ...
        if hex_str.len() == 64 {
            let mut seed = [0u8; 32];
            // `as_chunks` pairs the digits, so there is no `i * 2` stride to
            // get wrong and no slice index that could panic on a short string.
    ...
    pub fn from_openssh_text(text: &str) -> Result<Self, String> {
        ...
        // in `posix`, which that crate deliberately does not depend on.
        // Checking that the two agree is therefore this caller's obligation,
        // and skipping it would accept a file whose halves disagree: the
        // daemon would then run under an identity that is not the one in the
        // matching `.pub`, and every client would report a changed host key
        // with nothing to point at.
        if posix::ed25519::public_key(&key.seed) != key.public {
            return Err("the public key in the file does not match the private seed".to_string());
        }
        Ok(Self::from_seed(key.seed))
    }
}
...
/// 0600.
///
/// The container itself is `sshwire::encode_openssh_private_key`; what is left
/// here is the part that is this daemon's own business — where the file goes,
/// what permissions it gets, and where the `checkint` comes from. Using the
...
impl ConnectionState {
    /// Open a server-side connection over `transport`.
    ///
    /// Draws its secrets from the kernel CSPRNG. This is the only constructor,
    /// which is what makes that unconditional for every connection the daemon
    /// accepts; [`ConnectionState::with_secret_source`] is the sole override
    /// and exists only in builds that ask for it by name.
    #[must_use]
    pub fn new(
        transport: Box<dyn Transport>,
        config: SshdConfig,
    ...
    ///
    /// The `cfg` is that sentence made structural: the shipped binary is not
    /// compiled with any way to substitute a source, so the guarantee does not
    /// rest on nobody calling this.
    ///
    /// It reads `any(test, feature = "deterministic-secrets")` rather than
    /// `test` alone because the interop test now lives in a crate of its own,
    /// and `cfg(test)` is per-crate — it is false while *this* crate is being
    /// compiled as a dependency of someone else's tests. The feature is the
    /// same guarantee at one crate's remove: only a `dev-dependency` turns it
    /// on, and a release artifact compiles no dev-dependencies, so this method
    /// is absent from one rather than merely uncalled.
    ...
    /// The session identifier this connection settled on, or `None` before the
    /// first key exchange completes.
    ///
    /// RFC 4253 §7.2 makes it the exchange hash of the *first* key exchange,
    /// and both ends compute it independently from the same transcript. That
    /// makes it the one value worth comparing across an interop test: if the
    /// client and the server derive different session ids, they disagreed about
    /// something in the handshake, and every later signature and MAC that binds
    /// to it will fail for reasons that look nothing like the cause. Six bugs
    /// in this protocol's two implementations were exactly that shape.
    #[must_use]
...
pub fn do_version_exchange(conn: &mut ConnectionState) -> Result<(), SshdError> {
...
fn do_user_auth(
    ...
    loop {
        ...
        if login_grace_expired(start, now, conn.config.login_grace_time) {
...
impl CliOptions {
    ...
    /// # Errors
    ///
    /// Returns the status the process should end with when the arguments do
    /// not describe a run to perform: `1` for a bad option or an unparseable
    /// port, `0` for `--help`. That is an `Err` rather than a `process::exit`
...
/// Run the daemon: parse `argv`, load the configuration and host key, bind the
/// listener, and serve connections until the process is killed.
///
/// Returns the status the process should end with. Ending the process is the
/// binary's job, not this function's: a library that calls `process::exit`
/// cannot be called from a test, and an in-process test driving the real client
/// against the real server is the only thing that can catch the two ends of
/// this protocol disagreeing. Six bugs have come out of exactly that. See the
/// module docs and known-issues.md
/// `TD-B-THE-SSH-WIRE-LAYER-IS-WRITTEN-TWICE-AND-NOTHING-MAKES-THE-TWO-COPIES-AGREE`.
///
/// On a successful start this does not return at all: the accept loop is
/// unbounded, so every `return` below is a refusal to start.
pub fn run_cli() -> i32 {
    let opts = match CliOptions::parse_args() {
        Ok(o) => o,
...
mod tests {
    ...
    /// The login grace timer counts in the units its two operands are actually
    /// written in.
    ///
    /// The numbers below are the ones that matter, not round ones. A real
    /// handshake to the first authentication packet is a few milliseconds --
    /// two `group14` modular exponentiations and a round trip -- and the default
    /// grace is 120 seconds, so "10 ms in, not expired" is the case every
    /// successful login passes through. It used to fail: the clock is in
    /// nanoseconds and the code divided by 1000, so `elapsed` was microseconds
    ...
    fn the_login_grace_timer_measures_seconds_and_not_microseconds() {
        ...
        // A clock that steps backwards is a kernel bug reachable by an
...
";
    ...
    //
    // The container codec itself lives in `sshwire` and is tested there. What
    // is tested here is the part that is this crate's own: that a well-formed
    // container yields a working host key, and that the seed/public agreement
    // check -- which `sshwire` deliberately leaves to its callers -- actually
    // happens.
    ...
    fn an_openssh_private_key_round_trips() {
        let seed = [0x42u8; 32];
        let public = posix::ed25519::public_key(&seed);
        let text = encode_openssh_private_key(&seed, &public, "test", 0xDEAD_BEEF);

        let key = HostKey::from_openssh_text(&text).expect("parses");
...
",
            sshwire::base64_encode_padded(&raw)
        );

        let err = HostKey::from_openssh_text(&text).expect_err("must refuse");
        assert!(err.contains("encrypted"), "{err}");
    }

    #[test]
    fn an_openssh_key_whose_halves_disagree_is_refused() {
        // This check is `sshd`'s, not `sshwire`'s: the codec returns the
        // public key the file holds precisely so that this comparison can be
        // made, and a caller that forgot to make it would run under an
        // identity that does not match its own `.pub` file.
        let seed = [0x11u8; 32];
        let wrong_public = posix::ed25519::public_key(&[0x22u8; 32]);
        let text = encode_openssh_private_key(&seed, &wrong_public, "test", 1);
    ...
    fn a_file_that_is_not_a_host_key_is_an_error_not_an_invented_key() {
        ...
        let err = HostKey::from_openssh_text(
            "-----BEGIN OPENSSH PRIVATE KEY-----
Z
",
        )
    ...
    // HMAC-SHA256's vector test moved to `sshwire` with the function.

    // Base64's tests moved to `sshwire` with the functions. Duplicating them
    // here would rebuild the arrangement this whole change is dismantling: a
    // second copy of a shared encoding, tested against its own expectation.
    ...
    // than moved, because it was the test that let the counter bug live: it
    // encrypted one packet and decrypted it again with the same wrong counter,
    // which returns the plaintext perfectly no matter what the counter was.
    // `sshwire` asserts the cipher against FIPS-197's and RFC 3686's published
    // vectors instead, and asserts that two successive packets do not share a
    // keystream -- which is what this one could never have caught. The
    // constant-time comparison's three tests moved there with the function.
