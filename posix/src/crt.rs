...
unsafe fn retrieve_initial_fds() {
    ...
    for entry in entries {
        ...
        //
        // The handle goes in as well as the type because one wire type,
        // `PTY`, names either end — the end lives in the handle's low bit.
        let kind = handle_type_to_kind_for(entry.handle_type, entry.handle);
...
pub unsafe extern "C" fn __libc_start_main(
    ...
    // else.  Child threads get theirs in `pthread_create` /
    // `__pthread_thread_start` (see `crate::tls`).  On a host (test) build
    // this is a no-op.
    #[cfg(target_os = "none")]
    // SAFETY: this is the process's startup path, running before any other
    // code in the program, so the "at most once, before any TLS access"
    // contract holds.  A false return means no memory for TLS; the program
    // is allowed to continue (it only faults if it actually uses TLS).
    unsafe {
