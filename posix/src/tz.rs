//! POSIX `TZ`-string timezone rules.
//!
...
//! hours wrong outside Greenwich, and `%z`/`%Z` answered `+0000`/`UTC`
//! unconditionally.  See `known-issues.md` →
//! `TD-OILS-BROKEN-DOWN-TIME-IS-ALWAYS-UTC`, which is the shell-level symptom
//! of this same gap.
//!
//! ## What is supported
//!
...
impl TzName {
    /// The empty name.  Only used as a placeholder before parsing succeeds.
    const fn empty() -> Self {
        Self { bytes: [0; TZ_NAME_CAP], len: 0 }
    }
...
impl Tz {
    ...
    pub fn utc() -> Self {
        Self {
            // `TzName::new` only fails on an over-long name; "UTC" is 3 bytes.
            std_name: TzName::new(b"UTC").unwrap_or_else(TzName::empty),
            std_gmtoff: 0,
            dst: None,
        }
    }
