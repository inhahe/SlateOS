...
fn run_tengine(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Tengine v3.1 (Slate OS) — High-performance web server (Nginx fork by Alibaba)");
        println!();
        println!("Options:");
    ...
    {
        println!("Tengine/3.1.0 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "tengine".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_tengine, strip_ext};
