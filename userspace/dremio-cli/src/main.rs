...
fn run_dremio(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  --arctic               Dremio Arctic (Iceberg catalog with Git-like branching)"
        );
    ...
    println!(
        "          Apache Arrow (started 2016, Drill-derived) is one of the most important data formats today"
    );
    println!(
        "          Wendy Stirman + Kelly Stirman early team — early data engineering veterans"
    );
    ...
    println!(
        "                    primary competitor: Snowflake (closed format), Databricks (Delta/Spark), Starburst (Trino-based)"
    );
    println!(
        "                    Dremio's wedge: 'lakehouse from the people who created Arrow' — purpose-built for open formats"
    );
    println!(
        "                    smaller than Snowflake/Databricks but technically deep — Arrow + Sabot + Iceberg expertise"
    );
    println!(
        "                    Apache Arrow underlies Dremio Sonar (query engine) — vectorized + columnar execution"
    );
    println!(
        "                    enterprise positioning: 'cheaper than Snowflake by 50-80%' — typical price comparison pitch"
    );
    ...
    println!(
        "    Dremio Software (self-hosted) — annual license + support, $100K-$2M+/yr enterprise"
    );
    ...
    println!(
        "    - Data Reflections: smart materialized acceleration layer (transparent to query)"
    );
    ...
    println!(
        "    - Optimizer transparently routes queries to fastest reflection without user knowing"
    );
    println!(
        "    - Similar concept to: BigQuery materialized views, Snowflake search optimization"
    );
    ...
    println!(
        "    - Multiple data 'branches' for experimentation + staging before production merge"
    );
    println!("    - Open-source: based on Apache Polaris (also adopted by Snowflake)");
    println!(
        "    - Direct competitor: Nessie (open-source by Project Nessie / Dremio's own contribution)"
    );
    ...
    println!(
        "              BI: Tableau, Power BI, Looker, Mode, Sigma, Hex (JDBC/ODBC/Arrow Flight)"
    );
    println!("              Data science: dbt, Pandas, Polars, Apache Superset, Jupyter");
    println!("  Customers: ~600 paying customers");
    println!(
        "            UBS, S&P Global, NCR, Capital One (some teams), Royal Caribbean, Diageo, Ford Direct"
    );
    println!(
        "            Maersk, US Department of Defense (Lakehouse Architecture, Mission Partner Environment)"
    );
    println!(
        "            sweet spot: F1000 enterprises with S3/ADLS lakes + Iceberg adoption + multi-cloud"
    );
    println!(
        "            cost-sensitive vs Snowflake — Dremio Cloud Standard FREE for dev/staging"
    );
    println!("            US Federal + DoD strong (sovereignty + open-source angle)");
    println!(
        "  Critique: smaller than Snowflake + Databricks — fewer connectors + fewer integrations"
    );
    println!("           ecosystem of partners + marketplace smaller than Snowflake's");
    println!(
        "           market positioning between Starburst (also lakehouse) + Snowflake (warehouse) sometimes confusing"
    );
    println!(
        "           Snowflake's Polaris (open Iceberg catalog) launched 2024 — directly threatens Arctic differentiator"
    );
    println!("           AI/GenAI features behind Snowflake Cortex + Databricks AI in marketing");
    println!(
        "           Reflections sometimes mysterious to non-experts (acceleration without user control)"
    );
    println!("           Nadeau departure 2023 raised technical leadership questions");
    println!(
        "  Differentiator: Apache Arrow native + Iceberg-leading + Reflections acceleration + 50% cheaper than Snowflake + Dremio Arctic Git-like catalog — for enterprises seeking Iceberg-native lakehouse without lock-in"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "dremio".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_dremio, strip_ext};
