...
//! btrfs-cli — OurOS Btrfs filesystem tools
//!
...
fn run_btrfs(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!();
        println!("btrfs — Btrfs filesystem management (OurOS).");
        println!();
    ...
    if args.iter().any(|a| a == "--version") {
        println!("btrfs-progs v6.7 (OurOS)");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_btrfs(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_btrfs(&[]), 0);
    }
}
