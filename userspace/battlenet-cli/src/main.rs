...
fn run_bnet(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "  --launch GAME          Launch a specific game by tag (wow/d4/ow2/sc2/hs/coh/cod)"
        );
    ...
    println!("Battle.net 2.39.0.15573 (Slate OS)");
    println!(
        "  Vendor: Blizzard Entertainment — part of Activision Blizzard, owned by Microsoft since Oct 2023"
    );
    ...
    println!(
        "  Available titles: World of Warcraft, Diablo II/III/IV, Overwatch 2, StarCraft / SC2,"
    );
    println!(
        "                   Hearthstone, Heroes of the Storm, Call of Duty (Modern Warfare, Warzone),"
    );
    ...
    println!(
        "  Killer feature: Battle.net Authenticator (mobile) — pioneered mobile MFA for games (2008)"
    );
    println!("  Social: friends list, group chat, voice (since 2018), parties, broadcast invite");
    println!(
        "  Other Microsoft Gaming: Xbox Game Pass (CoD now on Game Pass since Microsoft deal)"
    );
...
#[cfg(test)]
mod tests {
    use super::{basename, run_bnet, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/battlenet"), "battlenet");
        assert_eq!(basename(r"C:\bin\battlenet.exe"), "battlenet.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("battlenet.exe"), "battlenet");
