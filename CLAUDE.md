...
## Two Simultaneous Sessions — Merge Conflict Prevention

Two Claude sessions will work on this repo at the same time. Merge conflicts waste significant time and break flow. Follow these rules strictly.
...
Some files are touched by multiple zones (e.g., `Cargo.toml` workspace members, shared trait definitions, the syscall number table). For these:

1. **Trait definitions and public APIs** (`kernel/src/traits/`, `kernel/src/api/`): Only one session modifies a shared trait at a time. If you need to change a shared interface, add a comment `// ZONE-LOCK: <zone-name> <date>` at the top of the file while working on it. Remove it when done. If you see another session's lock, do not edit that file — work on something else or add your new items to a *separate* file and integrate later.
2. **Cargo workspace root** (`Cargo.toml`): Only add new `[workspace.members]` entries; never reorder or modify existing entries. Append to the end of the members list. This makes merges trivial.
3. **Syscall number table**: Allocate numbers from non-overlapping ranges. kernel-core uses 0-199, kernel-ipc uses 200-399, kernel-security uses 400-499, kernel-process uses 500-599, fs uses 600-799, net uses 800-999. Each range has room to grow.
4. **This file (CLAUDE.md)**: Do not edit during normal development. Only the human operator edits this file.

### Branch Strategy

Each session works on its own branch: `session-a/<feature>` and `session-b/<feature>`. Merge to `main` only when the feature compiles and passes tests. If using git worktrees, each session gets its own worktree.

Before merging to main:
- `git pull --rebase origin main`
- Fix any conflicts (they should be rare if zone boundaries are respected)
- Run the full test suite
- Only then merge

### If You Accidentally Touch Another Zone's File

Stop. Revert your changes to that file (`git checkout -- <file>`). If you need a change in another zone to proceed, leave a note in `todo.txt` describing what you need and why, then move on to other work. The other session will pick it up.
...
6. **Would this be obvious to the next session?** The other Claude session (or a future you with no memory) will read this code. Clear naming, doc comments on public items, module-level `//!` docs explaining the subsystem's purpose and design.
...
4. Check `todo.txt` for any notes left by the other session that affect your work.
5. Study existing implementations per the table above.
6. Write the benchmark first (if applicable).
