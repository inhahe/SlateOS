...
//! linphone-cli — OurOS Linphone SIP/VoIP client
//!
...
fn run_linphone(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("linphone v5.2 (OurOS) — SIP/VoIP desktop client");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("linphone v5.2 (OurOS)"); return 0; }
...
fn run_linphonec(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("linphonec v5.2 (OurOS) — Console SIP client");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("linphonec v5.2 (OurOS)"); return 0; }
