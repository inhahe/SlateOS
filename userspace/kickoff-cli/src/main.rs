...
fn run_kickoff(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("kickoff v0.6 (Slate OS) — Minimalist Wayland program launcher");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("kickoff v0.6 (Slate OS)"); return 0; }
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_kickoff};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/kickoff"), "kickoff");
        assert_eq!(basename(r"C:\bin\kickoff.exe"), "kickoff.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("kickoff.exe"), "kickoff");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_kickoff(&["--help".to_string()], "kickoff"), 0);
        assert_eq!(run_kickoff(&["-h".to_string()], "kickoff"), 0);
        let _ = run_kickoff(&["--version".to_string()], "kickoff");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_kickoff(&[], "kickoff");
    }
}
