...
//! hydra-cli — OurOS Hydra password cracker CLI
//!
...
fn run_hydra(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Hydra — network login cracker (OurOS).");
        println!();
        println!("Options:");
    ...
    println!("Hydra v9.5 (OurOS) starting at 2024-01-15 12:00:00");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_hydra(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_hydra(vec![]), 0);
    }
}
