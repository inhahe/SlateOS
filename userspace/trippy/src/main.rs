...
//! trippy — OurOS network diagnostic tool (mtr alternative)
//!
...
fn run_trip(args: Vec<String>) -> i32 {
    ...
    match mode {
        ...
        _ => {
            // TUI mode
            println!("trippy 0.11.0 (OurOS) — TUI launched");
            println!("Tracing route to {} ...", target);
            println!();
            println!("  Hop  Host              Loss%  Sent  Recv  Last   Avg   Best  Worst  StdDev");
            println!("    1  192.168.1.1        0.0%    10    10   1.2   1.5   0.8    3.2     0.6");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_trip(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_trip(vec![]), 0);
    }
}
