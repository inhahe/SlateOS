...
fn run_maim(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("maim v5.7 (Slate OS) — Screenshot utility (Make Image)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("maim v5.7 (Slate OS)");
        return 0;
    }
    let file = args.last().map(|s| s.as_str()).unwrap_or("screenshot.png");
    println!("maim: captured to {}", quoteaf_os(file));
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_maim, strip_ext};
