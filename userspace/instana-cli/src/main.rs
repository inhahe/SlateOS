...
//! instana-cli — OurOS IBM Instana (APM + observability, IBM subsidiary since 2020)
...
fn run_instana(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("IBM Instana (OurOS) — automatic full-stack APM + observability");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("IBM Instana 2024 (OurOS)"); return 0; }
    println!("IBM Instana 2024 (OurOS) — Enterprise Observability");
