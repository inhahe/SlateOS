...
fn run_grpcurl(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!();
        println!("grpcurl — gRPC CLI client (Slate OS).");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/grpc"), "grpc");
        assert_eq!(basename(r"C:\bin\grpc.exe"), "grpc.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("grpc.exe"), "grpc");
