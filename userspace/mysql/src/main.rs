...
fn run_mysql_client(args: Vec<String>) -> i32 {
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("mysql  Ver 8.4.0 for Slate OS on x86_64 (Slate OS Community Server - GPL)");
        return 0;
    }
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_mysqld(vec!["--help".to_string()]), 0);
        assert_eq!(run_mysqld(vec!["-h".to_string()]), 0);
        let _ = run_mysqld(vec!["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_mysqld(vec![]);
    }
}
