...
impl Kerning {
    ...
    /// `corr` is the size the pair will be drawn at, which selects the row of
    ...
    pub(crate) fn pair(
        ...
        for group in &self.gpos {
            ...
            for &off in &group.subtables {
                if let Some(v) = pair_pos(data, off, left, right, corr) {
                    return v;
                }
            }
        }
...
fn pair_pos(data: &[u8], sub: usize, left: u16, right: u16, corr: Corrections<'_>) -> Option<i16> {
    let (first, _, _) = pair_values(data, sub, left, right, corr)?;
...
mod tests {
    ...
    fn pair_pos_format_1_finds_the_pair_and_only_the_pair() {
        ...
        assert_eq!(pair_pos(&t, 0, 1, 2, Corrections::NONE), Some(-80));
        assert_eq!(pair_pos(&t, 0, 1, 3, Corrections::NONE), Some(-50));
        assert_eq!(pair_pos(&t, 0, 1, 9, Corrections::NONE), Some(0));
        // 'A' does not kern before this one.
        assert_eq!(pair_pos(&t, 0, 1, 4, Corrections::NONE), None);
        // Nothing kerns before 'A': the first glyph is not covered.
        assert_eq!(pair_pos(&t, 0, 2, 1, Corrections::NONE), None);
    }
    ...
    fn pair_pos_format_2_indexes_the_class_grid() {
        ...
        assert_eq!(pair_pos(&t, 0, 1, 6, Corrections::NONE), Some(-30));
        assert_eq!(pair_pos(&t, 0, 2, 7, Corrections::NONE), Some(-70));
        // Class 0 on either axis is a real cell, and this font puts 0 there.
        assert_eq!(pair_pos(&t, 0, 1, 99, Corrections::NONE), Some(0));
        // An uncovered first glyph is not kerned at all, even though its class
        // would index a valid row.
        assert_eq!(pair_pos(&t, 0, 3, 6, Corrections::NONE), None);
    }
    ...
    fn the_legacy_table_is_read_when_there_is_no_gpos() {
        ...
        assert_eq!(k.pair(&data, 4, 2, &[], Corrections::NONE), -20);
        assert_eq!(k.pair(&data, 2, 1, &[], Corrections::NONE), 0);
    }
    ...
    fn a_truncated_table_yields_no_kerning_rather_than_a_panic() {
        ...
        for cut in 0..full.len() {
            ...
            if let Some(k) = Kerning::parse(data, None, Some(span(0, data.len())), None) {
                let _ = k.pair(data, 1, 2, &[], Corrections::NONE);
                let _ = k.pair(data, 1, 3, &[], Corrections::NONE);
                let _ = k.pair(data, 7, 7, &[], Corrections::NONE);
            }
        }
    }
    ...
    fn an_extension_lookup_is_followed_to_the_subtable_it_wraps() {
        ...
        assert_eq!(k.pair(&data, 1, 2, &[], Corrections::NONE), -65);
    }
    ...
    fn a_kern_lookup_that_ignores_marks_reads_the_pair_across_one() {
        ...
        assert_eq!(k.pair(&data, 1, 2, &[90], Corrections::NONE), -80);
        // Several of them, still all marks.
        assert_eq!(k.pair(&data, 1, 2, &[90, 90], Corrections::NONE), -80);
    }
    ...
    fn ignoring_marks_does_not_mean_ignoring_letters() {
        ...
        assert_eq!(k.pair(&data, 1, 2, &[90, 7], Corrections::NONE), 0);
    }
