...
fn run_openocd(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("openocd — Open On-Chip Debugger (Slate OS).");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Open On-Chip Debugger 0.12.0 (Slate OS)");
        return 0;
    }
...
fn run_st_flash(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("st-flash — STMicroelectronics flash tool (Slate OS).");
        println!();
    ...
    println!("st-flash 1.8.0 (Slate OS)");
    match cmd {
        "write" => {
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_openocd(&["--version".to_string()]);
    }
