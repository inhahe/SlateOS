...
mod tests {
    ...
    /// Measured against GNU on 2026-09-25: `POSIXLY_CORRECT=1 comm a b -1` takes
    /// `-1` for an extra operand, where without the variable it is an option.
    #[test]
    fn posixly_correct_makes_an_option_after_an_operand_an_operand() {
        let argv: Vec<OsString> = ["a", "b", "-1"].iter().map(OsString::from).collect();
        let Err(e) = super::parse_args(&argv, true) else {
            panic!("a third operand should be refused");
        };
        assert!(e.sentence.starts_with("extra operand"), "{}", e.sentence);
        assert!(super::parse_args(&argv, false).is_ok());
    }
