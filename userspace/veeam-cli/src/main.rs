...
fn run_vm(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Veeam Data Platform 12.2 (Slate OS) — Enterprise backup, recovery, replication");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Veeam Backup & Replication 12.2.0.334 (Slate OS)");
        return 0;
    }
    println!("Veeam Backup & Replication 12.2.0.334 (Slate OS)");
    ...
    println!(
        "  Recovery: Instant Recovery (mount + boot VM from backup), SureBackup verification,"
    );
    println!(
        "           Secure Restore (AV scan), DataLabs sandbox, item-level restore (AD, Exchange, SQL)"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "veeam".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_vm, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/veeam"), "veeam");
        assert_eq!(basename(r"C:\bin\veeam.exe"), "veeam.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("veeam.exe"), "veeam");
