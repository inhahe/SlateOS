...
fn compute_usable(level: RaidLevel, disks: &[ArrayDisk]) -> u64 {
    let active: Vec<&ArrayDisk> = disks
        .iter()
        .filter(|d| d.role == DiskRole::Active || d.role == DiskRole::Rebuilding)
        .collect();
...
pub fn create_array(
    ...
    with_state(|state| {
        ...
        let disks: Vec<ArrayDisk> = disk_ids
            .iter()
            .map(|id| ArrayDisk {
                disk_id: String::from(*id),
                role: DiskRole::Active,
...
pub fn remove_disk(array_id: u32, disk_id: &str) -> KernelResult<()> {
    with_state(|state| {
        ...
        if arr.disks.len() == before {
            return Err(KernelError::NotFound);
        }
        let active = arr
            .disks
            .iter()
            .filter(|d| d.role == DiskRole::Active)
            .count();
...
pub fn list_arrays() -> Vec<RaidArray> {
    STATE
        .lock()
        .as_ref()
        .map_or(Vec::new(), |s| s.arrays.clone())
}
...
pub fn get_array(id: u32) -> Option<RaidArray> {
    STATE
        .lock()
        .as_ref()
        .and_then(|s| s.arrays.iter().find(|a| a.id == id).cloned())
}
