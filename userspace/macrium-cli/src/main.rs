...
fn run_mc(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Origin: started as 'StoreItForeverPlus' (cassette tape backup) — pivoted to disk imaging"
    );
    ...
    println!(
        "  Features: image whole disk/SSD/NVMe, clone OS to new drive, ReDeploy (drive HW change),"
    );
    println!(
        "            schedule, mount images as virtual drives, file/folder restore from image,"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "macrium".to_string());
