//! `ls`, `dir` and `vdir` — list directory contents.
//!
//! # Three programs, one listing
//!
//! Upstream builds `ls.c` three times with a different `ls_mode`, and so
//! does this module: each bin is a [`Mode`] and a call to [`main`]. The
//! modes differ in their defaults and nothing else — every option means the
//! same in all three, and an explicit option overrides the mode:
//!
//! | | output is a terminal | output is not a terminal | quoting default |
//! |---|---|---|---|
//! | `ls` | columns, `?` for control characters | one per line | shell-escape on a terminal, else literal |
//! | `dir` | columns | columns | escape (`-b`) |
//! | `vdir` | long (`-l`) | long (`-l`) | escape (`-b`) |
//!
//! and each names itself in its diagnostics, `--help` and `--version`.
//! `dir` and `vdir` exist for systems where `ls`'s terminal-dependent defaults
//! are unwanted: their output is the same whether or not anyone is watching.
//!
//! # The port
//!
...
use crate::vercmp::version;
use crate::xnum::{self, Status, strtol_fatal};
...
/// Which of upstream's builds of `ls.c` is running: its `ls_mode`.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum Mode {
    /// `ls` (`LS_LS`): its defaults follow whether output is a terminal.
    #[default]
    Ls,
    /// `dir` (`LS_MULTI_COL`): columns and escape quoting, always.
    Dir,
    /// `vdir` (`LS_LONG_FORMAT`): the long format and escape quoting, always.
    Vdir,
}
...
/// The mode this process runs in: upstream's `ls_mode`, a global set once
/// before anything reads it, as upstream's is. A process is one program, so a
/// process-wide value is the faithful shape -- and it keeps a mode out of the
/// signature of every function that merely prints a diagnostic. Unset (a unit
/// test) reads as `ls`.
...
/// The name every diagnostic starts with: `ls`, `dir` or `vdir`.
fn program_name() -> &'static str {
    mode().name()
}

/// The usage status is **2**, not the 1 that almost every other utility uses:
/// 1 is already spent on "something went wrong with a file", which `ls`
/// reports while still producing a listing.
fn ls_program() -> Program {
    Program::new(program_name(), 2)
}
...
impl Refusal {
    fn from_getopt(error: &getopt::Error) -> Self {
        Self {
            lines: vec![format!("{}: {}", program_name(), error.sentence)],
    ...
    fn fatal(sentence: String) -> Self {
        Self {
            lines: vec![format!("{}: {sentence}", program_name())],
            referral: false,
            status: 2,
        }
    }
...
fn block_size_options(spec: Option<&[u8]>, posixly_correct: bool) -> (u64, Opts) {
    match spec {
        Some(text) => {
            let (size, opts, _) = crate::human::human_options(text, posixly_correct);
            (size, opts)
        }
...
fn parse_args(
    ...
    for item in ls_program().parse(argv, SHORT_OPTIONS, LONG_OPTIONS) {
        ...
        let (flag, value, spelling): (Flag, Option<OsString>, String) = match item {
            ...
            Opt::Short(letter, value) => {
                let Some(flag) = short_flag(letter) else {
                    ...
                    return Err(Refusal::from_getopt(&ls_program().invalid_option(letter)));
                };
            ...
            Opt::Long(name, value) => {
                let Some(flag) = long_flag(name) else {
                    return Err(Refusal::from_getopt(
                        &ls_program().unrecognized_option(format!("--{name}").as_bytes()),
                    ));
                };
        ...
        match flag {
            ...
            Flag::Sort => {
                set.sort_opt = Some(
                    ls_program()
                        .argmatch(arg, &spelling, SORT_ARGS)
                        .map_err(|e| Refusal::from_getopt(&e))?,
                );
            }
            Flag::Time => {
                cfg.time_type = ls_program()
            ...
            Flag::Format => {
                set.format_opt = Some(
                    ls_program()
                        .argmatch(arg, &spelling, FORMAT_ARGS)
                        .map_err(|e| Refusal::from_getopt(&e))?,
                );
            }
            Flag::IndicatorStyle => {
                cfg.indicator_style = ls_program()
            ...
            Flag::QuotingStyle => {
                set.quoting_style_opt = Some(
                    ls_program()
                        .argmatch(arg, &spelling, Style::WORDS)
                        .map_err(|e| Refusal::from_getopt(&e))?,
                );
            }
            Flag::Width => {
            ...
            Flag::BlockSize => {
                let (size, opts, status) = crate::human::human_options(arg, env.posixly_correct);
...
fn finish(
    ...
    // `ls` asks the terminal; `dir` and `vdir` do not.
    cfg.format = set.format_opt.unwrap_or(match mode() {
        Mode::Ls if env.stdout_isatty => Format::ManyPerLine,
        Mode::Ls => Format::OnePerLine,
        Mode::Dir => Format::ManyPerLine,
        Mode::Vdir => Format::Long,
    });
    ...
    // `ls_mode == LS_LS && stdout_isatty ()`: `dir` and `vdir` escape control
    // characters instead (their default style is `escape`), so they never
    // substitute `?`.
    cfg.qmark_funny_chars = set
        .hide_control_chars_opt
        .unwrap_or(mode() == Mode::Ls && env.stdout_isatty);
    ...
    {
        match ls_program().argmatch(text, "--quoting-style", Style::WORDS) {
    ...
    cfg.quoting_style = style.unwrap_or(match mode() {
        Mode::Ls if env.stdout_isatty => Style::ShellEscape,
        Mode::Ls => Style::Literal,
        Mode::Dir | Mode::Vdir => Style::Escape,
    });
...
/// none of).
fn help_text() -> String {
    format!("Usage: {} [OPTION]... [FILE]...\n", program_name()) + HELP_BODY
}

/// Everything in `--help` after the usage line, which is the same in all
/// three modes.
const HELP_BODY: &str = "\
...
/// The whole program, for one mode. On a target without `cfg(unix)` there
/// is nothing to list with, as upstream's portability layer would say.
#[cfg(not(unix))]
#[must_use]
pub fn main(mode: Mode) -> ExitCode {
    ...
    let _ = MODE.set(mode);
    diag!(
        "{}: unix-only utility; not supported on this platform",
        program_name()
    );
    ExitCode::from(2)
}
...
/// [`stdfd::close_stderr`].
///
/// `mode` is the program: [`Mode::Ls`] for `ls`, and so on.
#[cfg(unix)]
#[must_use]
pub fn main(mode: Mode) -> ExitCode {
    // Ignored: set once, by the first call, which is the only call.
