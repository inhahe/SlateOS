...
fn run_cellranger(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Cell Ranger v8.0 (Slate OS) — 10x Genomics single-cell analysis");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("cellranger v8.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "cellranger".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_cellranger, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_cellranger(&["--version".to_string()], "cellranger");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_cellranger(&[], "cellranger");
    }
}
