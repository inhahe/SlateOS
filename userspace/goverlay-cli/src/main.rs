...
fn run_goverlay(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("goverlay v1.1 (Slate OS) — Graphics overlay configuration");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("goverlay v1.1 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "goverlay".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_goverlay, strip_ext};
