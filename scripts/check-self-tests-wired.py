...
ALLOWLIST = {
    "proc::spawn::self_test_ctest_pty": (
        "Still disabled 2026-09-11, and THE REASON HAS CHANGED AGAIN -- the "
        "previous one is now satisfied, which is why this text is longer than "
        "it wants to be. It said: re-enable once the fixture reaps before "
        "reporting a write failure. Lane B did exactly that at 6e19f88a1 "
        "(2026-09-10 21:08, 'ctest-pty: ask the child before blaming the pty'): "
        "child_verdict() now waitpid()s first and returns the CHILD's verdict "
        "-- 46/47/48/49/50/51 -- falling back to 44 only when the child is "
        "genuinely still alive. Verified by reading services/ctest-pty/main.c, "
        "not by re-running. "
        "WHAT BLOCKS IT NOW IS THE STAGED BINARY, not the fixture's logic. "
        "services/ctest-pty/ctest-pty.elf is dated 2026-09-10 07:43 and the "
        "source fix is 13 hours later, so the ELF this rung would run cannot "
        "contain child_verdict; re-enabling collects 44 from the old fixture "
        "and reddens every lane for a bug already fixed. Restaging needs the "
        "sysroot rebuilt -- ctest-fixtures.py sysroot-check reports libc.a "
        "behind posix/src/unistd.rs and posix/src/utsname.rs, both lane B's -- "
        "then the fixtures relinked under services/**. boot-test.sh calls that "
        "'a repair lane A must not make', and it is right: it would be lane A "
        "writing build artifacts the other lanes' runs depend on. "
        "RE-ENABLE when ctest-fixtures.py sysroot-check passes AND "
        "services/ctest-pty/ctest-pty.elf is newer than 6e19f88a1. Both are "
        "checkable in one command, so check rather than assume -- this entry "
        "has now named a satisfied condition twice. Asked of lane B in "
