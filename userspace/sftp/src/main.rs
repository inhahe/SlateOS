...
//! transport is the `Slate OS` TCP syscall layer).
//!
//! # Usage
//!
//! ```text
...
// Native Slate OS TCP syscall numbers (see kernel/src/syscall/number.rs).
// Local-filesystem operations are performed via `std::fs`/`std::env`, which the
// userspace `posix` crate routes to the native Slate OS syscalls — there are no
...
// userspace `posix` crate routes to the native Slate OS syscalls. We deliberately
// avoid raw Linux file syscalls — Slate OS does not expose generic read/write/open
...
///
/// On unix (the real Slate OS target) this reads the full POSIX stat fields. On a
...
fn cmd_shell(cmd_str: &str) -> Result<(), SftpError> {
    // On Slate OS there's no exec/fork path; print a notice. In a full
