...
fn run_rs(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "  Original company: Redshift Rendering Technologies (Newport Beach CA, founded 2012)"
    );
    println!("  Founders: Nicolas Burtnyk, Panagiotis Zompolas, Rob Slater");
    println!(
        "  Pricing: Maxon One $124.91/mo (bundle: C4D + ZBrush + Redshift + Forger + Red Giant)"
    );
    println!("  Engine: biased GPU renderer — uses irradiance cache for performance");
    println!(
        "         (faster than unbiased on noisy interiors, with slight bias artifact tradeoff)"
    );
    println!(
        "  Backend: NVIDIA CUDA + OptiX (RTX) initially, AMD Metal/HIP added 2022, Apple Silicon native"
    );
    println!("  Integrations: Cinema 4D (deepest), Houdini, Maya, 3ds Max, Blender (beta), Katana");
    println!(
        "  Multi-GPU: superb scaling — used in render farms with 8x4090 in single workstations"
    );
    ...
    println!(
        "  Differentiator: best speed/quality tradeoff in GPU rendering — pragmatic biased path"
    );
    0
}
