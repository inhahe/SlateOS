...
fn run_toxiproxy_cli(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("toxiproxy-cli v2.9.0 (Slate OS) — Fault injection proxy");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "version" || a == "--version") {
        println!("toxiproxy-cli v2.9.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "toxiproxy-cli".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_toxiproxy_cli, strip_ext};
