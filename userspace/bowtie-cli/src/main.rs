...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/bowtie"), "bowtie");
        assert_eq!(basename(r"C:\bin\bowtie.exe"), "bowtie.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("bowtie.exe"), "bowtie");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_bowtie2(&["--help".to_string()]), 0);
        assert_eq!(run_bowtie2(&["-h".to_string()]), 0);
        assert_eq!(run_bowtie2(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_bowtie2(&[]), 0);
    }
}
