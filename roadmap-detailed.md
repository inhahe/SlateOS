...
- [-] View options: list, thumbnails (any size), column view, order by any column *Verified 2026-09-16: `ViewMode` has List, Icons and Details, and columns sort via `ColumnManager::sort_by`, so three of the four hold. **"Any size" does not**: `thumb_config` is set once from `ThumbConfig::default()` and never assigned again, so there is one thumbnail size and no way to ask for another. The groundwork is in place -- `DiskCache::cache_filename` keys on the size cap, so changing it misses the cache and regenerates rather than serving a stale size -- what is missing is the control and its persistence.*
