...
fn run_scribus(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Scribus — desktop publishing (Slate OS).");
        println!();
        println!("Options:");
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_scribus(vec!["--help".to_string()]), 0);
        assert_eq!(run_scribus(vec!["-h".to_string()]), 0);
        let _ = run_scribus(vec!["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_scribus(vec![]);
    }
}
