...
///
/// A thin name over `sha2::sha256`; the 77 lines that used to be inside it
/// were one of ten copies under `userspace/`. `fio` uses this to verify what
/// it wrote back, so a private implementation would only ever agree with
/// itself -- which is exactly the check it is supposed to be making.
