...
fn run_dirvish_locate(args: &[String], _prog: &str) -> i32 {
    ...
    if let Some(pat) = args.first() {
        println!("dirvish-locate: searching for {}", quoteaf_os(pat));
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "dirvish".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_dirvish, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/dirvish"), "dirvish");
        assert_eq!(basename(r"C:\bin\dirvish.exe"), "dirvish.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("dirvish.exe"), "dirvish");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_dirvish(&["--help".to_string()], "dirvish"), 0);
        assert_eq!(run_dirvish(&["-h".to_string()], "dirvish"), 0);
        let _ = run_dirvish(&["--version".to_string()], "dirvish");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_dirvish(&[], "dirvish");
    }
}
