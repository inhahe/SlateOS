...
fn run_dagger(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Dagger 0.12.0 (Slate OS)");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/dagger"), "dagger");
        assert_eq!(basename(r"C:\bin\dagger.exe"), "dagger.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("dagger.exe"), "dagger");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_dagger(&["--help".to_string()]), 0);
        assert_eq!(run_dagger(&["-h".to_string()]), 0);
        let _ = run_dagger(&["--version".to_string()]);
    }
