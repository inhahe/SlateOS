...
fn run_bowtie2(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Bowtie 2 v2.5.4 (Slate OS) — Fast short-read alignment");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Bowtie 2 v2.5.4 (Slate OS)");
        return 0;
    }
...
fn run_bowtie2_build(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("bowtie2-build v2.5.4 (Slate OS) — Build Bowtie 2 index");
        return 0;
    }
...
fn run_bowtie2_inspect(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("bowtie2-inspect v2.5.4 (Slate OS) — Inspect Bowtie 2 index");
        return 0;
    }
