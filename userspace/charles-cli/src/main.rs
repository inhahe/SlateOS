...
fn run_charles(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("Charles Proxy v4.6.5 (Slate OS) — Web debugging proxy");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "-V" || a == "--version") {
        println!("Charles Proxy v4.6.5 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "charles".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_charles, strip_ext};
