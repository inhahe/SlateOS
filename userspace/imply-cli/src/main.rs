...
//! imply-cli — OurOS Imply (Apache Druid commercial — Polaris cloud, real-time analytics, Burlingame, private)
...
fn run_imply(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Imply (OurOS) — Apache Druid commercial platform (real-time + sub-second analytics)");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("Imply 2024 (OurOS) — imply CLI 2.x + Druid 30.x"); return 0; }
    println!("Imply 2024 (OurOS) — Apache Druid Commercial (Real-Time Analytics)");
