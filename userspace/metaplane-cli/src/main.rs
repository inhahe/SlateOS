...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/metaplane"), "metaplane");
        assert_eq!(basename(r"C:\bin\metaplane.exe"), "metaplane.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("metaplane.exe"), "metaplane");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_and_version_exit_zero() {
        assert_eq!(run_meta(&["--help".to_string()], "metaplane"), 0);
        assert_eq!(run_meta(&["-h".to_string()], "metaplane"), 0);
        assert_eq!(run_meta(&["--version".to_string()], "metaplane"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_meta(&[], "metaplane"), 0);
    }
}
