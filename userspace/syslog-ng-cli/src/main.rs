...
fn run_syslog_ng(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("syslog-ng v4.6 (Slate OS) — System logging daemon");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") { println!("syslog-ng v4.6 (Slate OS)"); return 0; }
...
fn run_syslog_ng_ctl(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("syslog-ng-ctl v4.6 (Slate OS) — syslog-ng control tool");
        println!();
        println!("Commands:");
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_syslog_ng};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/syslog-ng"), "syslog-ng");
        assert_eq!(basename(r"C:\bin\syslog-ng.exe"), "syslog-ng.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("syslog-ng.exe"), "syslog-ng");
