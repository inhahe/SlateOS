...
//! smartctl-cli — Slate OS S.M.A.R.T. disk monitoring tools
//!
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/smartctl"), "smartctl");
        assert_eq!(basename(r"C:\bin\smartctl.exe"), "smartctl.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("smartctl.exe"), "smartctl");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_smartctl(&["--help".to_string()]), 0);
        assert_eq!(run_smartctl(&["-h".to_string()]), 0);
        let _ = run_smartctl(&["--version".to_string()]);
    }
