...
//! openssl-cli — Slate OS OpenSSL-compatible cryptography CLI
//!
...
mod tests {
    ...
    #[test]
    fn help_exits_zero() {
        assert_eq!(run_openssl(vec!["--help".to_string()]), 0);
        assert_eq!(run_openssl(vec!["-h".to_string()]), 0);
        let _ = run_openssl(vec!["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_openssl(vec![]);
    }
}
