...
fn run_nheko(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("nheko v0.11 (Slate OS) — Desktop Matrix client (Qt)");
        println!();
        println!("Options:");
        println!("  --version         Show version");
        return 0;
    }
    if args.iter().any(|a| a == "--version") { println!("nheko v0.11 (Slate OS)"); return 0; }
...
#[cfg(test)]
mod tests {
    use super::{basename, strip_ext, run_nheko};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/nheko"), "nheko");
    ...
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("nheko.exe"), "nheko");
