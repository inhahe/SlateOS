...
fn run_tcpdump(args: Vec<String>) -> i32 {
    ...
    if let Some(file) = read_file {
        ...
        println!(
            "tcpdump: listening on {}, link-type EN10MB (Ethernet), snapshot length 262144 bytes",
            iface
        );
    }
    ...
    if verbose {
        println!(
            "14:00:00.000001 IP (tos 0x0, ttl 64, id 12345, offset 0, flags [DF], proto TCP (6), length 60)"
        );
        println!(
            "    192.168.1.10.45678 > 93.184.216.34.443: Flags [S], cksum 0xabcd, seq 1234567890, win 65535, options [mss 1460,sackOK,TS val 123456 ecr 0,nop,wscale 7], length 0"
        );
    } else {
        println!(
            "14:00:00.000001 IP 192.168.1.10.45678 > 93.184.216.34.443: Flags [S], seq 1234567890, win 65535, length 0"
        );
    }
    println!(
        "14:00:00.012345 IP 93.184.216.34.443 > 192.168.1.10.45678: Flags [S.], seq 987654321, ack 1234567891, win 65535, length 0"
    );
    println!(
        "14:00:00.012456 IP 192.168.1.10.45678 > 93.184.216.34.443: Flags [.], ack 1, win 512, length 0"
    );
    println!(
        "14:00:00.012567 IP 192.168.1.10.45678 > 93.184.216.34.443: Flags [P.], seq 1:518, ack 1, win 512, length 517"
    );
    println!(
        "14:00:00.025678 IP 93.184.216.34.443 > 192.168.1.10.45678: Flags [.], ack 518, win 501, length 0"
    );
...
mod tests {
    ...
    fn help_exits_zero() {
        ...
        let _ = run_tcpdump(vec!["--version".to_string()]);
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_tcpdump(vec![]);
    }
}
