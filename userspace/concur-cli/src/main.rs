...
fn run_concur(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("SAP Concur (Slate OS) — Enterprise travel, expense & invoice");
        println!();
        println!("Options:");
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/concur"), "concur");
        assert_eq!(basename(r"C:\bin\concur.exe"), "concur.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("concur.exe"), "concur");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_concur(&["--help".to_string()], "concur"), 0);
        assert_eq!(run_concur(&["-h".to_string()], "concur"), 0);
        let _ = run_concur(&["--version".to_string()], "concur");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_concur(&[], "concur");
    }
}
