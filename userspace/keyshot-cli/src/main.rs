...
fn run_ks(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "KeyShot 2024 (Slate OS) — Product / industrial design renderer (real-time on CPU)"
        );
        println!();
        println!("Options:");
        println!(
            "  --import FILE          Import CAD (SolidWorks/Creo/NX/Inventor/CATIA/Rhino/Fusion/STEP/IGES)"
        );
    ...
    println!(
        "  Pricing: KeyShot Pro $999/yr, KeyShot Enterprise $1999/yr, perpetual still available"
    );
    println!(
        "  Niche: product visualization / industrial design — virtually no competition in this segment"
    );
    println!(
        "  Engine: physically-correct unbiased PT, real-time interactive on CPU (multi-core scales)"
    );
    ...
    println!(
        "  HDRI editing: built-in HDRI Editor (paint highlights/reflections directly on sphere)"
    );
    ...
    println!(
        "  Workflow: open CAD → drag material → drag HDRI → render — under 60 seconds first image"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "keyshot".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ks, strip_ext};
