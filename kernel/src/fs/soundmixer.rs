...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_volume_change_count = VOLUME_CHANGE_COUNT.load(Ordering::Relaxed);
    let saved_stream_count = STREAM_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&MIXER, MixerState::new(), self_test_inner);
    VOLUME_CHANGE_COUNT.store(saved_volume_change_count, Ordering::Relaxed);
    STREAM_COUNT.store(saved_stream_count, Ordering::Relaxed);
    result
}
