...
fn run_meshio(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("meshio v5.3 (Slate OS) — Mesh I/O library and converter");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("meshio v5.3.5 (Slate OS)");
        return 0;
    }
    match prog {
        "meshio-info" => {
        ...
        _ => {
            if let Some(cmd) = args.first().map(|s| s.as_str()) {
                match cmd {
                    ...
                    _ => {
                        println!("meshio: unknown command {}. Use --help.", quoteaf_os(cmd));
                    }
                }
            } else {
                println!("meshio v5.3.5 (Slate OS) — use --help for commands");
            }
        }
    }
    0
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "meshio".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_meshio, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/meshio"), "meshio");
        assert_eq!(basename(r"C:\bin\meshio.exe"), "meshio.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("meshio.exe"), "meshio");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_meshio(&["--help".to_string()], "meshio"), 0);
        assert_eq!(run_meshio(&["-h".to_string()], "meshio"), 0);
        let _ = run_meshio(&["--version".to_string()], "meshio");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_meshio(&[], "meshio");
    }
}
