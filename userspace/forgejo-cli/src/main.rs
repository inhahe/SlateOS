...
fn run_forgejo(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "          community wanted: copyleft license, community governance, no corporate gatekeeping"
    );
    ...
    println!(
        "  Pricing: free + open source — self-host on your own server, or use codeberg.org (free)"
    );
    println!("  Tech: Go single binary, ~50MB RAM, SQLite/MySQL/Postgres backend");
    println!(
        "       still close enough to Gitea that switching is one-command (`gitea` binary symlink works)"
    );
    ...
    println!(
        "    - ForgeFed (federation): experimental ActivityPub-based federation between forges"
    );
    println!("      → goal: cross-instance issues/PRs/comments");
    println!(
        "  Codeberg.org: largest public Forgejo instance — alternative to GitHub for FOSS projects"
    );
...
#[cfg(test)]
mod tests {
    use super::{basename, run_forgejo, strip_ext};
