...
pub fn self_test() -> KernelResult<()> {
    ...
    let saved_translate_count = TRANSLATE_COUNT.load(Ordering::Relaxed);
    let saved_switch_count = SWITCH_COUNT.load(Ordering::Relaxed);
    let result = crate::fs::selftest::with_pristine(&STATE, State::new(), self_test_inner);
    TRANSLATE_COUNT.store(saved_translate_count, Ordering::Relaxed);
    SWITCH_COUNT.store(saved_switch_count, Ordering::Relaxed);
    result
}
