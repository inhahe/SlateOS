...
//! ruff-cli — OurOS Ruff Python linter/formatter
//!
...
fn run_ruff(args: Vec<String>) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("Ruff — extremely fast Python linter and formatter (OurOS).");
        println!();
        println!("Commands:");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_ruff(vec!["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_ruff(vec![]), 0);
    }
}
