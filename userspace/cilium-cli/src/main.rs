...
//! cilium-cli — OurOS Cilium eBPF networking
//!
...
fn run_cilium(args: &[String], prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        match prog {
            "hubble" => {
                println!("Hubble v0.13 (OurOS) — Network observability (Cilium)");
            ...
            _ => {
                println!("Cilium v1.15 (OurOS) — eBPF-based networking");
    ...
    if args.iter().any(|a| a == "--version") { println!("Cilium v1.15.6 (OurOS)"); return 0; }
    match prog {
        "hubble" => {
            println!("Hubble v0.13 (OurOS)");
            println!("  Nodes: 5");
