...
//!
//! The custom `linker.ld` is only valid for the bare-metal `x86_64-ouros`
...
//! target triple and skip it for anything that isn't ouros.
fn main() {
    println!("cargo:rerun-if-changed=linker.ld");
    // `TARGET` is set by cargo to the target triple (or the JSON spec's file
    // stem, `x86_64-ouros`, for our custom target). Only the ouros target
    // wants the bare-metal linker script.
    let target = std::env::var("TARGET").unwrap_or_default();
    if !target.contains("ouros") {
        return;
    }
