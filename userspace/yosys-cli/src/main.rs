...
fn run_yosys_smtbmc(args: &[String]) -> i32 {
    ...
    let file = args
        .iter()
        .find(|a| a.ends_with(".smt2"))
        .map(|s| s.as_str())
        .unwrap_or("design.smt2");
    let depth = args
        .windows(2)
        .find(|w| w[0] == "-t")
        .map(|w| w[1].as_str())
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "yosys".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_yosys, strip_ext};

    #[test]
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/yosys"), "yosys");
        assert_eq!(basename(r"C:\bin\yosys.exe"), "yosys.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("yosys.exe"), "yosys");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_yosys(&["--help".to_string()]), 0);
        assert_eq!(run_yosys(&["-h".to_string()]), 0);
        let _ = run_yosys(&["--version".to_string()]);
    }
