...
fn run_ctx(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Citrix Workspace 2410 (Slate OS) — Enterprise VDI, app delivery, DaaS");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Citrix Workspace 2410.10.0.16 (Slate OS)");
        return 0;
    }
    println!("Citrix Workspace 2410.10.0.16 (Slate OS)");
    println!("  Vendor: Cloud Software Group (formed Sep 2022 — Citrix + TIBCO merger)");
    println!(
        "  Citrix founded: 1989 by Ed Iacobucci (Texas) — pioneered terminal session protocols"
    );
    ...
    println!(
        "    DaaS — Citrix Cloud-hosted control plane, customer-owned workloads in Azure/AWS/GCP"
    );
    println!(
        "    Hypervisor — bare-metal Xen-based (formerly XenServer, now Citrix Hypervisor 8.2 EOL)"
    );
    ...
    println!(
        "  Strengths: WAN-tolerant graphics, app virtualization, mature broker, F500 standard"
    );
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "citrix".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_ctx, strip_ext};
