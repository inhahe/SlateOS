...
fn run_quip(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Quip (Slate OS) — collaborative docs + spreadsheets + chat (Salesforce-owned, end-of-life Jan 2025)"
        );
        println!();
        println!("Options:");
        ...
        println!(
            "  --salesforce-anywhere  Quip embedded in Salesforce Lightning (live docs in CRM)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Quip 2024 (Slate OS) — EOL Jan 2025");
        return 0;
    }
    ...
    println!(
        "          went on to be Twitter board chair, CEO of Sierra (AI 2023), board member at OpenAI"
    );
    ...
    println!(
        "          acquired by Salesforce Aug 2016 for $750M (Taylor became Salesforce COO, then president, left 2022)"
    );
    println!("          relaunched as 'Salesforce Anywhere' briefly (rebrand abandoned)");
    println!(
        "          gradually deprioritized vs Slack (Salesforce acquired Slack Dec 2020 for $27.7B)"
    );
    println!(
        "          Salesforce announced END OF LIFE for Quip in 2024, full shutdown Jan 31 2025"
    );
    println!("  Strategic position (was): 'docs + chat in one for sales teams in Salesforce':");
    println!(
        "                    competitor at launch: Google Docs, Microsoft Office Online, Dropbox Paper, Notion (newer)"
    );
    println!(
        "                    Salesforce-pitched as: 'collaborative content layer inside CRM workflows'"
    );
    println!(
        "                    Salesforce mobile/desktop apps embedded Quip for account plans, opportunity briefs, internal wikis"
    );
    println!(
        "                    failed strategy: Quip never broke out of Salesforce ecosystem post-acquisition"
    );
    println!(
        "                    Slack acquisition + Microsoft 365 + Notion ate the use case Quip aimed to own"
    );
    ...
    println!(
        "    - Live Apps: embeddable widgets (Salesforce records, Jira tickets, polls, polls)"
    );
    ...
    println!(
        "    - Salesforce never integrated Quip deeply enough into CRM workflows beyond static templates"
    );
    println!(
        "    - Slack acquisition (2020) overlapped Quip's chat function — Salesforce had no incentive to invest"
    );
    ...
    println!(
        "    - Recommended targets: Salesforce Files, Slack Canvas (acquired internally), Notion, Confluence"
    );
    println!("    - Migration deadline strict: data inaccessible after Jan 31 2025");
    println!("  Lessons for the industry:");
    println!(
        "    - Notable failure of post-acquisition product strategy (vs Slack at Salesforce which thrived)"
    );
    println!(
        "    - Confirmed that 'docs inside CRM' is a feature, not a product (CRM-native docs in Salesforce Lightning largely replace Quip)"
    );
    println!(
        "    - The acquired-team-founder-leaves pattern: when Bret Taylor left, Quip lost its champion"
    );
    println!(
        "    - Microsoft Loop + Notion + Slack Canvas all converging on what Quip pioneered (chat + docs together) — but better"
    );
    println!(
        "  Differentiator (historically): chat-inside-docs UX years ahead of competitors + best mobile editing of the 2010s + Salesforce live data embeds — pioneering work whose category got commoditized while Salesforce focused on Slack"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_quip, strip_ext};
