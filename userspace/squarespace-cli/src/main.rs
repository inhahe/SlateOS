...
fn run_sqsp(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Squarespace (Slate OS) — beautifully-designed website + commerce + scheduling + domains in one"
        );
        println!();
        println!("Options:");
        ...
        println!(
            "  --commerce-advanced    Commerce Advanced — $52/mo (abandoned cart, subscriptions)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Squarespace 2024 (Slate OS)");
        return 0;
    }
    println!("Squarespace 2024 (Slate OS)");
    ...
    println!(
        "          Permira buyout announced May 2024 at $46.50/share (~$7.2B), closed Q4 2024"
    );
    println!("          taken PRIVATE — Casalena rolled equity + remains CEO");
    println!("          FY2023 revenue ~$1B, ~4M paying subs");
    println!(
        "  Strategic position: 'design-first website + commerce + appointments + bio for creators':"
    );
    println!(
        "                    primary competitor: Wix (slightly cheaper, more features, less polished)"
    );
    println!(
        "                    competitor in commerce: Shopify (more commerce-deep), Wix Stores"
    );
    println!("                    competitor in scheduling: Calendly, SimplyBook.me");
    println!("                    competitor in bio-link: Linktree, Beacons");
    println!(
        "                    pitch: 'beautiful brand site that also sells' for creatives + small biz"
    );
    println!(
        "                    famous for super-polished podcast ads (every podcast circa 2014-2023)"
    );
    ...
    println!(
        "    Commerce Advanced — $52/mo — abandoned cart recovery, subscriptions, advanced shipping"
    );
    ...
    println!(
        "    - 'designer-grade' vs Wix's 'kitchen-sink' aesthetic — Squarespace prized for visual restraint"
    );
    ...
    println!(
        "    - Multi-currency NOT natively supported (vs Shopify/Wix) — perennial customer complaint"
    );
    ...
    println!(
        "            heavy in: photographers, restaurants, freelancers, B&Bs, wedding planners, podcasters"
    );
    println!("            often the 'site' part of 'IG + Squarespace' creator stack");
    println!(
        "            celebrity sites: Idris Elba, Jay-Z, Lana Del Rey, Reese Witherspoon (via Hello Sunshine)"
    );
    println!(
        "            sweet spot: $0-$1M/yr boutique businesses + creators wanting one polished brand"
    );
    println!("  Critique: less customizable than Wix or WordPress — by design");
    println!(
        "           apps marketplace is tiny vs Shopify (limits extensibility for commerce-heavy use)"
    );
    println!("           multi-currency missing + international tax handling weak");
    println!("           commerce reporting basic vs Shopify Analytics");
    println!(
        "           Fluid Engine editor transition (2022) caused complaints from long-time customers"
    );
    ...
    println!(
        "  Differentiator: best-designed templates + design-coherent end-result + Acuity scheduling + creator-friendly + integrated domains — for people who care more about how their site looks than what it does"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "squarespace".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_sqsp, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_sqsp(&["--version".to_string()], "squarespace");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_sqsp(&[], "squarespace");
    }
}
