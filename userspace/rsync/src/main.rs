...
// The implementation used to stand here -- round constants, message schedule
// and compression function, one of ten copies under `userspace/`. It comes
// from `sha2` now. `-c` compares checksums with the far end, so a private
// SHA-256 here decides whether files are considered identical.
use sha2::Sha256;
