...
fn run_kakoune(args: &[String], prog: &str) -> i32 {
    ...
    let file = args
        ...
        .map(|s| s.as_str());
    if let Some(f) = file {
        println!("kak: Editing {}", quoteaf_os(f));
    } else {
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kakoune, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_kakoune(&["--version".to_string()], "kakoune");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_kakoune(&[], "kakoune");
    }
}
