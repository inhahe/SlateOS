...
fn run_procreate(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Procreate (Slate OS) — Award-winning illustration & painting");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Procreate 5.3.10 (Slate OS)");
        return 0;
    }
    println!("Procreate 5.3.10 (Slate OS)");
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "procreate".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_procreate, strip_ext};
