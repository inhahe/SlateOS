...
//! icinga-cli — OurOS Icinga2 monitoring system
//!
...
fn run_icinga2(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("icinga2 v2.14 (OurOS) — Monitoring and alerting system");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("icinga2 v2.14 (OurOS)"); return 0; }
...
fn run_icinga2_check(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("icinga2-check v2.14 (OurOS) — Run check plugins");
        return 0;
    }
...
fn run_icinga2_api(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("icinga2-api v2.14 (OurOS) — REST API client");
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_icinga2(&["--version".to_string()], "icinga"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_icinga2(&[], "icinga"), 0);
    }
}
