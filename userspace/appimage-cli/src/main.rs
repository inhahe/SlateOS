...
//! appimage-cli — OurOS AppImage tools
//!
...
fn run_appimagetool(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("appimagetool 13 (OurOS), build abc123");
        return 0;
    }
...
fn run_appimage_update(args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("AppImageUpdate 2.0.0 (OurOS)");
        return 0;
    }
    let file = args.iter()
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_appimagetool(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_appimagetool(&[]), 0);
    }
}
