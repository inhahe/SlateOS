...
fn run_splunk(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "          acquired by Cisco Mar 2024 for $28B (largest cybersecurity acquisition ever)"
    );
    println!(
        "  Founders: Michael Baum, Rob Das, Erik Swan (former systems engineers — 'spelunking' logs)"
    );
    println!("  History: pioneered the 'log everything, search later' model");
    println!("          IPO NYSE:SPLK 2012 → Cisco subsidiary 2024");
    println!(
        "  Pricing: notoriously expensive — workload-based (was data-volume-based, capped pricing reform 2020)"
    );
    ...
    println!(
        "    - Splunk Observability Cloud (SignalFx + VictorOps + Omnition acquisitions — APM/RUM/traces/logs)"
    );
    ...
    println!("  Components:");
    println!(
        "    - Universal Forwarder (light agent) → Indexer (storage+search) ← Search Head (query UI)"
    );
    println!(
        "    - Distributed: indexer cluster + search head cluster + deployer + license master"
    );
    println!(
        "  Use cases: SIEM, IT operations, fraud detection, business analytics, IoT, compliance"
    );
    println!("  Differentiator: still THE benchmark for ad-hoc log search at petabyte scale");
    println!(
        "                  enterprises pay millions because nothing else handles that volume of unstructured data"
    );
    0
}
...
#[cfg(test)]
mod tests {
    use super::{basename, run_splunk, strip_ext};
