...
fn run_luarocks(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("LuaRocks 3.11.0 (Slate OS)");
        println!();
        println!("Commands:");
    ...
    match subcmd {
        "--version" => println!("luarocks 3.11.0 (Slate OS)"),
        "install" => {
...
mod tests {
    ...
    fn basename_strips_path() {
        assert_eq!(basename("/usr/bin/luarocks"), "luarocks");
        assert_eq!(basename(r"C:\bin\luarocks.exe"), "luarocks.exe");
        assert_eq!(basename("plain"), "plain");
    }

    #[test]
    fn strip_ext_removes_extension() {
        assert_eq!(strip_ext("luarocks.exe"), "luarocks");
        assert_eq!(strip_ext("no-ext"), "no-ext");
    }

    #[test]
    fn help_exits_zero() {
        assert_eq!(run_luarocks(&["--help".to_string()]), 0);
        assert_eq!(run_luarocks(&["-h".to_string()]), 0);
        let _ = run_luarocks(&["--version".to_string()]);
    }
