...
fn run_eid(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Microsoft Entra ID (Slate OS) — Cloud identity & access management");
        println!();
        println!("Options:");
        println!(
            "  --p1                    Entra ID P1 (Conditional Access, included with M365 E3)"
        );
    ...
    if args.iter().any(|a| a == "--version") {
        println!("Microsoft Entra ID 2024 (Slate OS)");
        return 0;
    }
    println!("Microsoft Entra ID 2024 (Slate OS)");
    ...
    println!(
        "          renamed 'Microsoft Entra ID' July 2023 (part of broader 'Entra' identity family)"
    );
    ...
    println!(
        "    - Entra Internet Access + Entra Private Access (Secure Service Edge — Microsoft's SSE)"
    );
    ...
    println!(
        "          Entra ID P1: $6/user/mo (Conditional Access, dynamic groups, password writeback) — in M365 E3"
    );
    println!(
        "          Entra ID P2: $9/user/mo (Identity Protection risk policies, PIM JIT admin) — in M365 E5"
    );
    println!(
        "          Microsoft Entra ID Governance: $7/user/mo (access reviews, entitlement mgmt)"
    );
    ...
    println!(
        "    - Conditional Access (rich policy engine — location, device, app, risk, user, group)"
    );
    ...
    println!(
        "    - Identity Protection (risk detection: leaked creds, anonymous IP, atypical travel)"
    );
    ...
    println!(
        "  Strategy: bundled with M365 → 'free' for most enterprises → massive default-IdP advantage"
    );
    println!(
        "           Microsoft pricing pressure has hurt Okta + standalone IAM vendors significantly"
    );
    println!("  Critique:");
    println!(
        "    - Conditional Access policy interactions can become a maze (CA debugger only partial relief)"
    );
    println!(
        "    - Token theft (cookie pass-the-cookie) attacks high-profile — Storm-0558 May 2023 breached MS-issued GovCloud tokens"
    );
    println!(
        "    - Microsoft itself was breached repeatedly 2023-2024 (Storm-0558, Midnight Blizzard)"
    );
    println!(
        "    - Sometimes overlapping product lines (Entra Permissions vs Defender for Cloud Apps)"
    );
    println!(
        "  Differentiator: bundled-with-M365 economic moat + deepest Windows/Office/Azure integration"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "entraid".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_eid, strip_ext};
