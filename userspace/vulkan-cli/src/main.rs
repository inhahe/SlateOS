...
//! vulkan-cli — OurOS Vulkan tools CLI
//!
...
fn run_vulkaninfo(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("vulkaninfo — Vulkan GPU information (OurOS).");
        println!();
        println!("Options:");
...
fn run_vkcube(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("vkcube — Vulkan spinning cube demo (OurOS).");
        println!();
        println!("Options:");
...
fn run_vkvia(args: &[String]) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!();
        println!("vkvia — Vulkan installation analyzer (OurOS).");
        return 0;
    }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_vulkaninfo(&["--version".to_string()]), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_vulkaninfo(&[]), 0);
    }
}
