...
fn run_vboxmanage(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("VBoxManage v7.0 (Slate OS, VirtualBox)");
        return 0;
    }
...
fn run_vboxheadless(args: &[String], _prog: &str) -> i32 {
    ...
    if args.iter().any(|a| a == "--version") {
        println!("VBoxHeadless v7.0 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "VBoxManage".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_vboxmanage, strip_ext};
