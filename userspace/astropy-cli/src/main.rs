...
//! astropy-cli — OurOS Astropy toolkit CLI
//!
...
fn run_astropy(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("astropy v6.0 (OurOS) — Astronomical Python library tools");
        println!();
        println!("Commands:");
    ...
    if args.iter().any(|a| a == "--version") { println!("astropy v6.0.1 (OurOS)"); return 0; }
...
mod tests {
    ...
    fn help_and_version_exit_zero() {
        ...
        assert_eq!(run_astropy(&["--version".to_string()], "astropy"), 0);
    }

    #[test]
    fn default_invocation_exits_zero() {
        assert_eq!(run_astropy(&[], "astropy"), 0);
    }
}
