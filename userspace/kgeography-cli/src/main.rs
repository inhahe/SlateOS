...
fn run_kgeography(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("kgeography v23.08 (Slate OS) — Geography learning tool");
        println!();
        println!("Options:");
    ...
    if args.iter().any(|a| a == "--version") {
        println!("kgeography v23.08 (Slate OS)");
        return 0;
    }
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "kgeography".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_kgeography, strip_ext};
