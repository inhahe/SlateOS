...
fn run_klaviyo(args: &[String], _prog: &str) -> i32 {
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!(
            "Klaviyo (Slate OS) — owned marketing for e-commerce (email + SMS + reviews + CDP)"
        );
        println!();
        println!("Options:");
    ...
    println!(
        "                    primary competitor: Mailchimp (smaller deals), Attentive (SMS-only), Omnisend"
    );
    println!(
        "                    upmarket competitor: Iterable, Braze (Klaviyo less polished but cheaper)"
    );
    ...
    println!(
        "    - Shopify integration: deepest of any ESP, sync orders/carts/products/customers in seconds"
    );
    println!("    - BigCommerce, Magento, WooCommerce equally deep");
    println!(
        "    - Pre-built flow library: abandoned cart, post-purchase, win-back, browse abandonment, replenishment"
    );
    println!(
        "    - Product feed dynamic blocks: send price + image + 'buy now' link with real-time stock check"
    );
    println!(
        "    - Revenue attribution: see exact $ each campaign + flow generated, with multi-touch credit"
    );
    ...
    println!(
        "    - Trigger flows on Shopify events (placed order, cart abandoned, viewed product, etc.)"
    );
    println!(
        "    - Welcome series, browse abandonment, abandoned cart, post-purchase, replenishment"
    );
    ...
    println!(
        "              Shopify (deepest), Shopify Plus, Magento, BigCommerce, WooCommerce, Salesforce Commerce Cloud"
    );
    println!(
        "              Recharge (subscriptions), Loop Returns, Gorgias (support), Smile.io (loyalty)"
    );
    ...
    println!(
        "  Critique: not designed for B2B — flow library/triggers all assume e-commerce data model"
    );
    println!(
        "           SMS pricing surprising as profile counts grow + international message rates expensive"
    );
    ...
    println!(
        "  Differentiator: deepest e-commerce data model + Shopify integration + transparent pricing — built-for-DTC marketing automation"
    );
    0
}
...
fn main() {
    ...
    let _prog = args
        ...
        .unwrap_or_else(|| "klaviyo".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_klaviyo, strip_ext};
