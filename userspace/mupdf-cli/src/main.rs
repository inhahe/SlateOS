...
fn run_mupdf(args: &[String], prog: &str) -> i32 {
    if prog == "mupdf" {
        if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
            ...
            println!("MuPDF 1.24.2 (Slate OS) — Lightweight PDF viewer");
            return 0;
        }
        let file = args
            .iter()
    ...
    if args.iter().any(|a| a == "--help" || a == "-h") || args.is_empty() {
        ...
        println!("mutool (MuPDF 1.24.2, Slate OS)");
        println!();
        println!("Commands:");
...
fn main() {
    ...
    let prog = args
        ...
        .unwrap_or_else(|| "mutool".to_string());
...
#[cfg(test)]
mod tests {
    use super::{basename, run_mupdf, strip_ext};
    ...
    fn help_exits_zero() {
        ...
        let _ = run_mupdf(&["--version".to_string()], "mupdf");
    }

    #[test]
    fn default_invocation_does_not_panic() {
        let _ = run_mupdf(&[], "mupdf");
    }
}
