...
fn run_utm(args: &[String], _prog: &str) -> i32 {
    ...
    println!(
        "         Emulation (full QEMU TCG/TCI, any arch — x86, ARM, RISC-V, PPC, SPARC, MIPS)"
    );
    println!(
        "  iOS limit: JIT not available on stock iOS; UTM SE (slow interpreter only) on App Store,"
    );
    ...
    println!(
        "  Free: distributed free from getutm.app (Mac App Store version paid to fund development)"
    );
...
#[cfg(test)]
mod tests {
    use super::{basename, run_utm, strip_ext};
